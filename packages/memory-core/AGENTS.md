...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/memory-core/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** This directory contains the working documentation and task artifacts for the comprehensive refactor of the Cortex-OS Local Memory system. The refactor aims to create a unified, ...
- **Responsibilities:**
  - Implement the Memory Core domain logic and exported APIs.
...
MEMORY_CORE_BASE_URL=https://example.local
MEMORY_CORE_DB_URL=postgres://user:pass@localhost:5432/memory_core
...
```bash
pnpm --filter memory-core test:coverage
pnpm security:scan --scope=memory-core
pnpm structure:validate --scope=memory-core
```
...
- SBOM: CycloneDX generated at `./sbom/memory-core.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"memory-core"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"memory-core","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter memory-core test:e2e
pnpm --filter memory-core test:smoke
pnpm --filter memory-core chaos:probe
```
...
# Dev & run
pnpm --filter memory-core dev
pnpm --filter memory-core start

# Build & verify
pnpm --filter memory-core build
pnpm --filter memory-core lint && pnpm --filter memory-core typecheck
pnpm --filter memory-core test && pnpm --filter memory-core test:coverage
...
pnpm --filter memory-core mcp:smoke
```
