...
export class CDNCacheManager {
    ...
    generateCacheKey(content, context) {
        const parts = [this.config.cacheKeyPrefix];
        // Add content type
     ...
     */
    async cacheContent(content, url, options = {}) {
     ...
     */
    async getCachedContent(cacheKey, options = {}) {
        if (!this.config.enabled) {
    ...
    updateMetrics(contentType, region, originalSize, compressedSize, latency) {
    ...
    updateRegionalStats(region, latency) {
        if (!this.metrics.regionalStats[region]) {
            this.metrics.regionalStats[region] = {
                requests: 0,
                hitRatio: 0,
        ...
        const regionHits = Array.from(this.cacheEntries.values()).filter((entry) => entry.region === region && entry.cacheStatus === 'HIT').length;
        const regionRequests = Array.from(this.cacheEntries.values()).filter((entry) => entry.region === region).length;
    ...
    calculateNextExecution(strategy) {
        const now = Date.now();
    ...
    collectMetrics() {
        ...
        for (const [contentType, stats] of Object.entries(this.metrics.contentTypeStats)) {
            const typeEntries = Array.from(this.cacheEntries.values()).filter((entry) => entry.contentType === contentType);
...
let cdnCacheManager = null;
export function getCDNCacheManager(config) {
    if (!cdnCacheManager) {
        if (!config) {
...
export async function stopCDNCacheManager() {
    if (cdnCacheManager) {
