...
export class LocalMemoryProvider implements MemoryProvider {
    ...
    */
	...
	private async mapRelationshipGraph(input: any): Promise<MemoryGraph> {
		...
		const edges = new Map<
			string,
			{ source: string; target: string; weight: number; type: RelationshipType; directed: boolean }
		>();
		...
		const memoryRow = this.db
			...
			.get(input.memory_id) as SQLiteMemoryRow;
		if (memoryRow) {
				...
				for (const row of rows) {
					...
					if (!visited.has(otherId) && nodes.size < maxNodes) {
						...
						const otherRow = this.db
							.prepare('SELECT * FROM memories WHERE id = ?')
							.get(otherId) as SQLiteMemoryRow;
						if (otherRow) {
							nodes.set(otherId, {
								id: otherId,
