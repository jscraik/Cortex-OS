...
export class DistributedCache {
    ...
    startMetricsCollection() {
        ...
        setInterval(async () => {
            try {
                ...
                this.metrics.memoryUsage = parseInt(memoryInfo.used_memory || '0', 10) / 1024 / 1024; // MB
                this.metrics.keyCount = keyCount;
                this.metrics.hitRatio =
                    this.metrics.totalHits / (this.metrics.totalHits + this.metrics.totalMisses) || 0;
    ...
    async get(key, namespace = 'default') {
        ...
        try {
            ...
            const data = await this.deserialize(buffer);
            if (data === null) {
    ...
    async healthCheck() {
        try {
            ...
            const status = this.metrics.hitRatio > 0.5 && this.metrics.averageLatency < 100
                ? 'healthy'
                : this.metrics.hitRatio > 0.3 && this.metrics.averageLatency < 500
                    ? 'degraded'
                    : 'unhealthy';
            return {
                status,
...
export function getDistributedCache(config) {
    if (!distributedCache) {
        if (!config) {
...
export function closeDistributedCache() {
    if (distributedCache) {
