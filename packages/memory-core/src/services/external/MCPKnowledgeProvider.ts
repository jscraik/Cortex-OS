 ...
 */
export class MCPKnowledgeProvider implements ExternalCitationProvider {
	 ...
	 */
	async initialize(config: ExternalProviderConfig): Promise<void> {
		if (this.isDisposed) {
	 ...
	 */
	private transformMcpResponse(data: unknown, serverSlug: string): ExternalCitation[] {
		if (!Array.isArray(data)) {
			return [];
		}

		return data
			.filter((item: unknown) => this.isValidArxivItem(item))
			.map((item: any) => ({
				path: `arxiv:${item.id}`,
	 ...
	 */
	private normalizeDate(dateInput?: string | number): string {
		if (!dateInput) {
