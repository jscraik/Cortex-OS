...
import { z } from 'zod';
import { getGPUAccelerationManager, stopGPUAccelerationManager, } from '../acceleration/GPUAcceleration.js';
import { getCDNCacheManager } from '../cdn/CDNCacheManager.js';
import { prisma, shutdownPrisma } from '../db/prismaClient.js';
import { GraphEdgeType, GraphNodeType } from '../db/prismaEnums.js';
import { getMLOptimizationManager } from '../ml/MLOptimizationManager.js';
import { performanceMonitor } from '../monitoring/PerformanceMonitor.js';
import { getQueryPrecomputer } from '../precomputation/QueryPrecomputer.js';
import { assembleContext } from '../retrieval/contextAssembler.js';
import { expandNeighbors } from '../retrieval/expandGraph.js';
import { QdrantConfigSchema, QdrantHybridSearch, } from '../retrieval/QdrantHybrid.js';
import { getAutoScalingManager } from '../scaling/AutoScalingManager.js';
...
export const GraphRAGServiceConfigSchema = z.object({
    ...
    cdnCaching: z
        ...
        enabled: z.boolean().default(false),
        provider: z
            .enum(['cloudflare', 'aws-cloudfront', 'fastly', 'akamai', 'custom'])
            .default('cloudflare'),
...
export class GraphRAGService {
    ...
    async initialize(embedDenseFunc, embedSparseFunc) {
        ...
        if (this.config.gpuAcceleration.enabled) {
            try {
                await this.gpuAccelerationManager.initialize(async (texts) => {
        ...
        try {
            ...
            if (this.config.precomputation.enabled) {
                ...
                if (precomputedResults) {
                    ...
                    const context = await assembleContext(focusNodeIds, Math.min(validated.maxChunks, this.config.limits.maxContextChunks), precomputedResults);
            ...
            const expansion = await expandNeighbors(focusNodeIds, {
                allowedEdges: this.config.expansion.allowedEdges,
            ...
            const context = await assembleContext(allNodeIds, Math.min(validated.maxChunks, this.config.limits.maxContextChunks), seeds);
            performanceMonitor.recordQuery({
            ...
            if (this.externalProvider && this.config.externalKg.provider === 'mcp') {
                try {
                    ...
                    performanceMonitor.recordExternalProviderCall('mcp', Date.now() - mcpStartTime, mcpCitations.length > 0);
            ...
            if (this.config.mlOptimization.enabled && queryFeatures) {
                try {
                    await this.mlOptimizationManager.recordQueryResult(validated, queryFeatures, result, queryLatency);
                }
                catch (error) {
    ...
    async healthCheck() {
        try {
            ...
            else if (perfSummary.status !== 'healthy' ||
            ...
            const result = {
                ...
                components: {
                    ...
                    ...(this.config.autoScaling.enabled && {
                        autoScaling: autoScalingHealth?.healthy || false,
                    }),
                    ...(this.config.mlOptimization.enabled && {
                        mlOptimization: mlOptimizationHealth?.healthy || false,
                    }),
    ...
    async getStats() {
        ...
        const nodeTypeDistribution = Object.fromEntries(nodeStats.map((stat) => [stat.type, stat._count.type]));
        const edgeTypeDistribution = Object.fromEntries(edgeStats.map((stat) => [stat.type, stat._count.type]));
    ...
    async liftToGraphNodes(seedResults) {
    ...
    buildResult(context, expansion, startTime, seeds) {
        return {
    ...
    formatCitations(chunks) {
        return chunks.map((chunk) => ({
            path: chunk.path,
            lines: chunk.lineStart !== undefined && chunk.lineEnd !== undefined
    ...
    async fetchExternalCitations(nodeIds) {
        if (!this.externalKg)
            return [];
        const citations = [];
        ...
        for (const nodeId of nodeIds) {
            try {
                const neighborhood = await this.externalKg.driver.neighborhood(nodeId, this.externalKg.maxDepth);
    ...
    async fetchMcpCitations(question, _seeds) {
    ...
    async emitQueryEvent(type, data) {
        try {
            const event = {
...
export function createGraphRAGService(config) {
    const baseConfig = {
        ...
        autoScaling: {
            ...
            prediction: {
                ...
                algorithm: process.env.AUTO_SCALING_PREDICTION_ALGORITHM || 'linear',
            ...
            monitoring: {
                ...
                logLevel: process.env.AUTO_SCALING_LOG_LEVEL || 'info',
            },
        },
        mlOptimization: {
            ...
            mlModels: {
                latencyPrediction: {
                    ...
                    modelType: process.env.ML_LATENCY_MODEL_TYPE || 'linear',
    ...
    const externalKgBase = {
        ...
        provider: process.env.EXTERNAL_KG_PROVIDER || 'none',
        // Neo4j settings
    ...
    const mergedConfig = {
        ...
        cdnCaching: {
            ...
            compression: {
                ...
                types: (process.env.CDN_COMPRESSION_TYPES ||
                    'text/,application/json,application/javascript,application/xml').split(','),
            },
            optimization: {
            ...
            monitoring: {
                ...
                logLevel: process.env.CDN_LOG_LEVEL || 'info',
            },
            geographic: {
