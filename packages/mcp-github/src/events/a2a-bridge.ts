...
export class A2AEventBridge extends EventEmitter {
	...
	constructor(config: A2AConfig, routingConfig?: RoutingConfiguration) {
		super();
		this.config = config;
		this.router = new GitHubEventRouter(
			routingConfig || DEFAULT_GITHUB_ROUTING_CONFIG,
		);
	...
	private setupEventListeners(): void {
		...
		process.on("unhandledRejection", (reason, promise) => {
			logger.error(
				"Unhandled promise rejection in A2A bridge",
				reason as Error,
				{
	...
	async disconnect(): Promise<void> {
		...
		try {
			...
			if (this.eventQueue.length > 0) {
				logger.info(
					`Processing ${this.eventQueue.length} remaining events before disconnect`,
				);
	...
	async publishEvent(
		...
		const correlationId =
			options?.correlationId || CorrelationManager.generateCorrelationId();
		...
		try {
			...
			// Publish the event
			const _result = await this.publishEnvelope(envelope, routes);
	...
	private updateStats(
		...
		this.stats.averageProcessingTime =
			(this.stats.averageProcessingTime * (this.stats.totalEvents - 1) +
				processingTime) /
			this.stats.totalEvents;
	}
	...
	async publishEvents(
		...
		logger.info(
			`Publishing ${events.length} events in batches of ${batchSize}`,
		);
