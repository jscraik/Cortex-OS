	...
	const envConfig: Partial<GitHubMCPConfig> = {
		...
		rateLimits: {
			requestsPerHour: parseInt(
				process.env.CORTEX_GITHUB_RATE_LIMIT_HOUR || "5000",
				10,
			),
			requestsPerMinute: parseInt(
				process.env.CORTEX_GITHUB_RATE_LIMIT_MIN || "100",
				10,
			),
			burstLimit: parseInt(process.env.CORTEX_GITHUB_RATE_BURST || "10", 10),
		},
	};
