 ...
 */
export const validateCodeReview = async (state: PRPState): Promise<CodeReviewResult> => {
...
const runQualityChecks = async (): Promise<ReviewIssue[]> => {
  ...
  issues.push(...largeFunctions.map(func => ({
  ...
  issues.push(...todoComments.map(todo => ({
 ...
 */
const findLargeFunctions = async (): Promise<Array<{ name: string; file: string; line: number; }>> => {
...
const findTodoComments = async (): Promise<Array<{ text: string; file: string; line: number; }>> => {
  try {
    ...
    const { stdout } = await execAsync('grep -rn "TODO\\|FIXME" --include="*.ts" --include="*.js" --include="*.tsx" --include="*.jsx" .', {
      timeout: 10000,
    });
