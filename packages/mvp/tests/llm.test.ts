...
describe('LLM Integration', () => {
  it('uses MLX endpoint when available', async () => {
    ...
    const result = await generateText('hello', { model: 'mlx-model', fallbackModel: 'ollama' });
  ...
  it('falls back to Frontier when MLX fails', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({ ok: true, json: async () => ({ response: 'frontier' }) });
    vi.stubGlobal('fetch', fetchMock);
    const result = await generateText('hi', { model: 'mlx-model', fallbackModel: 'ollama' });
