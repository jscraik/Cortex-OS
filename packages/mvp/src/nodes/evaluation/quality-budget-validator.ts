 ...
 */
export const validateQualityBudgets = async (state: PRPState): Promise<QualityBudgetResult> => {
...
const getPerformanceMetrics = async (): Promise<{ score: number } | null> => {
  try {
    ...
    try {
      const { stdout } = await execAsync('npx lighthouse --only-categories=performance --output=json --chrome-flags="--headless" http://localhost:3000', {
        timeout: 60000,
      });
...
const runSecurityAnalysis = async () => {
  ...
  try {
    ...
    const { stdout } = await execAsync('semgrep --config=auto --json .', { timeout: 60000 });
    const results = JSON.parse(stdout);
 ...
 */
const calculateSecurityScore = (issues: { vulnerabilities: number; warnings: number; info: number }): number => {
...
const buildQualityDetails = (
  ...
  details.push(`Accessibility: ${accessibility.score}% (threshold: ${accessibility.threshold}%) - ${accessibility.passed ? 'PASS' : 'FAIL'}`);
  details.push(`Performance: ${performance.score}% (threshold: ${performance.threshold}%) - ${performance.passed ? 'PASS' : 'FAIL'}`);
  details.push(`Security: ${security.score}% (threshold: ${security.threshold}%) - ${security.passed ? 'PASS' : 'FAIL'}`);
