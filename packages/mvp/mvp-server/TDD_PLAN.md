  ...
  it('should handle rate limit errors', async () => {
    ...
    try {
      ...
      const requests = Array(100)
        .fill(null)
        .map(() => app.inject({ method: 'GET', url: '/api/rate-limited' }));
...
describe('Enhanced Rate Limiting', () => {
  it('should support different rate limits for different routes', async () => {
    ...
    try {
      ...
      const highFreqPromises = Array(50)
        .fill(null)
        .map(() => app.inject({ method: 'GET', url: '/api/high-freq' }));
      ...
      const lowFreqPromises = Array(10)
        .fill(null)
        .map(() => app.inject({ method: 'POST', url: '/api/low-freq' }));
...
describe('Distributed Tracing', () => {
  ...
  it('should propagate trace context to downstream services', async () => {
    ...
    try {
      ...
      expect(downstreamCalls).toContainEqual(
        expect.objectContaining({
