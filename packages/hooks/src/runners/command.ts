...
export async function runCommand(cmd: string, ctx: HookContext, timeoutMs: number, allowlist?: string[]): Promise<HookResult> {
  ...
  return new Promise<HookResult>((resolve) => {
    ...
    let out = '';
    let err = '';
    child.stdout.on('data', (d) => { out += String(d); if (out.length > 1024 * 1024) child.kill('SIGKILL'); });
    ...
    child.on('close', (code) => {
      resolve(code === 0 ? { action: 'exec', output: out.trim() } : { action: 'deny', reason: err.trim() || `exit ${code}` });
    });
  });
}
...
function minimalEnv() {
  return {
    PATH: process.env.PATH?.split(':').filter(p => /\/bin|\/usr\//.test(p)).join(':') ?? '/usr/bin:/bin',
