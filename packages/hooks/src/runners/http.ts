...
export async function runHTTP(url: string, ctx: HookContext, timeoutMs: number): Promise<HookResult> {
  if (!url) return { action: 'emit', note: 'empty http url' };
  try {
    const controller = new AbortController();
    const t = setTimeout(() => controller.abort('timeout'), timeoutMs);
    const res = await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(ctx), signal: controller.signal });
    clearTimeout(t);
