...
class TestCortexHooks extends CortexHooks {
	...
	setConfig(cfg: HookConfig) {
		(this as unknown as { cfg: HookConfig }).cfg = cfg;
	}
	getConfig(): HookConfig {
		return (this as unknown as { cfg: HookConfig }).cfg;
	}
}
...
describe('command allowlist via settings', () => {
	it('denies non-allowed binary and allows configured one', async () => {
		...
		const res = await hooks.run('PreToolUse', {
			event: 'PreToolUse',
			cwd: process.cwd(),
			user: 'test',
		});
		...
		const res2 = await hooks.run('PreToolUse', {
			event: 'PreToolUse',
			cwd: process.cwd(),
			user: 'test',
		});
