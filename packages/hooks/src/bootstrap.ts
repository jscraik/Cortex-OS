 ...
 */
export async function initHooksSingleton(opts?: { projectDir?: string; userDir?: string }): Promise<CortexHooks> {
  ...
  if (!instance) {
    ...
    let recordOperation: ((op: string, success: boolean, runId: string, labels?: Labels) => void) | null = null;
    let withSpan: (<T>(name: string, fn: (runId: string, ctx: { runId: string; traceId: string; spanId: string }) => Promise<T>, options?: { attributes?: Record<string, string | number | boolean> }) => Promise<T>) | null = null;
    try {
      const dynamicImport = new Function('m', 'return import(m)') as (m: string) => Promise<unknown>;
      const obsMod = (await dynamicImport('@cortex-os/observability')) as Record<string, unknown>;
      const level = (process.env.CORTEX_HOOKS_LOG_LEVEL ?? 'info') as 'debug' | 'info' | 'warn' | 'error';
      const maybeCreate = obsMod?.['createLogger'];
      if (typeof maybeCreate === 'function') {
        logger = (maybeCreate as (c: string, l: 'debug' | 'info' | 'warn' | 'error') => { info: (obj: unknown, msg?: string) => void })(
          '@cortex-os/hooks',
          level,
        );
      }
      // Metrics and tracing
      if (typeof obsMod['recordLatency'] === 'function') recordLatency = (obsMod['recordLatency'] as unknown) as (op: string, ms: number, labels?: Labels) => void;
      if (typeof obsMod['recordOperation'] === 'function') recordOperation = (obsMod['recordOperation'] as unknown) as (op: string, success: boolean, runId: string, labels?: Labels) => void;
      if (typeof obsMod['withSpan'] === 'function') withSpan = (obsMod['withSpan'] as unknown) as <T>(name: string, fn: (runId: string, ctx: { runId: string; traceId: string; spanId: string }) => Promise<T>, options?: { attributes?: Record<string, string | number | boolean> }) => Promise<T>;
    } catch {
    ...
    instance.on('hook:result', (payload: {
      event: string;
      matcher: string;
      ...
      try {
        ...
        const extra = {
          ...
          mutated: payload.result.action === 'allow' && 'input' in payload.result && (payload.result as { input?: unknown }).input !== undefined,
        };
        const emit = () => { logger?.info({ hooks: extra }, 'hook result'); };
        if (withSpan) {
          // Wrap emission in a span and record basic metrics
          withSpan('hooks.hook_result', async (runId: string) => {
            const start = Date.now();
            emit();
            const elapsed = Date.now() - start;
            recordLatency?.('hooks.hook_result', elapsed, { event: payload.event, action: payload.result.action });
            ...
            recordOperation?.('hooks.hook_result', success, runId, { event: payload.event, action: payload.result.action, decision });
          ...
          if (recordOperation) {
            ...
            recordOperation('hooks.hook_result', !isDeny, runId, { event: payload.event, action: payload.result.action, decision });
          }
        }
      } catch {
