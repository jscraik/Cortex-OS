...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/cortex-mcp/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** The Cortex MCP package now ships a production-fast FastMCP server with
- **Responsibilities:**
  - Implement the Cortex MCP domain logic and exported APIs.
...
CORTEX_MCP_BASE_URL=https://example.local
CORTEX_MCP_DB_URL=postgres://user:pass@localhost:5432/cortex_mcp
...
Run locally:
```bash
pnpm --filter cortex-mcp test:coverage
pnpm security:scan --scope=cortex-mcp
pnpm structure:validate --scope=cortex-mcp
```
...
- SBOM: CycloneDX generated at `./sbom/cortex-mcp.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"cortex-mcp"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"cortex-mcp","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter cortex-mcp test:e2e
pnpm --filter cortex-mcp test:smoke
pnpm --filter cortex-mcp chaos:probe
```
...
# Build & verify
pnpm --filter cortex-mcp build
pnpm --filter cortex-mcp lint && pnpm --filter cortex-mcp typecheck
pnpm --filter cortex-mcp test && pnpm --filter cortex-mcp test:coverage
...
pnpm --filter cortex-mcp mcp:smoke
```
