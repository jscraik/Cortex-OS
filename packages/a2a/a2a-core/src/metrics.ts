...
class BusMetricsCollector {
    eventsPublished = 0;
    duplicatesDropped = 0;
    incEvents(count = 1) { this.eventsPublished += count; }
    incDuplicates(count = 1) { this.duplicatesDropped += count; }
    snapshot(): BusMetricsSnapshot { return { eventsPublished: this.eventsPublished, duplicatesDropped: this.duplicatesDropped }; }
}
