...
export class MCPStreamingClient extends EventEmitter {
	...
	private async connectToMockServer(): Promise<void> {
		...
		try {
			...
			throw this.createBrAInwavError(`brAInwav MCP connection failed: ${msg}`, 'CONNECTION_FAILED');
		}
	}
	...
	private async establishConnection(): Promise<void> {
		return new Promise((resolve, reject) => {
			const timeout = setTimeout(() => {
				reject(this.createBrAInwavError('brAInwav MCP connection timeout', 'CONNECTION_TIMEOUT'));
			...
			if (this.retryCount < this.options.maxRetries - 1) {
				...
				setTimeout(
					...
					this.options.retryInterval * 2 ** (this.retryCount - 1),
				);

				reject(this.createBrAInwavError('brAInwav connection failed', 'CONNECTION_FAILED'));
				return;
			}
	...
	async subscribe(eventType: string): Promise<void> {
		if (!this.isConnected) {
			throw this.createBrAInwavError('brAInwav MCP client not connected', 'NOT_CONNECTED');
		}
