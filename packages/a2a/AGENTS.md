...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/a2a/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Event-driven messaging, CloudEvents 1.0 compliant, W3C Trace Context support
- **Responsibilities:**
  - Implement the A2A domain logic and exported APIs.
...
# Build
pnpm --filter a2a build

# Quality (affected-aware if available)
pnpm --filter a2a lint
pnpm --filter a2a typecheck
pnpm --filter a2a test
pnpm --filter a2a test:coverage
```
...
A2A_BASE_URL=https://example.local
A2A_DB_URL=postgres://user:pass@localhost:5432/a2a
...
```bash
pnpm --filter a2a test:coverage
pnpm security:scan --scope=a2a
pnpm structure:validate --scope=a2a
```
...
- SBOM: CycloneDX generated at `./sbom/a2a.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"a2a"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"a2a","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter a2a test:e2e
pnpm --filter a2a test:smoke
pnpm --filter a2a chaos:probe
```
...
# Dev & run
pnpm --filter a2a dev
pnpm --filter a2a start

# Build & verify
pnpm --filter a2a build
pnpm --filter a2a lint && pnpm --filter a2a typecheck
pnpm --filter a2a test && pnpm --filter a2a test:coverage
