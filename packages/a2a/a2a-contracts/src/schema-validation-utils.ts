...
export class SchemaValidationUtils {
  /**
   * Create a versioned schema with metadata
   */
  static createVersionedSchema<T extends z.ZodType>(
    eventType: string,
    version: string,
    schema: T,
    options: {
   ...
   */
  static validateAgainstVersions<T>(
    event: unknown,
   ...
   */
  static generateSchemaDocs(schema: z.ZodSchema, eventType: string): string {
    let shape: string[] = [];
    let isObject = false;
