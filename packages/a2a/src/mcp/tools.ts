...
function createQueueMessageTool(deps: {
	taskManager: TaskManager;
}): A2AMcpTool<A2AQueueMessageInput, A2AQueueMessageResult> {
	return {
...
export function createA2AEventStreamSubscribeTool(
	deps: EventStreamToolDependencies = {
	...
	return {
		...
		handler: async (input) =>
			withSpan({ name: 'a2a.mcp.eventStreamSubscribe' }, async () => {
				...
				const defaultEvents: import('../streaming.js').TaskEventName[] = [
					'taskCompleted',
					'taskFailed',
					'taskCancelled',
					'taskRunning',
				...
				const requestedEvents =
					params.events && params.events.length > 0
						? (params.events as import('../streaming.js').TaskEventName[])
						: defaultEvents;
				...
				try {
					result = A2AEventStreamSubscribeResultSchema.parse({
						...
						note:
							`Streaming active via SSE at ${deps.registry.buildSsePath(subscriptionId)}. ` +
