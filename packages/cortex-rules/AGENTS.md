...
```yaml
id: agents-cortex-rules-v1
package: Cortex Rules
type: package
runtime: python
...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/cortex-rules/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** A package for managing and rendering Cortex-OS rules for AI agents.
- **Responsibilities:**
  - Implement the Cortex Rules domain logic and exported APIs.
...
```bash
# Dev
uv run -m cortex_rules --dev
# or
python -m cortex_rules
...
CORTEX_RULES_BASE_URL=https://example.local
CORTEX_RULES_DB_URL=postgres://user:pass@localhost:5432/cortex_rules
...
Run locally:
```bash
pnpm --filter cortex-rules test:coverage
pnpm security:scan --scope=cortex-rules
pnpm structure:validate --scope=cortex-rules
```
...
- SBOM: CycloneDX generated at `./sbom/cortex-rules.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"cortex-rules"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"cortex-rules","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter cortex-rules test:e2e
pnpm --filter cortex-rules test:smoke
pnpm --filter cortex-rules chaos:probe
```
...
```bash
# Dev & run
pnpm --filter cortex-rules dev
pnpm --filter cortex-rules start

# Build & verify
pnpm --filter cortex-rules build
pnpm --filter cortex-rules lint && pnpm --filter cortex-rules typecheck
pnpm --filter cortex-rules test && pnpm --filter cortex-rules test:coverage
...
pnpm --filter cortex-rules mcp:smoke
```
