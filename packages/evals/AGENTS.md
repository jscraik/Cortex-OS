...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/evals/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for Evals within Cortex-OS.
- **Responsibilities:**
  - Implement the Evals domain logic and exported APIs.
...
EVALS_BASE_URL=https://example.local
EVALS_DB_URL=postgres://user:pass@localhost:5432/evals
...
```bash
pnpm --filter evals test:coverage
pnpm security:scan --scope=evals
pnpm structure:validate --scope=evals
```
...
- SBOM: CycloneDX generated at `./sbom/evals.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"evals"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"evals","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter evals test:e2e
pnpm --filter evals test:smoke
pnpm --filter evals chaos:probe
```
...
# Dev & run
pnpm --filter evals dev
pnpm --filter evals start

# Build & verify
pnpm --filter evals build
pnpm --filter evals lint && pnpm --filter evals typecheck
pnpm --filter evals test && pnpm --filter evals test:coverage
