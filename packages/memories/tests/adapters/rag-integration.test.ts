import type { Chunk, Embedder } from '@cortex-os/rag';
import { beforeEach, describe, expect, it, vi } from 'vitest';
import {
	MemoryStoreRAGAdapter,
	RAGEmbedderAdapter,
	RAGIntegration,
} from '../../src/adapters/rag-integration.js';
...
describe('RAG Integration', () => {
	...
	describe('RAGIntegration', () => {
		...
		it('should throw error for semantic search without embedder', async () => {
			...
			await expect(ragNoEmbedder.semanticSearch('test')).rejects.toThrow(
				'Embedder is required for semantic search',
			);
		});
	});
