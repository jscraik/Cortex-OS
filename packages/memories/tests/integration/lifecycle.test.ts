...
describe('LifecycleMemoryStore', () => {
	...
	describe('Stage Transitions', () => {
		...
		it('should enforce valid stage transitions', async () => {
			...
			// Valid transition
			const updated = await lifecycleStore.transitionTo(memory.id, 'active', namespace);
			expect(updated.lifecycle.stage).toBe('active');

			// Invalid transition - should throw
			await expect(lifecycleStore.transitionTo(memory.id, 'draft', namespace)).rejects.toThrow(
				'Invalid transition from active to draft',
			);
		});
		...
		it('should support custom transition logic', async () => {
			lifecycleStore = new LifecycleMemoryStore(baseStore, {
				stages: {
					...
					customTransitions: {
						'draft->review': async (memory, _context) => {
			...
			await expect(
				lifecycleStore.transitionTo(memory.id, 'review', namespace),
