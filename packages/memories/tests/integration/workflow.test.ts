...
describe('WorkflowAwareMemoryStore', () => {
	...
	describe('Workflow Triggers', () => {
		...
		it('should support trigger conditions based on memory count', async () => {
			...
			for (let i = 0; i < 12; i++) {
				await workflowStore.upsert(createMemory({ text: `Request #${i} processed` }), namespace);
			}
	...
	describe('Workflow Templates', () => {
		...
		it('should validate template fields', async () => {
			...
			await expect(workflowStore.upsert(memory, namespace)).rejects.toThrow(
				"Value for field 'priority' exceeds maximum",
			);
		});
	});
