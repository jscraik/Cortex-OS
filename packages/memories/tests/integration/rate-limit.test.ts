...
describe('RateLimitedMemoryStore', () => {
	...
	describe('Rate Limiting Strategies', () => {
		it('should enforce fixed window rate limiting', async () => {
			...
			await expect(rateLimitedStore.upsert(memory6, namespace)).rejects.toThrow(
				'Rate limit exceeded',
			);
		});
		...
		it('should enforce sliding window rate limiting', async () => {
			...
			await expect(rateLimitedStore.upsert(memory4, namespace)).rejects.toThrow(
				'Rate limit exceeded',
			);
		...
		it('should enforce token bucket rate limiting', async () => {
			...
			await expect(rateLimitedStore.upsert(memory9, namespace)).rejects.toThrow(
				'Rate limit exceeded',
			);
		});
	...
	describe('Per-Operation Limits', () => {
		it('should apply different limits for different operations', async () => {
			...
			await expect(rateLimitedStore.upsert(memory3, namespace)).rejects.toThrow(
				'Rate limit exceeded',
			);

			// But search should still work
			await expect(
				rateLimitedStore.searchByText({ text: 'test' }, namespace),
		...
		it('should use default limit for operations without specific limits', async () => {
			...
			await expect(rateLimitedStore.searchByText({ text: 'search 6' }, namespace)).rejects.toThrow(
				'Rate limit exceeded',
			);
		});
	});
	...
	describe('Advanced Features', () => {
		...
		it('should support burst allowances', async () => {
			...
			await expect(rateLimitedStore.upsert(memory11, namespace)).rejects.toThrow(
				'Rate limit exceeded',
			);
		});
		...
		it('should support gradual backoff on repeated violations', async () => {
			...
			for (let attempt = 0; attempt < 3; attempt++) {
				...
				// Wait for backoff
				clock.advanceTimeByTime(5000 * 2 ** attempt);
			}
		...
		it('should support whitelisting and blacklisting', async () => {
			...
			for (let i = 0; i < 5; i++) {
				...
				await expect(
					rateLimitedStore.upsert(memory, namespace, {
			...
			await expect(
				rateLimitedStore.upsert(badMemory, namespace, {
