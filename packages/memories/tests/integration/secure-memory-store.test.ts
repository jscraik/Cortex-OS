...
describe('SecureMemoryStore with RBAC', () => {
  ...
  describe('Role-Based Access Control', () => {
    it('should enforce read permissions based on roles', async () => {
      ...
      const result = await secureStore.get(memory.id, namespace, { role: 'manager', subject: 'user3' });
    ...
    it('should support wildcard permissions', async () => {
      ...
      const result = await secureStore.get(memory.id, namespace, { role: 'guest', subject: 'guest1' });
      expect(result).toBeTruthy();
    });
    ...
    it('should handle ownership-based permissions', async () => {
      ...
      const result = await secureStore.get(memory.id, namespace, { role: 'user', subject: 'user1' });
      expect(result).toBeTruthy();
    ...
    it('should enforce permissions on search operations', async () => {
      ...
      const results = await secureStore.searchByText(
        { text: 'info' },
        namespace,
        { role: 'user', subject: 'user1' }
      );
    ...
    it('should support hierarchical roles', async () => {
      ...
      const result = await secureStore.get(
        memory.id,
        namespace,
        { role: 'superadmin', subject: 'super1' }
      );
      expect(result).toBeTruthy();
    });
  });
  ...
  describe('Role Management', () => {
    ...
    it('should support role revocation', async () => {
      ...
      // User can access
      let result = await secureStore.get(memory.id, namespace, { role: 'user', subject: 'user1' });
      expect(result).toBeTruthy();
  ...
  describe('Security Configuration', () => {
    it('should allow custom deny messages', async () => {
      ...
      await secureStoreWithCustomMessage.upsert(memory, namespace, { role: 'admin', subject: 'admin1' });
    ...
    it('should support audit trail retention', async () => {
      ...
      await secureStoreWithRetention.upsert(memory, namespace, { role: 'admin', subject: 'admin1' });
    ...
    it('should allow permission caching', async () => {
      ...
      const result = await secureStoreWithCache.get(memory.id, namespace, { role: 'admin', subject: 'admin1' });
      expect(result).toBeTruthy();
    });
  });
});