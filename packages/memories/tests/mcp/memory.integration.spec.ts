...
import { MAX_MEMORY_TEXT_LENGTH, memoryMcpTools } from '../../src/mcp/tools.js';
...
function parseEnvelope(result: unknown): SuccessEnvelope | FailureEnvelope {
	...
	const payloadTimestamp = assertString(
		payload.timestamp,
		'MCP tool payload must include timestamp',
	);
	...
	if (payloadSuccess) {
		...
		const data = assertRecord(
			payload.data,
			'MCP tool payload must include data for success responses',
		);
		return {
			success: true,
			tool,
...
describe('memories MCP integration', () => {
	...
	it('completes maximum length memories.store payload within performance budget', async () => {
		...
		expect(
			assertNumber(
				envelope.data.textLength,
				'Successful memories.store response must report textLength',
			),
