...
} from '../../src/observability/observable-store.js';
...
describe('Observable Memory Store', () => {
	...
	beforeEach(() => {
		...
		mockProvider = {
			createSpan: vi.fn().mockImplementation(async (_name, fn) => {
				const mockSpan = {
					setAttribute: vi.fn(),
					setAttributes: vi.fn(),
					setStatus: vi.fn(),
	...
	describe('Operation Wrapping', () => {
		...
		it('should wrap get operation with observability', async () => {
			...
			expect(mockProvider.createSpan).toHaveBeenCalledWith('memories.get', expect.any(Function), {
				'memory.id': testMemory.id,
				'operation.type': 'get',
				'memory.namespace': 'test-ns',
			});
