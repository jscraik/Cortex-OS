...
describe('Enhanced privacy redaction', () => {
  it('redacts various types of PII from text', () => {
    ...
    expect(redactPII('Contact me at test@example.com'))
      .toBe('Contact me at [REDACTED]');
    
    // Test phone number redaction (new functionality)
    expect(redactPII('Call me at 555-123-4567'))
      .toBe('Call me at [REDACTED]');
    
    expect(redactPII('My number is (555) 123-4567'))
      .toBe('My number is [REDACTED]');
    
    expect(redactPII('Reach out at +1-555-123-4567'))
      .toBe('Reach out at +1-[REDACTED]');
    
    // Test credit card redaction (new functionality)
    expect(redactPII('My card is 4111-1111-1111-1111'))
      .toBe('My card is [REDACTED]');
    
    expect(redactPII('Card: 4111111111111111'))
      .toBe('Card: [REDACTED]');
    
    // Test SSN redaction (new functionality)
    expect(redactPII('SSN: 123-45-6789'))
      .toBe('SSN: [REDACTED]');
    
    // Test address redaction (new functionality)
    expect(redactPII('I live at 123 Main St'))
      .toBe('I live at [REDACTED]');
  });
