...
export class GraphMemoryStore implements MemoryStore {
	...
	async traverse(
		...
		while (queue.length > 0) {
			...
			if (!item) break;
			await this.processTraverseItem(item, visited, result, namespace, queue, {
				relationshipTypes,
				direction,
				maxDepth,
			});
	...
	private async processTraverseItem(
		...
		node.edges = relationships.filter((rel) =>
			this.relationshipMatches(rel, { relationshipTypes, direction, nodeId: id }),
		);
		result.push(node);
	...
	private async processStartNodeForQuery(
		...
		for (const rel of relationships) {
			if (
				...
				this.relationshipMatches(rel, { direction: query.direction, nodeId: query.startNodeId })
			) {
				edges.add(rel);
	...
	private async handleConnectedNodeIfWithinDepth(
		rel: Relationship,
		query: GraphQuery,
		namespace: string,
		nodes: Map<string, MemoryNode>,
	...
	private async handlePathIfMatches(
		path: string[],
		pattern: GraphPattern,
		nodes: Map<string, MemoryNode>,
		namespace: string,
	...
	private async handlePathPattern(
		nodes: Map<string, MemoryNode>,
		pattern: GraphPattern,
		namespace: string,
	): Promise<void> {
	...
	private async addNodeIfMissing(
		nodes: Map<string, MemoryNode>,
		id: string,
		namespace: string,
	): Promise<void> {
	...
	private relationshipMatches(
		rel: Relationship,
		opts: { relationshipTypes?: string[]; direction?: string; nodeId?: string },
	): boolean {
