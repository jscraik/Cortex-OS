...
export function computeVectorHybrid(query: HybridQuery, memories: Memory[]): HybridSearchResult[] {
  ...
  return memories
    .map((m) => {
      const memoryVec = Array.isArray(m.vector) ? m.vector : (m as unknown as { embedding?: number[] }).embedding;
      const score = Array.isArray(memoryVec) && memoryVec.length === vec.length
