...
export class QdrantMemoryStore implements MemoryStore {
  ...
  async upsert(memory: Memory, namespace?: string): Promise<Memory> {
    ...
    // SDK typing can be strict about optional vectors; cast to any for now to avoid type mismatch
    const options = { points: [point] } as unknown as Parameters<QdrantClient['upsert']>[1];
    await this.client.upsert(this.collection, options);
    return memory;
  }
