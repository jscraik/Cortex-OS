...
export class LocalMemoryStore implements MemoryStore {
    ...
    async delete(id: string, namespace?: string): Promise<void> {
        ...
        try {
            const url = new URL(`${this.baseUrl}/memories/${encodeURIComponent(id)}`);
            if (namespace ?? this.defaultNs) url.searchParams.set('namespace', String(namespace ?? this.defaultNs));
            const res = await fetch(url, { method: 'DELETE', headers: toHeaders(this.apiKey), signal: ctrl.signal });
            if (!res.ok && res.status !== 404) throw new Error(`local-memory: delete failed: ${res.status}`);
        } finally {
            clearTimeout(to);
        }
    }
    ...
    async searchByVector(q: VectorQuery, namespace?: string): Promise<Memory[]> {
        ...
        if (q.queryText) {
            return this.searchByText({ text: q.queryText, topK: q.topK, filterTags: q.filterTags }, namespace);
        }
        return [];
    }
