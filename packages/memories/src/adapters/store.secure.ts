...
export class SecureMemoryStore implements MemoryStore {
  ...
  async searchByText(q: TextQuery, namespace = 'default', context?: SecureContext): Promise<Memory[]> {
    if (!context) {
  ...
  async searchByVector(q: VectorQuery, namespace = 'default', context?: SecureContext): Promise<(Memory & { score: number })[]> {
    if (!context) {
      throw new Error('Security context required for search operation');
    }
  ...
  async list(namespace = 'default', limit?: number, offset?: number, context?: SecureContext): Promise<Memory[]> {
    if (!context) {
  ...
  private async checkPermission(
    ...
    const access = memory.metadata?.access as {
      read?: string[];
      write?: string[];
      delete?: string[];
    } || {};
