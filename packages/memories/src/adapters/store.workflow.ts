...
export class WorkflowAwareMemoryStore implements MemoryStore {
	...
	private async detectTriggers(memory: Memory, namespace: string): Promise<WorkflowTrigger[]> {
		...
		if (memory.metadata?.workflowTemplate) {
			const template = this.config.workflows.templates?.find(
				(t) => t.id === memory.metadata.workflowTemplate,
			);
	...
	private async evaluateRelationshipTrigger(
		condition: any,
		memory: Memory,
		namespace: string,
	): Promise<boolean> {
	...
	private async evaluateVolumeTrigger(
		condition: any,
		_memory: Memory,
		namespace: string,
	): Promise<boolean> {
		...
		const recentMemories = allMemories.filter(
			(m) => new Date(m.createdAt) >= since && condition.pattern.test(m.text),
		);
		...
		switch (action.type) {
			...
			case 'remind':
				if (action.message) {
					context.memory.metadata = {
						...context.memory.metadata,
						reminders: [...(context.memory.metadata?.reminders || []), action.message],
					};
				}
				break;
	...
	private async executeWorkflowChain(
		chain: WorkflowChain,
		memory: Memory,
		namespace: string,
	): Promise<void> {
