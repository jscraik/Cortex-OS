...
export class IntelligentMemoryStore implements MemoryStore {
	private readonly summaryCache = new Map<string, { result: SummaryResult; expires: number }>();
	private readonly keyPointsCache = new Map<string, { points: KeyPoint[]; expires: number }>();
	private readonly consolidationCandidates = new Map<string, Memory[]>();

	private readonly config: Required<IntelligentConfig>;
	...
	async extractKeyPointsFromMultiple(memoryIds: string[], namespace: string): Promise<KeyPoint[]> {
		...
		allKeyPoints.sort((a, b) => b.importance - a.importance);
		return allKeyPoints.slice(0, this.config.keyPointExtraction.maxPoints * memoryIds.length);
	}
	...
	private async createConsolidatedMemory(memories: Memory[]): Promise<Memory> {
		...
		const consolidatedMemory: Memory = {
			id: `consolidated_${Date.now()}_${randomUUID()}`,
			...
			createdAt: memories.length
				? memories.reduce(
						(oldest, m) => (new Date(m.createdAt) < new Date(oldest.createdAt) ? m : oldest),
						memories[0],
					).createdAt
	...
	private async generateInsightSuggestions(
		memories: Memory[],
		patterns: InsightsResult['patterns'],
	): Promise<string[]> {
