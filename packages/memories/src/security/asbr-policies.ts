...
export class ASBRSecurityPolicy {
	...
	sanitizeMemory(memory: Memory): Memory {
		...
		// Sanitize tags
		if (sanitized.tags) {
			sanitized.tags = sanitized.tags.map((tag) => tag.replace(/[<>"']/g, ''));
		}
	...
	canRead(memory: Memory, subject: string): boolean {
		...
		return memory.policy.read.includes(subject) || memory.policy.read.includes('*');
	}
	...
	canWrite(memory: Memory, subject: string): boolean {
		...
		return memory.policy.write.includes(subject) || memory.policy.write.includes('*');
	}
	...
	private validateMetadata(metadata: Record<string, unknown>): void {
		for (const [key, value] of Object.entries(metadata)) {
			...
			if (['__proto__', 'constructor', 'prototype'].includes(key)) {
				throw new MemoryValidationError(`Unsafe metadata key "${key}" detected`);
			}
