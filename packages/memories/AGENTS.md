...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/memories/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** *Neo4j graph storage, Qdrant vector search, MLX embeddings, and governed memory policies*
- **Responsibilities:**
  - Implement the Memories domain logic and exported APIs.
...
MEMORIES_BASE_URL=https://example.local
MEMORIES_DB_URL=postgres://user:pass@localhost:5432/memories
...
```bash
pnpm --filter memories test:coverage
pnpm security:scan --scope=memories
pnpm structure:validate --scope=memories
```
...
- SBOM: CycloneDX generated at `./sbom/memories.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"memories"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"memories","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter memories test:e2e
pnpm --filter memories test:smoke
pnpm --filter memories chaos:probe
```
...
# Dev & run
pnpm --filter memories dev
pnpm --filter memories start

# Build & verify
pnpm --filter memories build
pnpm --filter memories lint && pnpm --filter memories typecheck
pnpm --filter memories test && pnpm --filter memories test:coverage
...
pnpm --filter memories mcp:smoke
```
