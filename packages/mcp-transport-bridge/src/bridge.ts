...
export class McpBridge {
	...
	private async initializeTarget(): Promise<void> {
		...
		if (target.type === "stdio") {
			...
			await this.server.connect(transport);
			this.log(
				`📤 Exposed Streamable HTTP interface on http://${target.host}:${target.port}`,
			);
		}
	}
	...
	private async cleanup(): Promise<void> {
		...
		if (this.client) {
			cleanupTasks.push(
				this.client
					.close()
					.catch((err) => this.log(`⚠️  Error closing client: ${err.message}`)),
			);
		}
		...
		if (this.server) {
			cleanupTasks.push(
				this.server
					.close()
					.catch((err) => this.log(`⚠️  Error closing server: ${err.message}`)),
			);
		}
