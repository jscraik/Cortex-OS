...
program
	...
	.action(async (options) => {
		...
		try {
			...
			spinner.succeed(
				`Bridge started: ${chalk.blue(options.command)} → ${chalk.blue(url)}`,
			);
			console.log(chalk.gray("Bridge is running. Press Ctrl+C to stop."));
...
program
	...
	.action(async (options) => {
		...
		try {
			...
			const sourceDesc =
				config.source.type === "stdio"
					? `${config.source.command}`
					: config.source.url;
			const targetDesc =
				config.target.type === "stdio"
					? "stdio"
					: `${config.target.host}:${config.target.port}`;

			spinner.succeed(
				`Bridge started: ${chalk.blue(sourceDesc)} → ${chalk.blue(targetDesc)}`,
			);
			console.log(chalk.gray("Bridge is running. Press Ctrl+C to stop."));
...
program
	...
	.action(async (options) => {
		...
		try {
			...
			console.log(
				chalk.green(`✅ Example configuration written to ${options.output}`),
			);
