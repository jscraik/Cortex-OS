# AGENTS — Cortex Structure Github
...
```yaml
id: agents-cortex-structure-github-v1
package: Cortex Structure Github
type: package
runtime: node
...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/cortex-structure-github/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Production-ready repository structure analysis and enforcement GitHub App with automated violation detection and intelligent recommendations.
- **Responsibilities:**
  - Implement the Cortex Structure Github domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter cortex-structure-github dev

# Build
pnpm --filter cortex-structure-github build

# Quality (affected-aware if available)
pnpm --filter cortex-structure-github lint
pnpm --filter cortex-structure-github typecheck
pnpm --filter cortex-structure-github test
pnpm --filter cortex-structure-github test:coverage
```
...
CORTEX_STRUCTURE_GITHUB_BASE_URL=https://example.local
CORTEX_STRUCTURE_GITHUB_DB_URL=postgres://user:pass@localhost:5432/cortex_structure_github
...
```bash
pnpm --filter cortex-structure-github test:coverage
pnpm security:scan --scope=cortex-structure-github
pnpm structure:validate --scope=cortex-structure-github
```
...
- SBOM: CycloneDX generated at `./sbom/cortex-structure-github.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"cortex-structure-github"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"cortex-structure-github","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter cortex-structure-github test:e2e
pnpm --filter cortex-structure-github test:smoke
pnpm --filter cortex-structure-github chaos:probe
```
...
# Dev & run
pnpm --filter cortex-structure-github dev
pnpm --filter cortex-structure-github start

# Build & verify
pnpm --filter cortex-structure-github build
pnpm --filter cortex-structure-github lint && pnpm --filter cortex-structure-github typecheck
pnpm --filter cortex-structure-github test && pnpm --filter cortex-structure-github test:coverage
...
pnpm --filter cortex-structure-github mcp:smoke
```
...
```
packages/cortex-structure-github/
 ├─ src/                         # implementation
