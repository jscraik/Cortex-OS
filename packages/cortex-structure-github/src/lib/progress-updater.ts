...
export class LiveProgressUpdater {
  ...
  private generateProgressComment(progressState: ProgressState): string {
    ...
    const elapsed = endTime ? endTime.getTime() - startTime.getTime() : Date.now() - startTime.getTime();
    const elapsedSeconds = Math.round(elapsed / 1000);

    let comment = `@${user} **${taskType.toUpperCase()} Progress**\n\n`;

    // Status indicator
    const statusEmoji = status === 'completed' ? '✅' : status === 'error' ? '❌' : '⚙️';
    const statusText = status === 'completed' ? 'Completed' : status === 'error' ? 'Failed' : 'In Progress';
  ...
  private cleanupStaleProgress(): void {
    ...
    for (const [taskId, progress] of this.activeProgress.entries()) {
      ...
      if (taskAge > this.STALE_TASK_TIMEOUT) {
        ...
        console.log(`Cleaning up stale progress task: ${taskId} (age: ${Math.round(taskAge / 60000)}min)`);
      }
    }

    staleTasks.forEach(taskId => this.activeProgress.delete(taskId));
  }
