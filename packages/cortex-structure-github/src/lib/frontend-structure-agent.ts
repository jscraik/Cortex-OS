...
export async function analyzeFrontendStructure(
  ...
  const finalConfig: FrontendStructureConfig = {
    ...
    fileExtensions: config.fileExtensions || getDefaultExtensions(detectedFramework),
...
async function analyzeComponents(repoPath: string, config: FrontendStructureConfig) {
  const violations: FrontendViolation[] = [];
  const summary = {
    totalComponents: 0,
...
async function analyzeHooks(repoPath: string, config: FrontendStructureConfig) {
  const violations: FrontendViolation[] = [];
  const summary = {
    customHooks: 0,
...
async function analyzeRouting(repoPath: string, config: FrontendStructureConfig) {
  const violations: FrontendViolation[] = [];
  const summary = {
...
async function applyFrontendFix(repoPath: string, violation: FrontendViolation): Promise<void> {
