...
async function detectArchitecture(
  ...
  const hasControllers = directories.some((d) => d.includes('controller') || d.includes('controllers'));
  const hasServices = directories.some((d) => d.includes('service') || d.includes('services'));
  const hasModels = directories.some((d) => d.includes('model') || d.includes('models') || d.includes('entities'));
