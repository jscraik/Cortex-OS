...
export interface BackendViolation {
  type: 'controller' | 'service' | 'model' | 'middleware' | 'route' | 'config' | 'test' | 'database';
...
async function detectLanguage(repoPath: string): Promise<string> {
  try {
    ...
    const packageJsonExists = await import('fs').then(fs =>
      fs.promises.access(path.join(repoPath, 'package.json')).then(() => true).catch(() => false)
    );
    if (packageJsonExists) {
      const hasTs = await import('fs').then(fs =>
        fs.promises.access(path.join(repoPath, 'tsconfig.json')).then(() => true).catch(() => false)
      );
    ...
    for (const file of pythonFiles) {
      const exists = await import('fs').then(fs =>
        fs.promises.access(path.join(repoPath, file)).then(() => true).catch(() => false)
      );
      if (exists) return 'python';
    }

    // Check for Go
    const goModExists = await import('fs').then(fs =>
      fs.promises.access(path.join(repoPath, 'go.mod')).then(() => true).catch(() => false)
    );
    if (goModExists) return 'go';

    // Check for Rust
    const cargoTomlExists = await import('fs').then(fs =>
      fs.promises.access(path.join(repoPath, 'Cargo.toml')).then(() => true).catch(() => false)
    );
...
async function detectFramework(repoPath: string, language: string): Promise<string> {
  try {
    switch (language) {
      ...
      case 'go': {
        const goMod = await import('fs').then(fs =>
...
async function detectArchitecture(repoPath: string, framework: string): Promise<string> {
  // Check directory structure to infer architecture pattern
  const directories = await import('fs').then(fs =>
    fs.promises.readdir(repoPath, { withFileTypes: true })
...
async function analyzeBackendLayers(repoPath: string, config: BackendStructureConfig) {
  const violations: BackendViolation[] = [];
  const summary = {
    controllers: { count: 0, violations: 0, misplaced: [], oversized: [], missing: [] } as LayerInfo,
    services: { count: 0, violations: 0, misplaced: [], oversized: [], missing: [] } as LayerInfo,
    models: { count: 0, violations: 0, misplaced: [], oversized: [], missing: [] } as LayerInfo,
    middleware: { count: 0, violations: 0, misplaced: [], oversized: [], missing: [] } as LayerInfo,
    routes: { count: 0, violations: 0, misplaced: [], oversized: [], missing: [] } as LayerInfo,
  };
...
async function analyzeBackendSecurity(repoPath: string, config: BackendStructureConfig) {
  const violations: BackendViolation[] = [];
  const summary = {
...
async function analyzeTestCoverage(repoPath: string, config: BackendStructureConfig) {
  const violations: BackendViolation[] = [];
  const summary = {
    tested: [] as string[],
    untested: [] as string[],
...
function generateBackendRecommendations(
  ...
  const layerViolations = violations.filter(v => ['controller', 'service', 'model'].includes(v.type));
...
async function applyBackendFix(repoPath: string, violation: BackendViolation): Promise<void> {
