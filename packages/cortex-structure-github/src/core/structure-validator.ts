...
export class StructureValidator {
  ...
  validateFile(filePath: string): StructureViolation[] {
    ...
    for (const rule of this.rules) {
      ...
      // Check allowed paths
      const isInAllowedPath = rule.allowedPaths.some((path) => minimatch(filePath, path));
      ...
      if (rule.disallowedPaths) {
        const isInDisallowedPath = rule.disallowedPaths.some((path) => minimatch(filePath, path));
