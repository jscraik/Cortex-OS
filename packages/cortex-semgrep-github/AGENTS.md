...
```yaml
id: agents-cortex-semgrep-github-v1
package: Cortex Semgrep Github
type: package
runtime: node
...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/cortex-semgrep-github/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Production-ready security scanning GitHub App powered by Semgrep with comprehensive vulnerability detection and automated reporting.
- **Responsibilities:**
  - Implement the Cortex Semgrep Github domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter cortex-semgrep-github dev

# Build
pnpm --filter cortex-semgrep-github build

# Quality (affected-aware if available)
pnpm --filter cortex-semgrep-github lint
pnpm --filter cortex-semgrep-github typecheck
pnpm --filter cortex-semgrep-github test
pnpm --filter cortex-semgrep-github test:coverage
```
...
CORTEX_SEMGREP_GITHUB_BASE_URL=https://example.local
CORTEX_SEMGREP_GITHUB_DB_URL=postgres://user:pass@localhost:5432/cortex_semgrep_github
...
```bash
pnpm --filter cortex-semgrep-github test:coverage
pnpm security:scan --scope=cortex-semgrep-github
pnpm structure:validate --scope=cortex-semgrep-github
```
...
- SBOM: CycloneDX generated at `./sbom/cortex-semgrep-github.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"cortex-semgrep-github"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"cortex-semgrep-github","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter cortex-semgrep-github test:e2e
pnpm --filter cortex-semgrep-github test:smoke
pnpm --filter cortex-semgrep-github chaos:probe
```
...
# Dev & run
pnpm --filter cortex-semgrep-github dev
pnpm --filter cortex-semgrep-github start

# Build & verify
pnpm --filter cortex-semgrep-github build
pnpm --filter cortex-semgrep-github lint && pnpm --filter cortex-semgrep-github typecheck
pnpm --filter cortex-semgrep-github test && pnpm --filter cortex-semgrep-github test:coverage
...
pnpm --filter cortex-semgrep-github mcp:smoke
```
...
```
packages/cortex-semgrep-github/
 ├─ src/                         # implementation
