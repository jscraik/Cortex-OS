...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/mcp-server/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Minimal FastMCP v3 server exposing HTTP/SSE; loads the MCP registry; no business logic.
- **Responsibilities:**
  - Implement the MCP Server domain logic and exported APIs.
...
pnpm --filter mcp-server test:coverage
```
...
MCP_SERVER_BASE_URL=https://example.local
MCP_SERVER_DB_URL=postgres://user:pass@localhost:5432/mcp_server
...
```bash
pnpm --filter mcp-server test:coverage
pnpm security:scan --scope=mcp-server
pnpm structure:validate --scope=mcp-server
```
...
- SBOM: CycloneDX generated at `./sbom/mcp-server.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"mcp-server"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"mcp-server","level":"info","msg":"started","port":0}
```
...
pnpm --filter mcp-server test:smoke
pnpm --filter mcp-server chaos:probe
```
...
# Build & verify
pnpm --filter mcp-server build
pnpm --filter mcp-server lint && pnpm --filter mcp-server typecheck
pnpm --filter mcp-server test && pnpm --filter mcp-server test:coverage
...
pnpm --filter mcp-server mcp:smoke
```
