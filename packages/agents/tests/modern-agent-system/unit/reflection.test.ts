...
const buildReasoning = (
	strategy: PlannerReasoning['strategy'],
	alternatives = 0,
): PlannerReasoning => ({
	strategy,
...
describe('Reflection module', () => {
	...
	it('returns acceptance feedback when all steps succeed', async () => {
		...
		expect(outcome.feedback.improvements).toContain(
			'Leverage retrieved context to enhance final deliverable.',
		);
	...
	it('suggests retries and next goal when failures occur', async () => {
		...
		const execution = buildExecution(
			goal,
			[
				buildStep({ capability: 'plan' }),
				buildStep({ capability: 'deploy', status: 'failed', error: 'timeout' }),
			],
	...
	it('leverages tree-of-thought alternatives for improvements', async () => {
		...
		const plan = buildPlan(goal, planSteps);
		const execution = buildExecution(goal, planSteps, buildReasoning('tree-of-thought', 2));

		const outcome = await reflection.reflect({ goal, plan, lastResult: execution });

		expect(outcome.feedback.status).toBe('accepted');
		expect(
			outcome.feedback.improvements.some((msg) => msg.includes('Explore alternative branch')),
		).toBe(true);
	});
});
...
describe('FastMCP streaming compatibility', () => {
	it('buffers FastMCP async iterators without dropping events', async () => {
		...
		await manager.streamExecution(
			graph as any,
			{ currentStep: 'init' } as any,
			{
