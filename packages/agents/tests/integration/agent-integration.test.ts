...
describe('Agent Integration Tests', () => {
  ...
  describe('A2A Event Bus Integration', () => {
    it('should handle agent communication events', async () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('agent.task.started', 
        expect.objectContaining({ agentId: mockAgent.id, taskId: mockTask.id }));
      expect(eventBus.publish).toHaveBeenCalledWith('agent.task.completed', 
        expect.objectContaining({ agentId: mockAgent.id, taskId: mockTask.id }));
    });
    ...
    it('should handle agent coordination through events', async () => {
      ...
      expect(coordinator.eventBus.publish).toHaveBeenCalledWith('coordinator.task.assigned', 
    ...
    it('should handle error propagation through events', async () => {
      ...
      expect(eventBus.publish).toHaveBeenCalledWith('agent.task.error', 
  ...
  describe('MCP Bridge Integration', () => {
    it('should handle MCP tool integration', async () => {
      ...
      const mockMcpBridge = {
        tools: new Map([
          ['file-reader', { name: 'file-reader', execute: vi.fn().mockResolvedValue('file content') }],
          ['code-formatter', { name: 'code-formatter', execute: vi.fn().mockResolvedValue('formatted code') }]
        ]),
      ...
      const formattedResult = await mockMcpBridge.executeTool('code-formatter', { code: 'unformatted' });
    ...
    it('should handle MCP tool failures gracefully', async () => {
      ...
      await expect(mockMcpBridge.executeTool('failing-tool', {})).rejects.toThrow('Tool execution failed');
      await expect(mockMcpBridge.executeTool('nonexistent-tool', {})).rejects.toThrow('Tool nonexistent-tool not found');
    });
  });
  ...
  describe('End-to-End Workflows', () => {
    ...
    it('should handle workflow error recovery', async () => {
      const workflow = {
        steps: [
          { type: 'validate_input', handler: vi.fn().mockResolvedValue({ valid: true }) },
          { type: 'execute_agent', handler: vi.fn().mockRejectedValue(new Error('Execution failed')) },
        ...
        async execute(input: any) {
          const results = [];
          try {
            for (const step of this.steps.slice(0, 2)) { // Skip recovery step initially
              const result = await step.handler(input);
              results.push({ step: step.type, result });
            }
          } catch (error) {
