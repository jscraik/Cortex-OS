...
describe('CheckpointStore', () => {
    ...
    `);
		...
		db.exec(
			'CREATE INDEX IF NOT EXISTS idx_pending_writes_thread_task ON pending_writes (thread_id, task_id)',
		);
	...
	describe('getCheckpoint', () => {
		it('should retrieve checkpoint by ID', async () => {
			...
			expect(retrieved?.metadata.parentCheckpointId).toBe(
				mockCheckpoint.metadata.parentCheckpointId,
			);
		});
	...
	describe('getLatestCheckpoint', () => {
		it('should return most recent checkpoint for thread', async () => {
			...
			const _latestId = await checkpointStore.createCheckpoint(
				threadId,
	...
	describe('putWrites', () => {
		...
		it('should handle empty writes array', async () => {
			...
			const stmt = db.prepare(
				'SELECT writes FROM pending_writes WHERE thread_id = ? AND task_id = ?',
			);
			const stored = stmt.get(mockPendingWrite.threadId, mockPendingWrite.taskId);
	...
	describe('deleteCheckpoints', () => {
		it('should delete checkpoints older than specified date', async () => {
			...
			const oldId = await checkpointStore.createCheckpoint(
				threadId,
				oldCheckpoint,
				mockCheckpoint.metadata,
			);
