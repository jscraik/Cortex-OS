...
import { LogLevel, LoggerConfig, LogEntry, RotationConfig, RedactionConfig } from '../../src/logging/types';
...
describe('Logging Types', () => {
  ...
  describe('Type constraints', () => {
    it('should enforce required fields', () => {
      ...
      const invalidConfig1: LoggerConfig = {
        format: 'json',
      };

      // @ts-expect-error - Invalid log level
      const invalidConfig2: LoggerConfig = {
        level: 'invalid',
        format: 'json',
      };

      // @ts-expect-error - Invalid rotation type
      const invalidRotation: RotationConfig = {
        type: 'invalid',
        size: '10MB',
        maxFiles: 5,
      };
