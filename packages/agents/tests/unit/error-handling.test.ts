...
describe('Error Boundary Protection', () => {
	...
	describe('AgentError', () => {
		it('should create error with proper structure', () => {
			const error = new AgentError('Test error', ErrorCategory.VALIDATION, ErrorSeverity.HIGH);
		...
		it('should serialize to JSON correctly', () => {
			const error = new AgentError('Test error', ErrorCategory.SECURITY, ErrorSeverity.CRITICAL, {
				userId: '123',
			});
	...
	describe('ResourceManager', () => {
		...
		it('should handle cleanup errors gracefully', async () => {
			...
			expect(consoleErrorSpy).toHaveBeenCalledWith('Failed to cleanup failing:', expect.any(Error));
		});
