...
describe('Validation Utilities', () => {
  ...
  describe('validateSchema', () => {
    ...
    it('should throw ValidationError for invalid data', () => {
      ...
      expect(() => validateSchema(testSchema, invalidData, 'test-schema')).toThrow(ValidationError);
    });

    it('should include schema name in error message', () => {
      const invalidData = { name: 'John' }; // missing required age

      expect(() => validateSchema(testSchema, invalidData, 'user-schema')).toThrow(/user-schema/);
    });
  });
  ...
  describe('validateInput', () => {
    ...
    it('should throw ValidationError with capability context', () => {
      ...
      expect(() => validateInput(testSchema, invalidInput, 'code-analysis')).toThrow(
        /code-analysis-input/,
      );
    });
  });
  ...
  describe('validateOutput', () => {
    ...
    it('should throw ValidationError with capability context', () => {
      ...
      expect(() => validateOutput(testSchema, invalidOutput, 'test-generation')).toThrow(
  ...
  describe('parseAndValidateJSON', () => {
    ...
    it('should throw ValidationError for invalid JSON', () => {
      ...
      expect(() => parseAndValidateJSON(invalidJson, testSchema)).toThrow(ValidationError);
    });

    it('should throw ValidationError for malformed JSON', () => {
      const malformedJson = '{"name":"John","age":30'; // missing closing brace

      expect(() => parseAndValidateJSON(malformedJson, testSchema)).toThrow(ValidationError);
    });
  });
  ...
  describe('validateExecutionContext', () => {
    ...
    it('should throw for invalid model preference', () => {
      ...
      expect(() => validateExecutionContext(invalidContext)).toThrow(ValidationError);
    });
  ...
  describe('ValidationError', () => {
    it('should create ValidationError with correct properties', () => {
      const error = new ValidationError('Test message', 'test-schema', { field: 'error' });
