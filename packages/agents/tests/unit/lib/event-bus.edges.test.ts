...
import { cloudEventSchema, createEventBus } from '../../../src/lib/event-bus.js';
...
describe('event-bus edge cases', () => {
  it('coerces bare object events into envelope', async () => {
    ...
    bus.subscribe('edge.test', (e: Envelope) => { received = e; });
    await bus.publish({
      specversion: '1.0', type: 'edge.test', source: 'x', id: 'edge-1', time: new Date().toISOString(), headers: {}, ttlMs: 1,
    });
  ...
  it('publishes event with no subscribers without error', async () => {
    ...
    await bus.publish({
      specversion: '1.0', type: 'no.sub', source: 'edge', id: 'n1', time: new Date().toISOString(), headers: {}, ttlMs: 1,
      data: { info: 'none' },
    } as any);
