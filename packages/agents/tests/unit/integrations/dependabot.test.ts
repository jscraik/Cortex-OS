...
import { assessDependabotConfig, type DependabotConfig } from '../../../src/integrations/dependabot.js';
  ...
  it('scores configuration with mixed intervals', () => {
    const cfg: DependabotConfig = {
      ...
      projects: [
        ...
        { packageEcosystem: 'github-actions', directory: '/', scheduleInterval: 'monthly' },
      ],
    };
  ...
  it('handles configuration with no weak projects (all daily)', () => {
    const cfg: DependabotConfig = {
      ...
      projects: [
        ...
        { packageEcosystem: 'github-actions', directory: '/', scheduleInterval: 'daily' },
      ],
    };
    const res = assessDependabotConfig(cfg);
    expect(res.totalProjects).toBe(2);
