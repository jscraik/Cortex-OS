...
describe('maxTokens cap', () => {
  it('never exceeds 4096 across agents', async () => {
    ...
    const code = createCodeAnalysisAgent({ provider, eventBus: bus, mcpClient: mcp });
    await code.execute({
    ...
    const test = createTestGenerationAgent({ provider, eventBus: bus, mcpClient: mcp });
    await test.execute({
      sourceCode: 'x'.repeat(100000),
      language: 'javascript',
      testType: 'unit',
    ...
    const doc = createDocumentationAgent({ provider, eventBus: bus, mcpClient: mcp });
    await doc.execute({
      sourceCode: 'x'.repeat(100000),
      language: 'javascript',
    ...
    const sec = createSecurityAgent({ provider: secProv, eventBus: bus, mcpClient: mcp });
