...
function makeProvider(
	responder: (prompt: string) => string | Promise<string>,
): ModelProvider {
	return {
		id: 'mock-model',
		name: 'mock',
...
describe('security-agent coverage (branches)', () => {
	...
	it('heuristic decision escalates to block when raw text contains block keyword', async () => {
		...
		const provider = makeProvider(
			() => 'This should BLOCK due to policy violation',
		);
		const agent = createSecurityAgent({
			provider,
			eventBus: bus,
	...
	it('heuristic decision downgrades single block mention without violation phrase to flag', async () => {
		const bus = createEventBus();
		const provider = makeProvider(
			() => 'Consider whether to BLOCK this benign thing',
		);
		const agent = createSecurityAgent({
			provider,
			eventBus: bus,
