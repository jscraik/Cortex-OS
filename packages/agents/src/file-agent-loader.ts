...
export async function loadAgentTemplates(
	...
	if (errors.length > 0) {
		const details = errors
			.map((err) => `${err.filePath}: ${String(err.cause ?? err.message)}`)
			.join('; ');
		throw new AgentTemplateError(
			`brAInwav agent template validation failed: ${details}`,
			errors[0]?.filePath,
			errors,
		);
	}
...
async function listTemplateFiles(dir: string, pattern: string): Promise<string[]> {
	...
	return found.map((rel) => path.join(dir, rel)).sort((a, b) => a.localeCompare(b));
}
...
async function parseTemplate(filePath: string, scope: TemplateScope): Promise<AgentTemplate> {
	...
	try {
		...
		if (error instanceof ZodError) {
			throw new AgentTemplateError(
				`brAInwav agent template schema violation: ${error.message}`,
				filePath,
				error,
			);
		}
...
function buildConfigInput(args: {
	...
	const maxConcurrency = asInteger(
		data.maxConcurrency ?? (data as Record<string, unknown>)['max_concurrency'],
	);
