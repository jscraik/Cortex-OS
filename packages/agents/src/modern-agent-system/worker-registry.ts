import type { WorkerDefinition, WorkerRegistry } from './types.js';
...
const createIndexes = (definitions: WorkerDefinition[]) => {
	...
	for (const definition of definitions) {
		if (byName.has(definition.name)) {
			throw new Error(`brAInwav modern-agent-system: duplicate worker name "${definition.name}"`);
		}
...
const registerWorker = (
	...
	if (indexes.byName.has(definition.name)) {
		throw new Error(`brAInwav modern-agent-system: worker "${definition.name}" already registered`);
	}
