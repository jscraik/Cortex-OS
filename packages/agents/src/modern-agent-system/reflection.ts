...
const inferImprovements = (
        ...
        if (execution.reasoning.strategy === 'tree-of-thought' && execution.reasoning.alternatives?.length) {
...
const deriveNextGoal = (
        ...
        const updatedCapabilities = [failed.capability, ...goal.requiredCapabilities.filter((cap) => cap !== failed.capability)];
        return {
                ...goal,
