...
import { ToolAccessControl } from './types';
import { SubagentContext, SubagentResult, ISubagentRunner, DelegationRequest, DelegationResult } from './types';
...
export class SubagentRunner implements ISubagentRunner {
  ...
  async execute(context: SubagentContext): Promise<SubagentResult> {
    ...
    try {
      ...
      const modelRouter = createModelRouter({
        enableMLX: context.config.model_provider === 'mlx' || !context.config.model_provider,
        ollamaBaseUrl: context.config.model_provider === 'ollama' ?
          (context.config.model_config?.ollamaBaseUrl as string) : undefined,
      ...
      const result = await agent.execute({
        ...
        context: {
          isolationId: context.config.context_isolation ? context.id : undefined,
  ...
  private buildApiProvidersConfig(config: any) {
    ...
    } else if (config.model_provider === 'anthropic' && config.model_config?.apiKey) {
      providers.anthropic = {
        apiKey: config.model_config.apiKey,
        baseURL: config.model_config.baseURL,
      };
    } else if (config.model_provider === 'google' && config.model_config?.apiKey) {
      providers.google = {
        apiKey: config.model_config.apiKey,
        baseURL: config.model_config.baseURL,
      };
