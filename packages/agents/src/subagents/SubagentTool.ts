 ...
 */
export function materializeSubagentTool(
  ...
  const schema = z.object({
    ...
    }).optional().describe('Execution budget limits'),
  });
...
export function createAutoDelegateTool(
  ...
  const schema = z.object({
    ...
    }).optional().describe('Budget to split among subagents'),
  });
  ...
  return {
    ...
    async call(args, ctx?: { caller?: string; depth?: number }): Promise<any> {
      try {
        ...
        const selected = await selectSubagents?.(args.task, args.k) ||
        ...
        const perAgentBudget = args.budget ? {
          tokens: args.budget.tokens ? Math.floor((args.budget.tokens || 0) / selected.length) : undefined,
 ...
 */
export function createListSubagentsTool(
  subagents: Map<string, Subagent>
): Tool {
  return {
    name: 'agent.list',
 ...
 */
export function createSubagentHealthTool(
  subagents: Map<string, Subagent>
): Tool {
  const schema = z.object({
