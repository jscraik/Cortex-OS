...
import { DelegationRequest, SubagentConfig } from './types';
...
export class DelegationRouter {
  ...
  async route(message: string, context?: {
    ...
    const shouldDelegate = candidates.length > 0 &&
  ...
  async createDelegations(
    ...
    if (strategy === 'single') {
      ...
      const fanoutCount = Math.min(
        this.config.maxFanout,
        candidates.length
      );
  ...
  private async findCandidates(
    ...
    const candidates: Array<{ subagent: string; confidence: number; reason: string }> = [];
  ...
  private checkConditions(
    ...
    if (conditions.tools && conditions.tools.length > 0) {
      const hasTools = conditions.tools.every(tool =>
  ...
  addRule(rule: RoutingRule): void {
    ...
    logger.info(`Added routing rule for: ${Array.isArray(rule.targets) ? rule.targets.join(', ') : rule.targets}`);
  }
