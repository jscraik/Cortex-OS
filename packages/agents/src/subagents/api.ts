...
function adaptSubagent(
	...
	return {
		...
		async getHealth(): Promise<HealthStatus> {
			if (
				typeof (instance as { getHealth?: () => Promise<HealthStatus> }).getHealth === 'function'
			) {
...
function resolveRunner(
	...
	if (
		typeof (instance as { executeWithValidation?: (input: unknown) => Promise<unknown> })
			.executeWithValidation === 'function'
	) {
		return async (input: SubagentRunInput) =>
			await (
				instance as { executeWithValidation: (input: unknown) => Promise<unknown> }
			).executeWithValidation(input);
	}
