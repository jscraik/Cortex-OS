...
import { ISubagentRegistry, SubagentConfig, SubagentEvents } from './types';
import { SubagentToolFactory } from './tools';

const logger = createLogger('SubagentRegistry');

export class SubagentRegistry extends EventEmitter implements ISubagentRegistry {
  ...
  async reload(loader: any): Promise<void> {
    ...
    const toRemove = Array.from(current).filter(name => !loadedNames.has(name));
