...
export class MemoryMCPClient {
	...
	async search(params: unknown): Promise<any> {
		try {
			...
			const filtered = input.kind ? results.filter((r) => r.kind === input.kind) : results;
	...
	async get(params: unknown): Promise<any> {
		try {
			...
			throw new Error(
				`Memory get failed: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
	...
	async stats(params: unknown): Promise<any> {
		try {
			...
			throw new Error(
				`Memory stats failed: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
}
