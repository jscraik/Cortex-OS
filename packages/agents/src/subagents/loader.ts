...
import { SubagentConfig, SubagentConfigSchema, YamlSubagentFile, MarkdownSubagentFile } from './types';
...
export class SubagentLoader {
      ...
      try {
        ...
        for (const [name, config] of configs) {
          if (subagents.has(name)) {
            logger.warn(`Duplicate subagent name: ${name}. Using version from ${resolvedPath}`);
          }
   ...
   */
  private async loadFromDirectory(dirPath: string): Promise<Map<string, SubagentConfig>> {
    const subagents = new Map<string, SubagentConfig>();
  ...
  private parseMarkdownFile(content: string, filePath: string): SubagentConfig {
    try {
      ...
      logger.debug(`Loaded Markdown subagent: ${validated.name} from ${filePath}`);
      return validated;
    } catch (error) {
   ...
   */
  async watch(callback: (event: 'add' | 'change' | 'unlink', path: string) => void): Promise<fs.FSWatcher> {
