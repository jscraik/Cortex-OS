...
export const loadDependabotConfig = async (cwd: string = process.cwd(), path?: string): Promise<DependabotConfig | null> => {
  ...
  try {
    ...
    try {
      ...
      parsed = (mod as any).parse ? (mod as any).parse(raw) : (mod as any).default.parse(raw);
    } catch {
...
export const assessDependabotConfig = (cfg: DependabotConfig): DependabotAssessment => {
  ...
  for (const p of projects) {
    ...
    if (['npm', 'pnpm', 'yarn', 'npm_and_yarn'].includes(p.packageEcosystem)) hasJsEcosystem = true;
    if (!p.scheduleInterval || interval === 'monthly' || interval === '') weakProjects.push(p);
  }
