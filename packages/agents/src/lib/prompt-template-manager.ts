...
export class PromptTemplateManager {
  ...
  private readonly usageHistory: Map<string, Array<{
  ...
  recordUsage(templateId: string, context: PromptContext, effectiveness: number): void {
    ...
    console.log(`brAInwav Prompt Manager: Recorded usage for template ${templateId} with effectiveness ${effectiveness}`);
  }
      ...
      examples: [
        {
          ...
          expectedBehavior: 'Begin with explicit planning phase, use workspace tools for organization, break down into analysis/strategy/execution phases'
        }
      ],
      variables: ['capabilities', 'tools', 'taskId', 'complexity', 'priority', 'currentPhase', 'taskDescription'],
      ...
      examples: [
        {
          ...
          expectedBehavior: 'Systematic file reading, security pattern matching, organized reporting in workspace'
        }
      ],
      ...
      examples: [
        {
          ...
          expectedBehavior: 'Create structured plan with dependencies, resource allocation, and agent assignments'
        }
      ],
      variables: ['tools', 'complexity', 'agentCount', 'currentPhase', 'sessionId', 'taskDescription'],
      brainwavBranding: true,
      nOOptimized: true
    });
...
Apply brAInwav's commitment to reliable, resilient operation.`,
      ...
      variables: ['errorType', 'errorSeverity', 'currentPhase', 'affectedComponents', 'tools', 'errorDetails'],
      brainwavBranding: true,
      nOOptimized: true
    });
  ...
  private findCandidateTemplates(context: PromptContext): PromptTemplate[] {
    ...
    for (const template of this.templates.values()) {
      ...
      if (context.complexity >= template.complexity[0] &&
  ...
  private scoreTemplate(template: PromptTemplate, context: PromptContext): number {
    ...
    if (history && history.length > 0) {
      ...
      const avgEffectiveness = recentHistory.reduce((sum, use) => sum + use.effectiveness, 0) / recentHistory.length;
      score += avgEffectiveness * 0.3;
    }

    // Capability match
    const capabilityMatch = template.variables.includes('capabilities') && context.capabilities.length > 0;
    if (capabilityMatch) {
      score += 0.1;
    }
  ...
  private generateReasoningForSelection(template: PromptTemplate, context: PromptContext): string {
    return `brAInwav Template Manager: Selected "${template.name}" for complexity ${context.complexity}, ` +
  ...
  private getVariableValue(variable: string, context: PromptContext): string {
    switch (variable) {
      case 'taskId': return context.taskId;
      case 'agentId': return context.agentId;
      case 'sessionId': return context.sessionId || 'unknown';
      case 'complexity': return context.complexity.toString();
      case 'priority': return context.priority.toString();
      case 'capabilities': return context.capabilities.join(', ');
      case 'tools': return context.tools.join(', ');
      case 'currentPhase': return context.currentPhase || 'initialization';
      case 'agentCount': return '1'; // Default for single agent
      case 'taskDescription': return 'Task description not provided';
      case 'errorType': return 'unknown_error';
      case 'errorSeverity': return 'moderate';
      case 'affectedComponents': return 'unknown';
      case 'errorDetails': return 'Error details not available';
  ...
  private addBrainwavBranding(prompt: string, context: PromptContext): string {
    ...
    const branding = '\n**Powered by brAInwav** | Task: ' + context.taskId + ' | nO Architecture: ' + context.nOArchitecture + '\n';
  ...
  getStats(): {
    ...
    const nOOptimizedTemplates = Array.from(this.templates.values()).filter(t => t.nOOptimized).length;
