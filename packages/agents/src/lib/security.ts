...
export class InputSanitizer {
	...
	sanitizeString(input: string): SanitizationResult {
		...
		if (
			...
			(this.config.enableXSSProtection ||
				this.config.enableSQLInjectionProtection ||
				this.config.enableCommandInjectionProtection)
		) {
			result.blocked = true;
		}
 ...
 */
export { COMMAND_INJECTION_PATTERNS, SENSITIVE_PATTERNS, SQL_INJECTION_PATTERNS, XSS_PATTERNS };
