...
export class MetricsCollector extends EventEmitter {
     ...
     */
    getMetrics(): Metric[] {
        return this.metrics.keys().map(key => this.metrics.get(key)!).filter(Boolean);
    }
    ...
    private recordMetric(metric: Metric): void {
        ...
        this.metrics.set(
            `${metric.name}-${metric.timestamp}-${Math.random()}`,
            metric
        );
...
export class TracingSystem extends EventEmitter {
    ...
    getTrace(traceId: string): TraceSpan[] {
        const activeSpans = this.activeSpans.keys().map(key => this.activeSpans.get(key)!).filter(Boolean);
        const completedSpans = this.completedSpans.keys().map(key => this.completedSpans.get(key)!).filter(Boolean);
    ...
    getActiveSpans(): TraceSpan[] {
        return this.activeSpans.keys().map(key => this.activeSpans.get(key)!).filter(Boolean);
    }
    ...
    getCompletedSpans(): TraceSpan[] {
        return this.completedSpans.keys().map(key => this.completedSpans.get(key)!).filter(Boolean);
    }
    ...
    private addBrandingPrefix(operationName: string): string {
        return operationName.startsWith('brAInwav.')
            ? operationName
            : `brAInwav.${operationName}`;
    }
    ...
    private toJaegerFormat(spans: TraceSpan[]): unknown[] {
        const groupedByTrace = spans.reduce((acc, span) => {
            if (!acc[span.traceId]) {
                acc[span.traceId] = [];
            }
    ...
    traced<T extends (...args: unknown[]) => Promise<unknown>>(
        ...
        return (async (...args: unknown[]) => {
            ...
            try {
                ...
                this.metrics.histogram('brAInwav.operation.duration',
