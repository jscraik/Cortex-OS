...
export class MemoryBoundedStore<T = unknown> {
    ...
    cleanup(): void {
        ...
        if (this.options.enableMetrics && evictedCount > 0) {
            console.log(`🧹 Memory cleanup: evicted ${evictedCount} entries, current size: ${this.store.size}`);
        }
    }
    ...
    private calculateMissRate(): number {
        if (this.metrics.totalAccesses === 0) return 0;
        return (this.metrics.totalAccesses - this.metrics.hitRate * this.metrics.totalAccesses) / this.metrics.totalAccesses;
    }
}
...
export class GlobalMemoryManager {
    ...
    private performGlobalCleanup(): void {
        ...
        console.log(`  📊 Before cleanup: ${metrics.totalEntries} entries, ${metrics.totalMemoryUsageMB.toFixed(2)}MB`);
        ...
        console.log(`  ✅ After cleanup: ${afterMetrics.totalEntries} entries, ${afterMetrics.totalMemoryUsageMB.toFixed(2)}MB`);
    }
