...
export class HealthCheck extends EventEmitter {
    ...
    async execute(): Promise<HealthCheckResult> {
        ...
        try {
            ...
            // Update metrics
            observability.metrics.histogram('brAInwav.healthcheck.duration',
                result.duration,
                undefined,
                'ms',
    ...
    startPeriodicChecks(): void {
        ...
        console.log(`🏥 brAInwav health check ${this.config.name} started (interval: ${this.config.interval}ms)`);
    }
    ...
    private validateBranding(result: HealthCheckResult): boolean {
        ...
        const hasNameBranding = brandingPatterns.some(pattern =>
            pattern.test(result.name)
        );

        const hasMessageBranding = result.message ? brandingPatterns.some(pattern =>
...
export class HealthMonitor extends EventEmitter {
    ...
    registerCheck(config: HealthCheckConfig): void {
        ...
        console.log(`📋 brAInwav health check '${config.name}' registered (criticality: ${config.criticality})`);
    }
    ...
    async executeAllChecks(): Promise<HealthSummary> {
        ...
        try {
            ...
            if (this.config.enableMetrics) {
                observability.metrics.histogram('brAInwav.health.check_duration',
                    performance.now() - startTime,
                    undefined,
                    'ms'
                );

                observability.metrics.gauge('brAInwav.health.overall_status',
    ...
    getHealthHistory(limit?: number): HealthCheckResult[] {
        const history = this.healthHistory.keys().map(key => this.healthHistory.get(key)!).filter(Boolean);
    ...
    private startMonitoring(): void {
        if (this.config.healthCheckInterval > 0) {
            ...
            console.log(`🏥 brAInwav health monitoring started (interval: ${this.config.healthCheckInterval}ms)`);
        }
    }
    ...
    private generateHealthSummary(results: HealthCheckResult[]): HealthSummary {
        ...
        const importantUnhealthy = results.some(r =>
            r.status === 'unhealthy' &&
