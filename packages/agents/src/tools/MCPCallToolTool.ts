    ...
    try {
      ...
      if (serverInfo.status !== 'connected') {
        if (params.autoConnect) {
          ...
          if (!connected) {
            throw new Error(`Failed to connect to MCP server: ${params.serverName}`);
          }
        } else {
      ...
      const toolSchema = await client.getToolSchema(params.serverName, params.toolName);
      if (!toolSchema) {
        throw new Error(`Tool not found: ${params.toolName} on server ${params.serverName}`);
      }

      // Validate parameters against schema if provided
      if (toolSchema.inputSchema) {
        const validationResult = validateParameters(params.parameters || {}, toolSchema.inputSchema);
