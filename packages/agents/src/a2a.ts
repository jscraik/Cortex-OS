...
export function createAgentsBus(config: AgentsBusConfig = {}) {
  ...
  return {
    ...
    // Subscribe to events
    onAgentCreated(
      handler: (data: z.infer<typeof AgentCreatedSchema>) => void,
    ) {
      return bus.subscribe('agents.agent_created', handler);
    },

    onTaskStarted(
      handler: (data: z.infer<typeof AgentTaskStartedSchema>) => void,
    ) {
      return bus.subscribe('agents.task_started', handler);
    },

    onTaskCompleted(
      handler: (data: z.infer<typeof AgentTaskCompletedSchema>) => void,
    ) {
      return bus.subscribe('agents.task_completed', handler);
    },

    onCommunication(
      handler: (data: z.infer<typeof AgentCommunicationSchema>) => void,
    ) {
