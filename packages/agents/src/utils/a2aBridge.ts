...
export function createA2ABridge(_config?: A2ABridgeConfig) {
	...
	return {
		...
		async unsubscribe(
			...
			const index = subs.findIndex(
				(_, i) => generateEventId(`${eventType}-${i}`) === subscriptionId,
			);
		...
		async sendToAgent(
			...
			if (agent.endpoint) {
				// TODO: Implement HTTP call to agent endpoint
				logger.warn(`HTTP endpoint not implemented: ${agent.endpoint}`);
			}
		...
		getStats() {
			return {
				totalEvents: Array.from(eventBus.values()).reduce(
