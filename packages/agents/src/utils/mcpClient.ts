...
export class MCPClient {
  ...
  async callTool(
    ...
    try {
      ...
      logger.info(`Tool call completed: ${serverName}.${toolName} (${executionTime}ms)`);
   ...
   */
  async getServerInfo(serverName: string, detailed = false): Promise<ServerInfo> {
    const server = this.servers.get(serverName);
    if (!server) {
      return {
        name: serverName,
   ...
   * List all servers
   */
  async listServers(status: 'all' | 'connected' | 'disconnected' = 'all'): Promise<ServerInfo[]> {
   ...
   * Get tool schema
   */
  async getToolSchema(serverName: string, toolName: string): Promise<ToolInfo | null> {
    const server = this.servers.get(serverName);
    if (!server || server.status !== 'connected') {
      return null;
    }
  ...
  private async executeTool(_serverName: string, toolName: string, parameters: Record<string, any>): Promise<any> {
