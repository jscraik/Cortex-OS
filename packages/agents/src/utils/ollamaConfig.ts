...
export async function loadOllamaConfig(configPath?: string): Promise<OllamaConfig> {
  if (cachedConfig) {
    return cachedConfig;
  }
...
export function getOllamaModelByName(modelName: string, config: OllamaConfig): OllamaModelConfig | null {
...
export function getRecommendedOllamaModelForTask(taskType: string, config: OllamaConfig): string {
...
export function getOllamaBaseUrl(config?: OllamaConfig): string {
  return config?.service_configuration.ollama_endpoint || 'http://localhost:11434';
}