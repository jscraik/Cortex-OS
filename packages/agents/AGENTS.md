...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/agents/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** This package implements Cortex-OS’s master-agent coordination using LangGraphJS,
- **Responsibilities:**
  - Implement the Agents domain logic and exported APIs.
...
AGENTS_BASE_URL=https://example.local
AGENTS_DB_URL=postgres://user:pass@localhost:5432/agents
...
```bash
pnpm --filter agents test:coverage
pnpm security:scan --scope=agents
pnpm structure:validate --scope=agents
```
...
- SBOM: CycloneDX generated at `./sbom/agents.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"agents"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"agents","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter agents test:e2e
pnpm --filter agents test:smoke
pnpm --filter agents chaos:probe
```
...
# Build & verify
pnpm --filter agents build
pnpm --filter agents lint && pnpm --filter agents typecheck
pnpm --filter agents test && pnpm --filter agents test:coverage
