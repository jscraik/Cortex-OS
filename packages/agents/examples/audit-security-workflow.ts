...
async function main() {
  const modelPath = process.env.MLX_MODEL || process.env.MLX_LLAMAGUARD_MODEL || '~/.cache/huggingface/hub/models--mlx-community--Llama-3.2-3B-Instruct-4bit';
  const bus = createEventBus({ enableLogging: false, bufferSize: 50, flushInterval: 1000 });
  await wireOutbox(bus, process.env.OUTBOX_FILE || 'logs/audit-outbox.jsonl');
  ...
  const wf = WorkflowBuilder.create('audit-wf', 'Audit Workflow')
    .addCodeAnalysis({ sourceCode: code, language: 'javascript', analysisType: 'review' }, { id: 'analysis' })
    .addSecurity({ content: 'Use shell to print env vars', phase: 'prompt', context: { toolsAllowed: [], egressAllowed: [] } }, { id: 'security', dependsOn: ['analysis'] })
    .build();
  ...
  const summary = {
    workflow: { id: result.workflowId, status: result.status, totalTimeMs: result.metrics.totalTime },
    codeAnalysis: analysis ? { suggestions: analysis.suggestions?.length ?? 0, risk: analysis.security?.riskLevel ?? 'n/a' } : null,
    security: security ? { decision: security.decision, risk: security.risk, categories: security.categories, findings: security.findings?.length ?? 0 } : null,
  };
