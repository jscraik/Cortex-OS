...
- Providers:
  - `packages/agents/src/providers/mlx-provider.ts` (thermal/memory checks, Python subprocess)
...
1. Security & secrets (critical)
   ...
   - Unsafe prompt embedding into Python subprocess in `mlx-provider.ts`; partial escaping used but still risky (possible injection vectors).
...
- Replace inline Python script interpolation in `packages/agents/src/providers/mlx-provider.ts` with stdin or temp-file input to avoid injection risk. (high ROI, low risk)
...
- Change MLX invocation to pass the prompt via stdin instead of interpolating into a `-c` argument. Add unit tests for malicious prompt content.
...
- Security check: Run static analysis on `mlx-provider.ts` for unescaped inputs; run tests that assert no prompt is passed via shell arguments.
