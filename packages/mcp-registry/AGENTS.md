...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/mcp-registry/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** File-system backed registry for managing MCP server configurations with full Model Context Protocol integration.
- **Responsibilities:**
  - Implement the MCP Registry domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter mcp-registry dev

# Build
pnpm --filter mcp-registry build

# Quality (affected-aware if available)
pnpm --filter mcp-registry lint
pnpm --filter mcp-registry typecheck
pnpm --filter mcp-registry test
...
MCP_REGISTRY_BASE_URL=https://example.local
MCP_REGISTRY_DB_URL=postgres://user:pass@localhost:5432/mcp_registry
...
```bash
pnpm --filter mcp-registry test:coverage
pnpm security:scan --scope=mcp-registry
pnpm structure:validate --scope=mcp-registry
```
...
- SBOM: CycloneDX generated at `./sbom/mcp-registry.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"mcp-registry"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"mcp-registry","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter mcp-registry test:e2e
pnpm --filter mcp-registry test:smoke
pnpm --filter mcp-registry chaos:probe
```
...
# Dev & run
pnpm --filter mcp-registry dev
pnpm --filter mcp-registry start

# Build & verify
pnpm --filter mcp-registry build
pnpm --filter mcp-registry lint && pnpm --filter mcp-registry typecheck
pnpm --filter mcp-registry test && pnpm --filter mcp-registry test:coverage
...
pnpm --filter mcp-registry mcp:smoke
```
