...
import registrySchema from "../schemas/registry.schema.json" with {
	type: "json",
};
import serverManifestSchema from "../schemas/server-manifest.schema.json" with {
	type: "json",
};
...
export class McpValidator {
	...
	validateRegistry(registry: unknown) {
		return validateRegistryLib(this.ajv, registry, (m) =>
			validateServerManifestLib(this.ajv, m),
		);
	}
...
export const validateRegistry = (registry: unknown) =>
	validator.validateRegistry(registry);
export const validateSecurity = (manifest: unknown) =>
	validator.validateSecurity(manifest);
