...
function normalizeStartCommand(server: McpMarketServer): ServerInfo {
        ...
        if (type === 'http' || type === 'https' || type === 'sse' || type === 'ws' || type === 'streamablehttp') {
...
async function sendMarketplaceRequest(url: string, slug: string, signal?: AbortSignal): Promise<Response> {
        try {
