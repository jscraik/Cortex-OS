...
export function validateServerManifest(
	...
	if (typeof manifest === "object" && manifest !== null) {
		...
		if (
			m.oauth?.authType === "oauth2" &&
			(!m.oauth.scopes || m.oauth.scopes.length === 0)
		) {
			warnings.push({
				path: "oauth.scopes",
		...
		if (!m.repo) {
			warnings.push({
				...
				suggestion:
					"Adding a repository URL builds trust and allows verification",
			});
		}

		if (m.scopes?.includes("system:exec")) {
			warnings.push({
				path: "scopes",
				message: "Server requests system execution permissions",
				suggestion:
					"Consider if system:exec is necessary; it poses security risks",
			});
		}
