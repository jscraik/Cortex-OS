...
export function validateSecurity(manifest: unknown): ValidationResult {
		...
		for (const scope of m.scopes) {
			if (dangerousScopes.includes(scope)) {
				warnings.push({
					...
					suggestion:
						"Ensure this scope is absolutely necessary and well-documented",
				});
			}
		}
	}
	...
	if (m.transports) {
		["sse", "streamableHttp"].forEach((transport) => {
			if (
				m.transports[transport]?.url &&
				!m.transports[transport].url.startsWith("https://")
			) {
				errors.push({
	...
	if (m.license && !openSourceLicenses.includes(m.license)) {
		warnings.push({
			...
			suggestion:
				"Consider using a standard open-source license for better adoption",
		});
	}
