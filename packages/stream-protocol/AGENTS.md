...
```yaml
id: agents-stream-protocol-v1
package: Stream Protocol
type: package
runtime: node
...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/stream-protocol/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Server-side transport contract for Cortex-OS streaming. Provides a dual-buffer event dispatcher plus adapters for Server-Sent Events and WebSockets so UI and tooling receive con...
- **Responsibilities:**
  - Implement the Stream Protocol domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter stream-protocol dev

# Build
pnpm --filter stream-protocol build

# Quality (affected-aware if available)
pnpm --filter stream-protocol lint
pnpm --filter stream-protocol typecheck
pnpm --filter stream-protocol test
pnpm --filter stream-protocol test:coverage
```
...
STREAM_PROTOCOL_BASE_URL=https://example.local
STREAM_PROTOCOL_DB_URL=postgres://user:pass@localhost:5432/stream_protocol
...
```bash
pnpm --filter stream-protocol test:coverage
pnpm security:scan --scope=stream-protocol
pnpm structure:validate --scope=stream-protocol
```
...
- SBOM: CycloneDX generated at `./sbom/stream-protocol.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"stream-protocol"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"stream-protocol","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter stream-protocol test:e2e
pnpm --filter stream-protocol test:smoke
pnpm --filter stream-protocol chaos:probe
```
...
# Dev & run
pnpm --filter stream-protocol dev
pnpm --filter stream-protocol start

# Build & verify
pnpm --filter stream-protocol build
pnpm --filter stream-protocol lint && pnpm --filter stream-protocol typecheck
pnpm --filter stream-protocol test && pnpm --filter stream-protocol test:coverage
...
pnpm --filter stream-protocol mcp:smoke
```
