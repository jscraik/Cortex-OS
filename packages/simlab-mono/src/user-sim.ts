...
export class UserSimulator {
  ...
  private isHelpful(response: string): boolean {
    const helpfulIndicators = ['here is', 'i can help', 'solution', 'answer', 'information'];

    return helpfulIndicators.some((indicator) => response.toLowerCase().includes(indicator));
  }
  ...
  private needsClarification(response: string): boolean {
    ...
    return clarificationIndicators.some((indicator) => response.toLowerCase().includes(indicator));
  }
