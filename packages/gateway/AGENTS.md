...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/gateway/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Fastify-based HTTP gateway exposing Cortex agent endpoints (`/mcp`, `/a2a`, `/rag`, `/simlab`).
- **Responsibilities:**
  - Implement the Gateway domain logic and exported APIs.
...
pnpm --filter gateway test:coverage
```
...
GATEWAY_BASE_URL=https://example.local
GATEWAY_DB_URL=postgres://user:pass@localhost:5432/gateway
...
```bash
pnpm --filter gateway test:coverage
pnpm security:scan --scope=gateway
pnpm structure:validate --scope=gateway
```
...
- SBOM: CycloneDX generated at `./sbom/gateway.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"gateway"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"gateway","level":"info","msg":"started","port":0}
```
...
pnpm --filter gateway test:smoke
pnpm --filter gateway chaos:probe
```
...
# Build & verify
pnpm --filter gateway build
pnpm --filter gateway lint && pnpm --filter gateway typecheck
pnpm --filter gateway test && pnpm --filter gateway test:coverage
