...
function parseMcpArgs(raw?: string): unknown {
  ...
  try {
    ...
    console.warn('Failed to parse MCP_ARGS:', error instanceof Error ? error.message : 'Unknown error');
    return undefined;
  }
}
...
function buildHttpLikeServerInfo(name: string, transport: 'sse' | 'streamableHttp'): ServerInfo | null {
...
function getMCPServerInfo(): ServerInfo | null {
  ...
  if (transport === 'sse' || transport === 'streamableHttp') return buildHttpLikeServerInfo(name, transport);
  return null;
}
