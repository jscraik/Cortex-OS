...
describe("MCP Protocol Conformance Tests", () => {
		...
		transports.forEach((transportConfig) => {
			describe(`${transportConfig.type.toUpperCase()} Transport`, () => {
				it("should conform to JSON-RPC 2.0 message format", async () => {
					const _transport = createTransport(transportConfig);
	...
	describe("Capability Discovery Tests", () => {
		...
		it("should expose prompts capability correctly", async () => {
			...
			server.addPrompt(
				...
				async () => [{ role: "user", content: { type: "text", text: "test" } }],
			);
	...
	describe("Security Validation Tests", () => {
		it("should validate API key format", () => {
			...
			const _invalidKeys = ["password", "123456", "secret", "admin", "test"];
		...
		it("rejects invalid API key formats", () => {
			...
			invalidKeys.forEach((key) => {
				expect(validateApiKey(key), `Key should be invalid: ${key}`).toBe(
					false,
				);
			});
		});
	...
	describe("Tool Safety Tests", () => {
		...
		it("should validate tool input schemas", async () => {
			...
			expect((validResponse.result as any).result.processed).toBe(
				"hello world",
			);
		...
		it("should prevent dangerous tool operations", () => {
			...
			dangerousOperations.forEach(({ command, safe }) => {
				const isDangerous =
					/^(rm\s+.*-r|del\s+.*\/F|format\s+c:|dd\s+if=.*of=|sudo\s+.*|su\s+.*)/i.test(
						command,
					);
		...
		it("should validate server configurations", () => {
			...
			validConfigs.forEach((config, index) => {
				expect(
					...
					`Valid config ${index}: name not empty`,
				).toBe(true);
				expect(
			...
			invalidConfigs.forEach((config, index) => {
				const hasValidName =
					config.name.length > 0 && /^[a-zA-Z0-9_-]+$/.test(config.name);
				const hasValidTransport = ["stdio", "http"].includes(
					config.transport.type,
				);
				const hasRequiredFields =
					(config.transport.type === "stdio" &&
						"command" in config.transport) ||
					(config.transport.type === "http" && "url" in config.transport);

				const isValid = hasValidName && hasValidTransport && hasRequiredFields;
				expect(isValid, `Invalid config ${index}: should be invalid`).toBe(
					false,
				);
			});
		});
