...
const dangerousCommand = /^(rm\s|sudo\s|curl\s.+\|\s*sh|wget\s.+\|\s*bash|del\s|shutdown\s|reboot\s|format\s)/i;
...
export const stdioSchema = z
  .object({
    ...
    command: z
      ...
      .refine((cmd) => !unsafeMetacharacters.test(cmd), { message: 'Command contains shell metacharacters' })
