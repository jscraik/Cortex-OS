...
describe('MCP Protocol Compliance', () => {
  describe('Core Primitives Validation', () => {
    describe('Tools (AI Actions)', () => {
      it('should validate a compliant MCP tool definition', () => {
        ...
        expect(validated.inputSchema.required).toEqual(['origin', 'destination', 'date']);
      });
      ...
      it('should validate tool schema follows JSON Schema format', () => {
        ...
        expect(MCPProtocolCompliance.validateToolImplementation(schema)).toBe(true);
      });
    });
  ...
  describe('Protocol Version Negotiation', () => {
    ...
    it('should accept compatible versions within 1 year', () => {
      ...
      compatibleVersions.forEach((version) => {
        expect(MCPProtocolCompliance.checkVersionCompatibility(version)).toBe(true);
      });
    });
    ...
    it('should reject incompatible or malformed versions', () => {
      ...
      incompatibleVersions.forEach((version) => {
        expect(MCPProtocolCompliance.checkVersionCompatibility(version)).toBe(false);
      });
    });
  });
  ...
  describe('Error Handling', () => {
    it('should provide meaningful error messages', () => {
      expect(() => validateMCPTool({ name: '' })).toThrow(/Tool name is required/);
      expect(() => validateMCPResource({ uri: '' })).toThrow(/Resource URI is required/);
      expect(() => validateMCPPrompt({ name: '' })).toThrow(/Prompt name is required/);
    });
