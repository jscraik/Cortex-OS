...
class ListRepositoriesTool implements GitHubTool {
  ...
  async execute(args: Record<string, any>): Promise<ToolResult> {
    ...
    try {
      ...
      const { data: repositories } = await octokit.rest.repos.listForAuthenticatedUser(params);
...
class CreateRepositoryTool implements GitHubTool {
  ...
  async execute(args: Record<string, any>): Promise<ToolResult> {
    ...
    try {
      ...
      const { data: repository } = await octokit.rest.repos.createForAuthenticatedUser(params);
