...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/mcp/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for MCP within Cortex-OS.
- **Responsibilities:**
  - Implement the MCP domain logic and exported APIs.
...
MCP_BASE_URL=https://example.local
MCP_DB_URL=postgres://user:pass@localhost:5432/mcp
...
```bash
pnpm --filter mcp test:coverage
pnpm security:scan --scope=mcp
pnpm structure:validate --scope=mcp
```
...
- SBOM: CycloneDX generated at `./sbom/mcp.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"mcp"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"mcp","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter mcp test:e2e
pnpm --filter mcp test:smoke
pnpm --filter mcp chaos:probe
```
...
# Build & verify
pnpm --filter mcp build
pnpm --filter mcp lint && pnpm --filter mcp typecheck
pnpm --filter mcp test && pnpm --filter mcp test:coverage
