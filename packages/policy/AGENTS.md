...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/policy/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Standalone policy engine for Cortex-OS. Exposes:
- **Responsibilities:**
  - Implement the Policy domain logic and exported APIs.
...
pnpm --filter policy lint
pnpm --filter policy typecheck
pnpm --filter policy test
pnpm --filter policy test:coverage
```
...
POLICY_BASE_URL=https://example.local
POLICY_DB_URL=postgres://user:pass@localhost:5432/policy
...
```bash
pnpm --filter policy test:coverage
pnpm security:scan --scope=policy
pnpm structure:validate --scope=policy
```
...
- SBOM: CycloneDX generated at `./sbom/policy.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"policy"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"policy","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter policy test:e2e
pnpm --filter policy test:smoke
pnpm --filter policy chaos:probe
```
...
# Dev & run
pnpm --filter policy dev
pnpm --filter policy start

# Build & verify
pnpm --filter policy build
pnpm --filter policy lint && pnpm --filter policy typecheck
pnpm --filter policy test && pnpm --filter policy test:coverage
