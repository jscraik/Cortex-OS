...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/github/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for Github within Cortex-OS.
- **Responsibilities:**
  - Implement the Github domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter github dev

# Build
pnpm --filter github build

# Quality (affected-aware if available)
pnpm --filter github lint
pnpm --filter github typecheck
pnpm --filter github test
pnpm --filter github test:coverage
```
...
GITHUB_BASE_URL=https://example.local
GITHUB_DB_URL=postgres://user:pass@localhost:5432/github
...
```bash
pnpm --filter github test:coverage
pnpm security:scan --scope=github
pnpm structure:validate --scope=github
```
...
- SBOM: CycloneDX generated at `./sbom/github.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"github"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"github","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter github test:e2e
pnpm --filter github test:smoke
pnpm --filter github chaos:probe
```
...
# Build & verify
pnpm --filter github build
pnpm --filter github lint && pnpm --filter github typecheck
pnpm --filter github test && pnpm --filter github test:coverage
