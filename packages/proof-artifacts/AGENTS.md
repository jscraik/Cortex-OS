...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/proof-artifacts/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** brAInwav proof artifact envelope tooling aligned with SLSA provenance, Sigstore attestation, and Cortex-OS governance.
- **Responsibilities:**
  - Implement the Proof Artifacts domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter proof-artifacts dev

# Build
pnpm --filter proof-artifacts build

# Quality (affected-aware if available)
pnpm --filter proof-artifacts lint
pnpm --filter proof-artifacts typecheck
pnpm --filter proof-artifacts test
pnpm --filter proof-artifacts test:coverage
```
...
PROOF_ARTIFACTS_BASE_URL=https://example.local
PROOF_ARTIFACTS_DB_URL=postgres://user:pass@localhost:5432/proof_artifacts
...
```bash
pnpm --filter proof-artifacts test:coverage
pnpm security:scan --scope=proof-artifacts
pnpm structure:validate --scope=proof-artifacts
```
...
- SBOM: CycloneDX generated at `./sbom/proof-artifacts.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"proof-artifacts"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"proof-artifacts","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter proof-artifacts test:e2e
pnpm --filter proof-artifacts test:smoke
pnpm --filter proof-artifacts chaos:probe
```
...
# Dev & run
pnpm --filter proof-artifacts dev
pnpm --filter proof-artifacts start

# Build & verify
pnpm --filter proof-artifacts build
pnpm --filter proof-artifacts lint && pnpm --filter proof-artifacts typecheck
pnpm --filter proof-artifacts test && pnpm --filter proof-artifacts test:coverage
...
pnpm --filter proof-artifacts mcp:smoke
```
