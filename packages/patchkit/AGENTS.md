...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/patchkit/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Diffing, patch application, and pretty-print helpers powering executor-spool and the UI diff viewers. All functions are pure and rely on deterministic algorithms so changes can ...
- **Responsibilities:**
  - Implement the Patchkit domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter patchkit dev

# Build
pnpm --filter patchkit build

# Quality (affected-aware if available)
pnpm --filter patchkit lint
pnpm --filter patchkit typecheck
pnpm --filter patchkit test
pnpm --filter patchkit test:coverage
```
...
PATCHKIT_BASE_URL=https://example.local
PATCHKIT_DB_URL=postgres://user:pass@localhost:5432/patchkit
...
```bash
pnpm --filter patchkit test:coverage
pnpm security:scan --scope=patchkit
pnpm structure:validate --scope=patchkit
```
...
- SBOM: CycloneDX generated at `./sbom/patchkit.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"patchkit"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"patchkit","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter patchkit test:e2e
pnpm --filter patchkit test:smoke
pnpm --filter patchkit chaos:probe
```
...
# Dev & run
pnpm --filter patchkit dev
pnpm --filter patchkit start

# Build & verify
pnpm --filter patchkit build
pnpm --filter patchkit lint && pnpm --filter patchkit typecheck
pnpm --filter patchkit test && pnpm --filter patchkit test:coverage
...
pnpm --filter patchkit mcp:smoke
```
