...
import type { ModelCapability, ModelConfig, HybridMode } from '../model-router.js';
...
export class OrchestrationAdapter {
     ...
     */
    private mapComplexityThreshold(recommendedFor: string[]): 'simple' | 'moderate' | 'complex' | 'enterprise' {
        if (recommendedFor.includes('enterprise_architecture') || recommendedFor.includes('system_design')) {
            return 'enterprise';
        }
        if (recommendedFor.includes('complex_refactoring') || recommendedFor.includes('large_context')) {
            return 'complex';
        }
     ...
     */
    getModelForCapability(capability: ModelCapability, options?: ModelSelectionOptions): ModelConfig | null {
        try {
            ...
            if (!orchModel) {
                console.warn(`brAInwav Cortex-OS: No orchestration model found for capability: ${capability}`);
                return null;
            }
