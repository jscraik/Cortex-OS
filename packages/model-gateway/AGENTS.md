...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/model-gateway/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** The **Model Gateway** exposes `/chat`, `/embeddings`, and `/rerank` endpoints with
- **Responsibilities:**
  - Implement the Model Gateway domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter model-gateway dev

# Build
pnpm --filter model-gateway build

# Quality (affected-aware if available)
pnpm --filter model-gateway lint
pnpm --filter model-gateway typecheck
pnpm --filter model-gateway test
pnpm --filter model-gateway test:coverage
```
...
MODEL_GATEWAY_BASE_URL=https://example.local
MODEL_GATEWAY_DB_URL=postgres://user:pass@localhost:5432/model_gateway
...
```bash
pnpm --filter model-gateway test:coverage
pnpm security:scan --scope=model-gateway
pnpm structure:validate --scope=model-gateway
```
...
- SBOM: CycloneDX generated at `./sbom/model-gateway.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"model-gateway"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"model-gateway","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter model-gateway test:e2e
pnpm --filter model-gateway test:smoke
pnpm --filter model-gateway chaos:probe
```
...
# Dev & run
pnpm --filter model-gateway dev
pnpm --filter model-gateway start

# Build & verify
pnpm --filter model-gateway build
pnpm --filter model-gateway lint && pnpm --filter model-gateway typecheck
pnpm --filter model-gateway test && pnpm --filter model-gateway test:coverage
...
pnpm --filter model-gateway mcp:smoke
```
