...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/mvp-group/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for MVP Group within Cortex-OS.
- **Responsibilities:**
  - Implement the MVP Group domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter mvp-group dev

# Build
pnpm --filter mvp-group build

# Quality (affected-aware if available)
pnpm --filter mvp-group lint
pnpm --filter mvp-group typecheck
pnpm --filter mvp-group test
pnpm --filter mvp-group test:coverage
```
...
MVP_GROUP_BASE_URL=https://example.local
MVP_GROUP_DB_URL=postgres://user:pass@localhost:5432/mvp_group
...
```bash
pnpm --filter mvp-group test:coverage
pnpm security:scan --scope=mvp-group
pnpm structure:validate --scope=mvp-group
```
...
- SBOM: CycloneDX generated at `./sbom/mvp-group.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"mvp-group"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"mvp-group","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter mvp-group test:e2e
pnpm --filter mvp-group test:smoke
pnpm --filter mvp-group chaos:probe
```
...
# Dev & run
pnpm --filter mvp-group dev
pnpm --filter mvp-group start

# Build & verify
pnpm --filter mvp-group build
pnpm --filter mvp-group lint && pnpm --filter mvp-group typecheck
pnpm --filter mvp-group test && pnpm --filter mvp-group test:coverage
...
pnpm --filter mvp-group mcp:smoke
```
