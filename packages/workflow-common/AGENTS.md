...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/workflow-common/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** This package provides shared validation logic and evidence tracking used by both:
- **Responsibilities:**
  - Implement the Workflow Common domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter workflow-common dev

# Build
pnpm --filter workflow-common build

# Quality (affected-aware if available)
pnpm --filter workflow-common lint
pnpm --filter workflow-common typecheck
pnpm --filter workflow-common test
pnpm --filter workflow-common test:coverage
```
...
WORKFLOW_COMMON_BASE_URL=https://example.local
WORKFLOW_COMMON_DB_URL=postgres://user:pass@localhost:5432/workflow_common
...
```bash
pnpm --filter workflow-common test:coverage
pnpm security:scan --scope=workflow-common
pnpm structure:validate --scope=workflow-common
```
...
- SBOM: CycloneDX generated at `./sbom/workflow-common.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"workflow-common"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"workflow-common","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter workflow-common test:e2e
pnpm --filter workflow-common test:smoke
pnpm --filter workflow-common chaos:probe
```
...
# Dev & run
pnpm --filter workflow-common dev
pnpm --filter workflow-common start

# Build & verify
pnpm --filter workflow-common build
pnpm --filter workflow-common lint && pnpm --filter workflow-common typecheck
pnpm --filter workflow-common test && pnpm --filter workflow-common test:coverage
...
pnpm --filter workflow-common mcp:smoke
```
