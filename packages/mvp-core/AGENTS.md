...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/mvp-core/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for MVP Core within Cortex-OS.
- **Responsibilities:**
  - Implement the MVP Core domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter mvp-core dev

# Build
pnpm --filter mvp-core build

# Quality (affected-aware if available)
pnpm --filter mvp-core lint
pnpm --filter mvp-core typecheck
pnpm --filter mvp-core test
pnpm --filter mvp-core test:coverage
```
...
MVP_CORE_BASE_URL=https://example.local
MVP_CORE_DB_URL=postgres://user:pass@localhost:5432/mvp_core
...
```bash
pnpm --filter mvp-core test:coverage
pnpm security:scan --scope=mvp-core
pnpm structure:validate --scope=mvp-core
```
...
- SBOM: CycloneDX generated at `./sbom/mvp-core.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"mvp-core"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"mvp-core","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter mvp-core test:e2e
pnpm --filter mvp-core test:smoke
pnpm --filter mvp-core chaos:probe
```
...
# Dev & run
pnpm --filter mvp-core dev
pnpm --filter mvp-core start

# Build & verify
pnpm --filter mvp-core build
pnpm --filter mvp-core lint && pnpm --filter mvp-core typecheck
pnpm --filter mvp-core test && pnpm --filter mvp-core test:coverage
...
pnpm --filter mvp-core mcp:smoke
```
