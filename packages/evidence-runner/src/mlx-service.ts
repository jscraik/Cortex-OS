...
`);
    ...
    async performInference(request: MLXInferenceRequest): Promise<MLXInferenceResult> {
        ...
        try {
            let result: Partial<MLXInferenceResult> = {};
...
`;
    ...
    private createFallbackResult(request: MLXInferenceRequest, startTime: number): MLXInferenceResult {
        ...
        switch (request.task) {
            ...
            case 'embedding':
                ...
                result.embedding = Array.from({ length: 384 }, (_, i) => 
                    Math.sin(i * request.text.length) * 0.5
                );
                break;
        }
