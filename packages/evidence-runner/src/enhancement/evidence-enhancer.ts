...
function assessSeverity(
	text: string,
	categories: string[],
): EnhancementResult['insights']['severity'] {
	const lowerText = text.toLowerCase();
...
function generateRecommendations(categories: string[], severity: string): string[] {
	...
	if (categories.includes('authentication')) {
		recommendations.push(
			'brAInwav recommends reviewing authentication logs and session management',
		);
...
export async function enhanceEvidence(
	...
	if (trimmed.length === 0) {
		...
		return {
			enrichedText:
				'brAInwav Analysis: No evidence provided for enhancement. Please provide evidence text for analysis.',
