...
```yaml
id: agents-evidence-runner-v1
package: Evidence Runner
type: package
runtime: node
...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/evidence-runner/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Evidence Runner provides intelligent evidence analysis and enhancement capabilities using MLX models for privacy-sensitive processing and advanced AI techniques for comprehensiv...
- **Responsibilities:**
  - Implement the Evidence Runner domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter evidence-runner dev

# Build
pnpm --filter evidence-runner build

# Quality (affected-aware if available)
pnpm --filter evidence-runner lint
pnpm --filter evidence-runner typecheck
pnpm --filter evidence-runner test
pnpm --filter evidence-runner test:coverage
```
...
EVIDENCE_RUNNER_BASE_URL=https://example.local
EVIDENCE_RUNNER_DB_URL=postgres://user:pass@localhost:5432/evidence_runner
...
```bash
pnpm --filter evidence-runner test:coverage
pnpm security:scan --scope=evidence-runner
pnpm structure:validate --scope=evidence-runner
```
...
- SBOM: CycloneDX generated at `./sbom/evidence-runner.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"evidence-runner"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"evidence-runner","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter evidence-runner test:e2e
pnpm --filter evidence-runner test:smoke
pnpm --filter evidence-runner chaos:probe
```
...
# Dev & run
pnpm --filter evidence-runner dev
pnpm --filter evidence-runner start

# Build & verify
pnpm --filter evidence-runner build
pnpm --filter evidence-runner lint && pnpm --filter evidence-runner typecheck
pnpm --filter evidence-runner test && pnpm --filter evidence-runner test:coverage
...
pnpm --filter evidence-runner mcp:smoke
```
