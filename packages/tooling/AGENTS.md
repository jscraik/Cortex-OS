...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/tooling/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for Tooling within Cortex-OS.
- **Responsibilities:**
  - Implement the Tooling domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter tooling dev

# Build
pnpm --filter tooling build

# Quality (affected-aware if available)
pnpm --filter tooling lint
pnpm --filter tooling typecheck
pnpm --filter tooling test
pnpm --filter tooling test:coverage
```
...
TOOLING_BASE_URL=https://example.local
TOOLING_DB_URL=postgres://user:pass@localhost:5432/tooling
...
```bash
pnpm --filter tooling test:coverage
pnpm security:scan --scope=tooling
pnpm structure:validate --scope=tooling
```
...
- SBOM: CycloneDX generated at `./sbom/tooling.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"tooling"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"tooling","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter tooling test:e2e
pnpm --filter tooling test:smoke
pnpm --filter tooling chaos:probe
```
...
# Dev & run
pnpm --filter tooling dev
pnpm --filter tooling start

# Build & verify
pnpm --filter tooling build
pnpm --filter tooling lint && pnpm --filter tooling typecheck
pnpm --filter tooling test && pnpm --filter tooling test:coverage
