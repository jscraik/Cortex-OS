...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/orchestration/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Multi-Agent Workflow Coordination for Cortex-OS ASBR Runtime — LangGraph-only (TypeScript-first)
- **Responsibilities:**
  - Implement the Orchestration domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter orchestration dev

# Build
pnpm --filter orchestration build

# Quality (affected-aware if available)
pnpm --filter orchestration lint
pnpm --filter orchestration typecheck
pnpm --filter orchestration test
pnpm --filter orchestration test:coverage
```
...
ORCHESTRATION_BASE_URL=https://example.local
ORCHESTRATION_DB_URL=postgres://user:pass@localhost:5432/orchestration
...
```bash
pnpm --filter orchestration test:coverage
pnpm security:scan --scope=orchestration
pnpm structure:validate --scope=orchestration
```
...
- SBOM: CycloneDX generated at `./sbom/orchestration.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"orchestration"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"orchestration","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter orchestration test:e2e
pnpm --filter orchestration test:smoke
pnpm --filter orchestration chaos:probe
```
...
# Dev & run
pnpm --filter orchestration dev
pnpm --filter orchestration start

# Build & verify
pnpm --filter orchestration build
pnpm --filter orchestration lint && pnpm --filter orchestration typecheck
pnpm --filter orchestration test && pnpm --filter orchestration test:coverage
...
pnpm --filter orchestration mcp:smoke
```
