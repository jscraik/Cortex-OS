...
import type { CoordinationRequest } from '../../src/coordinator/adaptive-coordinator.js';
import type { CoordinationTelemetry } from '../../src/coordinator/adaptive-coordinator.js';
...
describe('Structured planning integration with cortex-sec security', () => {
        ...
        it('includes cortex-sec security evaluation in planning results and planning context', async () => {
                ...
                expect(activeContext?.preferences.notes.some((note) => note.includes('brAInwav security summary'))).toBe(true);
        });
        ...
        it('merges security telemetry and state patches during coordination', async () => {
                ...
                class StubSecurityCoordinator extends SecurityCoordinator {
                        review = vi.fn().mockReturnValue({ telemetry: securityTelemetry, statePatch: securityStatePatch });
                }
                ...
                expect(decision.statePatch.coordination).toMatchObject({ agents: ['agent-primary', 'agent-security', 'security-specialist'] });
        });
