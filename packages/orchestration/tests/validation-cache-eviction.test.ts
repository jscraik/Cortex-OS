...
import { _setValidationCacheForTests, clearValidationCache, createValidationCache, getValidationCacheStats, validateWorkflow } from '../src/workflow-validator.js';
...
describe('ValidationCache eviction', () => {
    ...
    beforeEach(() => {
        ...
        const cache = createValidationCache({ maxSize: 2, ttlMs: 1000, cleanupIntervalMs: 10_000, onEvict: (k, _e, r) => evictions.push({ key: k, reason: r }) });
    ...
    it('evicts least recently used entry when maxSize exceeded', () => {
        ...
        const evictions = (globalThis as any).__evictions as Array<{ key: string; reason: string }>;
