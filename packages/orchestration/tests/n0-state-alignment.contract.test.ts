	...
	it('keeps state keys aligned across agents, kernel, and orchestration', () => {
		...
		expect(sortedKeys(orchestrationN0)).toEqual([
			...
			'session',
                ...
                expect(agentN0.session.brainwavSession).toBe('session-trace-alignment');
                expect(kernelN0.session.brainwavSession).toBe('session-trace-alignment');
                expect(cortexN0.session.brainwavSession).toBe('session-trace-alignment');
                expect(orchestrationN0.session.brainwavSession).toBe('session-trace-alignment');

                const ctxMatcher = {
                        thermal: { level: 'warning' },
                };
                expect(agentN0.ctx).toMatchObject(ctxMatcher);
                expect(kernelN0.ctx).toMatchObject(ctxMatcher);
                expect(cortexN0.ctx).toMatchObject(ctxMatcher);
                expect(orchestrationN0.ctx).toMatchObject(ctxMatcher);
		...
		const initial = createInitialN0State('initial', session, {
			...
			output: 'pending',
                ...
                const patched = mergeN0State(initial, {
                        ctx: { checkpoints: 2, thermal: { status: 'nominal' } },
                        output: 'ready',
