...
describe('MLXFirstOrchestrator error handling', () => {
  it('throws when task decomposition fails', async () => {
    ...
    vi.spyOn((orchestrator as any).modelProvider, 'generate').mockRejectedValueOnce(
      new Error('model failure'),
    );
    await expect(orchestrator.decomposeTask('task', ['agent'])).rejects.toBeInstanceOf(
      OrchestrationError,
    );
  });
  ...
  it('throws when code orchestration fails', async () => {
    ...
    await expect(orchestrator.orchestrateCodeTask('task')).rejects.toBeInstanceOf(
      OrchestrationError,
    );
  });
  ...
  it('throws on invalid task decomposition response', async () => {
    const orchestrator = new MLXFirstOrchestrator();
    vi.spyOn((orchestrator as any).modelProvider, 'generate').mockResolvedValueOnce({
