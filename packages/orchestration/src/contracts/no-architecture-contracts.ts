...
export const ExecutionRequestSchema = z
	.union([FullExecutionRequestSchema, MinimalExecutionRequestSchema])
	.transform((input) => {
		...
		const complexity =
			typeof profile.complexity === 'number' ? (profile.complexity as unknown as number) : 0.4;
		const priority: 'low' | 'medium' | 'high' | 'urgent' = 'medium';
		const timeoutMs = minimal.constraints.timeoutMs;
		const memoryMB =
			typeof profile.memoryMB === 'number' ? (profile.memoryMB as unknown as number) : 512;
		const cpuPercent =
			typeof profile.cpuPercent === 'number' ? (profile.cpuPercent as unknown as number) : 50;
...
export const ExecutionPlanSchema = z
	.union([FullExecutionPlanSchema, MinimalExecutionPlanSchema])
	.transform((input) => {
...
export const ExecutionFeedbackSchema = z
	.union([FullExecutionFeedbackSchema, MinimalExecutionFeedbackSchema])
	.transform((input) => {
...
export const AgentScheduleSchema = z
	.union([FullAgentScheduleSchema, MinimalAgentScheduleSchema])
	.transform((input) => {
...
export const IntelligenceSchedulerSchema = z.object({
	planExecution: z.function().args(ExecutionRequestSchema).returns(z.promise(ExecutionPlanSchema)),
	scheduleAgents: z.function().args(ExecutionPlanSchema).returns(z.promise(AgentScheduleSchema)),
	adaptStrategy: z
		.function()
...
export const MasterAgentLoopSchema = z.object({
	initializeAgents: z.function().args(AgentConfigurationSchema).returns(z.promise(AgentPoolSchema)),
	coordinateExecution: z
		.function()
		.args(ExecutionPlanSchema)
		.returns(z.promise(ExecutionResultSchema)),
	handleAgentFailure: z
		.function()
		.args(z.string(), AgentErrorSchema)
		.returns(z.promise(RecoveryActionSchema)),
	persistAgentState: z.function().args(z.string(), AgentStateSchema).returns(z.promise(z.void())),
});
...
export const ToolLayerSchema = z.object({
	...
	invoke: z.function().args(z.string(), z.unknown()).returns(z.promise(ToolResultSchema)),
	getAvailableTools: z.function().returns(z.array(ToolManifestSchema)),
});
...
export const MessageHandlerSchema = z
	.function()
	.args(AgentMessageSchema)
	.returns(z.promise(z.void()));

export const UnsubscribeFnSchema = z.function().returns(z.void());
...
export const AgentNetworkSchema = z.object({
	sendMessage: z
		...
		.returns(z.promise(z.void())),
	broadcast: z.function().args(z.string(), BroadcastMessageSchema).returns(z.promise(z.void())),
	subscribeToAgent: z
		.function()
