...
describe('nO Architecture Contracts', () => {
  ...
  describe('ToolLayer Contract', () => {
    ...
    it('should enforce layer hierarchy constraints', () => {
      ...
      expect(dashboardTools.every(tool => tool.includes('visualize') || tool.includes('monitor') || tool.includes('report'))).toBe(true);

      // Execution layer should access execution tools
      expect(executionTools.every(tool => tool.includes('file') || tool.includes('network') || tool.includes('process'))).toBe(true);
    });
  });
