...
async function buildArtifactProposal(state: any) {
...
const nodeFns: Record<Node, NodeFn> = {
  ...
  synthesize: async (state, ctx) => {
    ...
    enforce(await loadGrant('fs.write'), 'write', { path: (proposal as any).path });
    record(auditEvent('fs', 'write', { runId: ctx.runId }, { path: (proposal as any).path }));
...
async function withRetry(node: Node, fn: () => Promise<any>, policy?: RetryPolicy) {
...
export async function runSupervisor(initialState: any, ctx: RunContext, opts: RunOptions = {}) {
