...
import { type RoutingPolicy, RoutingPolicySchema } from '@cortex-os/contracts';
...
type PriorityRule = NonNullable<RoutingPolicy['priority_rules']>[number];
type PriorityRuleAction = PriorityRule['then'];
type PriorityRuleCondition = PriorityRule['if'];
...
export class PolicyRouter {
	...
	private matchesCondition(
		...
		if (condition.operation_any && !condition.operation_any.includes(request.operation))
			return false;
		if (condition.tags_any) {
			const found = condition.tags_any.some((tag: string) => request.tags.includes(tag));
			if (!found) return false;
		}
		if (condition.command_prefix_any && request.command) {
			const match = condition.command_prefix_any.some((prefix: string) =>
