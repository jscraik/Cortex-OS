...
import {
	...
	type BypassResult,
	type BypassToken,
	type SkipPermissionsConfig,
	SkipPermissionsConfigSchema,
...
export class SkipPermissionsService {
	...
	private bypassTokens: Map<string, BypassToken> = new Map();
	...
	async canBypass(context: BypassContext): Promise<BypassResult> {
		...
		if (this.config.allowedIPs.length > 0 && context.ipAddress) {
			if (!this.config.allowedIPs.includes(context.ipAddress)) {
				return {
					allowed: false,
