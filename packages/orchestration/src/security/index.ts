...
export class SecuritySuite {
   ...
   */
  getValidationMiddleware(schema: z.ZodTypeAny) {
  ...
  getSecurityMetrics() {
    ...
    return {
      audit: {
        ...
        recentEvents: auditLog.filter((event) => {
          const ts =
            typeof event.timestamp === 'number'
              ? event.timestamp
              : event.timestamp
                ? new Date(event.timestamp as string | number | Date).getTime()
                : 0;
   ...
   */
  async encryptSensitiveData(data: Record<string, unknown>, fields: string[]): Promise<Record<string, unknown>> {
   ...
   */
  async decryptSensitiveData(data: Record<string, unknown>, fields: string[]): Promise<Record<string, unknown>> {
