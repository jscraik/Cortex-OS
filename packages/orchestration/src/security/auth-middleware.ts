...
import { AuthenticationResult, OAuthProvider } from './oauth-provider.js';
import { PolicyContext, rbacSystem } from './rbac-system.js';
...
export class AuthMiddleware {
    ...
    authenticate() {
        return async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
            try {
                ...
                if (this.config.enforceHttps && req.headers['x-forwarded-proto'] !== 'https' && req.protocol !== 'https') {
                    res.status(400).json({
                ...
                if (this.config.rateLimiting?.enabled) {
                    ...
                    if (!rateLimitResult.allowed) {
                        securityMetrics.rateLimitHits
                            .labels(this.getClientId(req), req.path)
                            .inc();
    ...
    handleOAuthCallback() {
        return async (req: Request, res: Response): Promise<void> => {
            try {
                ...
                const redirectUrl = (state && typeof state === 'string' ? Buffer.from(state, 'base64').toString() : '/');
    ...
    handleLogin() {
        return (req: Request, res: Response): void => {
            ...
            const state = redirect && typeof redirect === 'string' ? Buffer.from(redirect).toString('base64') : undefined;
    ...
    handleLogout() {
        return async (req: AuthenticatedRequest, res: Response): Promise<void> => {
            try {
                if (req.auth?.token) {
                    await this.config.oauthProvider.logout(
                        req.auth.token,
