...
export class RBACSystem {
	...
	async authorize(context: PolicyContext): Promise<AuthorizationResult> {
		...
		try {
			...
			securityMetrics.authzDecisions.labels(context.resource, context.action, 'error').inc();
	 ...
	 */
	private evaluateConditions(conditions: Record<string, unknown>, context: PolicyContext): boolean {
	...
	private initializeDefaultRoles(): void {
		const defaultRoles: Role[] = [
			...
			// Developer
			{
				...
				permissions: ['no:execute', 'no:read', 'agents:read', 'monitoring:read', 'config:read'],
			},
			...
			// Read-only User
			{
				...
				permissions: ['no:read', 'agents:read', 'monitoring:read'],
			},
			...
			// Service Account
			{
				...
				permissions: ['no:execute', 'no:read', 'agents:read'],
			},
		];
