...
export class LangGraphDSPBridge {
  ...
  private getMetadataFlag(workflowState: WorkflowState, key: string): boolean {
    ...
    const metaUnknown = (workflowState.context?.metadata as unknown) as Record<string, unknown> | undefined;
  ...
  private async validateIntegration(
    ...
    const assignments = Array.isArray(coordRecord['assignments']) ? (coordRecord['assignments'] as unknown[]) : [];
    const strategyVal = typeof coordRecord['strategy'] === 'string' ? String(coordRecord['strategy']) : undefined;
