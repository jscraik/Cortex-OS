...
export class AutoscalingManager {
    ...
    async executeScaling(decision: ScalingDecision): Promise<boolean> {
        ...
        const now = Date.now();
        const cooldownPeriod = decision.action === 'scale_up'
    ...
    private makeScalingDecision(
        ...
        if (shouldScaleUp) {
            ...
            targetAgents = Math.min(
                currentAgents + this.config.stepSize,
    ...
    private async getCurrentMetrics(): Promise<ScalingDecision['metrics']> {
        ...
        const baseLoad = this.getActiveAgentCount() > 0 ?
            Math.min(80, 30 + (this.getQueueDepth() * 10)) : 20;
