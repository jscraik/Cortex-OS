...
const LAYER_CAPABILITIES = {
  ...
  execution: ['file-system', 'process-management', 'network-operations', 'tool-chaining', 'resource-management'],
  primitive: ['atomic-operations', 'consistency-guarantees', 'rollback-capabilities', 'composition-primitives'],
} as const;
...
export class ToolLayer extends EventEmitter {
  ...
  async discoverTools(capability: string): Promise<ToolDefinition[]> {
    const tools = Array.from(this.registeredTools.values()).filter(
   ...
   */
  async invokeTool(toolId: string, input: unknown, context?: Partial<ToolExecutionContext>): Promise<unknown> {
  ...
  private updateLayerHealth(): void {
    ...
    this.layerHealth.errorRate = this.totalExecutions > 0 ? this.totalErrors / this.totalExecutions : 0;
  ...
  async shutdown(): Promise<void> {
    ...
    if (this.activeExecutions.size > 0) {
      console.warn(`Force terminating ${this.activeExecutions.size} active executions during shutdown`);
