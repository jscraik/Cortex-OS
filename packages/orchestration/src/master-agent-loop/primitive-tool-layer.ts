...
export class PrimitiveToolLayer extends ToolLayer {
	...
	private async executeAtomicOperation(input: any): Promise<any> {
		...
		switch (validated.operation) {
			case 'read': {
				const value = this.memoryStore.get(validated.key) || {
					data: `mock-value-${validated.key}`,
					version: 1,
				};
				result.value = value;
				result.atomicity = {
	...
	private validateAtomicInput(input: any): boolean {
		try {
			...
			if (
				input.target &&
				!['memory', 'disk', 'network', 'slow-storage', 'readonly-storage'].includes(input.target)
			) {
	...
	private validateCompositionInput(input: any): boolean {
		try {
			...
			if (input.operations && Array.isArray(input.operations)) {
				input.operations = input.operations.map(
					(op: { id: string; type?: string; params?: unknown; dependencies?: string[] }) => ({
						...op,
	...
	private updatePrimitiveMetrics(toolId: string, executionTime: number): void {
		...
		const totalTime =
			this.primitiveMetrics.averageOperationTime * (this.primitiveMetrics.totalOperations - 1) +
