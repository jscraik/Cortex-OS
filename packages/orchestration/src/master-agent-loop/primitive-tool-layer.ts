...
export class PrimitiveToolLayer extends ToolLayer {
   ...
   */
  async invoke(toolId: string, input: unknown): Promise<AtomicResult | TransactionResult | ConsistencyResult | RollbackResult | Record<string, unknown>> {
    ...
    try {
      ...
      return result as AtomicResult | TransactionResult | ConsistencyResult | RollbackResult | Record<string, unknown>;
    } catch (error) {
      const executionTime = Date.now() - startTime;
      this.updatePrimitiveMetrics(toolId, executionTime);
      throw error;
  ...
  private handleRestoreSavepoint(
    validated: RollbackOperation,
  ): {
