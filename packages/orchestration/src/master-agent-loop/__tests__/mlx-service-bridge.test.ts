...
import { MLXServiceBridge, MLXServiceError, MLXServiceErrorCode, createMLXServiceBridge } from '../mlx-service-bridge';
...
describe('MLXServiceBridge', () => {
  ...
  describe('Initialization', () => {
    ...
    it('should require initialization before use', async () => {
      await expect(
        serviceBridge.generateEmbedding({ text: 'test' })
      ).rejects.toThrow(
        expect.objectContaining({
          code: MLXServiceErrorCode.ADAPTER_NOT_AVAILABLE,
  ...
  describe('Embedding Generation', () => {
    ...
    it('should validate embedding requests', async () => {
      await expect(
        serviceBridge.generateEmbedding({ text: '' })
  ...
  describe('Chat Generation', () => {
    ...
    it('should validate chat requests', async () => {
      await expect(
        serviceBridge.generateChat({ messages: [] })
      ).rejects.toThrow(); // Zod validation error for empty messages
  ...
  describe('Timeout Handling', () => {
    ...
    it('should timeout long-running operations', async () => {
      ...
      await expect(
        serviceBridge.generateEmbedding({ text: 'test timeout' })
      ).rejects.toThrow(
        expect.objectContaining({
