...
vi.mock('../../observability/otel.js', () => ({
    withEnhancedSpan: vi.fn((name, fn) => fn()),
}));
...
describe('StatePersistenceManager', () => {
    ...
    describe('Transaction Management', () => {
        ...
        it('should handle transaction timeout', async () => {
            ...
            const transactionId = await testManager.beginTransaction('test-user', 'Timeout test');
    ...
    describe('Error Handling & Edge Cases', () => {
        ...
        it('should handle shutdown gracefully', async () => {
            await manager.updateState('agentState', 'agent-1', mockAgentState);
            const transactionId = await manager.beginTransaction('test-user', 'Shutdown test');
