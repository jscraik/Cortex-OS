...
import type { ToolChain, ToolOrchestrator } from '../tool-orchestrator';
...
describe('Tool Orchestration - Phase 3.6', () => {
  ...
  describe('Cross-Layer Tool Communication', () => {
    ...
    it('should validate message format between tool layers', async () => {
      ...
      await expect(orchestrator.executeChain(invalidMessageChain))
        .rejects.toThrow('Invalid message format between tool layers');
    });
  ...
  describe('Tool Dependency Management', () => {
    ...
    it('should detect and handle circular dependencies', async () => {
      ...
      await expect(orchestrator.executeChain(circularChain))
...
const createMultiLayerToolChain = (): ToolChain => createToolChain(
  'multi-layer-test',
  ...
  [
    ...
    createBasicToolDefinition('primitive-mem-write', 'primitive', 'memory-write', ['exec-file-read']),
    createBasicToolDefinition('exec-process', 'execution', 'process-execute', ['primitive-mem-write']),
    createBasicToolDefinition('dashboard-report', 'dashboard', 'generate-report', ['exec-process'])
  ],
...
const createDependentToolChain = (): ToolChain => createToolChain(
  'dependent-test',
...
const createOptimizableToolChain = (): ToolChain => createToolChain(
  'optimizable-test',
  'Optimizable Tool Chain',
  [
    createBasicToolDefinition('slow-tool-1', 'execution', 'slow-operation', [], { optimizable: true }),
    createBasicToolDefinition('fast-tool-1', 'primitive', 'fast-operation', [], { optimizable: true }),
    createBasicToolDefinition('parallel-tool-1', 'execution', 'parallel-operation', [], { parallelizable: true }),
    createBasicToolDefinition('parallel-tool-2', 'execution', 'parallel-operation', [], { parallelizable: true })
  ],
  'performance-optimized',
  10000
);

const createFailingToolChain = (): ToolChain => createToolChain(
  'failing-test',
  'Failing Tool Chain',
  [
    ...
    createBasicToolDefinition('dependent-tool', 'dashboard', 'dependent-operation', ['failing-tool'])
  ],
  'fail-fast',
  5000
);

const createCommunicatingToolChain = (): ToolChain => createToolChain(
  'communicating-test',
  ...
  [
    ...
    createBasicToolDefinition('receiver-tool', 'dashboard', 'receive-message', ['processor-tool'])
  ],
  'message-passing',
  10000
);

const createInvalidMessageChain = (): ToolChain => createToolChain(
  'invalid-message-test',
  'Invalid Message Chain',
  [
    createBasicToolDefinition('invalid-sender', 'primitive', 'send-invalid-message'),
    createBasicToolDefinition('strict-receiver', 'execution', 'strict-message-validation', ['invalid-sender'])
  ],
  'strict-validation',
  5000
);

const createSecureMessageChain = (): ToolChain => createToolChain(
  'secure-message-test',
  'Secure Message Chain',
  [
    createBasicToolDefinition('secure-sender', 'primitive', 'send-secure-message'),
    createBasicToolDefinition('secure-receiver', 'execution', 'receive-secure-message', ['secure-sender'])
  ],
  'security-first',
  8000,
  { securityLevel: 'high' }
...
const createComplexDependencyChain = (): ToolChain => createToolChain(
...
const createCircularDependencyChain = (): ToolChain => createToolChain(
...
const createDynamicDependencyChain = (): ToolChain => createToolChain(
  'dynamic-dependency-test',
  'Dynamic Dependency Chain',
  [
    createBasicToolDefinition('dynamic-root', 'primitive', 'dynamic-operation'),
    createBasicToolDefinition('conditional-tool', 'execution', 'conditional-operation', ['dynamic-root'])
  ],
  'dynamic-injection',
  15000,
...
const createParallelizableToolChain = (): ToolChain => createToolChain(
  'parallelizable-test',
  'Parallelizable Tool Chain',
  [
    createBasicToolDefinition('parallel-1', 'execution', 'parallel-operation-1', [], { parallelizable: true }),
    createBasicToolDefinition('parallel-2', 'execution', 'parallel-operation-2', [], { parallelizable: true }),
    createBasicToolDefinition('parallel-3', 'execution', 'parallel-operation-3', [], { parallelizable: true }),
    createBasicToolDefinition('sequential-end', 'dashboard', 'sequential-operation', ['parallel-1', 'parallel-2', 'parallel-3'])
  ],
  'parallel-optimized',
  10000
);

const createCacheableToolChain = (): ToolChain => createToolChain(
  'cacheable-test',
  'Cacheable Tool Chain',
  [
    createBasicToolDefinition('cacheable-1', 'primitive', 'cacheable-operation-1', [], { cacheable: true }),
    createBasicToolDefinition('cacheable-2', 'execution', 'cacheable-operation-2', [], { cacheable: true })
  ],
  'cache-optimized',
  5000
);

const createToolVariantChain = (): ToolChain => createToolChain(
  'variant-test',
  'Tool Variant Chain',
  [
...
const createMonitoredToolChain = (): ToolChain => createToolChain(
  'monitored-test',
  'Monitored Tool Chain',
  [
    createBasicToolDefinition('monitored-1', 'primitive', 'monitored-operation-1'),
    createBasicToolDefinition('monitored-2', 'execution', 'monitored-operation-2', ['monitored-1'])
  ],
  'telemetry-enabled',
  10000,
...
const createLongRunningToolChain = (): ToolChain => createToolChain(
  'long-running-test',
  'Long Running Tool Chain',
  [
    createBasicToolDefinition('long-1', 'execution', 'long-operation-1', [], { estimatedDuration: 2000 }),
    createBasicToolDefinition('long-2', 'execution', 'long-operation-2', [], { estimatedDuration: 3000 })
  ],
  'status-tracking',
  30000
);

const createRetryableToolChain = (): ToolChain => createToolChain(
  'retryable-test',
  'Retryable Tool Chain',
  [
    createBasicToolDefinition('flaky-tool', 'execution', 'flaky-operation', [], { retryable: true })
  ],
  'retry-enabled',
  15000,
...
const createFallbackToolChain = (): ToolChain => createToolChain(
  'fallback-test',
  'Fallback Tool Chain',
  [
...
const createDebuggableFailureChain = (): ToolChain => createToolChain(
