...
import {
    type MasterAgentLoopConfig,
    MasterAgentLoopCore
} from '../master-agent-loop-core';
...
class MockFailureRecoveryManager extends EventEmitter {
    ...
    constructor() {
        ...
        this.circuitBreakers.set('workflow-execution', { state: 'closed', failures: 0, lastFailure: null });
        this.circuitBreakers.set('checkpoint-creation', { state: 'closed', failures: 0, lastFailure: null });
    }
    ...
    getResilienceMetrics() {
        const closedCircuits = Array.from(this.circuitBreakers.values()).filter(cb => cb.state === 'closed').length;
...
describe('MasterAgentLoopCore', () => {
    ...
    describe('Component Integration', () => {
        it('should integrate with state persistence when enabled', async () => {
            const createCheckpointSpy = vi.spyOn(mockComponents.statePersistenceManager, 'createCheckpoint');
        ...
        it('should handle component failures gracefully', async () => {
            ...
            const failingPersistence = {
                ...
                createCheckpoint: vi.fn().mockRejectedValue(new Error('Persistence failed - simulated failure')),
    ...
    describe('Periodic Tasks and Coordination', () => {
        it('should perform periodic coordination when enabled', async () => {
            ...
            const coordinationSpy = vi.spyOn(fastLoop as unknown as Record<string, unknown>, 'performPeriodicCoordination' as any);
        ...
        it('should process workflow queue during coordination', async () => {
            const queueSpy = vi.spyOn(masterLoop as unknown as Record<string, unknown>, 'processWorkflowQueue' as any);
    ...
    describe('Shutdown and Cleanup', () => {
        ...
        it('should cancel active workflows during shutdown', async () => {
            ...
            let statusBefore = masterLoop.getExecutionStatus(executionId);
            const metricsBefore = masterLoop.getSystemMetrics();

            // Verify there's some activity before shutdown
            expect(metricsBefore.activeWorkflows + metricsBefore.queuedWorkflows).toBeGreaterThanOrEqual(0);
        ...
        it('should shutdown component managers', async () => {
            ...
            await Promise.all(shutdownSpies.map(spy =>
