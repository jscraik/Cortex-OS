...
import type { AtomicOperation, AtomicResult, ConsistencyResult, RollbackResult, TransactionResult } from '../primitive-tool-layer.js';
...
describe('PrimitiveToolLayer', () => {
  ...
  describe('Atomic Operations', () => {
    ...
    it('should perform atomic compare-and-swap operations', async () => {
      ...
      const result = (await primitiveLayer.invoke('atomic-operation', casOperation)) as AtomicResult;
    ...
    it('handleAtomicRead helper returns canonical atomicity and null for unknown keys', () => {
      primitiveLayer = new PrimitiveToolLayer();
      const helper = primitiveLayer as unknown as { handleAtomicRead: (op: AtomicResult & { key: string; isolation: string; consistency: string }) => { value: unknown; atomicity: { guaranteed: boolean; isolationLevel: string; consistency: string } } };
      const readOp = {
        operation: 'read',
        target: 'memory',
        key: 'non-existent',
      ...
      expect(out.atomicity).toEqual({ guaranteed: true, isolationLevel: 'serializable', consistency: 'strong' });
      expect(out.value).toBeNull();
    });

    it('should synthesize legacy atomic operation when key omitted for slow-storage timeout', () => {
      primitiveLayer = new PrimitiveToolLayer();
      const helper = primitiveLayer as unknown as { synthesizeAtomicOperation: (raw: Record<string, unknown>) => AtomicOperation };
      const raw = { target: 'slow-storage', timeout: 100, operation: 'write', value: { data: 'legacy' } };
    ...
    it('handleAtomicSimulations returns simulated error AtomicResult', () => {
      primitiveLayer = new PrimitiveToolLayer();
      const helper = primitiveLayer as unknown as { handleAtomicSimulations: (v: AtomicOperation) => AtomicResult | null };
    ...
    it('applyAtomicOperation returns correct read fragment', () => {
      primitiveLayer = new PrimitiveToolLayer();
      const helper = primitiveLayer as unknown as { applyAtomicOperation: (v: AtomicOperation) => Partial<AtomicResult> };
      const validated: AtomicOperation = { operation: 'read', target: 'memory', key: 'rtest', isolation: 'serializable', timeout: 5000, consistency: 'strong', durability: 'persistent' };
    ...
    it('applyAtomicOperation returns correct write fragment', () => {
      primitiveLayer = new PrimitiveToolLayer();
      const helper = primitiveLayer as unknown as { applyAtomicOperation: (v: AtomicOperation) => Partial<AtomicResult> };
      const validated: AtomicOperation = { operation: 'write', target: 'memory', key: 'wtest', value: { data: 'x' }, isolation: 'read-committed', timeout: 5000, consistency: 'strong', durability: 'persistent' };
      const out = helper.applyAtomicOperation(validated);
    ...
    it('applyAtomicOperation returns correct CAS fragment when expected matches', () => {
      primitiveLayer = new PrimitiveToolLayer();
      const helper = primitiveLayer as unknown as { applyAtomicOperation: (v: AtomicOperation) => Partial<AtomicResult> };
      const validated1: AtomicOperation = { operation: 'compare-and-swap', target: 'memory', key: 'castest', expectedValue: null, newValue: { counter: 1 }, isolation: 'read-committed', timeout: 5000, consistency: 'strong', durability: 'persistent' };
    ...
    it('applyAtomicOperation returns correct delete fragment', () => {
      primitiveLayer = new PrimitiveToolLayer();
      const helper = primitiveLayer as unknown as { applyAtomicOperation: (v: AtomicOperation) => Partial<AtomicResult> };
      const validated: AtomicOperation = { operation: 'delete', target: 'memory', key: 'deltest', isolation: 'read-committed', timeout: 5000, consistency: 'strong', durability: 'persistent' };
      const out = helper.applyAtomicOperation(validated);
  ...
  describe('Consistency Validation', () => {
    it('should validate data consistency', async () => {
      ...
      const result = (await primitiveLayer.invoke('consistency-validator', consistencyCheck)) as ConsistencyResult;
    ...
    it('should perform consistency repair operations', async () => {
      ...
      const result = (await primitiveLayer.invoke('consistency-validator', repairOperation)) as ConsistencyResult;
    ...
    it('should validate cross-reference integrity', async () => {
      ...
      const result = (await primitiveLayer.invoke('consistency-validator', integrityCheck)) as ConsistencyResult;
  ...
  describe('Rollback Operations', () => {
    it('should execute operation rollbacks', async () => {
      ...
      const result = (await primitiveLayer.invoke('rollback-handler', rollbackOperation)) as RollbackResult;
    ...
    it('should synthesize rollback operation from legacy input shapes', () => {
      primitiveLayer = new PrimitiveToolLayer();
      const helper = primitiveLayer as unknown as { synthesizeRollbackOperation: (input: Record<string, unknown>) => RollbackOperation };

      const cascading = helper.synthesizeRollbackOperation({ dependentOperations: [{ id: 'op-1', dependencies: [] }], cascadeLevel: 'full' });
      expect(cascading.action).toBe('cascading-rollback');

      const restore = helper.synthesizeRollbackOperation({ savepointId: 'sp-1', savepointState: { memory: {} } });
    ...
    it('summarizeRollbackOperations helper returns correct summary', () => {
      ...
      const helper = primitiveLayer as unknown as {
        summarizeRollbackOperations: (ops?: Array<{ id: string; dependencies: string[] }>) => { total: number; rolledBack: number; order: string[] } | undefined;
      };

      const ops = [{ id: 'a', dependencies: [] }, { id: 'b', dependencies: ['a'] }, { id: 'c', dependencies: ['b'] }];
    ...
    it('handleMissingRollbackOperation returns failure when operation not found and handling=compensate', () => {
      primitiveLayer = new PrimitiveToolLayer();
      const helper = primitiveLayer as unknown as {
        handleMissingRollbackOperation: (validated: any, input: Record<string, unknown>) => Record<string, unknown> | null;
      };

      const res = helper.handleMissingRollbackOperation({ operationId: 'does-not-exist', failureHandling: 'compensate' }, { failureHandling: 'compensate' });
