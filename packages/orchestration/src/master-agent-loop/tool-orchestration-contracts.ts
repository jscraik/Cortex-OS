...
export const ChainExecutionResultSchema = z.object({
    ...
    crossLayerMessages: z.array(z.object({
        from: z.string(),
        to: z.string(),
        type: z.string(),
        data: z.unknown()
...
export interface ToolOrchestrator {
     ...
     */
    registerToolVariant(toolId: string, variantId: string, metrics: OptimizationMetrics): Promise<void>;
...
export interface ToolChainExecutor {
     ...
     */
    executeParallel(tools: ToolDefinition[], context: ExecutionContext): Promise<ToolExecutionResult[]>;

    /**
     * Handle tool execution retry with backoff
     */
    retryTool(tool: ToolDefinition, context: ExecutionContext, attempt: number): Promise<ToolExecutionResult>;

    /**
     * Execute fallback tool if primary fails
     */
    executeFallback(tool: ToolDefinition, fallbackId: string, context: ExecutionContext): Promise<ToolExecutionResult>;
}
...
export interface DependencyManager {
     ...
     */
    injectDynamicDependencies(graph: DependencyGraph, dependencies: Record<string, string[]>): DependencyGraph;
}
