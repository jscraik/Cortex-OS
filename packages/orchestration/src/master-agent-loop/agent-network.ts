...
import { z } from 'zod';
import { AgentNetworkError, AgentNetworkErrorCode } from './agent-network-error';
	...
	async sendMessage(
		...
		if (this.isShutdown) {
			throw new AgentNetworkError(
				...
				{ sender, target, messageId: message.id, operation: 'sendMessage' },
			);
		}
		...
		for (attempts = 1; attempts <= this.config.routing.maxRetries; attempts++) {
			try {
				...
				// Deliver message
				const _deliveryResult = await this.deliverMessage(target, processedMessage);
	...
	async replayMessages(agentId: string): Promise<void> {
		...
		if (!handler) {
			throw new AgentNetworkError(
				...
				{ target: agentId, operation: 'replayMessages' },
			);
		}
	...
	private async deliverMessage(target: string, message: any): Promise<void> {
		...
		if (!handler) {
			throw new AgentNetworkError(
				AgentNetworkErrorCode.AGENT_NOT_FOUND,
				`No message handler registered for agent ${target}`,
				{ target, operation: 'deliverMessage' },
			);
		}
