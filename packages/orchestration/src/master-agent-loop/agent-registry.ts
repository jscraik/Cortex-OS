...
import { z } from 'zod';
import { AgentRegistryError, AgentRegistryErrorCode } from './agent-registry-error';
...
export class AgentRegistry extends EventEmitter {
    ...
    async registerAgent(agent: AgentRegistration): Promise<void> {
        ...
        if (this.registeredAgents.has(validatedAgent.agentId)) {
            throw AgentRegistryError.agentAlreadyExists(validatedAgent.agentId);
        }
    ...
    async updateAgent(agentId: string, updates: Partial<AgentRegistration>): Promise<void> {
        ...
        if (!existingAgent) {
            throw AgentRegistryError.agentNotFound(agentId, 'update');
        }
     ...
     */
    async subscribeToCapability(capability: string, callback: (agent: AgentRegistration) => void): Promise<void> {
    ...
    async getRegistryStatistics(): Promise<RegistryStatistics> {
        ...
        const averageHealth = healthStatuses.length > 0
