...
export class MLXAgent extends EventEmitter implements Agent {
   ...
   */
  async processInference(request: MLXInferenceRequest): Promise<MLXInferenceResponse> {
   ...
   */
  async generateCode(specification: string, language: string = 'typescript'): Promise<string> {
  ...
  async planTask(taskDescription: string): Promise<{
    ...
    const request: MLXInferenceRequest = {
      ...
      systemPrompt: 'You are an expert project planner. Create comprehensive, realistic plans.',
      maxTokens: 1024,
      temperature: 0.5,
    };
  ...
  async getStats(): Promise<MLXStats> {
    ...
    return {
      ...
      averageInferenceTime: this.totalInferenceTime / Math.max(this.inferenceCount, 1),
    };
  }
  ...
  private async loadModel(): Promise<void> {
    ...
    try {
      ...
      if (!loadResponse.ok) {
        ...
        throw new Error(`Failed to load model: ${errorData.error || loadResponse.statusText}`);
      }
  ...
  private async executeInference(request: MLXInferenceRequest): Promise<MLXInferenceResponse> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`MLX inference failed: ${errorData.error || response.statusText}`);
      }
