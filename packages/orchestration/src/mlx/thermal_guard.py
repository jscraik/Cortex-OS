...
class ThermalGuard:
    ...
    def __init__(
        ...
        # Current state
        self.current_metrics: Optional[ThermalMetrics] = None
    ...
    async def _collect_metrics(self) -> Optional[ThermalMetrics]:
    ...
    async def _get_gpu_temperature(self) -> float:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                import re
                temp_match = re.search(r'<key>gpu_temp</key>\s*<real>([\d.]+)</real>', output)
                if temp_match:
    ...
    async def _get_gpu_utilization(self) -> float:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                import re
                util_match = re.search(r'"PerformanceStatistics".*?"GPU".*?"utilization".*?(\d+)', output, re.DOTALL)
                if util_match:
    ...
    def get_thermal_stats(self) -> Dict[str, Any]:
        ...
        avg_gpu_temp = sum(m.gpu_temp_celsius for m in recent_metrics) / len(recent_metrics)
        max_gpu_temp = max(m.gpu_temp_celsius for m in recent_metrics)
        avg_power = sum(m.power_draw_watts for m in recent_metrics) / len(recent_metrics)
    ...
    def export_metrics_history(self, last_n: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    async def demo():
        ...
        for i in range(6):
            ...
            if thermal_guard.current_metrics:
                print(f"GPU Temp: {thermal_guard.current_metrics.gpu_temp_celsius:.1f}°C")
