...
class BatchedMLXInference:
    ...
    def __init__(
        ...
        logger.info(f"Batched inference initialized (batch_size={batch_size}, timeout={timeout_ms}ms)")
        ...
        for priority in sorted(RequestPriority, key=lambda p: p.value, reverse=True):
            ...
            while len(batch) < target_batch_size:
                try:
                    ...
                    request = await asyncio.wait_for(queue.get(), timeout=self.timeout_ms / 1000.0)
    ...
    async def _process_batch(self, batch: List[InferenceRequest]) -> None:
        ...
        try:
            ...
            for model_name, model_requests in model_groups.items():
                await self._process_model_group(model_name, model_requests, batch_start_time)
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        avg_processing_time = (
            self.total_processing_time / max(self.total_batches, 1)
        )
