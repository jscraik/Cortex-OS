...
class TestAdvancedKVCacheManager:
    ...
    def test_cache_system_prompt_success(self, cache_manager):
        ...
        with (
            patch("kv_cache_manager.load") as mock_load,
    ...
    def test_load_cached_prompt_from_metadata(self, cache_manager, temp_cache_dir):
        ...
        metadata = {"prompt_text": "Metadata prompt content", "model_id": "test-model"}
    ...
    def test_cleanup_memory_cache_over_limit(self, cache_manager):
        ...
        for i in range(cache_manager.max_memory_cache_items + 10):
            ...
            cache_manager.cache_metadata[f"cache_{i}"] = {
                "last_used": base_time
                - (100 - i)  # Older items have smaller timestamps
            }
        ...
        expected_keys = {
            f"cache_{i}" for i in range(10, cache_manager.max_memory_cache_items + 10)
        }
    ...
    async def test_warm_cache(self, cache_manager):
        ...
        with patch.object(cache_manager, "cache_system_prompt") as mock_cache:
            ...
            mock_cache.assert_any_call(
                "test-model", "System prompt 1 content", "prompt1"
            )
            mock_cache.assert_any_call(
                "test-model", "System prompt 2 content", "prompt2"
            )
    ...
    def test_cleanup_cache_directory_with_old_files(
    ...
    def test_default_system_prompts_structure(self):
        ...
        for _key, prompt in DEFAULT_SYSTEM_PROMPTS.items():
...
class TestKVCacheManagerPerformance:
    ...
    def test_cache_stats_performance(self, cache_manager):
        ...
        assert calculation_time < 0.01, (
            f"Stats calculation too slow: {calculation_time:.4f}s"
        )
    ...
    async def test_cache_warming_performance(self, cache_manager):
        ...
        large_prompts = {
            f"prompt_{i}": "x" * 1000
            for i in range(10)  # 10 large prompts
        }
...
class TestKVCacheManagerIntegration:
    ...
    async def test_full_cache_lifecycle(self, cache_manager):
        ...
        with (
            ...
            result = cache_manager.cache_system_prompt(
                model_id, system_prompt, cache_key
            )
            assert result is True
