...
class TestThermalGuard:
    ...
    async def test_collect_metrics_success(self, thermal_guard):
        ...
        with (
            patch.object(thermal_guard, "_get_gpu_temperature", return_value=80.0),
    ...
    async def test_collect_metrics_error(self, thermal_guard):
        ...
        with patch.object(
            thermal_guard, "_get_gpu_temperature", side_effect=Exception("GPU error")
        ):
            metrics = await thermal_guard._collect_metrics()
    ...
    async def test_get_gpu_temperature_apple_silicon(self, thermal_guard):
        ...
        with patch("asyncio.create_subprocess_exec") as mock_subprocess:
            ...
            mock_process.communicate = AsyncMock(
                return_value=(b"<key>gpu_temp</key><real>82.5</real>", b"")
            )
    ...
    async def test_get_gpu_temperature_fallback_parsing(self, thermal_guard):
        ...
        with patch("asyncio.create_subprocess_exec") as mock_subprocess:
            ...
            mock_process.communicate = AsyncMock(
                return_value=(
                    b"Temperature: 78.3C",  # Different format
                    b"",
                )
            )
            mock_process.returncode = 0
    ...
    async def test_get_gpu_utilization_apple_silicon(self, thermal_guard):
        ...
        with patch("asyncio.create_subprocess_exec") as mock_subprocess:
            mock_process = Mock()
            mock_process.communicate = AsyncMock(
                return_value=(b'"PerformanceStatistics" "GPU" "utilization" 75', b"")
            )
            mock_process.returncode = 0
            mock_subprocess.return_value = mock_process
    ...
    async def test_get_power_draw_error(self, thermal_guard):
        ...
        with patch(
            ...
            side_effect=Exception("Power command failed"),
        ):
            power = await thermal_guard._get_power_draw()
    ...
    def test_update_history_trim(self, thermal_guard):
        ...
        assert (
            thermal_guard.metrics_history[-1].timestamp
            > thermal_guard.metrics_history[0].timestamp
        )
    ...
    def test_get_thermal_recommendation_cpu_only(self, thermal_guard):
        ...
        thermal_guard.current_metrics = ThermalMetrics(
            ...
            gpu_utilization=80.0,
            power_draw_watts=150.0,
            thermal_state=ThermalState.CPU_ONLY,
...
class TestResourceMonitor:
    ...
    def test_record_inference_latency_overflow(self, resource_monitor):
        ...
        assert (
            len(resource_monitor.latency_samples)
            == resource_monitor.max_latency_samples
        )
...
class TestThermalGuardIntegration:
    ...
    async def test_thermal_state_transitions(self):
        ...
        assert (
            thermal_guard._determine_thermal_state(normal_metrics)
            == ThermalState.NORMAL
        )
        assert (
            thermal_guard._determine_thermal_state(throttled_metrics)
            == ThermalState.THROTTLED
        )
        assert (
            thermal_guard._determine_thermal_state(critical_metrics)
            == ThermalState.CPU_ONLY
        )
