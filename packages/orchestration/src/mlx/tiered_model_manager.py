...
class TieredMLXModelManager:
    ...
    def __init__(self, max_memory_gb: int = 28):
        ...
        self._always_on_task = asyncio.create_task(self._load_always_on_models())
    ...
    def calculate_task_complexity(
        ...
        if any(
            ...
            for word in ["simple", "quick", "basic", "utility"]
        ):
            complexity += 0.1
        elif any(
            word in description_lower for word in ["moderate", "standard", "normal"]
        ):
            complexity += 0.3
        elif any(
            word in description_lower
            ...
            word in description_lower for word in ["expert", "research", "analysis"]
        ):
            complexity += 0.8
        else:
        ...
        if any(
            word in description_lower
            for word in ["code", "programming", "debug", "refactor"]
        ):
            complexity += 0.2
        if any(
            word in description_lower for word in ["image", "vision", "visual", "photo"]
        ):
            complexity += 0.1
        if any(
            word in description_lower
            for word in ["reasoning", "logic", "math", "calculation"]
        ):
            complexity += 0.2
        if any(
            word in description_lower
            for word in ["creative", "story", "writing", "content"]
        ):
            complexity += 0.1
    ...
    def smart_load(self, task_description: str, context_length: int = 0) -> str | None:
        ...
        for model_name in candidates:
            if self._can_load_model(model_name):
                ...
                if success:
                    logger.info(
                        f"Selected {model_name} for complexity {complexity:.2f}"
                    )
                    return model_name
        ...
        logger.warning(
            f"No suitable model available for task complexity {complexity:.2f}"
        )
        return None
    ...
    def _can_load_model(self, model_name: str) -> bool:
        ...
        if config.ram_gb > available_memory:
            ...
            return bool(self._evict_for_space(config.ram_gb))
    ...
    def _evict_for_space(self, required_gb: float) -> bool:
        ...
        if freed_memory >= needed:
            ...
            logger.warning(
                f"Could not free enough memory (needed {needed:.1f}GB, freed {freed_memory:.1f}GB)"
            )
            return False
    ...
    def get_loaded_model(self, model_name: str) -> LoadedModel | None:
    ...
    def get_performance_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (self.total_loads - self.load_failures)
            / max(self.total_loads, 1),
            "models_by_tier": {
                tier.value: [
                    name
