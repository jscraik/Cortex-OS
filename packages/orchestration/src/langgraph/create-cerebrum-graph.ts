...
import { withThermalGuard, type ThermalGuardPatch, type ThermalGuardState } from './middleware/thermal-guard.js';
import {
        THERMAL_CTX_KEY,
        getThermalContext,
        type ThermalContext,
} from './state/thermal-history.js';
import { ThermalPolicy, type ThermalDecision, type ThermalEvent } from './thermal/thermal-policy.js';
...
function buildTelemetryEvent(
        ...
        response: { action: 'pause' | 'resume'; paused: boolean; fallbackProvider?: string; cooldownUntil?: number },
): NoTelemetryEvent | undefined {
        if (!event) return undefined;
        const session = state.session;
        const cooldownIso = response.cooldownUntil ? new Date(response.cooldownUntil).toISOString() : undefined;
