...
export class MultiAgentCoordinator extends EventEmitter {
        ...
        async coordinateWorkflows(
                ...
                const requests: DistributedWorkflowRequest[] = Object.entries(inputs).map(
                        ([id, value]) => ({ id, input: value.input, metadata: value.metadata }),
                );
                const results = await this.coordinateDistributedWorkflows(requests);
                return results.reduce<Record<string, { output?: string; state: N0State; coordinationId: string }>>(
                        (acc, result) => {
                                acc[result.id] = {
        ...
        private async runWorkflow(workflow: RegisteredWorkflow, input: string): Promise<string | undefined> {
...
function cloneState(state: N0State): N0State {
        const raw = typeof structuredClone === 'function' ? structuredClone(state) : JSON.parse(JSON.stringify(state));
