...
import { loadSubagents as discoverSubagents, subagentTools } from '@cortex-os/agent-contracts';
import { runSlash as defaultRunSlash, parseSlash } from '@cortex-os/commands';
...
export async function buildN0(options) {
	...
	const kernelDefinitions = (kernelBinding?.tools ?? kernelTools).map((tool) =>
	...
	let subagentManager;
	let subagentMap = options.subagents ?? new Map();
	...
	const graph = new StateGraph(N0Annotation)
		.addNode('parse_or_command', async (state) => {
			const ctxPatch = { sessionStarted: true };
		...
		.addNode('pre_prompt_hooks', async (state) => {
		...
		.addNode('plan_or_direct', async (state) => {
		...
		.addNode('llm_with_tools', async (state) => {
		...
		.addNode('tool_dispatch', async (state) => {
		...
		.addNode('compact_if_needed', async (state) => {
			const messages = state.messages ?? [];
		...
		.addNode('stream_and_log', async (state) => {
	...
	graph.addConditionalEdges('parse_or_command', (state) =>
	...
	graph.addConditionalEdges('llm_with_tools', (state) => {
		const messages = state.messages ?? [];
		const last = messages[messages.length - 1];
		if (last && isAIMessage(last) && last.tool_calls && last.tool_calls.length > 0) {
	...
	graph.addConditionalEdges('tool_dispatch', (state) => {
...
function resolveKernelBinding(options) {
...
function createKernelBindingFromTools(tools, options) {
	const allowLists = {
...
function collectAllowList(tools, surface) {
...
async function ensureHooks(hooks, options) {
	if (hooks) {
	...
	return instance;
	function _ensureHooks(hooks, options) {
		if (hooks) {
			return Promise.resolve(hooks);
		}
		const instance = new CortexHooks();
	...
	function _createKernelRunBash(tools) {
		const tool = findKernelTool(tools, ['kernel.bash', 'shell.exec']);
		if (!tool) return undefined;
		return async (cmd, _allowlist) => {
	...
	function _createKernelReadFile(tools) {
	...
	function findKernelTool(tools, names) {
		return tools.find((tool) => names.includes(tool.name));
	}
	function normaliseKernelExitCode(payload) {
	...
	function extractKernelFileContent(payload) {
	...
	function _createToolHookAdapter(hooks) {
		return {
			run: async (event, ctx) => hooks.run(event, ctx),
		};
	}
	function _kernelToolToDefinition(tool) {
		return {
			name: tool.name,
	...
	function _subagentToolToDefinition(binding) {
		return {
	...
	function _toStructuredTool(definition) {
	...
	function _extractCommandMetadata(metadata) {
		if (!metadata) {
			return undefined;
		}
		if (isRecord(metadata)) {
			const command = Object.hasOwn(metadata, 'command') ? metadata.command : undefined;
			if (isRecord(command)) {
				return command;
			}
	...
	function _extractStringArray(value) {
	...
	function _extendCtx(base, patch) {
		return { ...(base ?? {}), ...patch };
	}
	function _ensureSystemPrompt(messages, prompt, ctx) {
	...
	function _normaliseToAIMessage(message) {
	...
	async function _safeRunHook(hooks, event, ctx, logger) {
		try {
			return await hooks.run(event, ctx);
		} catch (error) {
			logger.warn?.('brAInwav hook execution failed', { event, error });
			return undefined;
		}
	}
	function _sessionHookContext(state) {
		return {
			event: 'SessionStart',
			cwd: state.session.cwd,
			user: state.session.user,
	...
	function _stopHookContext(state, output) {
		return {
			event: 'Stop',
			cwd: state.session.cwd,
			user: state.session.user,
	...
	function _compactMessages(messages, maxMessages, maxChars) {
		const preserved = [];
	...
	function _deriveOutputFromMessages(messages) {
	...
	function _defaultPlanResolver(state) {
...
//# sourceMappingURL=n0-graph.js.map
