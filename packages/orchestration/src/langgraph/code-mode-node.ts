...
export class CodeModeDispatcher {
        ...
        async dispatch(state: N0State, actions: CodeModeAction[], ctx: CodeModeContext): Promise<N0State> {
...
export function recordCodeModeBenchmark(fileWriter: { write: (content: string) => Promise<void> }, result: CodeModeResult): Promise<void> {
        const payload = {
