...
function toAttributes(tool, metadata, session) {
...
function addToGauge(delta, attributes) {
...
export function recordDispatchStart(tool, metadata, session) {
	snapshot.started += 1;
	snapshot.active += 1;
	const attributes = toAttributes(tool, metadata, session);
	addToGauge(1, attributes);
}
export function recordDispatchSkip(tool, metadata, session) {
	recordDispatchOutcome(tool, 'skipped', undefined, undefined, metadata, session);
}
export function recordDispatchOutcome(tool, outcome, durationMs, tokensUsed, metadata, session) {
	const attributes = toAttributes(tool, metadata, session);
	if (outcome !== 'skipped') {
...
export function getDispatchMetricsSnapshot() {
	return {
...
export function resetDispatchMetricsSnapshot() {
	snapshot.started = 0;
	snapshot.active = 0;
...
//# sourceMappingURL=tool-dispatch-metrics.js.map
