...
export function evaluatePersonaCompliance(persona: Persona): PersonaCompliance {
	...
	const hasA11yCategory = allCategories.includes('a11y') || allCategories.includes('accessibility');
	...
	if (!a11y)
		reasons.push('a11y requirements not fully satisfied (need a11y category and WCAG 2.2 AA)');
	if (!security)
		reasons.push(
