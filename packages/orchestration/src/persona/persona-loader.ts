...
export async function loadPersona(): Promise<Persona> {
  ...
  const personaPath = resolveConfigPath(override ?? '.cortex/library/personas/cerebrum.yaml');
  ...
  const explicitPolicies: PersonaPolicy[] = (parsed?.policies ?? []).map((p, i: number) => ({
  ...
  const checkItems: PersonaPolicy[] = Array.isArray(parsed?.checks)
    ? (parsed?.checks as unknown[])
        ...
        .map((c, i) => {
          ...
          const category = idx > 0 ? text.slice(0, idx).trim().toLowerCase() : undefined;
          const desc = idx > 0 ? text.slice(idx + 1).trim() : text;
          return { id: `check:${i + 1}`, description: desc, category } as PersonaPolicy;
        })
    : [];
...
export function evaluatePersonaCompliance(persona: Persona): PersonaCompliance {
  const texts = (persona.policies ?? []).map((p) => p.description ?? '').filter(Boolean);
  const categories = texts
    .map((t) => t.split(':')[0]?.trim().toLowerCase())
    .filter((c) => c && c.length > 0);
  const hasA11yCategory = categories.includes('a11y') || categories.includes('accessibility');
  const hasSecurityCategory = categories.includes('security');
  const hasWcagRule = texts.some((t) => /wcag\s*2\.2\s*aa/i.test(t));
  const hasSecurityStandards = texts.some((t) => /owasp\s*asvs/i.test(t) || /llm\s*top-?10/i.test(t));

  const a11y = hasA11yCategory && hasWcagRule;
  const security = hasSecurityCategory && hasSecurityStandards;
  const reasons: string[] = [];
  if (!a11y) reasons.push('a11y requirements not fully satisfied (need a11y category and WCAG 2.2 AA)');
  if (!security) reasons.push('security requirements not fully satisfied (need security category and OWASP ASVS or LLM Top-10)');
