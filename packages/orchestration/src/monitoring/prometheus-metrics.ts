...
export class MetricsCollector {
	...
	recordRequest(
		...
		requestMetrics.totalRequests.labels(priority, complexityLevel, status).inc();

		requestMetrics.requestDuration.labels(priority, complexityLevel).observe(duration);
	}

	/**
	 * Update agent pool metrics
	 */
	updateAgentMetrics(available: number, active: number, specialization: string = 'general'): void {
		agentMetrics.availableAgents.labels(specialization).set(available);

		agentMetrics.activeAgents.labels(specialization).set(active);
	 ...
	 */
	recordPerformance(cpuPercent: number, memoryPercent: number, throughput: number): void {
	...
	recordError(
		...
		errorMetrics.totalErrors.labels(errorType, component, severity).inc();
	}
		...
		switch (eventType) {
			...
			case 'violation':
				securityMetrics.securityViolations
					.labels(details.violation_type || 'unknown', details.severity || 'medium')
					.inc();
				break;
			case 'rate_limit':
				securityMetrics.rateLimitHits
					.labels(details.client_id || 'unknown', details.endpoint || 'unknown')
					.inc();
				break;
		}
	}
