...
// Schemas for workflow orchestration tools
const PlanWorkflowInputSchema = z.object({
	workflowName: z.string().min(1).max(180),
	goal: z.string().min(1).max(4096),
...
const PlanWorkflowResultSchema = z.object({
...
// Schemas for task management tools
...
const UpdateTaskStatusResultSchema = z.object({
	taskId: z.string().min(1).max(128),
	status: z.nativeEnum(TaskStatus),
...
// Schemas for process monitoring tools
const GetProcessStatusInputSchema = z.object({
	workflowId: z.string().uuid(),
	includeTimeline: z.boolean().optional(),
	includeMetrics: z.boolean().optional(),
});

const GetProcessStatusResultSchema = z.object({
	workflowId: z.string().uuid(),
	status: z.nativeEnum(TaskStatus),
