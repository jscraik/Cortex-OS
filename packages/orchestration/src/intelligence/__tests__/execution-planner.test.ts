...
describe('Phase 1.4: Enhanced ExecutionPlanner (nO Architecture)', () => {
  ...
  describe('DAG Creation and Dependency Resolution', () => {
    ...
    it('should detect and reject cyclic dependencies', async () => {
      ...
      await expect(planner.createPlanFromWorkflow(cyclicWorkflow))
  ...
  describe('Resource-Aware Planning', () => {
    it('should create plans that respect resource constraints', async () => {
      ...
      const plan = await planner.createResourceAwarePlan(resourceConstrainedWorkflow, resourceConstraints);

      expect(plan.resourceAllocation.memoryMB).toBeLessThanOrEqual(resourceConstraints.maxMemoryMB);
      expect(plan.resourceAllocation.cpuPercent).toBeLessThanOrEqual(resourceConstraints.maxCpuPercent);
  ...
  describe('Dynamic Plan Adaptation', () => {
    ...
    it('should handle step failures with contingency plans', async () => {
      ...
      expect(recoveredPlan.estimatedDuration).toBeGreaterThan(planWithContingencies.estimatedDuration);
    });
  });
  ...
  describe('Advanced DAG Operations', () => {
    it('should perform topological sorting for complex workflows', async () => {
      const complexWorkflow = {
        ...
        steps: {
          'a': { name: 'Step A', type: 'execution', agentRequirements: ['general'], estimatedDuration: 1000, parameters: {}, next: 'c' },
          'b': { name: 'Step B', type: 'execution', agentRequirements: ['general'], estimatedDuration: 1000, parameters: {}, next: 'c' },
          'c': { name: 'Step C', type: 'execution', agentRequirements: ['general'], estimatedDuration: 1000, parameters: {}, branches: [{ to: 'd', condition: 'always' }, { to: 'e', condition: 'always' }] },
          'd': { name: 'Step D', type: 'execution', agentRequirements: ['general'], estimatedDuration: 1000, parameters: {}, next: 'f' },
          'e': { name: 'Step E', type: 'execution', agentRequirements: ['general'], estimatedDuration: 1000, parameters: {}, next: 'f' },
          'f': { name: 'Step F', type: 'execution', agentRequirements: ['general'], estimatedDuration: 1000, parameters: {} },
        },
      };
    ...
    it('should calculate critical path and execution time estimates', async () => {
      const timedWorkflow = {
        ...
        steps: {
          'start': { name: 'Start', type: 'initialization', agentRequirements: ['coordinator'], estimatedDuration: 500, parameters: {}, branches: [{ to: 'path-a', condition: 'always' }, { to: 'path-b', condition: 'always' }] },
          'path-a': { name: 'Path A', type: 'execution', agentRequirements: ['executor'], estimatedDuration: 3000, parameters: {}, next: 'end' },
          'path-b': { name: 'Path B', type: 'execution', agentRequirements: ['executor'], estimatedDuration: 5000, parameters: {}, next: 'end' },
          'end': { name: 'End', type: 'finalization', agentRequirements: ['coordinator'], estimatedDuration: 1000, parameters: {} },
        },
      };
