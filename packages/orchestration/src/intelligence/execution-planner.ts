...
export interface WorkflowDefinition {
	...
	steps: Record<
		string,
		WorkflowStep & { next?: string; branches?: Array<{ to: string; condition: string }> }
	>;
	resourceRequirements?: {
		memoryMB: number;
		cpuPercent: number;
...
export class ExecutionPlanner {
	...
	private linkNextDependencies(
		byId: Map<string, WorkflowStep>,
		stepsObject: Record<string, { next?: string }>,
	): void {
		for (const [stepId, stepData] of Object.entries(stepsObject)) {
	...
	private linkBranchDependencies(
		byId: Map<string, WorkflowStep>,
		stepsObject: Record<string, { branches?: Array<{ to: string }> }>,
	): void {
		for (const [stepId, stepData] of Object.entries(stepsObject)) {
	...
	private linkDependencies(
		...
		stepsObject: Record<
			string,
			{ next?: string; branches?: Array<{ to: string; when?: string; condition?: string }> }
		>,
	): void {
		const byId = new Map<string, WorkflowStep>(stepsArray.map((s) => [s.id, s]));
		this.linkNextDependencies(byId, stepsObject);
		this.linkBranchDependencies(
			byId,
			stepsObject as Record<string, { branches?: Array<{ to: string }> }>,
		);
	}
	 ...
	 */
	async createPlanFromWorkflow(
		workflow: WorkflowDefinition | MinimalWorkflowInput,
	): Promise<MinimalExecutionPlan> {
		// Convert steps object to array with dependencies resolved
		const stepsArray = this.convertStepsObjectToArray(
			workflow.steps as MinimalWorkflowInput['steps'],
		);
	...
	async optimizeExecutionOrder(workflow: WorkflowDefinition): Promise<OptimizedPlan> {
		...
		const totalSequentialDuration = stepsArray.reduce(
			(sum: number, step: WorkflowStep) => sum + step.estimatedDuration,
			0,
		);
	...
	async analyzeResourceConflicts(workflow: WorkflowDefinition): Promise<ResourceConflictAnalysis> {
		...
		const resourceRequirements = workflow.resourceRequirements || {
			memoryMB: 1024,
			cpuPercent: 80,
			timeoutMs: 30000,
		};
		...
		for (const group of parallelGroups) {
			const totalMemory = group.reduce(
				(sum, step) => sum + (step.resourceRequirements?.memoryMB || 256),
				0,
			);
	...
	async validatePlan(workflow: WorkflowDefinition): Promise<PlanValidation> {
			...
			for (const step of stepsArray) {
				for (const requirement of step.agentRequirements) {
					if (!workflow.constraints.availableAgentTypes.includes(requirement)) {
						...
						suggestions.push(
							`Add '${requirement}' agent type or modify step '${step.id}' to use available agent types`,
						);
					}
				}
			}
		...
		const totalMemory = stepsArray.reduce(
			(sum: number, step: WorkflowStep) => sum + (step.resourceRequirements?.memoryMB || 256),
			0,
		);

		if (totalMemory > resourceRequirements.memoryMB * 2) {
			// Allow some overhead
			violations.push({
				type: 'resource_limit',
				stepId: 'workflow',
			...
			suggestions.push(
				'Reduce individual step memory requirements or increase workflow memory limit',
			);
		}
	...
	async optimizePlan(
		...
		const stepsArray = this.convertStepsObjectToArray(workflow.steps);
		const originalDuration = stepsArray.reduce(
			(sum: number, step: WorkflowStep) => sum + step.estimatedDuration,
			0,
		);
	 ...
	 */
	private convertStepsObjectToArray(
		stepsObject: Record<
			string,
			{
				id?: string;
		...
		if (totalMemory > maxMemory) {
			...
			for (const stepId in balancedWorkflow.steps) {
				...
				if (step.resourceRequirements) {
					step.resourceRequirements.memoryMB = Math.max(
						64,
						Math.floor(step.resourceRequirements.memoryMB * scaleFactor),
					);
				}
			}
		}
	...
	async adaptPlan(
		originalPlan: MinimalExecutionPlan,
		feedback: { stepFailures: string[]; performance: number },
	): Promise<MinimalExecutionPlan> {
	...
	async executeContingency(
		...
		const contingency = plan.contingencyPlans?.find(
			(cp: { condition: string; alternativeSteps: string[] }) =>
	...
	async createIntegratedPlan(
		workflow: WorkflowDefinition,
		options: { useStrategySelector: boolean; optimizeResources: boolean },
	): Promise<MinimalExecutionPlan> {
