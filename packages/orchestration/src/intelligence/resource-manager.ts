...
export class ResourceManager {
  ...
  async allocateResources(plan: ExecutionPlan): Promise<ResourceAllocation> {
    ...
    const estimatedCost = this.calculateCost(totalMemoryMB, totalCpuPercent, durationHours, actualAgentCount);
   ...
   */
  async createAgentPool(requirements: Array<{
    specialization: string;
    count: number;
  }>): Promise<AgentPool> {
  ...
  async enforceConstraints(
    ...
    if (plan.resourceAllocation.memoryMB > constraints.maxMemoryMB) {
      violations.push(`Memory usage ${plan.resourceAllocation.memoryMB}MB exceeds limit ${constraints.maxMemoryMB}MB`);
    ...
    if (plan.resourceAllocation.cpuPercent > constraints.maxCpuPercent) {
      violations.push(`CPU usage ${plan.resourceAllocation.cpuPercent}% exceeds limit ${constraints.maxCpuPercent}%`);
    ...
    if (requiredAgents > constraints.maxConcurrentAgents) {
      violations.push(`Required agents ${requiredAgents} exceeds limit ${constraints.maxConcurrentAgents}`);
  ...
  async optimizeForCost(
    ...
    if (!costParams.prioritizeSpeed) {
      ...
      optimizedPlan.resourceAllocation.cpuPercent = Math.ceil(plan.resourceAllocation.cpuPercent * 0.9);
  ...
  async analyzeResourceEfficiency(
    ...
    const avgMemoryUtilization = historicalData.reduce((sum, data) =>
      sum + (data.usedMemoryMB / data.allocatedMemoryMB), 0
    ) / historicalData.length;

    const avgCpuUtilization = historicalData.reduce((sum, data) =>
      sum + (data.usedCpuPercent / data.allocatedCpuPercent), 0
    ) / historicalData.length;
    ...
    return {
      ...
      trends: {
        ...
        cpuUsageTrend: 'stable',
        efficiencyTrend: avgMemoryUtilization > 0.7 && avgCpuUtilization > 0.7 ? 'improving' : 'stable',
      },
    };
  }
  ...
  async evaluatePreemption(
    ...
    const canPreempt = lowPriorityExecutions.length > 0 &&
      lowPriorityExecutions.reduce((sum, exec) => sum + exec.allocatedAgents.length, 0) >= requiredAgents;

    const affectedExecutions = lowPriorityExecutions.slice(0, Math.ceil(requiredAgents / 2)).map(exec => ({
      planId: exec.planId,
  ...
  async checkQuotas(
    ...
    if (totalMemoryRequired > userQuotas.dailyMemoryLimitMB) {
      ...
      adjustedPlan.resourceAllocation.memoryMB = Math.max(64, userQuotas.dailyMemoryLimitMB - userQuotas.currentUsage.memoryMB);
      withinLimits = false;
    }
    ...
    if (totalCpuRequired > userQuotas.dailyCpuLimitPercent) {
      ...
      adjustedPlan.resourceAllocation.cpuPercent = Math.max(10, (userQuotas.dailyCpuLimitPercent - userQuotas.currentUsage.cpuPercent) / durationHours);
      withinLimits = false;
    }
  ...
  private assignStepsToAgent(steps: ExecutionPlan['steps'], agentIndex: number, totalAgents: number): string[] {
    if (totalAgents === 1) {
  ...
  private calculateCost(memoryMB: number, cpuPercent: number, durationHours: number, agentCount: number): number {
  ...
  private getResourceUsageForSpecialization(specialization: string): { memoryMB: number; cpuPercent: number } {
