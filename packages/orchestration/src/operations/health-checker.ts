...
export class StandardHealthChecks {
     ...
     * Memory usage check
     */
    static memory(thresholds: { warning: number; critical: number } = { warning: 0.8, critical: 0.95 }): HealthCheck {
        return {
            name: 'memory',
            check: async (): Promise<HealthCheckResult> => {
     ...
     */
    static agentPool(getPoolStatus: () => Promise<{ active: number; total: number; healthy: number }>): HealthCheck {
        return {
            name: 'agent-pool',
            critical: true,
            check: async (): Promise<HealthCheckResult> => {
