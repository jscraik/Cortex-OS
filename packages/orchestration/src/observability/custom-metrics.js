...
export class OrchestrationMetrics {
	...
	plannerPersistenceEvents;
	plannerPersistenceDuration;
	...
	recordTaskCreated(workflowId, status, priority) {
	...
	recordTaskCompleted(workflowId, agentRole, status, result, duration) {
	...
	recordAgentRegistered(agentRole, agentType) {
	...
	recordAgentExecution(agentId, agentRole, status, taskType, duration) {
	...
	recordPlannerPersistenceEvent(eventType, status, errorType, durationSeconds) {
	...
	updateCircuitBreakerState(circuitBreakerName, state) {
		let stateValue;
		if (state === 'closed') {
			stateValue = 0;
		} else if (state === 'open') {
	...
	recordCircuitBreakerRequest(circuitBreakerName, result) {
