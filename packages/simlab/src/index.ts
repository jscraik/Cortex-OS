...
import { createInMemoryStore, createStdOutput, createJsonOutput, withTimestamp } from '@cortex-os/lib';
...
export function handleSimlab(input: unknown): string {
  ...
  if (!parsed.success) {
    ...
    log({ event: 'invalid-input', issues: parsed.error.issues });
    return createJsonOutput({ error: err.toJSON() });
  }
  const { config, command, json } = parsed.data;
  if (!ALLOWED_SCENARIOS.includes(command.scenario)) {
    const err = new StructuredError('SCENARIO_NOT_ALLOWED', 'Scenario not allowed', { scenario: command.scenario });
    log({ event: 'scenario-blocked', scenario: command.scenario });
    return createJsonOutput({ error: err.toJSON() });
  }
  ...
  log({ event: 'executed', scenario: command.scenario, step: command.step });
