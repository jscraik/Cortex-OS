...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/simlab/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** SimLab is an internal simulation and testing framework for evaluating Cortex-OS agent interactions. It provides deterministic, local-first simulation capabilities with comprehen...
- **Responsibilities:**
  - Implement the Simlab domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter simlab dev

# Build
pnpm --filter simlab build

# Quality (affected-aware if available)
pnpm --filter simlab lint
pnpm --filter simlab typecheck
pnpm --filter simlab test
pnpm --filter simlab test:coverage
```
...
SIMLAB_BASE_URL=https://example.local
SIMLAB_DB_URL=postgres://user:pass@localhost:5432/simlab
...
```bash
pnpm --filter simlab test:coverage
pnpm security:scan --scope=simlab
pnpm structure:validate --scope=simlab
```
...
- SBOM: CycloneDX generated at `./sbom/simlab.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"simlab"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"simlab","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter simlab test:e2e
pnpm --filter simlab test:smoke
pnpm --filter simlab chaos:probe
```
...
# Dev & run
pnpm --filter simlab dev
pnpm --filter simlab start

# Build & verify
pnpm --filter simlab build
pnpm --filter simlab lint && pnpm --filter simlab typecheck
pnpm --filter simlab test && pnpm --filter simlab test:coverage
