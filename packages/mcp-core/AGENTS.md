...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/mcp-core/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Minimal building blocks for the Model Context Protocol.
- **Responsibilities:**
  - Implement the MCP Core domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter mcp-core dev

# Build
pnpm --filter mcp-core build

# Quality (affected-aware if available)
pnpm --filter mcp-core lint
pnpm --filter mcp-core typecheck
pnpm --filter mcp-core test
...
MCP_CORE_BASE_URL=https://example.local
MCP_CORE_DB_URL=postgres://user:pass@localhost:5432/mcp_core
...
```bash
pnpm --filter mcp-core test:coverage
pnpm security:scan --scope=mcp-core
pnpm structure:validate --scope=mcp-core
```
...
- SBOM: CycloneDX generated at `./sbom/mcp-core.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"mcp-core"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"mcp-core","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter mcp-core test:e2e
pnpm --filter mcp-core test:smoke
pnpm --filter mcp-core chaos:probe
```
...
# Dev & run
pnpm --filter mcp-core dev
pnpm --filter mcp-core start

# Build & verify
pnpm --filter mcp-core build
pnpm --filter mcp-core lint && pnpm --filter mcp-core typecheck
pnpm --filter mcp-core test && pnpm --filter mcp-core test:coverage
...
pnpm --filter mcp-core mcp:smoke
```
