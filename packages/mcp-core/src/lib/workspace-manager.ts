...
export class WorkspaceManager {
        ...
        private resolveBaseDirectory(): string {
                const baseDir = this.configuredBaseDir ?? process.env.CORTEX_WORKSPACES_DIR ?? join(process.cwd(), '.cortex-workspaces');
        ...
        private async loadWorkspaceMetadata(workspacePath: string): Promise<WorkspaceMetadata | null> {
                try {
                        ...
                        const rawMetadata = JSON.parse(metadataContent) as Omit<WorkspaceMetadata, 'createdAt' | 'lastAccessed'> & {
        ...
        private async saveWorkspaceMetadata(workspacePath: string, metadata: WorkspaceMetadata): Promise<void> {
                const metadataPath = join(workspacePath, '.workspace-metadata.json');
                const payload = {
                        ...metadata,
