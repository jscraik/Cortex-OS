...
export class CreateCoordinationSessionTool implements McpTool<CreateCoordinationSessionInput, CreateCoordinationSessionResult> {
    readonly name = 'coordination-create-session';
    readonly description = 'Creates a new multi-agent coordination session with security controls and isolation';
    ...
    async execute(
        ...
        try {
            ...
            console.log(`brAInwav Coordination: Created secure coordination session ${coordinationId} with ${input.strategy} strategy`);
...
export class RegisterAgentTool implements McpTool<RegisterAgentInput, RegisterAgentResult> {
    ...
    async execute(
        ...
        try {
            ...
            if (!session) {
                throw new ToolExecutionError(`brAInwav Coordination: Session ${input.coordinationId} not found`, {
            ...
            if (existingAgent) {
                throw new ToolExecutionError(`brAInwav Coordination: Agent ${input.agent.id} already registered`, {
            ...
            console.log(`brAInwav Coordination: Registered agent ${agent.id} in session ${input.coordinationId} with security validation`);
            ...
            const message = error instanceof Error ? error.message : String(error);
            throw new ToolExecutionError(`brAInwav Coordination: Agent registration failed - ${message}`, {
    ...
    private async validateAgentSecurity(agent: Partial<Agent>, securityContext: SecurityContext): Promise<boolean> {
...
const findBestAgentByCapabilities = (agents: Agent[], task: CoordinationTask): Agent | undefined => {
    ...
    for (const agent of agents) {
        ...
        const matchCount = requiredCapabilities.filter(cap => agent.capabilities.includes(cap)).length;
...
export class AssignTaskTool implements McpTool<AssignTaskInput, AssignTaskResult> {
    ...
    readonly description = 'Assigns a task to an agent with security validation and strategy selection';
    ...
    async execute(
        ...
        try {
            ...
            if (assignedAgent) {
                await this.processAgentAssignment(task, assignedAgent, session.securityContext, input.validateSecurity !== false);
            }
            ...
            console.log(`brAInwav Coordination: Assigned task ${task.id} to agent ${assignedAgent?.id || 'unassigned'} using ${input.strategy} strategy`);
    ...
    private async processAgentAssignment(
        ...
        if (validateSecurity) {
            ...
            if (!isValid) {
                throw new ToolExecutionError('brAInwav Coordination: Task assignment failed security validation', {
                    code: 'E_SECURITY_VALIDATION_FAILED',
                });
            }
        }
