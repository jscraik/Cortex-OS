...
import {
        ...
        type PlanningTask,
} from '../lib/planning-session-manager.js';
...
export class CreatePlanningSessionTool implements McpTool<CreatePlanningSessionInput, CreatePlanningSessionResult> {
        readonly name = 'planning-create-session';
        readonly description = 'Creates a new DSP-integrated planning session with brAInwav context management';
        readonly inputSchema = CreatePlanningSessionInputSchema;

        constructor(private readonly manager: PlanningSessionManager = sharedPlanningSessionManager) {}
        ...
        async execute(
                ...
                try {
                        ...
                        console.log(`brAInwav Planning: Created DSP planning session ${sessionId} with nO architecture support`);
...
export class ExecutePlanningPhaseTool implements McpTool<ExecutePlanningPhaseInput, ExecutePlanningPhaseResult> {
        readonly name = 'planning-execute-phase';
        readonly description = 'Executes a specific planning phase using DSP patterns with brAInwav optimization';
        ...
        async execute(
                ...
                try {
                        const planningContext = this.manager.getContext(input.sessionId);
                        if (!planningContext) {
                        ...
                        console.log(`brAInwav Planning: Executed DSP phase ${input.phase} for session ${input.sessionId}`);
        ...
        private async executeDSPPhase(
                ...
                switch (phase) {
                        ...
                        case PlanningPhase.ANALYSIS:
                                return { success: true, result: { analyzed: true, complexity: context.metadata.complexity } };
...
export class GetPlanningStatusTool implements McpTool<GetPlanningStatusInput, GetPlanningStatusResult> {
        readonly name = 'planning-get-status';
        readonly description = 'Retrieves current status of a DSP planning session with brAInwav telemetry';
        ...
        async execute(
                ...
                try {
                        ...
                        const completedSteps = planningContext.steps.filter((step) => step.status === 'completed').length;
                        ...
                        console.log(
                                `brAInwav Planning: Retrieved status for DSP session ${input.sessionId} - ${status} (${Math.round(
                                        progress * 100,
                                )}%)`,
                        );
...
export const planningSessionManager = sharedPlanningSessionManager;

export { PlanningPhase } from '../lib/planning-session-manager.js';
export type { PlanningContext, PlanningTask } from '../lib/planning-session-manager.js';
