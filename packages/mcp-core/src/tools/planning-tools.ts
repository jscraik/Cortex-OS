    ...
    private async executeDSPPhase(
        ...
        switch (phase) {
            ...
            case PlanningPhase.COMPLETION:
                return { success: true, result: { completed: true, duration: Date.now() - context.metadata.createdAt.getTime() } };
            default:
...
export class GetPlanningStatusTool implements McpTool<GetPlanningStatusInput, GetPlanningStatusResult> {
    ...
    async execute(
        ...
        try {
            ...
            console.log(`brAInwav Planning: Retrieved status for DSP session ${input.sessionId} - ${status} (${Math.round(progress * 100)}%)`);
