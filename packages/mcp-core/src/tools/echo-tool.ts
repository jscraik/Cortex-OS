import { z } from 'zod';
import type { ToolExecutionContext, McpTool } from '../tools.js';
...
export class EchoTool implements McpTool<EchoToolInput, EchoToolResult> {
        readonly name = 'echo';
        readonly description = 'Echoes the provided message with optional transformations for connectivity testing.';
        ...
        async execute(input: EchoToolInput, context?: ToolExecutionContext): Promise<EchoToolResult> {
                if (context?.signal?.aborted) {
                        throw new ToolExecutionError('Echo tool execution aborted.', { code: 'E_TOOL_ABORTED' });
                }
