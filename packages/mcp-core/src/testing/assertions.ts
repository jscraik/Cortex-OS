...
export function assertRecord<T extends Record<string, unknown>>(value: unknown, message: string): T {
        if (!isRecord(value)) {
...
export function toStructuredZodIssues(issues: ZodIssue[]): { issues: ZodIssue[] } {
        return { issues };
}
