...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/tool-spec/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for Tool Spec within Cortex-OS.
- **Responsibilities:**
  - Implement the Tool Spec domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter tool-spec dev

# Build
pnpm --filter tool-spec build

# Quality (affected-aware if available)
pnpm --filter tool-spec lint
pnpm --filter tool-spec typecheck
pnpm --filter tool-spec test
pnpm --filter tool-spec test:coverage
```
...
TOOL_SPEC_BASE_URL=https://example.local
TOOL_SPEC_DB_URL=postgres://user:pass@localhost:5432/tool_spec
...
```bash
pnpm --filter tool-spec test:coverage
pnpm security:scan --scope=tool-spec
pnpm structure:validate --scope=tool-spec
```
...
- SBOM: CycloneDX generated at `./sbom/tool-spec.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"tool-spec"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"tool-spec","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter tool-spec test:e2e
pnpm --filter tool-spec test:smoke
pnpm --filter tool-spec chaos:probe
```
...
# Dev & run
pnpm --filter tool-spec dev
pnpm --filter tool-spec start

# Build & verify
pnpm --filter tool-spec build
pnpm --filter tool-spec lint && pnpm --filter tool-spec typecheck
pnpm --filter tool-spec test && pnpm --filter tool-spec test:coverage
...
pnpm --filter tool-spec mcp:smoke
```
