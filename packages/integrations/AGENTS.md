...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/integrations/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for Integrations within Cortex-OS.
- **Responsibilities:**
  - Implement the Integrations domain logic and exported APIs.
...
# Build
pnpm --filter integrations build

# Quality (affected-aware if available)
pnpm --filter integrations lint
pnpm --filter integrations typecheck
pnpm --filter integrations test
pnpm --filter integrations test:coverage
```
...
INTEGRATIONS_BASE_URL=https://example.local
INTEGRATIONS_DB_URL=postgres://user:pass@localhost:5432/integrations
...
```bash
pnpm --filter integrations test:coverage
pnpm security:scan --scope=integrations
pnpm structure:validate --scope=integrations
```
...
- SBOM: CycloneDX generated at `./sbom/integrations.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"integrations"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"integrations","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter integrations test:e2e
pnpm --filter integrations test:smoke
pnpm --filter integrations chaos:probe
```
...
# Dev & run
pnpm --filter integrations dev
pnpm --filter integrations start

# Build & verify
pnpm --filter integrations build
pnpm --filter integrations lint && pnpm --filter integrations typecheck
pnpm --filter integrations test && pnpm --filter integrations test:coverage
...
pnpm --filter integrations mcp:smoke
```
