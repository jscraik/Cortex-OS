...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/commands/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Slash command loader/parser/runner for Cortex-OS.
- **Responsibilities:**
  - Implement the Commands domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter commands dev

# Build
pnpm --filter commands build

# Quality (affected-aware if available)
pnpm --filter commands lint
pnpm --filter commands typecheck
pnpm --filter commands test
pnpm --filter commands test:coverage
```
...
COMMANDS_BASE_URL=https://example.local
COMMANDS_DB_URL=postgres://user:pass@localhost:5432/commands
...
```bash
pnpm --filter commands test:coverage
pnpm security:scan --scope=commands
pnpm structure:validate --scope=commands
```
...
- SBOM: CycloneDX generated at `./sbom/commands.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"commands"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"commands","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter commands test:e2e
pnpm --filter commands test:smoke
pnpm --filter commands chaos:probe
```
...
# Dev & run
pnpm --filter commands dev
pnpm --filter commands start

# Build & verify
pnpm --filter commands build
pnpm --filter commands lint && pnpm --filter commands typecheck
pnpm --filter commands test && pnpm --filter commands test:coverage
...
pnpm --filter commands mcp:smoke
```
