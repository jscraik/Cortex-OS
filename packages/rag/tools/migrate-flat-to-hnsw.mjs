...
function parseArgs() {
    ...
    const cfg = { input: null, out: './hnsw-index', space: 'cosine', M: 16, efConstruction: 200, efSearch: 64 };
    for (const a of args) {
...
export async function migrateFromJson(input, out, opts = {}) {
    ...
    const idx = new HNSWIndex({ space: opts.space ?? 'cosine', M: opts.M ?? 16, efConstruction: opts.efConstruction ?? 200, efSearch: opts.efSearch ?? 64 });
    await idx.init(dim);
