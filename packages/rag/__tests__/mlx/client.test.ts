...
describe('MLXClient', () => {
	...
	describe('text generation', () => {
		...
		it('should track token usage accurately', async () => {
			...
			expect(result.tokens.total).toBeGreaterThan(result.tokens.prompt);
			expect(result.tokens.total).toBeGreaterThan(result.tokens.completion);
		});
	});
	...
	describe('memory management', () => {
		...
		it('should unload models when not in use', async () => {
			// First setup the mock for loading
			...
			// Then setup the mock for unloading
			mockRunProcess.mockResolvedValueOnce({
				unloaded: true,
				model_path: '/test/model',
	...
	describe('error handling', () => {
		it('should handle Python bridge errors gracefully', async () => {
			...
			const health = await client.health();
			expect(health.status).toBe('unhealthy');
			expect(health.details.lastError).toContain('MLX Bridge error: Python execution failed');
		});
