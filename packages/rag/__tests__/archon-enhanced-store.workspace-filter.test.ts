...
const localStore = {
	...
	async query(_vector: number[], k = 10) {
		const rows = [
			{
				id: 'l1',
				text: 'alpha workspace A',
				score: 0.95,
				metadata: { text: 'alpha', workspace: 'A' },
			},
			{
				id: 'l2',
				text: 'beta workspace B',
				score: 0.9,
				metadata: { text: 'beta', workspace: 'B' },
			},
			{
				id: 'l3',
				text: 'gamma workspace A',
				score: 0.85,
				metadata: { text: 'gamma', workspace: 'A' },
			},
			{
				id: 'l4',
				text: 'delta workspace C',
				score: 0.8,
				metadata: { text: 'delta', workspace: 'C' },
			},
		];
...
describe('RemoteMCPEnhancedStore workspace filtering with fusion + topK', () => {
	...
	it('applies workspace filter before topK (rrf fusion)', async () => {
		...
		const workspaces = results.map(
			(r) => (r.metadata as Record<string, unknown> | undefined)?.workspace,
		);
