...
import { sanitizeMetadata, validateContentSize, validateEmbeddingDim } from '../src/lib/validation.js';

describe('security validation', () => {
  it('rejects invalid embedding dimensions and non-finite values', () => {
    expect(() => validateEmbeddingDim([0.1, 0.2], [768, 1536])).toThrow('Invalid embedding dimension');
  ...
  it('enforces content size limits', () => {
    ...
    expect(() => validateContentSize(123 as unknown as string, 10)).toThrow('Content must be a string');
  });
  ...
  it('sanitizes metadata and prevents prototype pollution', () => {
    ...
    expect(() => sanitizeMetadata({ __proto__: { hacked: true } } as unknown as Record<string, unknown>)).toThrow('forbidden key');

    // Non-plain objects rejected
    class X { x = 1 }
    expect(() => sanitizeMetadata(new X() as unknown as Record<string, unknown>)).toThrow('non-plain object');

    // Exotic types rejected
    const fnVal = (() => { /* noop */ }) as unknown as Record<string, unknown>;
