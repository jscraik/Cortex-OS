...
export function enqueueMockError(method: MethodName, message: string | Error) {
        responseQueue.push({ method, error: message instanceof Error ? message : new Error(message) });
}
...
function defaultResponse(method: MethodName): MockResponse {
        switch (method) {
                ...
                case 'mcp_create_task':
                        return { method, value: { taskId: 'task-mock', url: 'mock://task-mock' } };
                case 'mcp_update_task_status':
                        return { method, value: { updated: true } };
                case 'mcp_upload_document':
                        return { method, value: { documentId: 'doc-mock', url: 'mock://doc-mock' } };
...
export class AgentMCPClient {
        ...
        async initialize() {
                ...
                mockCallLog.push({ method: 'mcp_initialize', params: { config: this.config } });
        ...
        async callTool(name: string, args: Record<string, unknown>, timeout?: number) {
                ...
                mockCallLog.push({ method: 'mcp_call_tool', params: { name, arguments: args, timeout } });
        ...
        async searchKnowledgeBase(
                ...
                this.ensureConnected();
                mockCallLog.push({ method: 'mcp_search_knowledge_base', params: { query, options } });
        ...
        async createTask(title: string, description: string, options: Record<string, unknown> = {}) {
                this.ensureConnected();
                mockCallLog.push({ method: 'mcp_create_task', params: { title, description, options } });
        ...
        async updateTaskStatus(taskId: string, status: string, notes?: string) {
                this.ensureConnected();
                mockCallLog.push({ method: 'mcp_update_task_status', params: { taskId, status, notes } });
        ...
        async uploadDocument(
                ...
                this.ensureConnected();
                mockCallLog.push({ method: 'mcp_upload_document', params: { content, filename, options } });
