...
export function createEnhancedRAGPipeline(config) {
    ...
    return {
        ...
        retrieveDocs: (queryEmbedding, docs) => retrieveDocs(embedder, queryEmbedding, docs, finalConfig.topK),
        rerankDocs: (query, docs) => rerankDocs(reranker, query, docs, finalConfig.rerank.topK),
        generateAnswer: (query, docs, options) => generateAnswer(generator, query, docs, options),
    };
}
