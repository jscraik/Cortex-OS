...
class HTTPMCPClient implements AgentMCPClient {
  ...
  constructor(config: MCPIntegrationConfig) {
    ...
    this.apiKey = (cfg['apiKey'] as string | undefined) || (typeof process !== 'undefined' ? process.env?.MCP_API_KEY : undefined);
  }
  ...
  private async postRPC(method: string, params: Record<string, unknown> = {}, timeoutMs?: number): Promise<unknown> {
    ...
    const f = (globalThis as { fetch?: (input: RequestInfo, init?: RequestInit) => Promise<Response> }).fetch;
    if (typeof f !== 'function') {
      throw new Error('fetch is not available in this runtime. Node 18+ or a fetch polyfill is required.');
    }
    ...
    const res = await f(this.baseUrl, { method: 'POST', headers, body: JSON.stringify(body), signal: controller?.signal ?? null });
    ...
    type JSONRPCResponse = { jsonrpc?: string; id?: unknown; result?: unknown; error?: JSONRPCError } | Record<string, unknown>;
    ...
    const hasResult = (v: JSONRPCResponse): v is { result: unknown } => Boolean(v && typeof v === 'object' && 'result' in v);
    if (hasError(data)) {
  ...
  async initialize(): Promise<unknown> {
    try {
      ...
      const result = (await this.postRPC('list_capabilities').catch(() => ({}))) as Record<string, unknown>;
  ...
  private ensureInit() {
    if (!this.initialized) throw new Error('MCP client is not initialized. Call initialize() first.');
  }
  ...
  async searchKnowledgeBase(
    ...
    const result = (await this.postRPC('search', { query, max_results: limit })) as Record<string, unknown>;
  ...
  async createTask(title: string, description: string, options?: Record<string, unknown>): Promise<unknown> {
    this.ensureInit();
