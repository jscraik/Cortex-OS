...
function getAgentClientFactory() {
    const g = globalThis as unknown as { __createAgentMCPClient__?: (cfg: MCPIntegrationConfig) => AgentMCPClient };
    return typeof g.__createAgentMCPClient__ === 'function' ? g.__createAgentMCPClient__ : realCreateAgentMCPClient;
}
...
export class RemoteMCPEmbedder implements Embedder {
    ...
    constructor(config: RemoteRAGConfig, fallbackEmbedder?: Embedder) {
        ...
        this.mcpClient = getAgentClientFactory()(config as MCPIntegrationConfig);
    }
...
export class RemoteMCPEnhancedStore implements Store {
    ...
    private async extendedQuery(
        ...
        if (!options.remoteOnly) {
            try {
                ...
                const localResults: QueryResult[] = localChunks.map((c) => ({
                    id: c.id,
                    score: c.score ?? 0,
                    // source is not guaranteed on Chunk; guard via optional access
                    metadata: { text: c.text, source: (c as unknown as { source?: string }).source, ...c.metadata, provider: 'local' },
                }));
        ...
        if (this.config.enableRemoteRetrieval && options.useRemoteKnowledge !== false) {
            try {
                const remoteResults = await this.queryRemoteKnowledgeBase(vector, options);
                if (options.hybridSearch && results.length > 0) {
                    return this.combineResults(results, remoteResults, options);
                } else {
    ...
    private async queryRemoteKnowledgeBase(
        ...
        const remoteResults = await this.mcpClient.searchKnowledgeBase(searchQuery, {
