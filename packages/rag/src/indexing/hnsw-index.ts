...
interface HNSWNativeIndex {
    ...
    searchKnn(vec: Float32Array, topK: number): { neighbors: ArrayLike<number>; distances: ArrayLike<number> };
...
export class HNSWIndex implements VectorIndex {
    ...
    async load(path: string): Promise<void> {
        ...
        this.labelToId = new Map(Array.from(this.idToLabel.entries()).map(([id, lbl]: [string, number]) => [lbl, id]));
        this.index = new HierarchicalNSW(this.space, this.dim);
