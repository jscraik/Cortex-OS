...
export class ScalarQuantizedFlatIndex implements VectorIndex {
    ...
    async add(id: string, vector: number[]): Promise<void> {
        if (vector.length !== this.dim) throw new Error(`Vector dimension ${vector.length} != index dimension ${this.dim}`);
...
export class PQFlatIndex implements VectorIndex {
    ...
    async init(dimension: number): Promise<void> {
        this.dim = dimension;
        if (this.dim % this.m !== 0) throw new Error(`dim ${this.dim} must be divisible by m=${this.m}`);
    ...
    async add(id: string, vector: number[]): Promise<void> {
        ...
        if (!this.trained) {
            ...
            if (this.trainBuffer.length >= this.k * 10) {
                ...
                // Encode buffered
                for (const v of this.trainBuffer) this.codes.push({ id: `buf-${this.codes.length}`, codes: this.encodeVec(v) });
                this.trainBuffer = [];
            }
    ...
    async query(vector: number[], topK: number): Promise<Array<{ id: string; distance: number }>> {
        ...
        const scored = this.codes.map((e) => ({ id: e.id, distance: this.adcDistance(vnorm, e.codes) }));
        scored.sort((a, b) => a.distance - b.distance);
        return scored.slice(0, topK);
    }
    ...
    async save(basePath: string): Promise<void> {
        ...
        const meta = { dim: this.dim, m: this.m, k: this.k, iters: this.iters, subDim: this.subDim, size: this.codes.length };
    ...
    async load(basePath: string): Promise<void> {
        ...
        const meta = JSON.parse(fs.readFileSync(`${base}.pq.meta.json`, 'utf-8')) as {
            dim: number; m: number; k: number; iters: number; subDim: number; size: number;
        };
        this.dim = meta.dim;
        this.m = meta.m;
        this.k = meta.k;
