...
export class PooledEmbedder implements Embedder {
  ...
  private async runTask(task: Task): Promise<void> {
    ...
    try {
      ...
      this.failures++;
      task.reject(e);
      if (this.failures >= this.opt.failureRestartThreshold && this.currentWorkers > this.opt.minWorkers) {
...
export function createPooledEmbedder(inner: Embedder, options?: EmbeddingPoolOptions): PooledEmbedder {
