...
export class Qwen3Reranker implements Reranker {
	...
	constructor(options: Qwen3RerankOptions = {}) {
		...
		// Security: reject unsafe python executable paths (command injection protection)
		if (!isSafeExecutablePath(this.pythonPath)) {
			throw new Error(
				'Unsafe pythonPath: only absolute/relative paths and alphanumerics, dash, underscore, dot, and slash are allowed',
			);
		}
...
// Allow only safe characters and path separators; disallow whitespace and shell metacharacters
function isSafeExecutablePath(p: string): boolean {
	// Accept typical names like python3, /usr/bin/python3, ./venv/bin/python; disallow whitespace and backslash
	if (!p || typeof p !== 'string') return false;
	if (/\s/.test(p)) return false;
	if (p.includes('\\')) return false;
	// Strict allowlist of characters
