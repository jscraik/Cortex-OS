...
export const remoteRAGConfigSchema = mcpIntegrationConfigSchema.extend({
...
export const processingConfigSchema = z.object({
  maxSizeBytes: z.number().int().positive().max(100 * 1024 * 1024), // 100MB max
...
export const mimePolicyConfigSchema = z.object({
  ...
  maxFileSize: z.number().int().positive().max(1024 * 1024 * 1024), // 1GB max
...
export const pgVectorConfigSchema = z.object({
  ...
  tableName: z.string().regex(/^[a-zA-Z_]\w*$/).max(100),
...
export const enhancedRAGConfigSchema = ragPipelineConfigSchema.extend({
...
export function validateConfig<T>(
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      throw new Error(
        `Invalid ${configName} configuration: ${details.join(', ')}`
      );
    }
    throw error;
  }
