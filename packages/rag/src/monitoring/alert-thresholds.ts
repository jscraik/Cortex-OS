...
export function evaluateHealthAlerts(
    ...
    if (memoryAlerts.level === 'critical' || (memoryAlerts.level === 'warning' && maxLevel === 'ok')) {
...
): { level: 'ok' | 'warning' | 'critical'; alerts: ReturnType<typeof evaluateHealthAlerts>['alerts'] } {
    const alerts: ReturnType<typeof evaluateHealthAlerts>['alerts'] = [];
 ...
 */
export function generatePrometheusAlerts(thresholds: AlertThresholds = DEFAULT_ALERT_THRESHOLDS): string {
    return `
groups:
 ...
 */
export function generateGrafanaDashboard(thresholds: AlertThresholds = DEFAULT_ALERT_THRESHOLDS): Record<string, unknown> {
    return {
        dashboard: {
