...
export class MultiModelGenerator {
    ...
    async chat(messages, config) {
        ...
        return {
            ...
            usage: {
                ...
                completionTokens: Math.floor(result.length / 4),
                totalTokens: Math.floor((messages.reduce((sum, m) => sum + m.content.length, 0) + result.length) / 4),
            },
        };
    }
    /**
    ...
    async generateWithMLX(model, prompt, config) {
        ...
        const timer = new Promise((_, reject) =>
            setTimeout(() => reject(new Error(`MLX process timed out after ${this.timeout}ms`)), this.timeout),
        );
