...
// Helper to obtain an AbortSignal with timeout without assuming global AbortSignal presence.
const abortSignalTimeout = (ms) => {
	try {
		if (
...
export class MultiModelGenerator {
	...
	async generateWithOllama(model, prompt, config) {
		try {
			const response = await fetch('http://localhost:11434/api/generate', {
				...
				signal: abortSignalTimeout(this.timeout),
			});
			if (!response.ok) {
				throw new Error(
	...
	async generateWithMLX(model, prompt, config) {
		...
		const result = await runProcess('python3', ['-c', pythonScript], {
			input,
