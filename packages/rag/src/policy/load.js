...
export async function loadRetrievalPolicy(configPath = resolvePath(process.cwd(), 'config/retrieval.policy.json'), schemaPath = resolvePath(process.cwd(), 'schemas/retrieval.policy.schema.json')) {
    ...
    if (override.processing && typeof override.processing === 'object') {
        if (Object.hasOwn(override.processing, 'maxPages')) {
            nextProcessing.maxPages =
                override.processing.maxPages ?? null;
        }
    }
    return { ...decision, processing: nextProcessing };
}
export async function planAndDispatch(file, mimeType, engine, dispatcher, policy) {
