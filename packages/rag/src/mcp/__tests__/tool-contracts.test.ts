...
describe('RAG MCP tool contracts', () => {
	...
	it('requires at least one document when ingesting', () => {
		const result = ragDocumentIngestionTool.inputSchema.safeParse({
			documents: [],
		});
		...
		if (!result.success) {
			...
			expect(
				messages.some((msg) =>
					msg.toLowerCase().includes('at least one document'),
				),
			).toBe(true);
		}
	});
	...
	it('enforces non-empty search queries and default settings', () => {
		...
		const parsed = ragSearchTool.inputSchema.parse({
			query: 'what is cortex rag?',
		});
	...
	it('limits search result size to prevent runaway requests', () => {
		const result = ragSearchTool.inputSchema.safeParse({
			query: 'a',
			topK: 500,
		});
		expect(result.success).toBe(false);
