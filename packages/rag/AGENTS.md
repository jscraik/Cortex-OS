...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/rag/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Enhanced chunking, multi-model embeddings, Qwen3 reranking, and batch ingest capabilities
- **Responsibilities:**
  - Implement the RAG domain logic and exported APIs.
...
# Build
pnpm --filter rag build

# Quality (affected-aware if available)
pnpm --filter rag lint
pnpm --filter rag typecheck
pnpm --filter rag test
pnpm --filter rag test:coverage
```
...
RAG_BASE_URL=https://example.local
RAG_DB_URL=postgres://user:pass@localhost:5432/rag
...
```bash
pnpm --filter rag test:coverage
pnpm security:scan --scope=rag
pnpm structure:validate --scope=rag
```
...
- SBOM: CycloneDX generated at `./sbom/rag.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"rag"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"rag","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter rag test:e2e
pnpm --filter rag test:smoke
pnpm --filter rag chaos:probe
```
...
# Dev & run
pnpm --filter rag dev
pnpm --filter rag start

# Build & verify
pnpm --filter rag build
pnpm --filter rag lint && pnpm --filter rag typecheck
pnpm --filter rag test && pnpm --filter rag test:coverage
