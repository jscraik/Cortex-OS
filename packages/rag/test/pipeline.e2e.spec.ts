...
describe('Pipeline E2E (ingest -> retrieve -> bundle)', () => {
  it('produces a citation bundle with sources', async () => {
    ...
    const S = memoryStore();
    const pipeline = new RAGPipeline({ embedder: E, store: S, freshnessEpsilon: 0.02 });

    await pipeline.ingest([
      { id: 'a', text: 'Cortex-OS overview', source: 'readme.md', updatedAt: Date.now() - 1000 },
      { id: 'b', text: 'Agent communication patterns', source: 'arch.md', updatedAt: Date.now() },
    ]);
