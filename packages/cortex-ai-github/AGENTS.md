...
```yaml
id: agents-cortex-ai-github-v1
package: Cortex AI Github
type: package
runtime: node
...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/cortex-ai-github/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Production-ready AI-powered GitHub automation engine with MLX integration for intelligent code analysis and automated responses.
- **Responsibilities:**
  - Implement the Cortex AI Github domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter cortex-ai-github dev

# Build
pnpm --filter cortex-ai-github build

# Quality (affected-aware if available)
pnpm --filter cortex-ai-github lint
pnpm --filter cortex-ai-github typecheck
pnpm --filter cortex-ai-github test
pnpm --filter cortex-ai-github test:coverage
```
...
CORTEX_AI_GITHUB_BASE_URL=https://example.local
CORTEX_AI_GITHUB_DB_URL=postgres://user:pass@localhost:5432/cortex_ai_github
...
```bash
pnpm --filter cortex-ai-github test:coverage
pnpm security:scan --scope=cortex-ai-github
pnpm structure:validate --scope=cortex-ai-github
```
...
- SBOM: CycloneDX generated at `./sbom/cortex-ai-github.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"cortex-ai-github"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"cortex-ai-github","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter cortex-ai-github test:e2e
pnpm --filter cortex-ai-github test:smoke
pnpm --filter cortex-ai-github chaos:probe
```
...
# Dev & run
pnpm --filter cortex-ai-github dev
pnpm --filter cortex-ai-github start

# Build & verify
pnpm --filter cortex-ai-github build
pnpm --filter cortex-ai-github lint && pnpm --filter cortex-ai-github typecheck
pnpm --filter cortex-ai-github test && pnpm --filter cortex-ai-github test:coverage
...
pnpm --filter cortex-ai-github mcp:smoke
```
