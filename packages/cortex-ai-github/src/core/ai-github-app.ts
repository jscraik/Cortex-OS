...
export class CortexAiGitHubApp extends EventEmitter<AiAppEvents> {
  ...
  private async callModel(model: GitHubModel, messages: ModelMessage[]): Promise<{
    content: string;
    tokensUsed: number;
    finishReason: string;
  }> {
...
Only suggest fixes you're confident are safe and correct.`
  ...
  private validateTaskParams(params: AITaskParams): AITaskParams {
    const schema = z.object({
      taskType: z.enum(['code_review', 'pr_analysis', 'security_scan', 'documentation', 'issue_triage', 'workflow_optimize', 'repo_health', 'auto_fix']),
      githubContext: z.object({
        owner: z.string(),
        repo: z.string(),
        pr: z.optional(z.object({
          number: z.number(),
          title: z.string(),
          body: z.string(),
          base: z.string(),
      ...
      model: z.optional(z.enum(['gpt-4o', 'gpt-4o-mini', 'claude-3-5-sonnet', 'claude-3-haiku', 'phi-3-medium-128k', 'phi-3-mini-128k', 'llama-3.1-70b', 'llama-3.1-405b']))
    });
  ...
  get queueSize(): number { return this.taskQueue.size; }
  get activeTaskCount(): number { return this.activeRequests.size; }
  get rateLimit(): { remaining: number; resetAt: Date } { return { ...this.rateLimitInfo }; }
}