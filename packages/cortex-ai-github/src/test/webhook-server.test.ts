...
describe('CortexWebhookServer', () => {
  ...
  describe('Command Processing', () => {
    ...
    it('should handle multiple commands correctly', async () => {
      ...
      expect(mockAiApp.queueTask).toHaveBeenNthCalledWith(1, expect.objectContaining({
        taskType: 'code_review'
      }));
      expect(mockAiApp.queueTask).toHaveBeenNthCalledWith(2, expect.objectContaining({
    ...
    it('should extract instructions from commands', async () => {
      ...
      expect(mockAiApp.queueTask).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('Trigger Management', () => {
    ...
    it('should remove triggers by pattern', () => {
      ...
      const triggers = (server as any).triggers;
      expect(triggers.find((t: any) => t.pattern.source === patternToRemove.source)).toBeUndefined();
    });
  });
