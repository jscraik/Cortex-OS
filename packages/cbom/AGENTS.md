...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/cbom/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** The Context Bill of Materials (CBOM) package captures AI run context, policies, and
- **Responsibilities:**
  - Implement the Cbom domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter cbom dev

# Build
pnpm --filter cbom build

# Quality (affected-aware if available)
pnpm --filter cbom lint
pnpm --filter cbom typecheck
pnpm --filter cbom test
pnpm --filter cbom test:coverage
```
...
CBOM_BASE_URL=https://example.local
CBOM_DB_URL=postgres://user:pass@localhost:5432/cbom
...
```bash
pnpm --filter cbom test:coverage
pnpm security:scan --scope=cbom
pnpm structure:validate --scope=cbom
```
...
- SBOM: CycloneDX generated at `./sbom/cbom.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"cbom"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"cbom","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter cbom test:e2e
pnpm --filter cbom test:smoke
pnpm --filter cbom chaos:probe
```
...
# Dev & run
pnpm --filter cbom dev
pnpm --filter cbom start

# Build & verify
pnpm --filter cbom build
pnpm --filter cbom lint && pnpm --filter cbom typecheck
pnpm --filter cbom test && pnpm --filter cbom test:coverage
