...
```yaml
id: agents-memory-rest-api-v1
package: Memory Rest API
type: package
runtime: node
...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/memory-rest-api/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for Memory Rest API within Cortex-OS.
- **Responsibilities:**
  - Implement the Memory Rest API domain logic and exported APIs.
...
# Build
pnpm --filter memory-rest-api build

# Quality (affected-aware if available)
pnpm --filter memory-rest-api lint
pnpm --filter memory-rest-api typecheck
pnpm --filter memory-rest-api test
pnpm --filter memory-rest-api test:coverage
```
...
MEMORY_REST_API_BASE_URL=https://example.local
MEMORY_REST_API_DB_URL=postgres://user:pass@localhost:5432/memory_rest_api
...
```bash
pnpm --filter memory-rest-api test:coverage
pnpm security:scan --scope=memory-rest-api
pnpm structure:validate --scope=memory-rest-api
```
...
- SBOM: CycloneDX generated at `./sbom/memory-rest-api.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"memory-rest-api"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"memory-rest-api","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter memory-rest-api test:e2e
pnpm --filter memory-rest-api test:smoke
pnpm --filter memory-rest-api chaos:probe
```
...
# Dev & run
pnpm --filter memory-rest-api dev
pnpm --filter memory-rest-api start

# Build & verify
pnpm --filter memory-rest-api build
pnpm --filter memory-rest-api lint && pnpm --filter memory-rest-api typecheck
pnpm --filter memory-rest-api test && pnpm --filter memory-rest-api test:coverage
...
pnpm --filter memory-rest-api mcp:smoke
```
