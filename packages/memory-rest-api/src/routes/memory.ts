...
import { MemoryStoreInputSchema, MemorySearchInputSchema, MemoryAnalysisInputSchema, MemoryRelationshipsInputSchema, MemoryStatsInputSchema } from '@cortex-os/tool-spec';
...
export function memoryRoutes(provider: MemoryProvider): Router {
  ...
  router.get('/search', validateRequest(MemorySearchInputSchema, 'query'), async (req, res, next) => {
    try {
  ...
  router.post('/relationships', validateRequest(MemoryRelationshipsInputSchema), async (req, res, next) => {
    try {
