	...
	async validateMcpServer(
		...
		if (request.url) {
			try {
				...
				if (!isAllowedDomain) {
					warnings.push(
						`Domain ${url.hostname} is not in the allowlist - requires approval`,
					);
		...
		if (request.apiKey) {
			...
			if (
				insecurePatterns.some((pattern) =>
					request.apiKey?.toLowerCase().includes(pattern),
				)
			) {
				errors.push("API key appears to contain insecure patterns");
			}
		} else if (
			this.defaultSecurityPolicy.requireApiKey &&
		...
		const hasDangerousCapabilities =
			request.capabilities?.some((cap) =>
				dangerousCapabilities.includes(cap.toLowerCase()),
		...
		if (request.transport === "stdio" && request.command) {
			...
			if (
				dangerousCommands.some((cmd) =>
					request.command?.toLowerCase().includes(cmd),
				)
			) {
