...
async function addGpt5Server() {
  try {
    ...
    if (result.success) {
      ...
      console.log('\n📝 Next steps:');
      console.log('1. Replace "your-openai-api-key-here" with your actual OpenAI API key');
      console.log(
        '2. Update "/path/to/gpt5mcp/servers/gpt5-server/build/index.js" with the correct path',
      );
      console.log('3. Ensure the GPT-5 server build is available at the specified path');
      console.log('4. Consider using environment variables from .env file for production');
    }
  } catch (error) {
