...
export class PluginRegistry {
	...
	async searchPlugins(
		...
		if (searchOptions.query) {
			...
			plugins = plugins.filter(
				(plugin: PluginMetadata) =>
					...
					plugin.keywords.some((keyword: string) =>
		...
		if (searchOptions.maintainerVerified !== undefined) {
			plugins = plugins.filter(
				(plugin: PluginMetadata) =>
					plugin.maintainerVerified === searchOptions.maintainerVerified,
			);
		}
		...
		if (searchOptions.minRating !== undefined) {
			plugins = plugins.filter(
				(plugin: PluginMetadata) =>
					(plugin.rating ?? 0) >= searchOptions.minRating!,
			);
		}
		...
		if (searchOptions.minDownloads !== undefined) {
			plugins = plugins.filter(
				(plugin: PluginMetadata) =>
					(plugin.downloads ?? 0) >= searchOptions.minDownloads!,
			);
		}
	...
	async getPlugin(name: string): Promise<PluginMetadata | null> {
		...
		return (
			this.marketplaceIndex?.plugins.find(
				(p: PluginMetadata) => p.name === name,
			) || null
		);
	}
	...
	private async saveInstalledPlugins(): Promise<void> {
		try {
			...
			const data = JSON.stringify(
				Array.from(this.installedPlugins.values()),
				null,
				2,
			);
