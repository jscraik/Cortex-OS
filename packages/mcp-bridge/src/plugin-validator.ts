...
export class PluginValidator {
	...
	async validatePlugin(
		...
		if (
			plugin.installSize &&
			plugin.installSize > this.SECURITY_RULES.MAX_INSTALL_SIZE
		) {
			warnings.push(
				`Large install size: ${this.formatBytes(plugin.installSize)}`,
			);
			securityScore -= 10;
		}

		// Security checks
		securityScore -= this.checkPermissions(
			plugin.permissions,
			warnings,
			errors,
		);
		...
		securityScore -= this.checkDependencies(
			plugin.dependencies,
			warnings,
			errors,
		);
		...
		const result: PluginValidationResult = {
			...
			details: {
				...
				permissionCount: Array.isArray(plugin.permissions)
					? plugin.permissions.length
					: 0,
				dependencyCount: Array.isArray(plugin.dependencies)
	...
	getSecurityRecommendations(
		...
		if (validationResult.securityScore < 70) {
			recommendations.push(
				"This plugin has security concerns. Consider alternatives.",
			);
		}

		if (!plugin.verified) {
			recommendations.push(
				"This plugin is not verified. Review carefully before installation.",
			);
		}

		if (!plugin.signature) {
			recommendations.push(
				"Install plugins with digital signatures when possible.",
			);
		}

		if (
			plugin.permissions.some((p: string) =>
				this.SECURITY_RULES.DANGEROUS_PERMISSIONS.includes(p),
			)
		) {
			recommendations.push(
		...
		if (plugin.dependencies.length > 5) {
			recommendations.push(
				"This plugin has many dependencies. Review the dependency tree.",
			);
		}
	...
	private checkKeywords(
		...
		for (const keyword of keywords) {
			if (
				this.SECURITY_RULES.SUSPICIOUS_KEYWORDS.includes(keyword.toLowerCase())
			) {
