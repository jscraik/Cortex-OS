...
export class MLXMcpServer {
	...
	constructor(configPath?: string) {
		this.configPath =
			configPath || join(process.cwd(), "mlx-server-config.json");
	}
	...
	async initialize(): Promise<void> {
		try {
			...
			throw new Error(
				`Failed to load MLX config from ${this.configPath}: ${error}`,
			);
		}
	}
	...
	async chat(request: MLXRequest): Promise<MLXResponse> {
		...
		return new Promise((resolve, reject) => {
			...
			process.on("close", (code) => {
				if (code !== 0) {
					reject(
						new Error(`MLX process failed with code ${code}: ${errorOutput}`),
					);
					return;
				}
