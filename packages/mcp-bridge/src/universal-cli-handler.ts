...
export class UniversalCliHandler {
	...
	async processMcpCommand(
		...
		try {
			...
			const result = await universalMcpManager.addMcpServer(
				normalizedCommand,
				autoApprove,
			);
...
• 'cortex mcp status ${result.config && typeof result.config === "object" && "name" in result.config ? (result.config as { name: string }).name : "server-name"}' - Check server health`,
				...
				if (result.requiresApproval && interactive) {
					return {
						success: false,
						message: this.formatApprovalMessage(
							result.message,
...
${instructions}`;
	 ...
	 */
	private getSecurityLevelInfo(level?: string): {
		level: string;
		guidance: string;
	} {
		switch (level) {
	...
	async approveServer(
		...
		if (!force) {
			return {
				success: false,
				message:
					"Server approval requires explicit confirmation. Use --force flag to proceed.",
			};
		}
