...
class ManagedConnection {
	...
	isIdle(idleTimeout: number): boolean {
		return (
			!this.isAcquired && Date.now() - this.lastUsed.getTime() > idleTimeout
		);
	}
}
...
export class McpConnectionManager extends EventEmitter {
	...
	> {
		...
		> = {};
		const healthy = Array.from(this.servers.entries()).filter(
			([_, s]) => s.status === "healthy",
		);
	...
	private async cleanupIdleConnections(): Promise<void> {
		...
		for (const [serverId, connections] of this.connections.entries()) {
			const idleConnections = connections.filter((c) =>
				c.isIdle(this.poolConfig.idleTimeout),
			);
