...
describe("PluginRegistry", () => {
	...
	describe("refreshMarketplace", () => {
		it("should fetch marketplace data successfully", async () => {
			...
			expect(fetch).toHaveBeenCalledWith(
				"https://plugins.brainwav.ai/index.json",
			);
		});
		...
		it("should handle fetch failure in production environment", async () => {
			...
			try {
				...
				await expect(registry.refreshMarketplace()).rejects.toThrow(
					"Network error",
				);
			} finally {
	...
	describe("searchPlugins", () => {
		...
		it("should return empty array when marketplace is not loaded", async () => {
			...
			try {
				...
				await expect(newRegistry.searchPlugins()).rejects.toThrow(
					"Network error",
				);
			} finally {
				env.NODE_ENV = original;
	...
	describe("schema conformance", () => {
		it("should validate mock marketplace index against schema", () => {
			...
			const claudePlugin = mockIndex.plugins.find(
				(p: any) => p.name === "claude-code-templates",
			);
