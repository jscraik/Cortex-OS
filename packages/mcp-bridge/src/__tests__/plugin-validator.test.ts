...
describe("PluginValidator", () => {
	...
	describe("validatePlugin", () => {
		...
		it("should validate version format", async () => {
			...
			expect(
				result.warnings.some((warning) => warning.includes("version")),
			).toBe(true);
		});
		...
		it("should flag dangerous permissions", async () => {
			...
			expect(
				result.warnings.some(
					(warning) =>
						warning.includes("dangerous") || warning.includes("permission"),
				),
			).toBe(true);
		});
		...
		it("should flag oversized plugins", async () => {
			...
			expect(result.warnings.some((warning) => warning.includes("size"))).toBe(
				true,
			);
		});
		...
		it("should handle plugins without signature", async () => {
			...
			expect(
				result.warnings.some((warning) => warning.includes("signed")),
			).toBe(true);
		});
	...
	describe("getSecurityRecommendations", () => {
		it("should provide recommendations for low security score", () => {
			...
			expect(
				recommendations.some((rec) => rec.includes("security concerns")),
			).toBe(true);
			expect(recommendations.some((rec) => rec.includes("not verified"))).toBe(
				true,
			);
			expect(
				recommendations.some((rec) => rec.includes("digital signatures")),
			).toBe(true);
		});
