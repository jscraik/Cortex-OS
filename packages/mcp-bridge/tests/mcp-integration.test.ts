...
import {
	MCP_OBSERVABILITY_TOOLS,
	MCPProtocolHandlers,
} from "../mcp-protocol-handlers";
import {
	createObservability,
...
describe("MCP Integration", () => {
	...
	describe("Real-time Monitoring", () => {
		...
		test("should emit real-time events for monitoring", async () => {
			...
			expect(events.some((e) => e.type === "connection-established")).toBe(
				true,
			);
	...
	describe("Performance and Resource Management", () => {
		...
		test("should cleanup resources properly", async () => {
			...
			expect(mcpDataAfter.connectedServers).not.toContain(
				mockServerConfig.serverId,
			);
		});
	});
