...
async function testMarketplace() {
  ...
  try {
    ...
    console.log(`Marketplace loaded: ${success ? '✅' : '❌ (using fallback)'}\n`);
    ...
    const codeGen = await registry.searchPlugins({ category: 'code-generation' });
    codeGen.forEach((p) => console.log(`  - ${p.name} (${p.category})`));

    // Test 4: High-rated plugins
    console.log('\n⭐ Plugins with rating >= 4.5:');
    const highRated = await registry.searchPlugins({ minRating: 4.5 });
    highRated.forEach((p) => console.log(`  - ${p.name} (⭐${p.rating}/5.0)`));
    ...
    if (claude) {
      ...
      console.log(`  Maintainer Verified: ${claude.maintainerVerified ? '✅' : '❌'}`);
