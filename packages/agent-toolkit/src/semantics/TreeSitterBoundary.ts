...
export async function createTreeSitterProvider(): Promise<TsProvider | null> {
    const enabled = process.env.CORTEX_TS_BOUNDARIES === '1' || process.env.CORTEX_TS_BOUNDARIES === 'true';
...
async function loadAllGrammars(TS: WTS): Promise<GrammarMap> {
    ...
    const TS_TS_FB = process.env.CORTEX_TS_GRAMMAR_TS_FB ?? '@tree-sitter/typescript/wasm/tree-sitter-typescript.wasm';
    const TS_JS = process.env.CORTEX_TS_GRAMMAR_JS ?? '@tree-sitter/javascript.wasm';
    const TS_JS_FB = process.env.CORTEX_TS_GRAMMAR_JS_FB ?? '@tree-sitter/javascript/wasm/tree-sitter-javascript.wasm';
    const TS_PY = process.env.CORTEX_TS_GRAMMAR_PY ?? '@tree-sitter/python.wasm';
    const TS_PY_FB = process.env.CORTEX_TS_GRAMMAR_PY_FB ?? '@tree-sitter/python/wasm/tree-sitter-python.wasm';
    const TS_GO = process.env.CORTEX_TS_GRAMMAR_GO ?? '@tree-sitter/go.wasm';
    const TS_GO_FB = process.env.CORTEX_TS_GRAMMAR_GO_FB ?? '@tree-sitter/go/wasm/tree-sitter-go.wasm';
    const TS_JAVA = process.env.CORTEX_TS_GRAMMAR_JAVA ?? '@tree-sitter/java.wasm';
    const TS_JAVA_FB = process.env.CORTEX_TS_GRAMMAR_JAVA_FB ?? '@tree-sitter/java/wasm/tree-sitter-java.wasm';
    const TS_CSHARP = process.env.CORTEX_TS_GRAMMAR_CSHARP ?? '@tree-sitter/c-sharp.wasm';
    const TS_CSHARP_FB = process.env.CORTEX_TS_GRAMMAR_CSHARP_FB ?? '@tree-sitter/c-sharp/wasm/tree-sitter-c-sharp.wasm';

    try { grammars.ts = await dynamicLoadGrammar(TS, TS_TS, TS_TS_FB); } catch { /* no-op */ }
    try { grammars.js = await dynamicLoadGrammar(TS, TS_JS, TS_JS_FB); } catch { /* no-op */ }
    try { grammars.python = await dynamicLoadGrammar(TS, TS_PY, TS_PY_FB); } catch { /* no-op */ }
    try { grammars.go = await dynamicLoadGrammar(TS, TS_GO, TS_GO_FB); } catch { /* no-op */ }
    try { grammars.java = await dynamicLoadGrammar(TS, TS_JAVA, TS_JAVA_FB); } catch { /* no-op */ }
    try { grammars.csharp = await dynamicLoadGrammar(TS, TS_CSHARP, TS_CSHARP_FB); } catch { /* no-op */ }
    return grammars;
}
...
function makeBoundaryFinder(parser: WTSParser, grammars: GrammarMap) {
    ...
    return (source: string, language: 'js' | 'ts' | 'python' | 'go' | 'java' | 'csharp' | 'any'): TsBoundary[] => {
