...
export class ContextualCoach {
  ...
  private getPhaseSpecificGuidance(
    ...
    const baseGuidance = {
      'RED': {
        ...
        explanation: 'The RED phase is about defining what you want to build through failing tests. Implementation comes later.',
        steps: [
      ...
      'GREEN': {
        ...
        explanation: 'The GREEN phase is about making tests pass with the simplest implementation possible. Resist the urge to over-engineer.',
        steps: [
      ...
      'REFACTOR': {
        ...
        explanation: 'The REFACTOR phase is about improving code quality without changing behavior. Tests should remain green throughout.',
        steps: [
  ...
  private getViolationExplanation(violation: string, skillLevel: TDDSkillLevel): string {
    const explanations: Record<string, string> = {
      'implementation_before_test': 'Writing implementation before tests skips the RED phase, which is crucial for defining requirements.',
      'test_after_implementation': 'Writing tests after implementation often leads to tests that confirm existing behavior rather than drive design.',
      'over_implementation': 'Implementing more than needed violates the GREEN phase principle of minimal implementation.',
      'refactor_with_failing_tests': 'Refactoring with failing tests is dangerous and violates the safety net that TDD provides.'
    };
  ...
  updateSkillAssessment(developerId: string, session: CoachingSession): void {
    ...
    if (metrics.testFirstPercentage > 0.8 && metrics.violationCount < 2 && metrics.cyclesCompleted > 5) {
      switch (newSkillLevel) {
        ...
        case TDDSkillLevel.ADVANCED:
          if (metrics.averageCycleTime < 300000) { // 5 minutes
