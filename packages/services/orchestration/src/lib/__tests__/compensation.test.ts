...
describe('SagaManager', () => {
	...
	describe('Step execution', () => {
		it('should execute step successfully and track for compensation', async () => {
			...
			const result = await sagaManager.executeStep(
				'test-step',
				stepFn,
				mockCompensation,
				{ metadata: 'test' },
			...
			expect(
				sagaManager.getCompensationManager().hasCompensation('test-step'),
			).toBe(true);
		});
		...
		it('should trigger compensation when step fails', async () => {
			...
			await expect(
				sagaManager.executeStep('test-step', stepFn, mockCompensation),
		...
		it('should not allow step execution during compensation', async () => {
			...
			(sagaManager as unknown as { isCompensating: boolean }).isCompensating =
				true;

			// While compensation flag is set, try to execute a step
			await expect(
				sagaManager.executeStep('test-step', stepFn),
			).rejects.toThrow('Cannot execute steps during compensation');
		...
		it('should not allow step execution during compensation (alternative test)', async () => {
			...
			const failingStep = vi
				.fn()
				.mockRejectedValue(new Error('First step failed'));
	...
	describe('Saga lifecycle', () => {
		...
		it('should not allow completion during compensation', async () => {
			...
			expect(() => sagaManager.complete()).toThrow(
				'Cannot complete saga during compensation',
			);
...
describe('compensationPatterns', () => {
	...
	it('should provide file cleanup pattern', async () => {
		const cleanup = compensationPatterns.cleanupFiles([
			'file1.tmp',
			'file2.tmp',
		]);
		const context: CompensationContext = { stepId: 'test-step' };
		...
		expect(consoleWarnSpy).toHaveBeenCalledWith(
			'Cleaning up files for step: test-step',
			['file1.tmp', 'file2.tmp'],
		);
	});
	...
	it('should provide API reversal pattern', async () => {
		...
		expect(consoleWarnSpy).toHaveBeenCalledWith(
			'Reversing API call for step: test-step',
		);
	});

	it('should provide resource deallocation pattern', async () => {
		const deallocation =
			compensationPatterns.deallocateResource('resource-456');
		const context: CompensationContext = { stepId: 'test-step' };
