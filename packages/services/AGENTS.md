...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/services/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for Services within Cortex-OS.
- **Responsibilities:**
  - Implement the Services domain logic and exported APIs.
...
SERVICES_BASE_URL=https://example.local
SERVICES_DB_URL=postgres://user:pass@localhost:5432/services
...
```bash
pnpm --filter services test:coverage
pnpm security:scan --scope=services
pnpm structure:validate --scope=services
```
...
- SBOM: CycloneDX generated at `./sbom/services.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"services"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"services","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter services test:e2e
pnpm --filter services test:smoke
pnpm --filter services chaos:probe
```
...
# Dev & run
pnpm --filter services dev
pnpm --filter services start

# Build & verify
pnpm --filter services build
pnpm --filter services lint && pnpm --filter services typecheck
pnpm --filter services test && pnpm --filter services test:coverage
...
pnpm --filter services mcp:smoke
```
