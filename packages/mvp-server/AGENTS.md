...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/mvp-server/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for MVP Server within Cortex-OS.
- **Responsibilities:**
  - Implement the MVP Server domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter mvp-server dev

# Build
pnpm --filter mvp-server build

# Quality (affected-aware if available)
pnpm --filter mvp-server lint
pnpm --filter mvp-server typecheck
pnpm --filter mvp-server test
pnpm --filter mvp-server test:coverage
```
...
MVP_SERVER_BASE_URL=https://example.local
MVP_SERVER_DB_URL=postgres://user:pass@localhost:5432/mvp_server
...
```bash
pnpm --filter mvp-server test:coverage
pnpm security:scan --scope=mvp-server
pnpm structure:validate --scope=mvp-server
```
...
- SBOM: CycloneDX generated at `./sbom/mvp-server.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"mvp-server"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"mvp-server","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter mvp-server test:e2e
pnpm --filter mvp-server test:smoke
pnpm --filter mvp-server chaos:probe
```
...
# Dev & run
pnpm --filter mvp-server dev
pnpm --filter mvp-server start

# Build & verify
pnpm --filter mvp-server build
pnpm --filter mvp-server lint && pnpm --filter mvp-server typecheck
pnpm --filter mvp-server test && pnpm --filter mvp-server test:coverage
...
pnpm --filter mvp-server mcp:smoke
```
