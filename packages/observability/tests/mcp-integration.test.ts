...
import {
        ...
        type ObservabilityToolHandlers,
        type TraceRecord,
} from '../src/mcp/runtime.js';
import { generateRunId } from '../src/ulids.js';

class FakeMcpClient {
        constructor(private readonly handlers: ObservabilityToolHandlers) {}
        ...
        async callTool<T = unknown>(name: string, input: unknown): Promise<T> {
                ...
                return handler(input) as Promise<T>;
        }
}
...
function createDataset(): ObservabilityDataset {
...
describe('observability MCP integration', () => {
        ...
        it('executes observability tools through the MCP client', async () => {
                ...
                const client = new FakeMcpClient(
                        createObservabilityToolHandlers(runtime),
                );
        ...
        it('returns descriptive errors for invalid dashboard requests', async () => {
                ...
                await expect(
                        client.callTool('generate_dashboard', {
        ...
        it('enforces limits to satisfy performance requirements', async () => {
                const runId = generateRunId();
                const noisyLogs: LogRecord[] = Array.from({ length: 220 }, (_, index) => ({
                        id: `bulk-log-${index}`,
