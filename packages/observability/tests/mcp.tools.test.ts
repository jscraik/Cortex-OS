...
import {
	createObservabilityErrorResponse,
	ObservabilityToolError,
	validateObservabilityToolInput,
...
describe('observability MCP tools validation', () => {
	...
	it('throws ObservabilityToolError for invalid trace ID', () => {
		...
		const error = (() => {
			try {
				...
				return err as ObservabilityToolError;
			}
	...
	it('rejects unsafe tag keys', () => {
		...
		expect(loggerSpies.warn).toHaveBeenCalledWith(
			expect.objectContaining({ tool: 'record_metric' }),
			'record_metric validation failed',
		);
	});
