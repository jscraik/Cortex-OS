...
type MinimalChildProcess = { on: (event: string, cb: (...args: unknown[]) => void) => MinimalChildProcess };
describe('generateFlamegraph', () => {
	it('spawns 0x with correct arguments', async () => {
		const on = vi.fn(function (this: MinimalChildProcess, event: string, cb: (...args: unknown[]) => void) {
				if (event === 'exit') cb(0);
				return this; // ChildProcess#on returns `this`
			});
			// Return an object whose `on` method matches the ChildProcess signature
			const child: MinimalChildProcess = { on: on as (event: string, cb: (...args: unknown[]) => void) => MinimalChildProcess };
