...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/observability/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Utilities for structured logging, metrics, and tracing with ULID propagation for Cortex-OS services.
- **Responsibilities:**
  - Implement the Observability domain logic and exported APIs.
...
OBSERVABILITY_BASE_URL=https://example.local
OBSERVABILITY_DB_URL=postgres://user:pass@localhost:5432/observability
...
```bash
pnpm --filter observability test:coverage
pnpm security:scan --scope=observability
pnpm structure:validate --scope=observability
```
...
- SBOM: CycloneDX generated at `./sbom/observability.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"observability"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"observability","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter observability test:e2e
pnpm --filter observability test:smoke
pnpm --filter observability chaos:probe
```
...
# Dev & run
pnpm --filter observability dev
pnpm --filter observability start

# Build & verify
pnpm --filter observability build
pnpm --filter observability lint && pnpm --filter observability typecheck
pnpm --filter observability test && pnpm --filter observability test:coverage
...
pnpm --filter observability mcp:smoke
```
