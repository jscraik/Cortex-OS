...
export class ObservabilityToolError extends Error {
	constructor(
		public readonly code: 'validation_error' | 'security_error' | 'internal_error',
		message: string,
...
function assertSafeRecord(obj: unknown, context: string): asserts obj is Record<string, string> {
...
function sanitizeStringMap(map?: Record<string, string>): Record<string, string> | undefined {
...
function ensureTimeRange(start?: string, end?: string) {
	if (start && end) {
...
export function validateObservabilityToolInput(
	...
	try {
		switch (tool) {
			case 'create_trace': {
				...
				const raw = input as { tags?: unknown } | undefined;
			...
			case 'get_metrics': {
				...
				const raw = input as { labels?: unknown } | undefined;
