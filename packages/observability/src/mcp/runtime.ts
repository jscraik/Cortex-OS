...
function cloneDataset(dataset: Partial<ObservabilityDataset> | undefined): ObservabilityDataset {
        return {
                ...
                dashboards: (dataset?.dashboards ?? []).map((dashboard) => ({ ...dashboard })),
        };
}
        ...
        return {
                async queryTraces(input) {
                        ...
                        const filtered = dataset.traces
                                ...
                                .sort(
                                        (a, b) =>
                                                (parseTime(b.startTime) ?? 0) -
                                                (parseTime(a.startTime) ?? 0),
                                );
                ...
                async searchLogs(input) {
                        ...
                        const filtered = dataset.logs
                                ...
                                .sort(
                                        (a, b) =>
                                                (parseTime(b.timestamp) ?? 0) -
                                                (parseTime(a.timestamp) ?? 0),
                                );
                ...
                async evaluateAlert(input) {
                        ...
                        const latestTimestamp = metricsForName.reduce(
                                (latest, metric) => {
                ...
                async generateDashboard(input) {
                        ...
                        if (!definition) {
                                throw new Error(
                                        `Dashboard not found: ${parsed.dashboardId}`,
                                );
                        }

                        const startIso = parsed.timeRange?.start;
                        const endIso = parsed.timeRange?.end;
                        const { start, end, startIso: normalizedStart, endIso: normalizedEnd } =
                        ...
                        return {
                                ...
                                traces: {
                                        total: totalTraces,
                                        errors: errorCount,
                                        errorRate: totalTraces
                                                ? errorCount / totalTraces
                                                : 0,
