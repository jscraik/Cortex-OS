...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/protocol/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Shared Cortex-OS runtime contracts and schemas used by orchestration, kernel, tooling, and UI layers. The module contains only pure TypeScript types and Zod validators so other ...
- **Responsibilities:**
  - Implement the Protocol domain logic and exported APIs.
...
PROTOCOL_BASE_URL=https://example.local
PROTOCOL_DB_URL=postgres://user:pass@localhost:5432/protocol
...
```bash
pnpm --filter protocol test:coverage
pnpm security:scan --scope=protocol
pnpm structure:validate --scope=protocol
```
...
- SBOM: CycloneDX generated at `./sbom/protocol.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"protocol"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"protocol","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter protocol test:e2e
pnpm --filter protocol test:smoke
pnpm --filter protocol chaos:probe
```
...
# Dev & run
pnpm --filter protocol dev
pnpm --filter protocol start

# Build & verify
pnpm --filter protocol build
pnpm --filter protocol lint && pnpm --filter protocol typecheck
pnpm --filter protocol test && pnpm --filter protocol test:coverage
...
pnpm --filter protocol mcp:smoke
```
