...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/prompts/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for Prompts within Cortex-OS.
- **Responsibilities:**
  - Implement the Prompts domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter prompts dev

# Build
pnpm --filter prompts build

# Quality (affected-aware if available)
pnpm --filter prompts lint
pnpm --filter prompts typecheck
pnpm --filter prompts test
pnpm --filter prompts test:coverage
```
...
PROMPTS_BASE_URL=https://example.local
PROMPTS_DB_URL=postgres://user:pass@localhost:5432/prompts
...
```bash
pnpm --filter prompts test:coverage
pnpm security:scan --scope=prompts
pnpm structure:validate --scope=prompts
```
...
- SBOM: CycloneDX generated at `./sbom/prompts.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"prompts"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"prompts","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter prompts test:e2e
pnpm --filter prompts test:smoke
pnpm --filter prompts chaos:probe
```
...
# Dev & run
pnpm --filter prompts dev
pnpm --filter prompts start

# Build & verify
pnpm --filter prompts build
pnpm --filter prompts lint && pnpm --filter prompts typecheck
pnpm --filter prompts test && pnpm --filter prompts test:coverage
