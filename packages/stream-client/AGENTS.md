...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/stream-client/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Client SDK for consuming Cortex-OS streaming events. Supports SSE with automatic reconnect and optional WebSocket transport, plus a lightweight recorder for diagnostics.
- **Responsibilities:**
  - Implement the Stream Client domain logic and exported APIs.
...
pnpm --filter stream-client typecheck
pnpm --filter stream-client test
pnpm --filter stream-client test:coverage
```
...
STREAM_CLIENT_BASE_URL=https://example.local
STREAM_CLIENT_DB_URL=postgres://user:pass@localhost:5432/stream_client
...
```bash
pnpm --filter stream-client test:coverage
pnpm security:scan --scope=stream-client
pnpm structure:validate --scope=stream-client
```
...
- SBOM: CycloneDX generated at `./sbom/stream-client.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"stream-client"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"stream-client","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter stream-client test:e2e
pnpm --filter stream-client test:smoke
pnpm --filter stream-client chaos:probe
```
...
# Dev & run
pnpm --filter stream-client dev
pnpm --filter stream-client start

# Build & verify
pnpm --filter stream-client build
pnpm --filter stream-client lint && pnpm --filter stream-client typecheck
pnpm --filter stream-client test && pnpm --filter stream-client test:coverage
...
pnpm --filter stream-client mcp:smoke
```
