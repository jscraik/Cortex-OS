...
```yaml
id: agents-workflow-dashboard-v1
...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/workflow-dashboard/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for Workflow Dashboard within Cortex-OS.
- **Responsibilities:**
  - Implement the Workflow Dashboard domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter workflow-dashboard dev

# Build
pnpm --filter workflow-dashboard build

# Quality (affected-aware if available)
pnpm --filter workflow-dashboard lint
pnpm --filter workflow-dashboard typecheck
pnpm --filter workflow-dashboard test
pnpm --filter workflow-dashboard test:coverage
```
...
WORKFLOW_DASHBOARD_BASE_URL=https://example.local
WORKFLOW_DASHBOARD_DB_URL=postgres://user:pass@localhost:5432/workflow_dashboard
...
```bash
pnpm --filter workflow-dashboard test:coverage
pnpm security:scan --scope=workflow-dashboard
pnpm structure:validate --scope=workflow-dashboard
```
...
- SBOM: CycloneDX generated at `./sbom/workflow-dashboard.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"workflow-dashboard"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"workflow-dashboard","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter workflow-dashboard test:e2e
pnpm --filter workflow-dashboard test:smoke
pnpm --filter workflow-dashboard chaos:probe
```
...
# Dev & run
pnpm --filter workflow-dashboard dev
pnpm --filter workflow-dashboard start

# Build & verify
pnpm --filter workflow-dashboard build
pnpm --filter workflow-dashboard lint && pnpm --filter workflow-dashboard typecheck
pnpm --filter workflow-dashboard test && pnpm --filter workflow-dashboard test:coverage
...
pnpm --filter workflow-dashboard mcp:smoke
```
