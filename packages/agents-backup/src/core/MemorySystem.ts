...
class InMemoryConversationHistory implements ConversationHistory {
  ...
  async add(message: Message): Promise<void> {
    ...
    if (message.id) { // Using message.id as conversationId for now
...
export class MemorySystem implements IMemorySystem {
  ...
  constructor(config: {
    ...
    this.contextualMemoryTTL = this.parseTTL(config.contextualMemoryTTL || '1h');
   ...
   */
  addToWorkingMemory(content: string, tags: string[] = [], metadata?: Record<string, unknown>): string {
   ...
   */
  async addToContextualMemory(content: string, tags: string[] = [], metadata?: Record<string, unknown>): Promise<string> {
    const id = this.generateId();
    const item: MemoryItem = {
      id,
      type: 'contextual',
  ...
  async search(query: string, options: {
    ...
    const { types = ['working', 'contextual', 'episodic'], limit = 20 } = options;
    ...
    if (types.includes('contextual')) {
      ...
      results.push(...contextualResults.filter(item => !this.isExpired(item)));
    }

    // Search episodic memory
    if (types.includes('episodic')) {
      const episodicResults = await this.episodic.search(query, options.filters);
      results.push(...episodicResults.map(msg => ({
        id: msg.id,
  ...
  async cleanup(): Promise<void> {
    ...
    const expiredContextual = contextualItems.filter(item => this.isExpired(item));
    ...
    const cutoffTime = new Date(now - this.episodicMemoryTTL).toISOString();
