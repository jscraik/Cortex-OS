...
export class CortexAgent extends EventEmitter {
  ...
  private initializeAgent(): void {
    ...
    this.isRunning = true;
    this.emit('ready', { agentId: this.config.id, timestamp: new Date().toISOString() });
  }
  ...
  async process(input: AgentInput): Promise<AgentResponse> {
    ...
    try {
      ...
      // 4. Generate response
      const response = await this.generateResponse(modelProvider, processedInput, context);
      ...
      this.emit('request_complete', { requestId, response, duration, timestamp: Date.now() });

      return response;
    } catch (error) {
      const duration = Date.now() - startTime;
      this.emit('request_error', { requestId, error, duration, timestamp: Date.now() });
      throw error;
    }
  }
  ...
  async *processStream(input: AgentInput): AsyncIterable<StreamEvent> {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.emit('stream_complete', { requestId, duration, timestamp: Date.now() });
    } catch (error) {
      const duration = Date.now() - startTime;
      this.emit('stream_error', { requestId, error, duration, timestamp: Date.now() });
      throw error;
    }
  }
   ...
   * Query memory
   */
  async queryMemory(query: string, type: 'working' | 'contextual' | 'episodic' = 'contextual'): Promise<MemoryItem[]> {
    switch (type) {
      case 'working':
      ...
      default:
        throw new AgentError(`Invalid memory type: ${type}`, 'INVALID_MEMORY_TYPE');
    }
  }
   ...
   * Add item to memory
   */
  async addToMemory(content: string, type: 'working' | 'contextual' | 'episodic' = 'contextual'): Promise<string> {
    ...
    switch (type) {
      case 'working':
        ...
        if (this.memorySystem.working.size > this.config.memory.working.maxItems) {
  ...
  private async prepareContext(input: AgentInput): Promise<ConversationContext> {
    ...
    if (input.memory) {
      const relevantMemories = await this.queryMemory(input.memory.query, input.memory.type);
      context.metadata = {
        ...context.metadata,
  ...
  private async selectModelProvider(input: AgentInput): Promise<ModelProvider> {
  ...
  private async updateMemory(input: AgentInput, response: AgentResponse): Promise<void> {
  ...
  private handleMemoryUpdate(memoryItem: MemoryItem): void {
