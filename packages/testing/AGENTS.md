...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/testing/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Comprehensive test suite for the Cortex Memory system, covering unit tests, integration tests, parity tests, and soak tests.
- **Responsibilities:**
  - Implement the Testing domain logic and exported APIs.
...
# Build
pnpm --filter testing build

# Quality (affected-aware if available)
pnpm --filter testing lint
pnpm --filter testing typecheck
pnpm --filter testing test
pnpm --filter testing test:coverage
```
...
TESTING_BASE_URL=https://example.local
TESTING_DB_URL=postgres://user:pass@localhost:5432/testing
...
```bash
pnpm --filter testing test:coverage
pnpm security:scan --scope=testing
pnpm structure:validate --scope=testing
```
...
- SBOM: CycloneDX generated at `./sbom/testing.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"testing"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"testing","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter testing test:e2e
pnpm --filter testing test:smoke
pnpm --filter testing chaos:probe
```
...
# Dev & run
pnpm --filter testing dev
pnpm --filter testing start

# Build & verify
pnpm --filter testing build
pnpm --filter testing lint && pnpm --filter testing typecheck
pnpm --filter testing test && pnpm --filter testing test:coverage
