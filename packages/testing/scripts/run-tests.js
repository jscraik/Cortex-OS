...
program.name('test-runner').description('Cortex Memory test suite runner').version('1.0.0');
...
program
	...
	.action(async (options) => {
		...
		try {
			...
			execSync('npx vitest run src/integration --config vitest.integration.config.ts', {
				stdio: 'inherit',
			});
