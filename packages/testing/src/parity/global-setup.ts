...
beforeAll(async () => {
	...
	stdioServer = spawn('node', ['../mcp-server/dist/index.js', '--transport', 'stdio'], {
		stdio: 'pipe',
		cwd: process.cwd(),
		env: {
			...process.env,
	...
	httpServer = spawn(
		'node',
		['../mcp-server/dist/index.js', '--transport', 'http', '--port', '9603', '--host', '0.0.0.0'],
		{
			stdio: 'pipe',
			cwd: process.cwd(),
			env: {
...
afterAll(async () => {
	...
	try {
		execSync('curl -X DELETE http://localhost:6333/collections/parity-test-stdio', {
			stdio: 'ignore',
		});
		execSync('curl -X DELETE http://localhost:6333/collections/parity-test-http', {
			stdio: 'ignore',
		});
	} catch {
