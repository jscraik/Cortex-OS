...
describe('8-Hour Soak Tests', () => {
	...
	let _lastMemoryId: string | null = null;
	...
	afterAll(async () => {
		...
		console.log(
			`Average latency: ${latencySamples.length > 0 ? latencySamples.reduce((a, b) => a + b) / latencySamples.length : 0}ms`,
		);
	...
	async function performBurst(): Promise<void> {
		...
		console.log(
			`[${new Date().toISOString()}] Burst complete: ${successCount}/${results.length} successful, avg latency: ${avgLatency}ms, max: ${maxLatency}ms`,
		);
		...
		if (highLatencyCount > results.length * 0.1) {
			// More than 10% slow operations
			console.warn(
				`High latency detected: ${highLatencyCount}/${results.length} operations exceeded ${MAX_LATENCY_MS}ms`,
			);
		}
	}

	it(
		'should maintain reliability over 8 hours',
		async () => {
			...
			console.log(
				`Success rate: ${(((operationCount - failureCount) / operationCount) * 100).toFixed(2)}%`,
			);
	...
	it(
		'should handle memory growth gracefully',
		async () => {
			...
			const memoryMonitor = setInterval(() => {
				...
				console.log(
					`Current memory: ${Math.round(currentMemory.heapUsed / 1024 / 1024)}MB, Max: ${Math.round(maxMemory / 1024 / 1024)}MB`,
				);
				...
				if (memoryGrowth > 100 * 1024 * 1024) {
					...
					console.warn(
						`High memory usage detected: ${Math.round(memoryGrowth / 1024 / 1024)}MB growth`,
					);
				}
			...
			console.log(
				`Memory growth: ${Math.round(memoryGrowth / 1024 / 1024)}MB over ${Math.round(memoryTestDuration / 60000)} minutes`,
			);
	...
	it('should recover from temporary Qdrant outage', async () => {
		...
		execSync(
			'docker run -d --name cortex-test-qdrant -p 6333:6333 -p 6334:6334 qdrant/qdrant:v1.8.3',
			{ stdio: 'pipe' },
		);
