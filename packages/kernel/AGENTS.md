...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/kernel/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for Kernel within Cortex-OS.
- **Responsibilities:**
  - Implement the Kernel domain logic and exported APIs.
...
KERNEL_BASE_URL=https://example.local
KERNEL_DB_URL=postgres://user:pass@localhost:5432/kernel
...
```bash
pnpm --filter kernel test:coverage
pnpm security:scan --scope=kernel
pnpm structure:validate --scope=kernel
```
...
- SBOM: CycloneDX generated at `./sbom/kernel.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"kernel"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"kernel","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter kernel test:e2e
pnpm --filter kernel test:smoke
pnpm --filter kernel chaos:probe
```
...
# Dev & run
pnpm --filter kernel dev
pnpm --filter kernel start

# Build & verify
pnpm --filter kernel build
pnpm --filter kernel lint && pnpm --filter kernel typecheck
pnpm --filter kernel test && pnpm --filter kernel test:coverage
