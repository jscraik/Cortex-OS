...
describe('bindKernelTools', () => {
	...
	it('reads files only from the allowlist', async () => {
		...
		await expect(fsTool.execute({ path: '../etc/passwd' })).rejects.toThrow(
			/brAInwav filesystem deny/,
		);
	});
	...
	it('enforces http allowlist and timeout', async () => {
		...
		await expect(httpTool.execute({ url: 'https://evil.example.com' })).rejects.toThrow(
			/brAInwav http deny/,
		);
	});
});
