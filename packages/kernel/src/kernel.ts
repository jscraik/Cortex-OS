...
export class CortexKernel {
  ...
  private buildLangGraph(): any {
    ...
    // Define edges and transitions using proper string constants
    graph.addEdge(START, 'strategy');

    graph.addConditionalEdges('strategy', (state: WorkflowState) => this.routeFromStrategy(state), {
      build: 'build',
      recycle: 'recycle',
    });

    graph.addConditionalEdges('build', (state: WorkflowState) => this.routeFromBuild(state), {
    ...
    graph.addEdge('recycle', 'strategy');
