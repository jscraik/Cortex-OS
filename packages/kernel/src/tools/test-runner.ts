...
export class TestRunner {
  ...
  private async detectPackageManager(): Promise<string> {
    try {
      ...
      const pmLock = await execAsync('ls package-lock.json yarn.lock pnpm-lock.yaml bun.lockb 2>/dev/null || true');
  ...
  private async checkCompilation(packageManager: string): Promise<{ success: boolean; error?: string }> {
    try {
  ...
  private async executeTests(framework: string, packageManager: string): Promise<{
    success: boolean;
    passed: number;
    failed: number;
  ...
  private parseTestResults(result: any, framework: string): {
    success: boolean;
    passed: number;
    failed: number;
    failures: TestFailure[];
  ...
  private extractTestStats(parsed: any, framework: string): {
    success: boolean;
    passed: number;
    failed: number;
    failures: TestFailure[];
    ...
    switch (framework) {
      ...
      case 'vitest':
        return {
          success: parsed.success || false,
          passed: parsed.testResults?.reduce((sum: number, file: any) => sum + (file.numPassingTests || 0), 0) || 0,
          failed: parsed.testResults?.reduce((sum: number, file: any) => sum + (file.numFailingTests || 0), 0) || 0,
      ...
      default:
        return {
          ...
          passed: 0,
          failed: 1,
          failures: [{ name: 'unsupported-framework', file: 'test-runner', error: `Framework ${framework} not supported for result parsing` }],
        };
    }
  }
