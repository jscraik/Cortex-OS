...
export class SecurityScanner {
    ...
    async scanProject(): Promise<SecurityScanResult> {
        ...
        const blockers = allVulnerabilities.filter(v =>
            v.severity === 'critical' || (v.severity === 'high' && v.confidence > 90)
        ).length;

        const majors = allVulnerabilities.filter(v =>
            v.severity === 'high' || (v.severity === 'medium' && v.confidence > 80)
        ).length;
    ...
    private mapSemgrepSeverity(severity: string): 'critical' | 'high' | 'medium' | 'low' {
        switch (severity.toUpperCase()) {
            ...
            case 'INFO': case 'LOW': default: return 'low';
        }
    }
    ...
    private mapCodeQLSeverity(level: string): 'critical' | 'high' | 'medium' | 'low' {
        switch (level.toLowerCase()) {
            ...
            case 'note': case 'info': default: return 'low';
        }
    }
    ...
    private async runSecretDetection(): Promise<Vulnerability[]> {
        try {
            ...
            const secretPatterns = [
                { pattern: /(?:password|pwd|pass)\s*[=:]\s*["']([^"']+)["']/gi, type: 'hardcoded-password' },
                        ...
                        for (const { pattern, type } of secretPatterns) {
                            ...
                            if (matches) {
                                vulnerabilities.push({
                                    ...
                                    confidence: 85,
                                    fixRecommendation: 'Move sensitive data to environment variables or secure configuration',
                                });
                            }
                        }
                        lineNumber++;
    ...
    private async getFilesToScan(): Promise<string[]> {
        try {
            const result = await execAsync('find . -type f \\( -name "*.ts" -o -name "*.js" -o -name "*.json" -o -name "*.env*" -o -name "*.yml" -o -name "*.yaml" \\) -not -path "./node_modules/*" -not -path "./.git/*"');
