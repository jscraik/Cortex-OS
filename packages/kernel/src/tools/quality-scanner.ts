...
export class QualityScanner {
    ...
    private async runLighthouseScan(url?: string): Promise<{
        ...
        try {
            ...
            return {
                score: Math.round(Object.values(categories).reduce((sum: number, cat: any) => sum + (cat.score * 100), 0) / Object.keys(categories).length),
                details: {
    ...
    private async runAxeScan(url?: string): Promise<{
        ...
        try {
            ...
            const severity = criticalViolations > 0 ? 'critical' :
                seriousViolations > 0 ? 'serious' :
                    moderateViolations > 0 ? 'moderate' : 'minor';
    ...
    private async detectLocalServer(): Promise<string | null> {
        ...
        for (const port of commonPorts) {
            try {
                ...
                const result = await execAsync(`curl -f -s --max-time 5 ${testUrl} > /dev/null`, { timeout: 6000 });
                if (result.exitCode === 0) {
                    return testUrl;
                }
            } catch {
