...
export class StrategyNode {
    ...
    async validateSecurityBaseline(state) {
        // OWASP ASVS L1 + MITRE ATLAS validation
        const requirements = state.blueprint.requirements || [];
        const hasSecurityReq = requirements.some((req) => req.toLowerCase().includes('security') ||
    ...
    async validateUXAccessibility(state) {
        // WCAG 2.2 AA compliance check
        const hasUXReq = state.blueprint.requirements?.some((req) => req.toLowerCase().includes('ux') ||
        ...
        return {
            ...
            details: {
                ...
                accessibilityFeatures: state.blueprint.requirements?.filter((req) => req.toLowerCase().includes('accessibility')) || [],
            },
        };
    }
    async validateArchitecture(state) {
        ...
        const requirements = state.blueprint.requirements || [];
        const hasArch = title.includes('architecture') ||
            description.includes('system') ||
            requirements.some((r) => ['architecture', 'system design'].some((k) => r.toLowerCase().includes(k)));
        return {
            passed: hasArch,
            details: {
                architectureElements: requirements.filter((req) => ['architecture', 'system', 'design'].some((k) => req.toLowerCase().includes(k))),
            },
        };
    }
}
