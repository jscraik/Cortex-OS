...
import { createInitialPRPState, validateStateTransition } from './state.js';
 ...
 */
export function createKernel(orchestrator) {
...
export class CortexKernel {
	 ...
	 * Execute build phase
	 */
	async executeBuildPhase(state, deterministic = false) {
		const newState = {
			...state,
			phase: 'build',
			metadata: {
	 ...
	 */
	async runPRPWorkflow(blueprint, options = {}) {
	 ...
	 * Execute strategy phase
	 */
	async executeStrategyPhase(state, deterministic = false) {
		const newState = {
			...state,
			phase: 'strategy',
			metadata: {
	 ...
	 */
	getExecutionHistory(runId) {
		return this.executionHistory.get(runId) || [];
	}
	/**
	 * Execute evaluation phase
	 */
	async executeEvaluationPhase(state, deterministic = false) {
		const newState = {
			...state,
			phase: 'evaluation',
			metadata: {
...
//# sourceMappingURL=graph-simple.js.map
