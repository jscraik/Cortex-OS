...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/security/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Zero-trust security with mTLS, workload identity management, and LLM-aware static analysis
- **Responsibilities:**
  - Implement the Security domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter security dev

# Build
pnpm --filter security build

# Quality (affected-aware if available)
pnpm --filter security lint
pnpm --filter security typecheck
pnpm --filter security test
pnpm --filter security test:coverage
```
...
SECURITY_BASE_URL=https://example.local
SECURITY_DB_URL=postgres://user:pass@localhost:5432/security
...
```bash
pnpm --filter security test:coverage
pnpm security:scan --scope=security
pnpm structure:validate --scope=security
```
...
- SBOM: CycloneDX generated at `./sbom/security.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"security"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"security","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter security test:e2e
pnpm --filter security test:smoke
pnpm --filter security chaos:probe
```
...
# Dev & run
pnpm --filter security dev
pnpm --filter security start

# Build & verify
pnpm --filter security build
pnpm --filter security lint && pnpm --filter security typecheck
pnpm --filter security test && pnpm --filter security test:coverage
...
pnpm --filter security mcp:smoke
```
