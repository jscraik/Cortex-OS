...
export function createLogger(options: LoggerOptions = {}): Logger {
    ...
    function baseLog(level: LogLevel, bindings: Record<string, unknown>, msg: string, meta?: Record<string, unknown>) {
...
export function applyLogging(app: Express, opts: ApplyLoggingOptions = {}): Logger {
    ...
    app.use((req: Request, res: Response, next: NextFunction): void => {
        ...
        const reqId = r.requestId || req.header('x-request-id') || (res.getHeader('x-request-id') as string | undefined) || '';
        const child = root.child({ requestId: reqId });
        r.log = child;
        const started = process.hrtime.bigint();
        if (level === 'debug') child.debug('request:start', {
            method: req.method,
            path: req.path,
            ip: req.ip,
        ...
        res.on('finish', () => {
            ...
            if (Math.random() <= sampleRate) child.info('request:finish', {
                method: req.method,
                path: req.path,
