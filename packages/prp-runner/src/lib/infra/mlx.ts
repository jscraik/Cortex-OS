...
const probe = async (cmd: string, args: string[]): Promise<{ ok: boolean; out: string; err: string }> => {
  try {
...
const tryRunner = async (runner: Runner): Promise<MlxAvailability> => {
  ...
  if (runner === 'uv') {
    ...
    return res.ok ? { available: true, runner: 'uv', details: 'mlx.core + mlx_lm OK (uv)' } : { available: false, runner: 'uv', error: res.err };
  }
  if (runner === 'python3') {
    ...
    return res.ok ? { available: true, runner: 'python3', details: 'mlx.core + mlx_lm OK (python3)' } : { available: false, runner: 'python3', error: res.err };
  }
