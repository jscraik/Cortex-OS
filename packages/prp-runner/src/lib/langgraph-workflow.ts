...
import { ErrorBoundary, ErrorType, CategorizedError } from './error-boundary.js';
...
export class LangGraphWorkflow {
	...
	private createWorkflowGraph(): StateGraph<WorkflowState> {
		...
		// Add edges
		workflow.addEdge('start', 'selectModel');
		workflow.addEdge('selectModel', 'execute');
		workflow.addEdge('execute', 'validate');

		// Add conditional edges
		workflow.addConditionalEdges('validate', this.shouldRetryOrComplete.bind(this), {
			retry: 'execute',
			complete: 'complete',
			error: 'handleError',
		});

		workflow.addConditionalEdges('handleError', this.shouldRetryOrFail.bind(this), {
			retry: 'execute',
			fail: END,
		});
