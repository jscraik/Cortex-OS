...
const DEFAULT_PATH = process.env.PRP_API_KEYS_FILE || path.resolve(process.cwd(), 'data/api-keys.json');
...
export function generateApiKey(role: 'admin' | 'user', label?: string, filePath = DEFAULT_PATH): ApiKeyRecord {
...
export function resolveRoleForKey(key?: string, filePath = DEFAULT_PATH): 'admin' | 'user' | undefined {
