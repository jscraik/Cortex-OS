...
import { z } from 'zod';
import { CircuitBreaker, retry } from './lib/resilience/circuit-breaker.js';
...
// Shared breakers per downstream
const ollamaBreakers = new Map<string, CircuitBreaker>();
function getOllamaBreaker(
	endpoint: string,
	cfg?: { threshold?: number; timeout?: number },
): CircuitBreaker {
	let b = ollamaBreakers.get(endpoint);
	if (!b) {
		b = new CircuitBreaker({
			threshold: cfg?.threshold ?? 3,
			timeout: cfg?.timeout ?? 200,
			onStateChange: (state) =>
				metrics.incCounter(
					'breaker_state_total',
		...
		ollamaBreakers.set(endpoint, b);
	}
	return b;
}

const mlxBreakers = new Map<string, CircuitBreaker>();
function getMlxBreaker(
	key = 'mlx-knife',
	cfg?: { threshold?: number; timeout?: number },
): CircuitBreaker {
	let b = mlxBreakers.get(key);
	if (!b) {
		b = new CircuitBreaker({
			threshold: cfg?.threshold ?? 3,
			timeout: cfg?.timeout ?? 200,
...
function createOllamaAdapter(cfg: LLMConfig): OllamaAdapter {
	...
	return {
		async generate({ prompt, temperature, maxTokens, model }) {
			...
			const breaker = getOllamaBreaker(String(cfg.endpoint), cfg.breakers?.ollama);
			try {
				const text = await breaker.call(() => retry(call, 3, 50));
				return { text };
			} catch (err) {
				metrics.incCounter(
					'breaker_events_total',
					'Breaker events',
					{ target: 'ollama', event: 'failure' },
					1,
				);
				throw err;
			}
...
async function generateWithMLX(
	...
	const adapter = state.mlxAdapter; // exists due to early guard
	const call = async () =>
		adapter.generate({
			prompt,
	...
	const knifeKey = `knife:${state.config.knifePath || 'mlx-knife'}:${state.config.mlxModel || 'unknown'}`;
	const breaker = getMlxBreaker(knifeKey, state.config.breakers?.mlx);
	try {
		return await breaker.call(() => retry(call, 3, 50));
	} catch (err) {
		metrics.incCounter(
			'breaker_events_total',
			'Breaker events',
			{ target: 'mlx', event: 'failure' },
			1,
		);
		throw err;
	}
