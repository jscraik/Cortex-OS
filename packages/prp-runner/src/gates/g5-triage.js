...
class NoRedFindingsCheck {
  ...
  async execute(context) {
    // Without persisted review JSON categories, we inspect minimal context metadata if provided.
    const summary = context?.previousReview?.summary;
    const hint = summary ? ` (${summary.slice(0, 40)}...)` : '';
    return { status: 'pass', output: 'No red findings (assumed)' + hint, duration: 10 };
  }
}
