...
class CoverageTargetsCheck {
    ...
    async execute(context) {
        ...
        const issues = [];
        if (!(coverageLines > 0))
            issues.push("coverageLines must be > 0");
        if (!(coverageBranches > 0))
            issues.push("coverageBranches must be > 0");
        const evidence = [{
                id: nanoid(),
                type: "validation",
...
class PerfAndA11yBudgetCheck {
    ...
    async execute(context) {
        ...
        const issues = [];
        if (!(performanceLCP > 0))
            issues.push("performanceLCP must be > 0");
        if (!(performanceTBT > 0))
            issues.push("performanceTBT must be > 0");
        if (!(a11yScore > 0))
            issues.push("a11yScore must be > 0");
        const evidence = [{
                id: nanoid(),
                type: "validation",
                source: "g2-perf-a11y-budgets",
                content: JSON.stringify({ performanceLCP, performanceTBT, a11yScore, issues }),
                timestamp: new Date().toISOString(),
                phase: "build",
                commitSha: context.repoInfo.commitSha,
            }];
        return {
            status: (issues.length === 0 ? "pass" : "fail"),
            output: issues.length === 0 ? "Performance and accessibility budgets present" : `Issues: ${issues.join(", ")}`,
            duration: 40,
            evidence,
        };
    }
...
class TestCategoriesPlanCheck {
    ...
    async execute(context) {
        ...
        return {
            ...
            output: missing.length === 0 ? "All required test categories planned" : `Missing categories: ${missing.join(", ")}`,
            duration: 35,
            evidence,
        };
    }
...
export class G2TestPlanGate extends BaseGate {
    id = "G2";
    name = "Test Plan";
    purpose = "Validate that test strategy meets coverage, performance, and accessibility requirements";
