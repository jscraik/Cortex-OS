...
def test_sentence_transformers():
    ...
    try:
        ...
        try:
            ...
            cache_path = os.environ.get('HF_CACHE_PATH', os.path.expanduser('~/.cache/huggingface'))
            model = SentenceTransformer('Qwen/Qwen3-Embedding-0.6B', cache_folder=cache_path)
    ...
    def embed_with_qwen(texts):
        ...
        try:
            ...
            try:
                ...
                model = AutoModel.from_pretrained('Qwen/Qwen3-Embedding-0.6B', cache_dir=cache_dir)
                ...
                # Fallback to mock
                import hashlib
                return [[hash(text + str(i)) % 100 / 100.0 for i in range(384)] for text in texts]
