...
export class ASBRAIMcpServer {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (capabilities && capabilities.features) {
        console.log(`✅ ASBR AI MCP Server initialized with ${capabilities.features.length} features`);
      } else {
    ...
    const tools: MCPTool[] = [
      {
        ...
        description: 'Generate text using MLX language models with optional system prompts and parameters',
        inputSchema: {
          type: 'object',
          properties: {
            prompt: {
      ...
      {
        name: 'ai_rag_query',
        description: 'Perform Retrieval-Augmented Generation (RAG) query combining search and generation',
        inputSchema: {
          type: 'object',
          properties: {
            query: {
      ...
      content: [{
        ...
        text: JSON.stringify({
          ...
          interpretation: similarity > 0.8 ? 'very similar' : 
                         ...
                         similarity > 0.3 ? 'somewhat similar' : 'not similar'
        }, null, 2)
      }]
    };
  }
  ...
  private async handleGetCapabilities(args: any): Promise<MCPToolCallResponse> {
    try {
      const capabilities = await this.aiCapabilities.getCapabilities();
  ...
  private async handleGetKnowledgeStats(args: any): Promise<MCPToolCallResponse> {
   ...
   */
  async getHealth(): Promise<{ status: string; tools: number; features: string[] }> {
    try {
      const capabilities = await this.aiCapabilities.getCapabilities();
