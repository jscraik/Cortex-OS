...
describe('🔥 Real Qwen Embedding Tests', () => {
  ...
  describe('Semantic Search with Qwen', () => {
    ...
    it('should handle domain-specific queries', async () => {
      ...
      const programmingRelated = results.some(
        (r) =>
          r.text.toLowerCase().includes('python') || r.text.toLowerCase().includes('javascript'),
      );
    ...
    it('should filter unrelated content', async () => {
      ...
      const hasUnrelated = results.some(
        (r) => r.text.toLowerCase().includes('cooking') || r.text.toLowerCase().includes('weather'),
      );
  ...
  describe('Performance with Real Model', () => {
    it('should handle batch embeddings efficiently', async () => {
      const texts = Array.from(
        { length: 10 },
        (_, i) => `This is test document number ${i} for batch processing evaluation.`,
      );
