...
import { createRerankerState, type RerankerConfig, rerank } from '../lib/reranker/index.js';
...
describe('Reranker', () => {
	it('orders documents by score using transformers', async () => {
		const inference = vi
			.fn()
			.mockResolvedValue([[{ score: 0.1 }], [{ score: 0.9 }], [{ score: 0.5 }]]);
	...
	it('throws for unknown provider', async () => {
		...
		expect(() => createRerankerState('unknown')).toThrow(/Unsupported reranker provider/);

		const badState = {
			config: { provider: 'unknown' as RerankerConfig['provider'] },
		};
		await expect(rerank(badState, 'q', ['a'])).rejects.toThrow(/Reranking not implemented/);
	});
});
