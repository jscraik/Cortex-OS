...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/prp-runner/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for PRP Runner within Cortex-OS.
- **Responsibilities:**
  - Implement the PRP Runner domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter prp-runner dev

# Build
pnpm --filter prp-runner build

# Quality (affected-aware if available)
pnpm --filter prp-runner lint
pnpm --filter prp-runner typecheck
pnpm --filter prp-runner test
pnpm --filter prp-runner test:coverage
```
...
PRP_RUNNER_BASE_URL=https://example.local
PRP_RUNNER_DB_URL=postgres://user:pass@localhost:5432/prp_runner
...
```bash
pnpm --filter prp-runner test:coverage
pnpm security:scan --scope=prp-runner
pnpm structure:validate --scope=prp-runner
```
...
- SBOM: CycloneDX generated at `./sbom/prp-runner.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"prp-runner"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"prp-runner","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter prp-runner test:e2e
pnpm --filter prp-runner test:smoke
pnpm --filter prp-runner chaos:probe
```
...
# Dev & run
pnpm --filter prp-runner dev
pnpm --filter prp-runner start

# Build & verify
pnpm --filter prp-runner build
pnpm --filter prp-runner lint && pnpm --filter prp-runner typecheck
pnpm --filter prp-runner test && pnpm --filter prp-runner test:coverage
...
pnpm --filter prp-runner mcp:smoke
```
