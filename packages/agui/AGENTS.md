...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/agui/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for Agui within Cortex-OS.
- **Responsibilities:**
  - Implement the Agui domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter agui dev

# Build
pnpm --filter agui build

# Quality (affected-aware if available)
pnpm --filter agui lint
pnpm --filter agui typecheck
pnpm --filter agui test
pnpm --filter agui test:coverage
```
...
AGUI_BASE_URL=https://example.local
AGUI_DB_URL=postgres://user:pass@localhost:5432/agui
...
```bash
pnpm --filter agui test:coverage
pnpm security:scan --scope=agui
pnpm structure:validate --scope=agui
```
...
- SBOM: CycloneDX generated at `./sbom/agui.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"agui"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"agui","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter agui test:e2e
pnpm --filter agui test:smoke
pnpm --filter agui chaos:probe
```
...
# Dev & run
pnpm --filter agui dev
pnpm --filter agui start

# Build & verify
pnpm --filter agui build
pnpm --filter agui lint && pnpm --filter agui typecheck
pnpm --filter agui test && pnpm --filter agui test:coverage
