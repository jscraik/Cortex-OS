...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/history-store/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Persistence layer for conversation history, event streams, and checkpoints. Ships with SQLite and file adapters by default plus hooks for PostgreSQL-backed deployments.
- **Responsibilities:**
  - Implement the History Store domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter history-store dev

# Build
pnpm --filter history-store build

# Quality (affected-aware if available)
pnpm --filter history-store lint
pnpm --filter history-store typecheck
pnpm --filter history-store test
pnpm --filter history-store test:coverage
```
...
HISTORY_STORE_BASE_URL=https://example.local
HISTORY_STORE_DB_URL=postgres://user:pass@localhost:5432/history_store
...
```bash
pnpm --filter history-store test:coverage
pnpm security:scan --scope=history-store
pnpm structure:validate --scope=history-store
```
...
- SBOM: CycloneDX generated at `./sbom/history-store.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"history-store"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"history-store","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter history-store test:e2e
pnpm --filter history-store test:smoke
pnpm --filter history-store chaos:probe
```
...
# Dev & run
pnpm --filter history-store dev
pnpm --filter history-store start

# Build & verify
pnpm --filter history-store build
pnpm --filter history-store lint && pnpm --filter history-store typecheck
pnpm --filter history-store test && pnpm --filter history-store test:coverage
...
pnpm --filter history-store mcp:smoke
```
