...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/registry/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** *Centralized schema validation and versioning for distributed agent communication*
- **Responsibilities:**
  - Implement the Registry domain logic and exported APIs.
...
pnpm --filter registry test:coverage
```
...
REGISTRY_BASE_URL=https://example.local
REGISTRY_DB_URL=postgres://user:pass@localhost:5432/registry
...
```bash
pnpm --filter registry test:coverage
pnpm security:scan --scope=registry
pnpm structure:validate --scope=registry
```
...
- SBOM: CycloneDX generated at `./sbom/registry.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"registry"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"registry","level":"info","msg":"started","port":0}
```
...
pnpm --filter registry test:smoke
pnpm --filter registry chaos:probe
```
...
# Build & verify
pnpm --filter registry build
pnpm --filter registry lint && pnpm --filter registry typecheck
pnpm --filter registry test && pnpm --filter registry test:coverage
...
pnpm --filter registry mcp:smoke
```
