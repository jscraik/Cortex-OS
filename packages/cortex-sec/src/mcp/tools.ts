...
function stableStringify(
	...
	if (value && typeof value === 'object') {
		...
		const result = `{${entries
			.map(
				([key, entry]) =>
					`${JSON.stringify(key)}:${stableStringify(entry, seen, depth + 1, maxDepth)}`,
			)
			.join(',')}}`;
...
function createSemgrepTool(): CortexSecTool {
	...
	return {
		...
		handler: async (params: unknown) => {
			...
			const rulesets =
				input.rulesets && input.rulesets.length > 0 ? input.rulesets : ['brAInwav/owasp-top10'];
	...
	return {
		...
		handler: async (params: unknown) => {
			...
			const payload = {
				...
				brand: BRAND,
				riskLevel: risk,
				codeSnippetHash: input.codeSnippet ? createFingerprint(input.codeSnippet) : undefined,
				recommendedActions: [
