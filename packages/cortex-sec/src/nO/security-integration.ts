import { createCompliancePlanner, type CompliancePlanner, type CompliancePlanningResult } from '../planning/compliance-planner.js';
...
export function createSecurityIntegrationService(planner: CompliancePlanner = createCompliancePlanner()): SecurityIntegrationService {
        return {
                evaluate(input: SecurityIntegrationInput): SecurityIntegrationResult {
...
function buildSummary(description: string, result: CompliancePlanningResult): string {
        const severity = result.aggregateRisk >= 0.7 ? 'critical' : result.aggregateRisk >= 0.4 ? 'elevated' : 'nominal';
