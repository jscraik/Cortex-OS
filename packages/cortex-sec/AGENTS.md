...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/cortex-sec/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Orchestrates compliance strategies, risk aggregation, and remediation planning
- **Responsibilities:**
  - Implement the Cortex SEC domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter cortex-sec dev

# Build
pnpm --filter cortex-sec build

# Quality (affected-aware if available)
pnpm --filter cortex-sec lint
pnpm --filter cortex-sec typecheck
pnpm --filter cortex-sec test
pnpm --filter cortex-sec test:coverage
```
...
CORTEX_SEC_BASE_URL=https://example.local
CORTEX_SEC_DB_URL=postgres://user:pass@localhost:5432/cortex_sec
...
```bash
pnpm --filter cortex-sec test:coverage
pnpm security:scan --scope=cortex-sec
pnpm structure:validate --scope=cortex-sec
```
...
- SBOM: CycloneDX generated at `./sbom/cortex-sec.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"cortex-sec"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"cortex-sec","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter cortex-sec test:e2e
pnpm --filter cortex-sec test:smoke
pnpm --filter cortex-sec chaos:probe
```
...
# Dev & run
pnpm --filter cortex-sec dev
pnpm --filter cortex-sec start

# Build & verify
pnpm --filter cortex-sec build
pnpm --filter cortex-sec lint && pnpm --filter cortex-sec typecheck
pnpm --filter cortex-sec test && pnpm --filter cortex-sec test:coverage
...
pnpm --filter cortex-sec mcp:smoke
```
