...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/rag-http/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for RAG Http within Cortex-OS.
- **Responsibilities:**
  - Implement the RAG Http domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter rag-http dev

# Build
pnpm --filter rag-http build

# Quality (affected-aware if available)
pnpm --filter rag-http lint
pnpm --filter rag-http typecheck
pnpm --filter rag-http test
pnpm --filter rag-http test:coverage
```
...
RAG_HTTP_BASE_URL=https://example.local
RAG_HTTP_DB_URL=postgres://user:pass@localhost:5432/rag_http
...
```bash
pnpm --filter rag-http test:coverage
pnpm security:scan --scope=rag-http
pnpm structure:validate --scope=rag-http
```
...
- SBOM: CycloneDX generated at `./sbom/rag-http.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"rag-http"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"rag-http","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter rag-http test:e2e
pnpm --filter rag-http test:smoke
pnpm --filter rag-http chaos:probe
```
...
# Dev & run
pnpm --filter rag-http dev
pnpm --filter rag-http start

# Build & verify
pnpm --filter rag-http build
pnpm --filter rag-http lint && pnpm --filter rag-http typecheck
pnpm --filter rag-http test && pnpm --filter rag-http test:coverage
...
pnpm --filter rag-http mcp:smoke
```
