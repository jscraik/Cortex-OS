...
import { EventSchema, type Event, type TaskInput } from '../../src/types/index.js';
...
describe('ASBR Event Contract', () => {
    ...
    it('rejects unknown event types', () => {
        const unknown = EventSchema.safeParse({ ...baseEvent('PlanStarted'), type: 'Unknown' });
        expect(unknown.success).toBe(false);
    });

    it('rejects invalid timestamp', () => {
        const invalid = EventSchema.safeParse({ ...baseEvent('PlanStarted'), timestamp: 'not-a-date' });
    ...
    it('emitPlanStarted produces a schema-valid event', async () => {
        ...
        await emitPlanStarted(async (ev: Event) => { collected.push(ev); }, task, input);
    ...
    it('fails validation when id is not a uuid', () => {
        const invalid = EventSchema.safeParse({ ...baseEvent('Failed'), id: '123' });
        expect(invalid.success).toBe(false);
    });
    ...
    it('allows optional data and ariaLiveHint', () => {
        const evt = baseEvent('DeliverableReady', { ariaLiveHint: 'ready', data: { foo: 'bar' } });
        const parsed = EventSchema.parse(evt);
        expect(parsed.data?.foo).toBe('bar');
    });

    it('rejects non-object data', () => {
        const invalid = EventSchema.safeParse({ ...baseEvent('Resumed'), data: 123 });
        expect(invalid.success).toBe(false);
    });
});
