...
class ServiceMetricsCollector {
    quotaGlobalReject = 0;
    quotaAgentReject = 0;
    incQuotaGlobal() { this.quotaGlobalReject += 1; }
    incQuotaAgent() { this.quotaAgentReject += 1; }
    snapshot(): ServiceMetricsSnapshot {
        return { quotaGlobalReject: this.quotaGlobalReject, quotaAgentReject: this.quotaAgentReject };
    }
}
