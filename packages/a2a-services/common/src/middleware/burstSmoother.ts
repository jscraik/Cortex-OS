...
export interface BurstSmootherMetrics { rejected: number; accepted: number; buckets: number }
...
export function createBurstSmoother({ ratePerSec, burst, keyHeader, now = () => Date.now() }: BurstSmootherOptions): BurstSmootherMiddleware {
    if (ratePerSec <= 0) throw new Error('ratePerSec must be > 0');
    if (burst <= 0) throw new Error('burst must be > 0');
    const buckets = new Map<string, Bucket>();
    const metrics: BurstSmootherMetrics = { rejected: 0, accepted: 0, buckets: 0 };
