...
```yaml
id: agents-a2a-services-v1
package: A2A Services
type: package
runtime: node
...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/a2a-services/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for A2A Services within Cortex-OS.
- **Responsibilities:**
  - Implement the A2A Services domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter a2a-services dev

# Build
pnpm --filter a2a-services build

# Quality (affected-aware if available)
pnpm --filter a2a-services lint
pnpm --filter a2a-services typecheck
pnpm --filter a2a-services test
pnpm --filter a2a-services test:coverage
```
...
A2A_SERVICES_BASE_URL=https://example.local
A2A_SERVICES_DB_URL=postgres://user:pass@localhost:5432/a2a_services
...
```bash
pnpm --filter a2a-services test:coverage
pnpm security:scan --scope=a2a-services
pnpm structure:validate --scope=a2a-services
```
...
- SBOM: CycloneDX generated at `./sbom/a2a-services.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"a2a-services"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"a2a-services","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter a2a-services test:e2e
pnpm --filter a2a-services test:smoke
pnpm --filter a2a-services chaos:probe
```
...
# Dev & run
pnpm --filter a2a-services dev
pnpm --filter a2a-services start

# Build & verify
pnpm --filter a2a-services build
pnpm --filter a2a-services lint && pnpm --filter a2a-services typecheck
pnpm --filter a2a-services test && pnpm --filter a2a-services test:coverage
...
pnpm --filter a2a-services mcp:smoke
```
