...
```yaml
id: agents-workflow-orchestrator-v1
...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `packages/workflow-orchestrator/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** Core module for Workflow Orchestrator within Cortex-OS.
- **Responsibilities:**
  - Implement the Workflow Orchestrator domain logic and exported APIs.
...
```bash
# Dev
pnpm --filter workflow-orchestrator dev

# Build
pnpm --filter workflow-orchestrator build

# Quality (affected-aware if available)
pnpm --filter workflow-orchestrator lint
pnpm --filter workflow-orchestrator typecheck
pnpm --filter workflow-orchestrator test
pnpm --filter workflow-orchestrator test:coverage
```
...
WORKFLOW_ORCHESTRATOR_BASE_URL=https://example.local
WORKFLOW_ORCHESTRATOR_DB_URL=postgres://user:pass@localhost:5432/workflow_orchestrator
...
```bash
pnpm --filter workflow-orchestrator test:coverage
pnpm security:scan --scope=workflow-orchestrator
pnpm structure:validate --scope=workflow-orchestrator
```
...
- SBOM: CycloneDX generated at `./sbom/workflow-orchestrator.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"workflow-orchestrator"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"workflow-orchestrator","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter workflow-orchestrator test:e2e
pnpm --filter workflow-orchestrator test:smoke
pnpm --filter workflow-orchestrator chaos:probe
```
...
# Dev & run
pnpm --filter workflow-orchestrator dev
pnpm --filter workflow-orchestrator start

# Build & verify
pnpm --filter workflow-orchestrator build
pnpm --filter workflow-orchestrator lint && pnpm --filter workflow-orchestrator typecheck
pnpm --filter workflow-orchestrator test && pnpm --filter workflow-orchestrator test:coverage
...
pnpm --filter workflow-orchestrator mcp:smoke
```
