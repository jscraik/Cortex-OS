...
test.describe('Automated accessibility compliance', () => {
        ...
        test('dashboard maintains a logical heading structure', async ({ page }) => {
                ...
                expect(headingLevels.length, 'brAInwav accessibility: missing headings in dashboard view').toBeGreaterThan(0);
                expect(headingLevels.includes(1), 'brAInwav accessibility: missing level-one heading for context').toBe(true);
        ...
        test('primary navigation supports keyboard focus', async ({ page }) => {
                ...
                await expect(focusableElements.first(), 'brAInwav accessibility: at least one focusable element is required').toBeVisible();

                await page.keyboard.press('Tab');

                const activeElementTag = await page.evaluate(() => document.activeElement?.tagName ?? '');
                expect(activeElementTag.length, 'brAInwav accessibility: focus did not move after Tab navigation').toBeGreaterThan(0);

                const navigationLocator = page.locator('nav, [role="navigation"]').first();
                await expect(navigationLocator, 'brAInwav accessibility: navigation landmark must be visible').toBeVisible();
                ...
                await expect(navigationFocusable.first(), 'brAInwav accessibility: navigation item is not keyboard focusable').toBeFocused();
        });
});
