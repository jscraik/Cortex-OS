/**
 * @file_path tests/security/security-wrappers.integration.test.ts
 * @description Integration tests for all security wrapper classes working together
...
describe('Security Wrappers - Integration Tests', () => {
	...
	describe('Unified Security Policy Enforcement Tests', () => {
		test('should enforce consistent input validation across all wrappers', async () => {
			...
			}).not.toThrow(); // Parameterized queries prevent SQL injection

			// Neo4j wrapper should reject
			const maliciousNode = {
				id: maliciousInput,
				label: 'User',
				props: { name: 'John' },
			...
			// Command executor should reject
			await expect(async () => {
