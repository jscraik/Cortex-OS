...
describe('Memory Adapter Migration - Phase 1.1', () => {
	describe('Legacy Direct Database Adapter Removal', () => {
		it('should reject direct database connections', async () => {
			...
			const { MemoryAdapter } = await import(
				'../../packages/memories/src/adapters/store.prisma/client.js'
			);
		...
		it('should route all operations through REST API', async () => {
			...
			const { LocalMemoryStore } = await import(
				'../../packages/memories/src/adapters/store.localmemory.js'
			);
	...
	describe('REST API Adapter Validation', () => {
		...
		beforeEach(async () => {
			const { RestApiClient } = await import(
				'../../packages/memories/src/adapters/rest-api/rest-adapter.js'
			);
