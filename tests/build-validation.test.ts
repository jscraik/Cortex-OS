            ...
            for (const entry of entries) {
                ...
                if (stats.isDirectory()) {
                    ...
                    if (pattern === '**/project.json' && entry === 'project.json') {
                        ...
                        files.push(fullPath);
                    } else if (pattern === '**/*.{ts,tsx}' && /\.(ts|tsx)$/.test(entry) && !entry.endsWith('.d.ts')) {
                        files.push(fullPath);
                    }
                }
            }
...
describe('brAInwav Build Validation Suite', () => {
    ...
    describe('📋 NX Configuration Compliance', () => {
                    ...
                    for (let i = 0; i < lines.length; i++) {
                        ...
                        if (line.includes('{workspaceRoot}')) {
                            ...
                            if (isInCommand) {
                                ...
                                if (commandMatch && commandMatch[1].includes(' ') && commandMatch[1].includes('{workspaceRoot}')) {
                                    violations.push({
        ...
        it('should build all packages without NX errors', async () => {
            ...
            const result = await buildSuite.runCommand('npx nx run-many --target=build --all --skip-nx-cache', {
                timeout: 300000
            });
