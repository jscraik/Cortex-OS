...
describe("memory guard", () => {
  it("kills process exceeding memory", async () => {
    const hog = spawn(process.execPath, ["--expose-gc", hogScript], { stdio: "ignore" });
    let logs = "";
    const guard = spawn(process.execPath, ["scripts/memory-guard.mjs", "--pid", String(hog.pid), "--max", "30", "--interval", "100"], {
      cwd: resolve(__dirname, ".."),
    });
    guard.stdout.on("data", (d) => { logs += d.toString(); });
