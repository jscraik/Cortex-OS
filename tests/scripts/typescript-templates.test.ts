...
function findPackagesWithTsConfig(): string[] {
  try {
    ...
    return output.trim().split('\n').map(p => path.dirname(p)).filter(Boolean);
  } catch {
    return [];
  }
}
...
describe('Phase 2: Package Conformance Validation', () => {
  ...
  describe('Composite Flag Compliance', () => {
    it.each(packages)(
      '%s should have composite: true (if buildable)',
      (pkg) => {
    ...
    it.each(packages)(
      ...
      (pkg) => {
        ...
        const hasSpecConfig = fs.existsSync(
          path.join(process.cwd(), pkg, 'tsconfig.spec.json')
        );
  ...
  describe('Module Resolution Standards', () => {
    it.each(packages)(
      '%s should use NodeNext module resolution',
      (pkg) => {
        const tsconfigPath = path.join(process.cwd(), pkg, 'tsconfig.json');
  ...
  describe('Include/Exclude Patterns', () => {
    it.each(packages)(
      '%s should have clean include patterns',
      (pkg) => {
        const tsconfigPath = path.join(process.cwd(), pkg, 'tsconfig.json');
        ...
        if (tsconfig.include) {
          ...
          const hasSrcInclude = tsconfig.include.some((p: string) =>
            p.includes('src')
          );
          expect(hasSrcInclude).toBe(true);
          
          // Test files should NOT be in main include (should be in spec config)
          const includesTests = tsconfig.include.some((p: string) =>
            p.includes('test') && !p.includes('setup')
          );
...
describe('Phase 2: brAInwav Standards Summary', () => {
  it('should report overall conformance statistics', () => {
    ...
    packages.forEach((pkgDir) => {
      ...
      const excludesStandard = tsconfig.exclude?.includes('dist') && 
    ...
    console.log(`   Conforming: ${conformingPackages}/${validPackages} (${conformanceRate.toFixed(1)}%)`);
