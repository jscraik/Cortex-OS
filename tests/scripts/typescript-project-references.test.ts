...
describe('Phase 3: TypeScript Project References', () => {
	...
	describe('Reference Configuration', () => {
		it.each(packagesWithReferences)('%s should have project references configured', (pkgPath) => {
		...
		it.each(packagesWithReferences)('%s references should point to valid paths', (pkgPath) => {
			const tsconfigPath = path.join(process.cwd(), pkgPath, 'tsconfig.json');
			const tsconfig = JSON.parse(fs.readFileSync(tsconfigPath, 'utf-8'));
	...
	describe('Phase 3 Summary', () => {
		it('should report overall Phase 3 conformance', () => {
			...
			console.log(
				`   Packages with references: ${packagesWithRefs}/${packagesWithReferences.length}`,
			);
			console.log(`   Total references added: ${totalReferences}`);
			console.log(
				`   Average references per package: ${(totalReferences / packagesWithRefs).toFixed(1)}`,
			);
