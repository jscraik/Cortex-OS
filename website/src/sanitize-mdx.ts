...
const startsWithFence = (s: string): RegExpExecArray | null => /^(`{3,5})(?:\s*(markdown|md|mdx)\b)?\s*$/i.exec(s);
...
export const upgradeDoubleBacktickFences = (s: string): string => {
    const langs = ['ts', 'tsx', 'js', 'cjs', 'mjs', 'jsx', 'json', 'jsonc', 'bash', 'sh', 'shell', 'zsh', 'yaml', 'yml', 'toml', 'ini', 'text', 'md', 'markdown', 'typescript', 'mermaid', 'http', 'sql', 'diff'];
    return s
        .replace(/(^|\n)``([a-zA-Z0-9_-]{1,15})\n/g, (m, pre, lang) => (langs.includes(lang) && !pre.endsWith('`') ? `${pre}\`\`\`${lang}\n` : m))
        .replace(/(^|\n)( {0,3})``([a-z]{1,5})\n([\s\S]*?)\n``(\n|$)/g, (_m, pre, indent, lang, body, tail) =>
...
export const protectInlineGenerics = (s: string): string => {
    ...
    let out = s.replace(/\b(?:Record|Array|Promise)<[^\n<>]{1,60}>/g, (m) => `\`${m.replace(/</g, OPEN).replace(/>/g, CLOSE)}\``);
    out = out.replace(/(?<!`)\b([A-Z][A-Za-z0-9_$.]{0,40})<([^\n<>]{1,80})>(?!`)/g, (_m, id, inner) => `\`${String(id)}${OPEN}${String(inner)}${CLOSE}\``);
    out = out.replace(/`([^`]+?):\s*`(Promise__GENO__[^`]+?__GENC__)`/g, (_m, left, p) => `\`${left}: ${p}\``);
...
export const escapePseudoJsx = (s: string): string => s
    .replace(/<([^\s<>]{1,90})>/g, (m, inner) => {
        ...
        if (isSimpleIdentifier(inner) || isBracketed(inner) || isBareToken(inner) || isAttrLike(inner)) {
...
const stripTrailingWhitespace = (s: string): string => s
    .split('\n')
    .map((l) => l.replace(/[ \t]+$/g, ''))
    .join('\n')
    .replace(/^\s+$/gm, '');
...
export const isMermaidContentLine = (l: string): boolean => {
    const t = l.trim();
    if (t.startsWith('participant ') || t.startsWith('note ') || /^(alt|loop|end|rect)\b/i.test(t)) return true;
...
export const findFenceLookahead = (lines: string[], start: number): { foundOpen: boolean; foundClose: number; codeLike: boolean } => {
    let codeLike = false; let foundClose = -1; let foundOpen = false;
    for (let j = start + 1; j < Math.min(lines.length, start + 120); j++) {
        const mm = fenceOpenRe.exec(lines[j] || '');
        if (mm) { if (mm[1]) { foundOpen = true; break; } else { foundClose = j; break; } }
...
const extendBlockOnEarlyClose = (s: string): string => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!m) continue;
        const lang = m[1];
        if (lang) { stack.push(i); continue; }
        if (stack.length === 0) continue;
        const res = findFenceLookahead(lines, i);
        if (!res.foundOpen && res.foundClose !== -1 && res.codeLike) { lines[i] = ''; }
        else { stack.pop(); }
    }
    return lines.join('\n');
};

const scoreMermaidContinuation = (lines: string[], start: number): { mermaidLike: number; total: number; hardStop: boolean } => {
    let mermaidLike = 0; let total = 0; let hardStop = false;
    for (let j = start + 1; j < Math.min(lines.length, start + 40); j++) {
        const ln = lines[j] || '';
        if (ln.startsWith('```')) break;
        if (/^\s*$/.test(ln)) { total++; continue; }
        if (isHeading(ln) || ln.trim() === '---') { hardStop = true; break; }
        total++;
...
const extendMermaidOnPrematureClose = (s: string): string => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!m) continue;
        if (m[1] === 'mermaid') { openMermaid = i; continue; }
        if (openMermaid == null) continue;
        const { mermaidLike, total, hardStop } = scoreMermaidContinuation(lines, i);
        if (!hardStop && mermaidLike > 0 && mermaidLike >= Math.min(3, total)) { lines[i] = ''; }
        else { openMermaid = null; }
    }
    return lines.join('\n');
};
...
const autoCloseDanglingFences = (s: string): string => {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!m) continue;
        if (m[1]) opens.push(i); else if (opens.length) opens.pop();
    }
    if (opens.length > 0) lines.push(...Array(opens.length).fill('```'));
    return lines.join('\n');
};

export const escapeInlineAnglesInCode = (s: string): string => s.replace(/`[^`]{1,160}`/g, (seg) => /[<>]/.test(seg) ? seg.replace(/</g, '&lt;').replace(/>/g, '&gt;') : seg);

export const normalizeWhitespace = (s: string): string => stripTrailingWhitespace(s.replace(/\n{4,}/g, '\n\n\n'));

export const sanitizeMdxContent = (raw: string): string => {
    let s = stripBom(raw);
    s = unwrapSuperFence(s);
    s = s.replace(/^````(?:markdown|md|mdx)?\n([\s\S]*?)\n``?`?\s*$/i, (_m, inner) => looksLikeDoc(inner) ? inner.replace(/^[\r\n]+/, '') : _m);
...
export const sanitizeMdxContentWithReport = (content: string): { content: string; report: SanitizationReport } => {
    const original = content;
    const sanitized = sanitizeMdxContent(content);
    let fencesRepaired = 0; let genericsEscaped = 0; let pseudoJsxEscaped = 0; const htmlTagsEscaped = 0; const spuriousFencesRepaired = 0;
    if (original !== sanitized) {
        const genericMatches = original.match(/\w+<[^>]+>/g) || [];
        const jsxMatches = original.match(/<[^>]*\/?>(?!\))/g) || [];
        const fenceMatches = original.match(/``[^`]/g) || [];
        genericsEscaped = genericMatches.filter((m) => !sanitized.includes(m) && sanitized.includes(m.replace(/</g, '&lt;').replace(/>/g, '&gt;'))).length;
        pseudoJsxEscaped = jsxMatches.filter((m) => !sanitized.includes(m) && sanitized.includes(m.replace(/</g, '&lt;').replace(/>/g, '&gt;'))).length;
        fencesRepaired = fenceMatches.filter((m) => !sanitized.includes(m)).length;
    }
    const totalChanges = fencesRepaired + genericsEscaped + pseudoJsxEscaped + htmlTagsEscaped + spuriousFencesRepaired;
    return { content: sanitized, report: { fencesRepaired, genericsEscaped, pseudoJsxEscaped, htmlTagsEscaped, spuriousFencesRepaired, totalChanges } };
};
