...
import { handleDocEntry, hasSlugCollision, normalizeReferenceLinks, sanitizeMdxContent } from '../sync-docs';
...
describe('handleDocEntry base file deletion logic', () => {
    it('skips and would delete base file when index.md present (dirBase set to package name)', async () => {
        ...
        const exists = await fs.access(join(targetDir, 'pkgA.md')).then(() => true).catch(() => false);
