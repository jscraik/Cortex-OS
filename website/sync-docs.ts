...
const log = (...args: unknown[]) => { if (VERBOSE) console.warn('[docs-sync]', ...args); };
const info = (...args: unknown[]) => { console.warn('[docs-sync]', ...args); }; // use warn (allowed)
...
const normalizeReferenceLinks = (md: string): string => {
  const pattern = /(\]\()(?:\/|\.\/|\.\.\/)*(CONTRIBUTING\.md|CODE_OF_CONDUCT\.md|COMMERCIAL-LICENSE\.md|CHANGELOG\.md|CHANGES\.md|SECURITY\.md|AGENTS\.md|POLICY\.md|POLICY-TERMS\.md)(#[^)]+)?\)/g;
...
const sanitizeMdxContentWithReport = (content: string): { content: string; report: SanitizationReport } => {
    ...
    if (lines.length > 2) {
      ...
      if (openMatch) {
        ...
        for (let i = lines.length - 1; i > 0; i--) {
          ...
          if (/^`{3,5}\s*$/.test(raw)) { endIdx = i; break; }
  ...
  out = out.replace(/\b(?:Record|Array|Promise)<[^\n<>]{1,60}>/g, (m) => {
    ...
    const protectedAngles = unescaped.replace(/</g, GENERIC_SENTINEL_OPEN).replace(/>/g, GENERIC_SENTINEL_CLOSE);
  ...
  //  4. Skip if already backticked or already sentinelized.
  {
    ...
    while ((m = fenceRe.exec(out)) !== null) {
      if (m.index > lastIndex) segments.push({ type: 'text', value: out.slice(lastIndex, m.index) });
      segments.push({ type: 'code', value: m[0] });
      lastIndex = fenceRe.lastIndex;
    }
    if (lastIndex < out.length) segments.push({ type: 'text', value: out.slice(lastIndex) });
    const multilineGenericRe = /(?<!`)(\b(?:Promise|Record|Array|Map|Set|[A-Z][A-Za-z0-9_$]{2,})<([\s\S]{1,400}?)>)(?!`)/g;
      ...
      seg.value = seg.value.replace(multilineGenericRe, (full, whole, inner) => {
        ...
        // Remove space right after '<' and before '>' with linear scan
        {
          ...
          while (i < collapsed.length) {
            ...
            if (ch === '<' && collapsed[i + 1] === ' ') { // skip extra space after '<'
              rebuilt += '<';
              i += 2;
              continue;
            }
            if (ch === ' ' && collapsed[i + 1] === '>') { // skip space before '>'
              i++;
              continue;
            }
            rebuilt += ch;
  ...
  out = out.replace(/`([^`]+?):\s*`(Promise__GENO__[^`]+?__GENC__)`/g, (_m, left, promisePart) => {
  ...
  const FENCE_LANGS = [
    'ts', 'tsx', 'js', 'cjs', 'mjs', 'jsx', 'json', 'jsonc', 'bash', 'sh', 'shell', 'zsh', 'yaml', 'yml', 'toml', 'ini', 'text', 'md', 'markdown',
    'typescript', 'mermaid', 'http', 'sql', 'diff'
  ];
  ...
  out = out.replace(/^(#{2,4}) ([A-Za-z0-9_$][A-Za-z0-9_$.<>-]*?\([^\n)]*\)):\s*(Promise__GENO__[A-Za-z0-9_<>, ]+__GENC__)(\s*)$/gm, (_m, hashes, left, promisePart, trail) => {
  ...
  out = out.replace(/^(?![#>-])([^`\n]{1,80}?)\(([^\n`]{0,80})\):\s*(Promise__GENO__[^`]+?__GENC__)(\s*)$/gm, (_m, fnName, args, promisePart, trail) => {
  ...
  const processLinks = (line: string): string => {
    ...
    while (i < line.length) {
      ...
      if (ch === '[') {
        ...
        if (info) { outLine += info.label; i = info.end; continue; }
      }
      outLine += ch;
      i++;
    }
  ...
  const SAFE_HTML = new Set(['div', 'span', 'a', 'p', 'br', 'img', 'strong', 'em', 'code', 'pre', 'ul', 'ol', 'li', 'table', 'thead', 'tbody', 'tr', 'th', 'td']);
  ...
  const needsEscape = (inside: string): boolean => {
    ...
    return isSimpleIdentifier(inside) || isBracketed(inside) || isBareToken(inside) || isAttrLike(inside);
  };
  const escapeSegment = (inside: string): string => {
    if (isAttrLike(inside)) return '`&lt;' + inside + '&gt;`';
    return '&lt;' + inside + '&gt;';
  };
  out = out.replace(/<([^\s<>]{1,90})>/g, (m, inner) => needsEscape(inner) ? escapeSegment(inner) : m);
  ...
  out = out.split('\n').map(line => {
    if (/^<([A-Z][A-Za-z0-9_$-]{0,60})(\s+[a-zA-Z_:][A-Za-z0-9_:.-]*=)/.test(line) && !line.trimEnd().endsWith('>')) {
    ...
    if (/^<([A-Z][A-Za-z0-9_$-]{0,60})(\s+[a-zA-Z_:][A-Za-z0-9_:.-]*=)[^>]*>\s*$/.test(line)) {
      ...
      return line.replace(/^<([A-Z][A-Za-z0-9_$-]{0,60})([^>]*)>\s*$/, (_m, nm, attrs) => '&lt;' + nm + attrs.replace(/>/g, '') + '&gt;');
    }
    return line;
  }).join('\n');
  // Additional pass: escape any remaining <Identifier> at start of headings that slipped through (rare)
  out = out.replace(/^(#{1,6} .*?)<([A-Z][\w.$-]{0,60})>(.*)$/gm, (_m, pre, id, post) => `${pre}&lt;${id}&gt;${post}`); // keep simple
  // Headings with raw generic tokens like ## Foo<Bar> -> ## Foo `&lt;Bar&gt;`
  out = out.replace(/^(#{1,6} [^`\n]{0,120}?)([A-Z][A-Za-z0-9_$-]*<[^<>\n]{1,60}>)(.*)$/gm, (_m, pre, genericToken, post) => {
  ...
  out = out.replace(new RegExp(GENERIC_SENTINEL_OPEN, 'g'), '<')
  ...
  out = out.replace(/&lt;\/(div|span|a|p|strong|em|code|pre|ul|ol|li|table|thead|tbody|tr|th|td)&gt;/g, (_m, tag) => `</${tag}>`);
  ...
  // Defensive line-based pass: upgrade any remaining orphan double-fence blocks (including indented) missed by regex due to interference
  {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (let j = i + 1; j < Math.min(lines.length, i + 800); j++) {
        ...
        if (new RegExp('^' + indent.replace(/ /g, ' ') + '``$').test(lines[j])) { closedAt = j; break; }
      }
      if (closedAt !== null) {
        lines[i] = `${indent}\`\`\`${lang}`;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      for (; j < Math.min(lines.length, i + 400); j++) {
        const l = lines[j];
        if (new RegExp('^' + indent.replace(/ /g, ' ') + '``$').test(l)) { // found closing `` -> handled in pass1
          break; // already closed; nothing to do
        }
        if (/^( {0,3})```/.test(l)) { // encountered another fenced block start; auto close before this
          needAutoClose = true; break;
        }
        if (l.startsWith('---') || l.startsWith('# ') || /^\s*$/.test(l)) { // structural boundary
          needAutoClose = true; break;
        }
      }
      lines[i] = `${indent}\`\`\`${lang}`;
  ...
  // --- Enhancement C: Multi-line pseudo-JSX attribute continuations ---
  {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^<([A-Z][A-Za-z0-9_$-]{0,60})(\s|$)/.test(line) && !line.includes('&lt;')) {
        let closed = false; let end = i;
        for (let j = i + 1; j < Math.min(lines.length, i + 8); j++) {
          const lj = lines[j];
          if (/^>$/.test(lj.trim()) || />\s*$/.test(lj)) { closed = true; end = j; break; }
  ...
  const codeLikeRe = /(?:^(?:\s{0,4}(?:import |const |let |await |async |type |interface |class |new |participant |[A-Za-z0-9_$]+\s*=|[A-Z]{1,5}-&gt;&gt;|[A-Z]{1,5}--&gt;&gt;|[A-Z]{1,5}->>|[A-Z]{1,5}-->>)))|[;{}()=]|--&gt;&gt;|-&gt;&gt;|-->>|->>|=>/;
    interface OpenFence { index: number; lang: string; }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (m) {
        ...
        let codeLike = false;
        for (; j < Math.min(lines.length, i + 120); j++) { // limit lookahead to 120 lines for safety
          const mj = fenceLineRe.exec(lines[j]);
            if (mj) {
              if (mj[1]) { // opening fence encountered
                foundInterveningOpen = true;
                break;
              } else { // another closing fence
                foundNextClose = j;
                break;
              }
            }
  ...
  // effectively extending the original mermaid block so arrow lines are not parsed as MDX/JSX.
  {
    ...
    const mermaidContentRe = /^(\s*(participant\s+|note\s+|alt\b|loop\b|end\b|rect\b|classDef\b|subgraph\b|[A-Za-z0-9_$]{1,20}(?:-{1,2}|={1,2})>>[A-Za-z0-9_$]{1,20}:|[A-Za-z0-9_$]{1,20}-->>[A-Za-z0-9_$]{1,20}:))/i;
      ...
      if (m) {
        ...
        if (openMermaid != null) {
          // Look ahead a few lines (until blank or heading) to see if mermaid-like
            let mermaidLikeCount = 0; let total = 0; let j = i + 1; let hardStop = false;
            for (; j < Math.min(lines.length, i + 40); j++) {
              const ln = lines[j];
              if (/^```/.test(ln)) break; // another fence; stop
              if (/^\s*$/.test(ln)) { total++; continue; } // allow blank
              if (/^\s*#/.test(ln)) { hardStop = true; break; }
              if (/^---$/.test(ln)) { hardStop = true; break; }
              total++;
  ...
  if (originalContent !== sanitizedContent) {
    ...
    report.genericsEscaped = genericMatches.filter(m => 
      !sanitizedContent.includes(m) && sanitizedContent.includes(m.replace(/</g, '&lt;').replace(/>/g, '&gt;'))
    ).length;
    
    report.pseudoJsxEscaped = jsxMatches.filter(m => 
      !sanitizedContent.includes(m) && sanitizedContent.includes(m.replace(/</g, '&lt;').replace(/>/g, '&gt;'))
    ).length;
    
    report.fencesRepaired = fenceMatches.filter(m => !sanitizedContent.includes(m)).length;
    
    report.totalChanges = report.fencesRepaired + report.genericsEscaped + report.pseudoJsxEscaped + report.htmlTagsEscaped + report.spuriousFencesRepaired;
  }
...
const handleDocEntry = async (params: {
  ...
  const { entryName, sourceDocsDir, targetDir, displayName, collision, dirBase, fileNames, existingTargetFiles } = params;
  if (!isMarkdownFile(entryName)) return false;
  if (shouldSkipFile(entryName)) { log(`Skipping ${entryName} (pattern match)`); return false; }
  if (collision) {
    const decision = decideSlugOverride({ fileName: entryName, dirBase, fileNames });
    if (decision === '__SKIP__') { log(`Skipping ${entryName} (collision with README.md – rename recommended).`); return false; }
  }
  if (shouldSkipBaseFile({ entryName, dirBase, existingTargetFiles, fileNames, collision })) {
...
const syncPackageDocs = async (category: string, packageName: string, displayName: string, options: SyncOptions = { dryRun: false }): Promise<SyncResult> => {
  ...
  try {
    ...
    const prep = await (async () => {
      ...
      if (debugSlugs && collision) log('Slug collision detected', { packageName, dirBase, files: fileNames });
...
const generateSidebarConfig = async (): Promise<object> => {
  ...
  try {
    ...
    if (mdFiles.length > 0) {
      const cortexCategory = sidebar.tutorialSidebar.find(item =>
        typeof item === 'object' && item !== null && 'label' in item && item.label === 'Cortex Platform'
      ) as SidebarCategory;
...
const syncCortexDocs = async (displayName: string, options: SyncOptions = { dryRun: false }): Promise<SyncResult> => {
  const cortexSourcePath = join(ROOT_DIR, '.cortex', 'docs');
  const cortexTargetPath = join(DOCS_DIR, 'cortex');
  try {
    if (!(await fs.access(cortexSourcePath).then(() => true).catch(() => false))) {
      return { success: false, packageName: 'cortex-platform', fileCount: 0, error: 'No cortex docs' };
    }
    ...
    return { success: false, packageName: 'cortex-platform', fileCount: 0, error: `Failed cortex sync: ${error}` };
  }
};
...
const summarizeResults = async (results: SyncResult[], options: SyncOptions = { dryRun: false }): Promise<void> => {
  ...
  const totalSanitizationReport = successful.reduce((acc, r) => {
    ...
    return {
      ...
      spuriousFencesRepaired: acc.spuriousFencesRepaired + r.sanitizationReport.spuriousFencesRepaired,
      totalChanges: acc.totalChanges + r.sanitizationReport.totalChanges
    };
  }, { fencesRepaired: 0, genericsEscaped: 0, pseudoJsxEscaped: 0, htmlTagsEscaped: 0, spuriousFencesRepaired: 0, totalChanges: 0 });
  ...
  if (options.dryRun) {
    ...
    if (totalSanitizationReport.totalChanges > 0) {
      ...
      if (totalSanitizationReport.fencesRepaired > 0) info(`  • ${totalSanitizationReport.fencesRepaired} fence repairs`);
      if (totalSanitizationReport.spuriousFencesRepaired > 0) info(`  • ${totalSanitizationReport.spuriousFencesRepaired} spurious fence repairs`);
      if (totalSanitizationReport.genericsEscaped > 0) info(`  • ${totalSanitizationReport.genericsEscaped} generic escapes`);
      if (totalSanitizationReport.pseudoJsxEscaped > 0) info(`  • ${totalSanitizationReport.pseudoJsxEscaped} pseudo-JSX escapes`);
      if (totalSanitizationReport.htmlTagsEscaped > 0) info(`  • ${totalSanitizationReport.htmlTagsEscaped} HTML tag escapes`);
    }
...
const syncCategory = async (category: string, packages: Record<string, string>, results: SyncResult[], options: SyncOptions = { dryRun: false }): Promise<void> => {
...
export { DOCS_STRUCTURE, ensureReferenceStubs, handleDocEntry, hasSlugCollision, normalizeReferenceLinks, sanitizeMdxContent, shouldSkipBaseFile, syncAllDocs };
