...
describe('HealthMonitor', () => {
		...
		const dependencies = [
			{
				...
				check: vi
					.fn()
					.mockResolvedValue({ id: 'database', status: 'ok', detail: 'connected', latencyMs: 0 }),
			},
			{
				id: 'queue',
	...
	it('fails fast when dependency list is empty', () => {
		expect(() => new HealthMonitor([])).toThrowError(
			/brAInwav health monitor requires at least one dependency/,
		);
	});
});
