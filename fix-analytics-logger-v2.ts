...
async function fixFile(filename: string) {
  ...
  content = content.replace(
    /this\.logger\.(info|warn|error|debug)\s*\(\s*['"`]([^'"`]+)['"`]\s*,\s*({[\s\S]*?})\s*\)/g,
    'this.logger.$1({ msg: $2, ...$3 })'
  );

  // Fix logger patterns - replace logger.method('message') with logger.method({ msg: 'message' })
  content = content.replace(
    /this\.logger\.(info|warn|error|debug)\s*\(\s*['"`]([^'"`]+)['"`]\s*\)/g,
    'this.logger.$1({ msg: $2 })'
  );

  // Fix error.message references - replace error.message with proper error handling (avoiding double replacements)
  content = content.replace(
    /(?<!error instanceof Error \? )error\.message(?!\s*: String\(\error\))/g,
