...
class MLXMemoryManager:
    ...
    def __init__(self, total_ram_gb: int = 36, mlx_reserved_gb: int = 28):
        ...
        logger.info(f"MLX Memory Manager initialized: {mlx_reserved_gb}GB reserved for MLX inference")
    ...
    def can_load_model_size(self, required_mb: int) -> bool:
        ...
        if not can_load:
            logger.warning(f"Insufficient MLX memory: need {required_mb}MB, have {available_mb}MB")
    ...
    def get_optimal_model_combination(self) -> List[str]:
        ...
        for model_name, memory_mb, priority in models:
    ...
    def get_memory_status_report(self) -> Dict:
        ...
        for model_name, memory_mb in self.model_memory.items():
            model_breakdown.append({
                ...
                "percent_of_mlx": round((memory_mb / self.mlx_reserved_mb) * 100, 1)
            })
    ...
    def _get_memory_warnings(self) -> List[str]:
        ...
        if usage["mlx_available_gb"] < 2:
            warnings.append(f"LOW MEMORY: Only {usage['mlx_available_gb']:.1f}GB available")
    ...
    def _get_memory_recommendations(self) -> List[str]:
        ...
        if "glm-4.5" in self.model_memory and "qwen3-instruct" in self.model_memory:
            recommendations.append("Both GLM-4.5 and Qwen3-Instruct loaded - consider unloading one")
        
        if usage["mlx_available_gb"] > 15:
            recommendations.append("Plenty of memory available - consider loading additional models")
