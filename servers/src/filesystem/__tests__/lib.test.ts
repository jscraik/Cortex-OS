...
// Mock fs/promises for both default and named exports so app and tests share the same spies
vi.mock("fs/promises", () => {
...
// Handy strongly-typed alias to the mocked implementation
	...
	describe("File Editing Functions", () => {
		...
		describe("tailFile", () => {
			...
			it("handles files with content and returns last lines", async () => {
				...
				const _result = await tailFile("/test/file.txt", 2);
		...
		describe("headFile", () => {
			...
			it("handles files with content and returns first lines", async () => {
				...
				const _result = await headFile("/test/file.txt", 2);
			...
			it("handles files with leftover content", async () => {
				...
				const _result = await headFile("/test/file.txt", 5);
