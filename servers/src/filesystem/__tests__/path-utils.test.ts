...
describe('Path Utilities', () => {
	describe('convertToWindowsPath', () => {
		it('leaves Unix paths unchanged', () => {
			...
			expect(convertToWindowsPath('/home/user/some path')).toBe(
				'/home/user/some path',
			);
		});

		it('converts WSL paths to Windows format', () => {
			expect(convertToWindowsPath('/mnt/c/NS/MyKindleContent')).toBe(
				'C:\\NS\\MyKindleContent',
			);
		});

		it('converts Unix-style Windows paths to Windows format', () => {
			expect(convertToWindowsPath('/c/NS/MyKindleContent')).toBe(
				'C:\\NS\\MyKindleContent',
			);
		});
	...
	describe('normalizePath', () => {
		it('preserves Unix paths', () => {
			...
			expect(normalizePath('/home/user/some path')).toBe(
				'/home/user/some path',
			);
			expect(normalizePath('"/usr/local/some app/"')).toBe(
				'/usr/local/some app',
			);
		});

		it('removes surrounding quotes', () => {
			expect(normalizePath('"C:\\NS\\My Kindle Content"')).toBe(
				'C:\\NS\\My Kindle Content',
			);
		});

		it('normalizes backslashes', () => {
			expect(normalizePath('C:\\\\NS\\\\MyKindleContent')).toBe(
				'C:\\NS\\MyKindleContent',
			);
		});
		...
		it('handles WSL paths', () => {
			expect(normalizePath('/mnt/c/NS/MyKindleContent')).toBe(
				'C:\\NS\\MyKindleContent',
			);
		});
		...
		it('handles paths with spaces and mixed slashes', () => {
			...
			expect(normalizePath('/mnt/c/NS/My Kindle Content')).toBe(
				'C:\\NS\\My Kindle Content',
			);
			expect(normalizePath('C:\\Program Files (x86)\\App Name')).toBe(
				'C:\\Program Files (x86)\\App Name',
			);
			expect(normalizePath('"C:\\Program Files\\App Name"')).toBe(
		...
		it('preserves spaces in all path formats', () => {
			expect(normalizePath('/mnt/c/Program Files/App Name')).toBe(
				'C:\\Program Files\\App Name',
			);
			expect(normalizePath('/c/Program Files/App Name')).toBe(
				'C:\\Program Files\\App Name',
		...
		it('handles special characters in paths', () => {
			...
			expect(
				normalizePath('/Users/NEMANS~1/FOLDER~2/SUBFO~1/Public/P12PST~1'),
			).toBe('/Users/NEMANS~1/FOLDER~2/SUBFO~1/Public/P12PST~1');

			// Test other special characters
			expect(normalizePath('C:\\Path with #hash')).toBe('C:\\Path with #hash');
			expect(normalizePath('C:\\Path with (parentheses)')).toBe(
				'C:\\Path with (parentheses)',
			);
			expect(normalizePath('C:\\Path with [brackets]')).toBe(
