...
const cases = [
  ...
  { path: '/rag', body: { config: { seed: 1, maxTokens: 1, timeoutMs: 1, memory: { maxItems: 1, maxBytes: 1 } }, query: { query: '' }, json: true } }, // invalid query
];
...
async function main() {
  ...
  for (const c of cases) {
    ...
    try {
      const res = await fetch(url, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(c.body) });
