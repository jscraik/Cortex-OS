...
const METRICS_URL =
	process.env.METRICS_URL || "http://localhost:3333/metrics/eval";
...
function parseResult(file) {
	try {
		...
		const total = Array.isArray(j.results)
			? j.results.length
			: Number(j.total ?? 0) || 0;
...
async function main() {
	...
	for (const f of iterJsonFiles(root)) {
		...
		await post(rec);
		console.log(
			`posted suite=${rec.suite} failures=${rec.failures} total=${rec.total}`,
		);
		count++;
	}
	if (count === 0) {
