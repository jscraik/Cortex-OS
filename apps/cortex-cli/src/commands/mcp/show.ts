...
export const mcpShow = new Command('show')
  ...
  .option('--client <type>', 'Show install command for specific client (claude, cline, devin, cursor, continue, windsurf)')
  .action(async (serverId: string, options: any) => {
    try {
      const client = createMarketplaceClient(options.registry ? { registryUrl: options.registry } : {});
      
      const server = await client.getServer(serverId);
      
      if (!server) {
        if (options.json) {
          process.stderr.write(JSON.stringify({ error: `Server "${serverId}" not found` }, null, 2) + '\n');
        } else {
...
function displayServerInfo(server: ServerManifest, showClient?: string) {
  ...
  process.stdout.write(`  Verified Publisher: ${server.security?.verifiedPublisher ? 'Yes' : 'No'}\n`);
  ...
  if (showClient && isValidClient(showClient)) {
    ...
    if (installCmd) {
      if (showClient === 'json') {
        ...
        process.stdout.write(JSON.stringify(installCmd, null, 4).split('\n').map(line => `    ${line}`).join('\n') + '\n');
      } else {
  ...
  if (clients.length > 0) {
    process.stdout.write(`  cortex mcp show ${server.id} --client ${clients[0]}    Show install command for ${clients[0]}\n`);
  }
}
