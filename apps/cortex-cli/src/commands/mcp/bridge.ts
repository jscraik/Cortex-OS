  ...
  .action(async (url?: string, options: any = {}) => {
    try {
      if (options.list) {
        ...
        await addRegistry(url, options.name, options.description, options.trusted || false, options.json);
        ...
        process.stdout.write('  cortex mcp bridge <url>                     Test registry connectivity\n');
        process.stdout.write('  cortex mcp bridge <url> --add --name <name> Add registry to bridge list\n');
        process.stdout.write('  cortex mcp bridge --list                    List configured registries\n');
...
async function addRegistry(url: string, name: string, description: string | undefined, trusted: boolean, json: boolean): Promise<void> {
  // Validate URL format
  try {
    new URL(url);
  } catch {
  ...
  try {
    ...
    throw new Error(`Registry test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
...
async function testRegistry(url: string, json: boolean): Promise<void> {
  ...
  try {
    ...
    if (json) {
      ...
      process.stdout.write(`  Updated: ${new Date(registry.metadata.updatedAt).toLocaleDateString()}\n`);
