...
describe('McpMarketplaceCommand (deprecated)', () => {
  ...
  it('show throws deprecation error', async () => {
    await expect(command.show('id')).rejects.toThrow('Deprecated command: use "cortex mcp show"');
  });

  it('add throws deprecation error', async () => {
    await expect(command.add('id')).rejects.toThrow('Deprecated command: use "cortex mcp add"');
  });
  ...
  it('list throws deprecation error', async () => {
    await expect(command.list()).rejects.toThrow('Deprecated command: use "cortex mcp list"');
  });
