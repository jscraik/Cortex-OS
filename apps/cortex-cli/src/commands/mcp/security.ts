...
export class SecurityValidator {
  ...
  private async validateSignature(server: ServerManifest): Promise<boolean> {
    ...
    if (!server.security?.sigstoreBundle) {
      console.warn(`Server ${server.name} has no Sigstore bundle - signature validation failed`);
      return false;
    }
    ...
    try {
      ...
      if (!bundle.mediaType || !bundle.verificationMaterial || !bundle.dsseEnvelope) {
        console.error('Invalid Sigstore bundle structure');
        return false;
      }

      // Step 3: Verify bundle format
      const expectedMediaType = 'application/vnd.dev.sigstore.bundle+json;version=0.2';
      ...
      if (!verificationMaterial.x509CertificateChain && !verificationMaterial.publicKey) {
  ...
  private async validateRegistrySignature(registry: RegistryIndex): Promise<boolean> {
    ...
    if (!registry.signing?.publicKey) {
      console.warn('Registry has no signing information - signature validation skipped');
      return false;
    }
    ...
    try {
      ...
      if (!supportedAlgorithms.includes(signingInfo.algorithm)) {
        console.error(`Unsupported signing algorithm: ${signingInfo.algorithm}`);
        return false;
      }
      ...
      console.log('Registry signing information validated (no signature to verify)');
      return true;
    } catch (error) {
