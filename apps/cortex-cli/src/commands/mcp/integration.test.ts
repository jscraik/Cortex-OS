...
describe("MCP Marketplace Integration", () => {
	...
	describe("Configuration Validation", () => {
		...
		it("should reject invalid config", () => {
			...
			expect(
				() =>
					new MarketplaceClient(invalidConfig as unknown as MarketplaceConfig),
			).toThrow();
		});
	});
	...
	describe("Error Handling", () => {
		it("should handle network errors gracefully", async () => {
			const offlineConfig = {
				...
				registries: {
					offline:
						"https://nonexistent-registry-12345.invalid/v1/registry.json",
				},
			};
	...
	describe("Caching Behavior", () => {
		it("should implement proper cache invalidation", () => {
			...
			expect(
				path.isAbsolute(config.cacheDir) || config.cacheDir.includes("tmp"),
			).toBe(true);
		});
	});
});
