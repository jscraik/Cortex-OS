...
describe("MarketplaceClient", () => {
		...
		servers: [
			...
			},
			{
				...
				install: {
					...
					json: {
						mcpServers: {
							"utility-server": { serverUrl: "https://api.utility.com/mcp" },
						},
					},
				},
	...
	describe("search", () => {
		...
		it("should return error when registry not loaded", async () => {
			...
			// Act
			const result = await uninitializedClient.search({
				q: "test",
				limit: 20,
				offset: 0,
			});
	...
	describe("addServer", () => {
		...
		it("should add server with specific transport", async () => {
			...
			// Act
			const result = await client.addServer("utility-server", {
	...
	describe("registry management", () => {
		it("should add custom registry", async () => {
			...
			const customRegistryUrl =
				"https://registry.cortex-os.dev/custom/registry.json";
			vi.mocked(fetch).mockResolvedValue({
				ok: true,
				json: () => Promise.resolve(mockRegistryIndex),
			} as Response);
			...
			// Act
			const result = await client.addRegistry(customRegistryUrl, {
				name: "custom",
			});
		...
		it("should reject invalid registry URL", async () => {
			// Act
			const result = await client.addRegistry("invalid-url", {
				name: "invalid",
			});
		...
		it("should validate registry connectivity", async () => {
			...
			// Act
			const result = await client.addRegistry(
				"https://unreachable.com/registry.json",
				{
					name: "unreachable",
				},
			);
