...
export const mcpSearch = new Command("search")
	...
	.option(
		"--category <category>",
		"Filter by category (development, productivity, data, etc.)",
	)
	...
	.action(
		async (query: string | undefined, options: Record<string, unknown>) => {
...
async function runSearchAction(
	...
	try {
		...
		const limitedServers =
			typeof limit === "number" ? servers.slice(0, limit) : servers;
...
function printPrettyResults(
	...
	process.stdout.write(
		"  cortex mcp show <server-id>     Show detailed info\n",
	);
	process.stdout.write(
