...
export const mcpSearch = new Command('search')
  ...
  .option('--tags <tags>', 'Filter by tags (comma-separated)', (val) => val.split(',').map(t => t.trim()))
  ...
  .action(async (query = '', options: any) => {
    try {
      ...
      if (options.json) {
        ...
        process.stdout.write(`Found ${limitedServers.length} MCP server${limitedServers.length === 1 ? '' : 's'}:\n\n`);
