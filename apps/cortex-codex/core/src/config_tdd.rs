...
impl SimpleConfig {
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self, ConfigError> {
        match fs::read_to_string(path) {
            Ok(content) => {
                let config: SimpleConfig = toml::from_str(&content).map_err(ConfigError::InvalidToml)?;
    ...
    pub fn load_with_profile<P: AsRef<Path>>(path: P, profile: &str) -> Result<Self, ConfigError> {
        ...
        let parsed: toml::Value = toml::from_str(&content).map_err(ConfigError::InvalidToml)?;

        // Look for the profiles section
        if let Some(profiles) = parsed.get("profiles")
            && let Some(profile_config) = profiles.get(profile)
        {
            // Convert the profile TOML value to our config struct
            let config = toml::Value::Table(
                profile_config
                    .as_table()
                    .ok_or_else(|| ConfigError::Validation(format!("Profile '{profile}' is not a table")))?
                    .clone(),
            )
        ...
        Err(ConfigError::Validation(format!("Profile '{profile}' not found")))
    }
    ...
    fn apply_override(config: &mut SimpleConfig, override_str: &str) -> Result<(), ConfigError> {
        ...
        if parts.len() != 2 {
            return Err(ConfigError::MalformedOverride(format!(
                "Invalid override format: {override_str}"
            )));
        }
        ...
        match key {
            ...
            "api.rate_limit.requests_per_minute" => {
                config.api.rate_limit.requests_per_minute = value
                    .parse()
    ...
    toml::to_string(self).map_err(ConfigError::Serialization)
    }
