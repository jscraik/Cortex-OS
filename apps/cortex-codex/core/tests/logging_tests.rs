...
fn test_logging_levels() {
    // Create a test subscriber with INFO level
...
fn test_concurrent_logging() {
    ...
    // Verify at least some concurrent logs were captured
    assert!(logs_contain("Concurrent logging test"));
}
...
fn test_component_filtering() {
    // Create subscriber that filters to only "core" component
    let _guard = tracing_subscriber::fmt()
        .with_env_filter(EnvFilter::new("core=debug"))
        .with_test_writer()
        .try_init();

    // This should be captured if we're in the core crate
