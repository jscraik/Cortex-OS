...
/// Tests the `codex chat` one-off path using a mock server (mirrors TUI coverage).
/// Ensures streaming works and the output arrives exactly once.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn chat_subcommand_streams_once() {
    if std::env::var(CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Codex sandbox."
        );
...
/// Same as `chat_subcommand_streams_once` but forces raw token streaming via
/// the `--no-aggregate` flag to ensure per‑token mode also yields exactly one
/// logical line of output for this simple single-token response.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn chat_subcommand_streams_no_aggregate() {
    if std::env::var(CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Codex sandbox."
        );
