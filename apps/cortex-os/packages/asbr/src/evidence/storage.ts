...
export class EvidenceStorage {
  ...
  async storeEvidence(evidence: Evidence): Promise<void> {
    ...
    try {
      ...
      throw new ValidationError(`Failed to store evidence ${evidence.id}: ${error.message}`);
    }
  }
  ...
  async getEvidence(evidenceId: string): Promise<Evidence | null> {
    ...
    try {
      ...
      throw new ValidationError(`Failed to retrieve evidence ${evidenceId}: ${error.message}`);
    }
  }
  ...
  async queryEvidence(query: EvidenceQuery): Promise<{
    ...
    try {
      ...
      throw new ValidationError(`Failed to query evidence: ${error.message}`);
    }
  }
  ...
  async deleteEvidence(evidenceId: string): Promise<void> {
    ...
    const receiptPath = getDataPath('receipts', `delete_${evidenceId}_${Date.now()}.json`);
  ...
  async cleanupOldEvidence(): Promise<{
    ...
    try {
      ...
      throw new ValidationError(`Failed to cleanup old evidence: ${error.message}`);
    }
  }
  ...
  async getStorageStats(): Promise<StorageStats> {
    ...
    try {
      ...
      throw new ValidationError(`Failed to get storage stats: ${error.message}`);
    }
  }
  ...
  private async createDeletionReceipt(
    ...
    const receiptDir = getDataPath('receipts');
