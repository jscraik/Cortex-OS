...
export class ASBRServer {
        ...
        const legacyInputs = Array.isArray(input?.inputs)
          ? input.inputs.map((item: unknown) => {
              ...
              if (item.type === 'text' && typeof item.content === 'string') {
                return { kind: 'text', value: item.content };
              }
              if (item.type === 'repo' && typeof item.path === 'string') {
                return { kind: 'repo', path: item.path };
              }
              if (item.type === 'doc' && typeof item.path === 'string') {
                return { kind: 'doc', path: item.path };
              }
              ...
              if (item.kind === 'file') {
                const path = typeof item.path === 'string' ? item.path : item.value;
  ...
  private async getServiceMap(req: Request, res: Response): Promise<void> {
