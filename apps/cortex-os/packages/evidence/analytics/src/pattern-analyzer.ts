      ...
      for (const [agentA, targetsA] of activationMap) {
        for (const agentB of targetsA) {
          if (activationMap.has(agentB)) {
            for (const agentC of activationMap.get(agentB)!) {
  ...
  private async analyzeDependency(
    ...
    try {
      ...
      return {
        ...
        strength,
        frequency,
        criticality: strength > 0.7 ? 'high' : strength > 0.4 ? 'medium' : 'low',
        latency: avgLatency,
        reliability: this.calculateReliability(targetInteractions),
        impact: {
          onFailure: strength > 0.7 ? 'cascade' : strength > 0.4 ? 'degraded' : 'isolated',
