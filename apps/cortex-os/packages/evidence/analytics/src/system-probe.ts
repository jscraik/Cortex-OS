...
export class NodeSystemProbe implements SystemProbe {
        ...
        async sample(): Promise<SystemProbeSnapshot> {
                ...
                const networkInbound = this.computeRate(
                        start.networkBytes.rx,
                        end.networkBytes.rx,
                        elapsedMs,
                );
                const networkOutbound = this.computeRate(
                        start.networkBytes.tx,
                        end.networkBytes.tx,
                        elapsedMs,
                );
        ...
        async getAgentUsage(_agentId: string): Promise<AgentResourceUsage> {
                ...
                const cpuPercent = uptimeSeconds <= 0
                        ? 0
                        : clamp((totalMicros / (uptimeSeconds * 1_000_000)) * 100);
