...
describe('Retrieval Policy Loader', () => {
  it('loads and validates default policy', async () => {
    ...
    const schemaPath = path.join(process.cwd(), 'schemas/retrieval.policy.schema.json');
  ...
  it('creates dispatcher from policy config', async () => {
    ...
    expect(dispatcher.getConfig().maxChunkSize).toBe(policy.dispatcher?.maxChunkSize);
    expect(dispatcher.getConfig().enableParallel).toBe(policy.dispatcher?.enableParallel);
  });
});
