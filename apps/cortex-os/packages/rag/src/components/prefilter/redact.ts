...
export class SecretsRedactor {
  ...
  async scanContent(content: string, filePath: string): Promise<RedactionResult> {
    ...
    const secretsResults = await this.detectSecrets(content, filePath);

    // Apply redaction to content
    let redactedContent = content;

    // Redact secrets first
    for (const secret of secretsResults.secrets) {
      redactedContent = this.redactSecret(redactedContent, secret);
    }

    // Security: Detect and sanitize prompt injections
    ...
    const riskScore = this.calculateRiskScore(secretsResults, securityAnalysis);

    const result: RedactionResult = {
      scanId,
      timestamp,
      redactedContent,
      secretsFound: secretsResults.secrets,
      entitiesFound: [], // Will be populated by PII redactor
      summary: {
        totalSecrets: secretsResults.secrets.length,
        highRiskSecrets: secretsResults.secrets.filter((s) => s.confidence > 0.8).length,
        totalPII: 0,
        highSensitivityPII: 0,
        secretTypes: [...new Set(secretsResults.secrets.map((s) => s.type))],
        entityTypes: [],
        customPatternMatches: secretsResults.customMatches,
        encodedSecrets: secretsResults.encodedSecrets,
        obfuscatedSecrets: secretsResults.obfuscatedSecrets,
        obfuscatedPatterns: 0,
        maskedPatterns: 0,
        environmentSecrets: secretsResults.environmentSecrets,
        indirectPII: 0,
