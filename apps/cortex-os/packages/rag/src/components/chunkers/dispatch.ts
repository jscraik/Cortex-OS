...
class TextChunker implements Chunker {
  async chunk(file: ProcessingFile, config: ProcessingConfig): Promise<DocumentChunk[]> {
    ...
    switch (config.chunker) {
      case "markdown":
        return this.chunkMarkdown(content, file);
      case "code":
        return this.chunkCode(content, file);
      case "structured":
        return this.chunkStructured(content, file);
      default:
  ...
  private chunkCode(content: string, file: ProcessingFile): DocumentChunk[] {
    ...
    lines.forEach((line, index) => {
      ...
      if (
        line.match(/^(function|class|def|public|private)\s/) &&
...
export class ProcessingDispatcher {
  ...
  private pdfChunker = new PdfChunker();
  private ocrChunker = new OcrChunker();
  private unstructuredChunker = new UnstructuredChunker();
