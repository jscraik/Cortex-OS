...
export class Circuit {
  ...
  async exec<T>(op: () => Promise<T>): Promise<T> {
    const now = Date.now();
    if (this.state === "open" && now - this.openedAt < this.resetMs) throw new Error("CIRCUIT_OPEN");
  ...
  private fail() { if (++this.failures >= this.threshold) { this.state = "open"; this.openedAt = Date.now(); } }
}
