    ...
    try {
      if (options.session) {
        ...
        if ((session as any).status !== "active") {
          console.log(
            chalk.yellow(
              `Session ${sessionId} is not active (status: ${(session as any).status})`,
            ),
          );
          return;
        }
        ...
        if (result) {
          console.log(
            chalk.green(`✓ Session ${sessionId} paused successfully`),
          );
          console.log(
            chalk.gray(
              `Use 'claude-flow hive-mind resume -s ${sessionId}' to resume`,
            ),
          );
        } else {
      ...
      console.error(
        chalk.red("Error pausing session:"),
