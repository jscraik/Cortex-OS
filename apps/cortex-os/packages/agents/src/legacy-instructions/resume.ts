  ...
  .action(async (options) => {
    ...
    try {
      if (options.session) {
        ...
        console.log(
          chalk.gray(`Swarm: ${(session as any).swarm_name || "N/A"}`),
        );
        console.log(
          chalk.gray(`Objective: ${(session as any).objective || "N/A"}`),
        );
        console.log(
          chalk.gray(`Progress: ${session.statistics.completionPercentage}%`),
        );
      } else {
      ...
      console.error(
        chalk.red("Error resuming session:"),
