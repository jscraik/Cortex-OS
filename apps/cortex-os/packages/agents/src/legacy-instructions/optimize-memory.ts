...
import {
  CollectiveMemory,
  MemoryOptimizer,
} from "../../simple-commands/hive-mind/memory.js";
    ...
    if (analytics.pools) {
      ...
      for (const [name, stats] of Object.entries(analytics.pools)) {
        if (typeof stats === "object" && stats.reuseRate !== undefined) {
          console.log(
            `   ${name}: ${chalk.bold(stats.reuseRate.toFixed(1))}% reuse rate`,
          );
        }
      }
      console.log("");
    ...
    const statusColor =
      healthCheck.status === "healthy"
        ...
        : healthCheck.status === "warning"
          ...
          : "red";
    console.log(
      `   Overall: ${chalk[statusColor].bold(healthCheck.status.toUpperCase())}`,
    );
      ...
      const color =
        alert.level === "critical"
          ...
          : alert.level === "warning"
            ...
            : "blue";
      console.log(
        chalk[color](`🚨 ${alert.level.toUpperCase()}: ${alert.message}`),
      );
    });
...
async function generateMemoryReport(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `   Cache Hit Rate: ${formatMetric(report.metrics.cacheHitRate, "%", 70)}`,
    );
    console.log(
    ...
    console.log(
      `   Pool Efficiency: ${formatMetric(report.metrics.poolEfficiency, "%", 50)}`,
    );
    console.log(
    ...
    console.log(
      `   Performance: ${getTrendIndicator(report.trends.performance)}`,
    );
    console.log(
      `   Memory Usage: ${getTrendIndicator(report.trends.memoryUsage)}`,
    );
    console.log(
      `   Cache Efficiency: ${getTrendIndicator(report.trends.cacheEfficiency)}\n`,
    );
          ...
          suggestion.priority === "critical"
            ...
            : suggestion.priority === "high"
              ...
              : suggestion.priority === "medium"
                ...
                : "gray";
        console.log(
          `   ${index + 1}. ${chalk[priorityColor].bold(suggestion.title)}`,
        );
    ...
    console.log(
      `   Cache Memory: ${(analytics.cache.memoryUsage / 1024 / 1024).toFixed(1)} MB`,
    );
