...
async function saveSession(
  ...
  try {
    ...
    const filePath = `${SESSION_DIR}/${session.id}.json`;
    {
      ...
      await PermissionEngine.guardWrite(
        `Write session file ${filePath} (${content.length} bytes)`,
        async () => {
...
async function restoreSession(sessionId: string, options: any): Promise<void> {
  try {
    ...
    const filePath = `${SESSION_DIR}/${session.id}.json`;
    {
      const content = JSON.stringify(session, null, 2);
      await PermissionEngine.guardWrite(
        `Update session file ${filePath} (${content.length} bytes)`,
        async () => {
          await fs.writeFile(filePath, content);
          return true;
        },
...
async function exportSession(
  ...
  try {
    ...
    await PermissionEngine.guardWrite(
      `Export session to ${outputFile} (${content.length} bytes)`,
      async () => {
...
async function importSession(inputFile: string, options: any): Promise<void> {
  try {
    ...
    const filePath = `${SESSION_DIR}/${sessionData.id}.json`;
    {
      ...
      await PermissionEngine.guardWrite(
        `Import session to ${filePath} (${content.length} bytes)`,
        async () => {
          await fs.writeFile(filePath, content);
          return true;
        },
