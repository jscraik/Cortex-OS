...
`);
  ...
  async submitConsensusVote(
    ...
    const status =
      currentRatio >= proposal.required_threshold ? "achieved" : "pending";
  ...
  getDatabaseAnalytics(): any {
    try {
      ...
      return {
        ...
        fragmentationDetails: fragmentation.tables,
        tableCount: tableStats.count,
        totalSize: tableStats.totalSize,
        indexCount: tableStats.indexCount,
        schemaVersion: "1.0.0",
        lastVacuum: this.getLastVacuumTime(),
        recommendations: this.getOptimizationRecommendations(fragmentation),
      };
    } catch (error) {
      const errorMessage =
  ...
  private calculateFragmentation(): {
    ...
    try {
      ...
      const tableFragmentation: Array<{
        name: string;
        fragmentation: number;
        pages: number;
        unusedPages: number;
      ...
      let totalPages = 0;
      let totalUnusedPages = 0;

      for (const table of tables) {
        try {
          // Get page count and freelist info for each table
          const pageInfo = this.db.prepare(`PRAGMA page_count`).get() as {
            page_count?: number;
          };
          const freelistInfo = this.db
          ...
          // Calculate approximate fragmentation for this table
          const pages = pageInfo.page_count || 0;
          const unusedPages = freelistInfo.freelist_count || 0;
          const fragmentation = pages > 0 ? (unusedPages / pages) * 100 : 0;

          tableFragmentation.push({
            name: table.name,
      ...
      const overallFragmentation =
        totalPages > 0 ? (totalUnusedPages / totalPages) * 100 : 0;

      return {
        overall: Math.round(overallFragmentation * 100) / 100,
  ...
  private getTableStatistics(): {
    ...
    try {
      ...
      // Get database size
      const sizeInfo = this.db.prepare(`PRAGMA page_size`).get() as {
        page_size?: number;
      };
      const pageInfo = this.db.prepare(`PRAGMA page_count`).get() as {
        page_count?: number;
      };
      const pageSize = sizeInfo.page_size || 4096;
  ...
  private getLastVacuumTime(): string | null {
    try {
      // SQLite doesn't track vacuum time directly, but we can check file modification time
      const fs = require("fs");
  ...
  private getOptimizationRecommendations(fragmentation: {
    overall: number;
    tables: Array<{ name: string; fragmentation: number }>;
  }): string[] {
    const recommendations: string[] = [];

    // Overall fragmentation recommendations
    if (fragmentation.overall > 25) {
      recommendations.push(
        "Database fragmentation is high (>25%). Consider running VACUUM to reclaim space.",
      );
    } else if (fragmentation.overall > 10) {
      recommendations.push(
        "Database fragmentation is moderate (>10%). Monitor and consider maintenance.",
      );
    }

    // Table-specific recommendations
    fragmentation.tables.forEach((table) => {
      if (table.fragmentation > 30) {
        recommendations.push(
          `Table '${table.name}' has high fragmentation (${table.fragmentation}%). Consider rebuilding.`,
        );
      }
    });
    ...
    if (stats.totalSize > 100 * 1024 * 1024) {
      // 100MB
      recommendations.push(
        "Database size is large (>100MB). Consider archiving old data or implementing partitioning.",
      );
    }
    ...
    if (stats.indexCount === 0) {
      recommendations.push(
        "No custom indexes found. Consider adding indexes for frequently queried columns.",
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        "Database is in good condition. No immediate optimization needed.",
      );
    }
  ...
  optimizeDatabase(): {
    ...
    try {
      const beforeStats = this.getDatabaseAnalytics();
      ...
      const afterStats = this.getDatabaseAnalytics();
