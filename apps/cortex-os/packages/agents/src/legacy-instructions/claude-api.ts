...
claudeApiCommand
  ...
  .action(async (options: any) => {
    try {
      ...
      console.error(
        chalk.red("❌ Failed to configure Claude API:"),
  ...
  .action(async (options: any) => {
    try {
      ...
      if (!configManager.isClaudeAPIConfigured()) {
        console.error(
          chalk.red(
            '❌ Claude API not configured. Run "claude-api configure" first.',
          ),
        );
        process.exit(1);
      }
      ...
      console.error(
        chalk.red("❌ Claude API test failed:"),
        getErrorMessage(error),
      );
      process.exit(1);
    }
  ...
  .action(async () => {
    try {
      ...
      if (isConfigured) {
        ...
        console.log(
          chalk.gray(`Model: ${config.model || "claude-3-sonnet-20240229"}`),
        );
        console.log(chalk.gray(`Temperature: ${config.temperature ?? 0.7}`));
        console.log(chalk.gray(`Max tokens: ${config.maxTokens || 4096}`));
        console.log(
          chalk.gray(`API key: ${config.apiKey ? "***masked***" : "Not set"}`),
        );

        if (process.env.ANTHROPIC_API_KEY && !config.apiKey) {
          console.log(
            chalk.yellow(
              "⚠️  Using API key from ANTHROPIC_API_KEY environment variable",
            ),
          );
        }
      } else {
        ...
        console.log(
          chalk.gray('Run "claude-api configure" to set up Claude API.'),
        );
      }
    } catch (error) {
      console.error(
  ...
  .action(async (options: any) => {
    try {
      ...
      if (Object.keys(updates).length === 0) {
        console.log(
          chalk.yellow(
            "⚠️  No updates specified. Use --model, --temperature, or --max-tokens.",
          ),
        );
        return;
      }
      ...
      console.error(
        chalk.red("❌ Failed to update configuration:"),
        getErrorMessage(error),
      );
      process.exit(1);
    }
