...
export class HelpFormatter {
  ...
  static formatHelp(info: CommandInfo): string {
    ...
    // NAME section
    sections.push(
      this.formatSection("NAME", [`${info.name} - ${info.description}`]),
    );
    ...
    if (info.commands && info.commands.length > 0) {
      sections.push(
        this.formatSection("COMMANDS", this.formatCommands(info.commands)),
      );
    }
    ...
    if (info.options && info.options.length > 0) {
      sections.push(
        this.formatSection("OPTIONS", this.formatOptions(info.options)),
      );
    }
    ...
    if (info.globalOptions && info.globalOptions.length > 0) {
      sections.push(
        this.formatSection(
          "GLOBAL OPTIONS",
          this.formatOptions(info.globalOptions),
        ),
      );
    }
    ...
    if (info.commands && info.commands.length > 0) {
      sections.push(
        `Run '${info.name} <command> --help' for more information on a command.`,
      );
    }
  ...
  private static formatCommands(commands: CommandItem[]): string[] {
    const maxNameLength = Math.max(
      ...
      ...commands.map((cmd) => {
        ...
        const aliasLength = cmd.aliases
          ? ` (${cmd.aliases.join(", ")})`.length
          : 0;
  ...
  private static formatOptions(options: OptionItem[]): string[] {
    ...
    return options.map((opt) => {
      const padding = " ".repeat(
        maxFlagsLength - opt.flags.length + this.COLUMN_GAP,
      );
      ...
      if (opt.validValues && opt.validValues.length > 0) {
        const validValuesLine =
          " ".repeat(maxFlagsLength + this.COLUMN_GAP) +
