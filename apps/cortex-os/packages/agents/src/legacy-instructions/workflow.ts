...
export const workflowCommand = new Command()
  ...
  .action(async (options: any) => {
    await listAvailableTemplates(options);
  })
  .command("init")
  ...
  .option("-v, --variables <vars>", "Variable overrides (JSON format)")
  .action(async (templateId: string, options: any) => {
    await initFromTemplate(templateId, options);
  });
...
async function generateTemplate(
  ...
  try {
    // Try to load template from the new enhanced template system
    const { workflowTemplateHandler } = await import(
      "../packages/slash-commands/src/handlers/workflow-templates.handler.js"
    );

    await workflowTemplateHandler.initFromTemplate(templateType, {
    ...
    console.log(
      chalk.yellow(
        "Enhanced template system not available, using legacy templates...",
      ),
    );
  }
  ...
  const templates: Record<string, WorkflowDefinition> = {
    ...
    "tdd-development": {
      ...
      agents: [
        { id: "test-designer", type: "tester", name: "Test Designer" },
        { id: "implementer", type: "coder", name: "Code Implementer" },
      ],
      tasks: [
        {
          id: "write-test",
    ...
    "security-audit": {
      ...
      agents: [
        { id: "security-scanner", type: "security", name: "Security Scanner" },
        { id: "analyst", type: "analyst", name: "Security Analyst" },
      ],
      tasks: [
        {
          id: "static-scan",
          type: "security-scan",
          description: "Perform SAST scanning",
        ...
        },
        {
          ...
          assignTo: "analyst",
          depends: ["static-scan", "dependency-audit"],
        },
      ],
      settings: {
        maxConcurrency: 2,
...
async function listAvailableTemplates(options: any): Promise<void> {
  try {
    // Try to use enhanced template system
    const { workflowTemplateHandler } = await import(
      "../packages/slash-commands/src/handlers/workflow-templates.handler.js"
    );
    await workflowTemplateHandler.listTemplates();
    return;
  } catch (error) {
    console.log(
      chalk.yellow(
        "Enhanced template system not available, showing legacy templates...",
      ),
    );
  }

  // Legacy template listing
  const templateIds = [
    "research",
    "implementation",
    "coordination",
  ...
  console.log(chalk.cyan.bold("Available Workflow Templates"));
  console.log("─".repeat(60));

  templateIds.forEach((id) => {
    const template = getLegacyTemplate(id);
    if (template) {
      console.log(`${chalk.green("●")} ${chalk.white(template.name)}`);
      console.log(`  ${chalk.gray(template.description)}`);
      console.log(
        `  ${chalk.blue("Tasks:")} ${template.tasks.length} | ${chalk.blue("Agents:")} ${template.agents?.length || 0}`,
      );
      console.log();
    }
  });

  console.log(chalk.gray(`Total templates: ${templateIds.length}`));
  console.log();
  console.log(chalk.blue("Usage:"));
  console.log(
    `  ${chalk.white("workflow init <template-id>")} - Initialize from template`,
  );
  console.log(
    `  ${chalk.white("workflow template <template-id>")} - Generate template (legacy)`,
  );
}
...
async function initFromTemplate(
  ...
  try {
    ...
    await workflowTemplateHandler.initFromTemplate(templateId, {
      output: options.output,
      format: options.format || "json",
      variables,
    });
    ...
    console.log(
      chalk.yellow(
        "Enhanced template system not available, using legacy initialization...",
      ),
    );

    // Fallback to legacy template generation
    await generateTemplate(templateId, options);
  }
}

function getLegacyTemplate(templateId: string): any {
  const templates: any = {
    research: {
      name: "Research Workflow",
      description: "Multi-stage research and analysis workflow",
    ...
    implementation: {
      ...
      tasks: [
        ...
        { id: "test", type: "testing", description: "Test the implementation" },
      ],
      agents: [
        {
          id: "implementer",
    ...
    coordination: {
      ...
      description: "Complex multi-agent coordination workflow",
      tasks: [
        { id: "plan", type: "planning", description: "Create execution plan" },
        { id: "work1", type: "implementation", description: "Execute part 1" },
        { id: "work2", type: "implementation", description: "Execute part 2" },
        {
          id: "integrate",
          type: "integration",
          description: "Integrate results",
