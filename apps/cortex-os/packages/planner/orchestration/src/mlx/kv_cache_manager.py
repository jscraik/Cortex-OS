...
class KVCacheManager:
    ...
    def __init__(
        ...
        """
        self.cache_dir = Path(cache_dir) if cache_dir else Path.home() / ".cortex" / "mlx_cache"
        ...
        logger.info(f"Config: window={max_token_window}, memory={max_memory_mb}MB, compression={compression_enabled}")
    ...
    def _generate_cache_key(self, model_name: str, tokens: List[int], context_hash: str = "") -> str:
    ...
    def _decompress_kv_states(self, compressed_data: bytes, is_compressed: bool = True) -> np.ndarray:
    ...
    def put(
        ...
        with self._lock:
            ...
            logger.debug(f"Cached KV states: {cache_key} ({len(tokens)} tokens, {compressed_size} bytes)")
            return cache_key
    ...
    def get(
        ...
        with self._lock:
            ...
            if entry is not None:
                ...
                # Decompress KV states
                kv_states = self._decompress_kv_states(entry.kv_states, entry.is_compressed)
                ...
                logger.debug(f"Cache miss for model {model_name} with {len(tokens)} tokens")
                return None
    ...
    def _apply_token_rotation(self, tokens: List[int]) -> List[int]:
        ...
        logger.debug(f"Applied token rotation: {len(tokens)} -> {len(rotated_tokens)} tokens")
        return rotated_tokens
    
    def _find_partial_match(self, model_name: str, tokens: List[int]) -> Optional[str]:
        ...
        for key in reversed(model_keys):  # Most recent first
            ...
            if len(entry.tokens) >= len(tokens):
                if entry.tokens[-len(tokens):] == tokens:
                    return key
            
            # Check for prefix match
            common_length = min(len(entry.tokens), len(tokens))
            if common_length > self.max_token_window // 2:  # At least half window
                if entry.tokens[:common_length] == tokens[:common_length]:
                    return key
    ...
    def _enforce_memory_limit(self):
        ...
        logger.info(f"Memory limit exceeded: {current_usage_mb:.1f}MB > {self.max_memory_mb}MB")
        ...
        logger.info(f"Evicted {evicted_count} entries, new usage: {current_usage_mb:.1f}MB")
    ...
    def _update_memory_usage(self):
        ...
        if self._access_times:
            self._metrics.average_access_time_ms = sum(self._access_times) / len(self._access_times)
        ...
        if self.compression_enabled and self._cache:
            total_compressed = sum(entry.compressed_size for entry in self._cache.values())
            total_uncompressed = sum(
                entry.kv_states.nbytes if hasattr(entry.kv_states, 'nbytes') 
        ...
        with self._lock:
            ...
            for model_name, keys in self._model_caches.items():
                ...
                model_stats[model_name] = {
                    ...
                    "memory_mb": sum(entry.compressed_size for entry in entries) / (1024 * 1024),
