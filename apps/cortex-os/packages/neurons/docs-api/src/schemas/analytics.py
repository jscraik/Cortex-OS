...
class UserEventCreate(BaseModel):
    ...
    user_id: Optional[str] = Field(None, description="User ID (optional for anonymous users)")
    ...
    document_path: Optional[str] = Field(None, description="Document path if applicable")
    page_url: Optional[str] = Field(None, description="Current page URL")
    referrer: Optional[str] = Field(None, description="Referrer URL")
    user_agent: Optional[str] = Field(None, description="User agent string")
    duration_ms: Optional[int] = Field(None, description="Event duration in milliseconds", ge=0)
    
    @validator('event_type')
    def validate_event_type(cls, v):
        allowed_types = {
            'page_view', 'search', 'click', 'bookmark_add', 'bookmark_remove',
            'progress_update', 'download', 'share', 'feedback', 'error',
            'session_start', 'session_end', 'scroll', 'time_on_page'
        }
...
class SearchEventCreate(BaseModel):
    ...
    clicked_position: Optional[int] = Field(None, description="Position of clicked result", ge=1)
    clicked_document_id: Optional[str] = Field(None, description="ID of clicked document")
...
class DashboardMetricsResponse(BaseModel):
    ...
    popular_pages: List[Dict[str, Union[str, int]]] = Field(..., description="Most popular pages")
    device_breakdown: Dict[str, float] = Field(..., description="Device type breakdown percentages")
...
class SearchAnalyticsQuery(BaseModel):
    ...
    avg_position: Optional[float] = Field(None, description="Average click position")
...
class SearchAnalyticsResponse(BaseModel):
    ...
    top_queries: List[SearchAnalyticsQuery] = Field(..., description="Top search queries")
...
class PerformanceEndpoint(BaseModel):
    ...
    p50_time_ms: Optional[float] = Field(None, description="50th percentile response time")
    p95_time_ms: Optional[float] = Field(None, description="95th percentile response time")
    p99_time_ms: Optional[float] = Field(None, description="99th percentile response time")
...
class PerformanceMetricsResponse(BaseModel):
    ...
    endpoints: List[PerformanceEndpoint] = Field(..., description="Endpoint-specific metrics")
    generated_at: Optional[datetime] = Field(None, description="Generation timestamp")
...
class UserEngagementMetrics(BaseModel):
    ...
    top_exit_pages: List[Dict[str, Union[str, float]]] = Field(..., description="Pages with highest exit rates")
    generated_at: Optional[datetime] = Field(None, description="Generation timestamp")
...
class ABTestResponse(BaseModel):
    ...
    metrics: Dict[str, Dict[str, float]] = Field(..., description="Performance metrics by variant")
    created_at: datetime = Field(..., description="Test creation timestamp")
    started_at: Optional[datetime] = Field(None, description="Test start timestamp")
    ended_at: Optional[datetime] = Field(None, description="Test end timestamp")
...
class AnalyticsFilter(BaseModel):
    ...
    def validate_operator(cls, v):
        allowed_operators = {'eq', 'ne', 'gt', 'gte', 'lt', 'lte', 'in', 'not_in', 'contains', 'not_contains'}
...
class AnalyticsQuery(BaseModel):
    ...
    filters: Optional[List[AnalyticsFilter]] = Field(None, description="Filters to apply")
    date_range: Dict[str, datetime] = Field(..., description="Date range with 'start' and 'end' keys")
    limit: Optional[int] = Field(100, description="Maximum results", ge=1, le=10000)
...
class AnalyticsExportRequest(BaseModel):
    ...
    def validate_report_type(cls, v):
        allowed_types = {
            'user_events', 'search_analytics', 'performance', 'content_analytics',
            'user_engagement', 'conversion_funnel', 'cohort_analysis'
        }
        if v not in allowed_types:
...
class AnalyticsExportResponse(BaseModel):
    ...
    data: Union[Dict[str, Any], str] = Field(..., description="Exported data or download URL")
...
class RealTimeMetrics(BaseModel):
    ...
    search_queries_last_hour: int = Field(..., description="Search queries in last hour")
    avg_response_time_ms: float = Field(..., description="Average response time in last 5 minutes")
    error_rate_last_hour: float = Field(..., description="Error rate in last hour")
    top_pages_last_hour: List[Dict[str, Union[str, int]]] = Field(..., description="Top pages in last hour")
