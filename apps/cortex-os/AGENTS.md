...
- Cannot contradict root rules. When in doubt, follow this order:
  ...
  4) `/CODESTYLE.md`  
  5) `/AGENTS.md` (root)  
  6) `apps/cortex-os/AGENTS.md` (this file)

---

## 2) Purpose & Scope

- **What this unit is for:** This app wires the `@cortex-os/memories` service using env-driven factories for storage and embedders.
- **Responsibilities:**
  - Implement the Cortex OS domain logic and exported APIs.
...
### 3.1 HTTP/GRPC/WS
- Inherits the ASBR control plane documented in apps/cortex-os/README.md; this package does not add extra HTTP routes.

### 3.2 MCP (Tools/Resources/Prompts)
- Uses shared MCP endpoints from packages/mcp-server; document new tools under packages/mcp-registry.

### 3.3 A2A Topics (Contracts)
- Publishes orchestration intents defined in packages/orchestration; describe additions in contracts/.

### 3.4 CLI
- Use the workspace commands below to run Cortex OS locally.
...
```bash
# Dev
pnpm --filter cortex-os dev

# Build
pnpm --filter cortex-os build

# Quality (affected-aware if available)
pnpm --filter cortex-os lint
pnpm --filter cortex-os typecheck
pnpm --filter cortex-os test
pnpm --filter cortex-os test:coverage
```
...
CORTEX_OS_BASE_URL=https://example.local
CORTEX_OS_DB_URL=postgres://user:pass@localhost:5432/cortex_os
...
```bash
pnpm --filter cortex-os test:coverage
pnpm security:scan --scope=cortex-os
pnpm structure:validate --scope=cortex-os
```
...
- SBOM: CycloneDX generated at `./sbom/cortex-os.json`.  
...
- Log schema requires `brand:"brAInwav"`, `component:"cortex-os"`, and `trace_id`.

```json
{"brand":"brAInwav","component":"cortex-os","level":"info","msg":"started","port":0}
```
...
```bash
pnpm --filter cortex-os test:e2e
pnpm --filter cortex-os test:smoke
pnpm --filter cortex-os chaos:probe
```
...
# Dev & run
pnpm --filter cortex-os dev
pnpm --filter cortex-os start

# Build & verify
pnpm --filter cortex-os build
pnpm --filter cortex-os lint && pnpm --filter cortex-os typecheck
pnpm --filter cortex-os test && pnpm --filter cortex-os test:coverage
...
pnpm --filter cortex-os mcp:smoke
```
