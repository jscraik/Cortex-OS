...
async function handleTasksRoute(
	...
	if (segments.length === 2) {
		...
		if (req.method === 'POST') {
			...
			const saved = await tasks.save(task as any);
	...
	if (segments.length === 3) {
		...
		if (req.method === 'PUT') {
			...
			if (mode === 'replace') {
				...
				record.id = id;
				const entry = await tasks.replace(id, record as any, { expectedDigest });
				sendJson(res, 200, { task: entry.record, digest: entry.digest });
				return;
			}
			...
			const entry = await tasks.update(id, patch as any, { expectedDigest });
			if (!entry) throw new HttpError(404, 'Task not found');
			sendJson(res, 200, { task: entry.record, digest: entry.digest });
			return;
		}
	...
	if (segments.length === 3) {
		...
		if (req.method === 'PUT') {
			...
			if (mode === 'replace') {
				...
				profile.id = id;
				const entry = await profiles.replace(id, profile as any, { expectedDigest });
				sendJson(res, 200, { profile: entry.record, digest: entry.digest });
				return;
			}
			...
			const entry = await profiles.update(id, patch as any, { expectedDigest });
			if (!entry) throw new HttpError(404, 'Profile not found');
			sendJson(res, 200, { profile: entry.record, digest: entry.digest });
			return;
		}
...
function sendNotFound(res: ServerResponse, message: string): void {
	sendJson(res, 404, { error: 'NotFound', message });
}
...
function handleError(res: ServerResponse, error: unknown): void {
	...
	sendJson(res, 500, { error: 'InternalError', message: 'Unexpected server error' });
}
