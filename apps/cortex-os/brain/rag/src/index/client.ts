...
export interface BuildIndexOptions
  extends Omit<BuildRequest, "documents" | "config"> {
...
export interface SearchIndexOptions
  extends Omit<SearchRequest, "query_vector"> {
    ...
    for (const doc of documents) {
      ...
      if (!doc.embedding || !Array.isArray(doc.embedding)) {
        throw new FaissClientError(
          `Document ${doc.doc_id} must have an embedding array`,
        );
      }
 ...
 */
export async function checkFaissService(
  endpoint: string,
  timeout = 5000,
): Promise<boolean> {
