...
export function getSingleDim(): number {
  try {
    const cfg = yaml.load(
      fs.readFileSync("configs/rag.config.yaml", "utf8"),
    ) as any;
...
export function validateEmbedding(
  embedding: unknown,
  dim: number = getSingleDim(),
): number[] {
  if (!Array.isArray(embedding)) throw new Error("embedding_must_be_array");
  if (embedding.length !== dim)
    throw new Error(
      `embedding_length_mismatch_${embedding.length}_expected_${dim}`,
    );
...
export async function setEmbeddingInSpace(
  chunkId: string,
  embedding: number[],
  dim: number,
) {
