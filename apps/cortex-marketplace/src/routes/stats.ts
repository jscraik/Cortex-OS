  ...
  fastify.get('/stats/top-rated', {
    schema: {
      ...
      querystring: {
        ...
        properties: {
          category: { type: 'string', description: 'Filter by category' },
          minDownloads: { type: 'integer', minimum: 0, description: 'Minimum download count', default: 100 },
          limit: { type: 'integer', minimum: 1, maximum: 50, default: 10 }
        }
      },
      response: {
    ...
    const query = request.query as { category?: string; minDownloads?: number; limit?: number };
