...
export async function serverRoutes(fastify: FastifyInstance): Promise<void> {
    ...
    schema: {
      ...
      querystring: {
        ...
        properties: {
          ...
          featured: { type: 'boolean', description: 'Filter featured servers' },
          publisher: { type: 'string', description: 'Filter by publisher name' },
          minRating: { type: 'number', minimum: 0, maximum: 5, description: 'Minimum rating' },
          tags: { type: 'string', description: 'Comma-separated tags' },
          capabilities: { type: 'string', description: 'Comma-separated capabilities (tools,resources,prompts)' },
  ...
  fastify.get('/servers/:id/install', {
    schema: {
      tags: ['servers'],
