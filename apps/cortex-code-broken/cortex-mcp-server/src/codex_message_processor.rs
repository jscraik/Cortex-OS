...
use cortex_core::config::ConfigToml;
use cortex_core::config::load_config_as_toml;
use cortex_core::git_info::git_diff_to_remote;
use cortex_core::protocol::ApplyPatchApprovalRequestEvent;
use cortex_core::protocol::Event;
use cortex_core::protocol::EventMsg;
use cortex_core::protocol::ExecApprovalRequestEvent;
use cortex_core::protocol::ReviewDecision;
use cortex_protocol::mcp_protocol::AuthMode;
use cortex_protocol::mcp_protocol::GitDiffToRemoteResponse;
...
use cortex_core::auth::CLIENT_ID;
use cortex_core::protocol::InputItem as CoreInputItem;
use cortex_core::protocol::Op;
use cortex_login::ServerOptions as LoginServerOptions;
use cortex_login::ShutdownHandle;
use cortex_login::run_login_server;
use cortex_protocol::mcp_protocol::APPLY_PATCH_APPROVAL_METHOD;
use cortex_protocol::mcp_protocol::AddConversationListenerParams;
use cortex_protocol::mcp_protocol::AddConversationSubscriptionResponse;
use cortex_protocol::mcp_protocol::ApplyPatchApprovalParams;
use cortex_protocol::mcp_protocol::ApplyPatchApprovalResponse;
use cortex_protocol::mcp_protocol::AuthStatusChangeNotification;
use cortex_protocol::mcp_protocol::ClientRequest;
use cortex_protocol::mcp_protocol::ConversationId;
use cortex_protocol::mcp_protocol::EXEC_COMMAND_APPROVAL_METHOD;
use cortex_protocol::mcp_protocol::ExecCommandApprovalParams;
use cortex_protocol::mcp_protocol::ExecCommandApprovalResponse;
use cortex_protocol::mcp_protocol::GetConfigTomlResponse;
use cortex_protocol::mcp_protocol::InputItem as WireInputItem;
use cortex_protocol::mcp_protocol::InterruptConversationParams;
use cortex_protocol::mcp_protocol::InterruptConversationResponse;
use cortex_protocol::mcp_protocol::LoginChatGptCompleteNotification;
use cortex_protocol::mcp_protocol::LoginChatGptResponse;
use cortex_protocol::mcp_protocol::NewConversationParams;
use cortex_protocol::mcp_protocol::NewConversationResponse;
use cortex_protocol::mcp_protocol::RemoveConversationListenerParams;
use cortex_protocol::mcp_protocol::RemoveConversationSubscriptionResponse;
use cortex_protocol::mcp_protocol::SendUserMessageParams;
use cortex_protocol::mcp_protocol::SendUserMessageResponse;
use cortex_protocol::mcp_protocol::SendUserTurnParams;
use cortex_protocol::mcp_protocol::SendUserTurnResponse;
use cortex_protocol::mcp_protocol::ServerNotification;
...
/// Handles JSON-RPC messages for Cortex conversations.
pub(crate) struct CortexMessageProcessor {
...
impl CortexMessageProcessor {
    ...
    async fn login_chatgpt(&mut self, request_id: RequestId) {
        ...
        let opts = LoginServerOptions {
            open_browser: false,
            ..LoginServerOptions::new(config.cortex_home.clone(), CLIENT_ID.to_string())
        };
    ...
    async fn cancel_login_chatgpt(&mut self, request_id: RequestId, login_id: Uuid) {
        ...
        if guard.as_ref().map(|l| l.login_id) == Some(login_id) {
            ...
            self.outgoing
                .send_response(
                    request_id,
                    cortex_protocol::mcp_protocol::CancelLoginChatGptResponse {},
                )
                .await;
        } else {
            drop(guard);
    ...
    async fn logout_chatgpt(&mut self, request_id: RequestId) {
        ...
        self.outgoing
            .send_response(
                request_id,
                cortex_protocol::mcp_protocol::LogoutChatGptResponse {},
            )
            .await;
    ...
    async fn get_auth_status(
        ...
        params: cortex_protocol::mcp_protocol::GetAuthStatusParams,
    ) {
        ...
        let response = match self.auth_manager.auth() {
            Some(auth) => {
                ...
                cortex_protocol::mcp_protocol::GetAuthStatusResponse {
                    auth_method: reported_auth_method,
                    preferred_auth_method,
                    auth_token: token_opt,
                }
            }
            None => cortex_protocol::mcp_protocol::GetAuthStatusResponse {
                auth_method: None,
                preferred_auth_method,
                auth_token: None,
            },
    ...
    async fn get_config_toml(&self, request_id: RequestId) {
        let toml_value = match load_config_as_toml(&self.config.cortex_home) {
            Ok(val) => val,
            Err(err) => {
                let error = JSONRPCErrorError {
                    code: INTERNAL_ERROR_CODE,
        ...
        let profiles: HashMap<String, cortex_protocol::config_types::ConfigProfile> = cfg
            .profiles
            .into_iter()
            .map(|(k, v)| {
                (
                    ...
                    // implement `From<cortex_core::config_profile::ConfigProfile>`
                    // for the `ConfigProfile` type and introduce a dependency on cortex_core
                    cortex_protocol::config_types::ConfigProfile {
                        model: v.model,
    ...
    async fn process_new_conversation(&self, request_id: RequestId, params: NewConversationParams) {
        let config = match derive_config_from_params(params, self.cortex_linux_sandbox_exe.clone()) {
            Ok(config) => config,
            Err(err) => {
                let error = JSONRPCErrorError {
                    code: INVALID_REQUEST_ERROR_CODE,
            ...
            loop {
                tokio::select! {
                    ...
                    event = conversation.next_event() => {
                        ...
                        // stable wire format.
                        let method = format!("cortex/event/{}", event.msg);
...
async fn apply_bespoke_event_handling(
    ...
    conversation: Arc<CortexConversation>,
    outgoing: Arc<OutgoingMessageSender>,
    pending_interrupts: Arc<Mutex<HashMap<Uuid, Vec<RequestId>>>>,
) {
...
async fn on_exec_approval_response(
    ...
    // Try to deserialize `value` and then make the appropriate call to `cortex`.
    let response =
