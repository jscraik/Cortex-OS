...
use crate::cortex_message_processor::CortexMessageProcessor;
use crate::cortex_tool_config::CortexToolCallParam;
use crate::cortex_tool_config::CortexToolCallReplyParam;
use crate::cortex_tool_config::create_tool_for_cortex_tool_call_param;
use crate::cortex_tool_config::create_tool_for_cortex_tool_call_reply_param;
use crate::error_code::INVALID_REQUEST_ERROR_CODE;
use crate::outgoing_message::OutgoingMessageSender;
use cortex_protocol::mcp_protocol::ClientRequest;

use cortex_core::AuthManager;
use cortex_core::ConversationManager;
use cortex_core::config::Config;
use cortex_core::protocol::Submission;
...
pub(crate) struct MessageProcessor {
    cortex_message_processor: CortexMessageProcessor,
    outgoing: Arc<OutgoingMessageSender>,
    initialized: bool,
    cortex_linux_sandbox_exe: Option<PathBuf>,
    conversation_manager: Arc<ConversationManager>,
    running_requests_id_to_cortex_uuid: Arc<Mutex<HashMap<RequestId, Uuid>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        let auth_manager =
            AuthManager::shared(config.cortex_home.clone(), config.preferred_auth_method);
        let conversation_manager = Arc::new(ConversationManager::new(auth_manager.clone()));
        let cortex_message_processor = CortexMessageProcessor::new(
            auth_manager,
            conversation_manager.clone(),
            outgoing.clone(),
            cortex_linux_sandbox_exe.clone(),
            config,
        );
        Self {
            cortex_message_processor,
            outgoing,
            initialized: false,
            cortex_linux_sandbox_exe,
            conversation_manager,
            running_requests_id_to_cortex_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }

    pub(crate) async fn process_request(&mut self, request: JSONRPCRequest) {
        if let Ok(request_json) = serde_json::to_value(request.clone())
            && let Ok(cortex_request) = serde_json::from_value::<ClientRequest>(request_json)
        {
            // If the request is a Cortex request, handle it with the Cortex
            // message processor.
            self.cortex_message_processor
                .process_request(cortex_request)
                .await;
            return;
        }
    ...
    async fn handle_list_tools(
        ...
        let result = ListToolsResult {
            tools: vec![
                create_tool_for_cortex_tool_call_param(),
                create_tool_for_cortex_tool_call_reply_param(),
                // Lightweight GitHub tools exposed via cortex-github
                mcp_types::Tool {
                    name: "github/repo_info".to_string(),
                    description: Some("Get repository info: { owner, repo }".to_string()),
                ...
                mcp_types::Tool {
                    ...
                    description: Some("List pull requests: { owner, repo, state?, per_page? }".to_string()),
                    input_schema: Some(serde_json::json!({
                        "type": "object",
                        "required": ["owner", "repo"],
                        "properties": {
    ...
    async fn handle_call_tool(
        ...
        match name.as_str() {
            "cortex" => self.handle_tool_call_cortex(id, arguments).await,
            "cortex-reply" => {
                self.handle_tool_call_cortex_session_reply(id, arguments)
                    .await
            }
            "github/repo_info" => {
                let (owner, repo) = match arguments.and_then(|v| {
                    let owner = v.get("owner").and_then(|s| s.as_str()).map(|s| s.to_string());
                    let repo = v.get("repo").and_then(|s| s.as_str()).map(|s| s.to_string());
                    match (owner, repo) { (Some(o), Some(r)) => Some((o, r)), _ => None }
                }) {
                    Some(x) => x,
                    None => {
                        let result = CallToolResult {
                            content: vec![ContentBlock::TextContent(TextContent { r#type: "text".into(), text: "Missing required fields: owner, repo".into(), annotations: None })],
                            is_error: Some(true),
                ...
                let result = crate::tool_handlers::github::repo_info(owner, repo).await;
                self.send_response::<mcp_types::CallToolRequest>(id, result).await;
            }
            "github/list_prs" => {
                let (owner, repo, state, per_page) = match arguments.and_then(|v| {
                    let owner = v.get("owner").and_then(|s| s.as_str()).map(|s| s.to_string());
                    let repo = v.get("repo").and_then(|s| s.as_str()).map(|s| s.to_string());
                    let state = v.get("state").and_then(|s| s.as_str()).map(|s| s.to_string());
                    let per_page = v.get("per_page").and_then(|n| n.as_u64()).map(|n| n as u32);
                    match (owner, repo) { (Some(o), Some(r)) => Some((o, r, state, per_page)), _ => None }
                }) {
                    Some(x) => x,
                    None => {
                        let result = CallToolResult {
                ...
                let result = crate::tool_handlers::github::list_prs(owner, repo, state, per_page).await;
                self.send_response::<mcp_types::CallToolRequest>(id, result).await;
            }
            _ => {
                let result = CallToolResult {
    ...
    async fn handle_tool_call_cortex(&self, id: RequestId, arguments: Option<serde_json::Value>) {
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<CortexToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.cortex_linux_sandbox_exe.clone()) {
                    Ok(cfg) => cfg,
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![ContentBlock::TextContent(TextContent {
                                ...
                                text: format!(
                                    "Failed to load Cortex configuration from overrides: {e}"
                                ),
                                annotations: None,
                            })],
                            is_error: Some(true),
                ...
                Err(e) => {
                    let result = CallToolResult {
                        content: vec![ContentBlock::TextContent(TextContent {
                            r#type: "text".to_owned(),
                            text: format!("Failed to parse configuration for Cortex tool: {e}"),
                            annotations: None,
                        })],
                        is_error: Some(true),
                        structured_content: None,
            ...
            None => {
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_string(),
                        text:
                            "Missing arguments for cortex tool-call; the `prompt` field is required."
                                .to_string(),
                        annotations: None,
                    })],
                    is_error: Some(true),
        ...
        let running_requests_id_to_cortex_uuid = self.running_requests_id_to_cortex_uuid.clone();

        // Spawn an async task to handle the Cortex session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Cortex session and stream events back to the client.
            crate::cortex_tool_runner::run_cortex_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                conversation_manager,
                running_requests_id_to_cortex_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_cortex_session_reply(
        &self,
        ...
        // parse arguments
        let CortexToolCallReplyParam { session_id, prompt } = match arguments {
            Some(json_val) => match serde_json::from_value::<CortexToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Cortex tool call reply parameters: {e}");
                    let result = CallToolResult {
                        content: vec![ContentBlock::TextContent(TextContent {
                            r#type: "text".to_owned(),
                            text: format!("Failed to parse configuration for Cortex tool: {e}"),
            ...
            None => {
                tracing::error!(
                    "Missing arguments for cortex-reply tool-call; the `session_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_owned(),
                        text: "Missing arguments for cortex-reply tool-call; the `session_id` and `prompt` fields are required.".to_owned(),
                        annotations: None,
                    })],
                    is_error: Some(true),
                    structured_content: None,
        ...
        let cortex = match self.conversation_manager.get_conversation(session_id).await {
            Ok(c) => c,
            Err(_) => {
        ...
        tokio::spawn({
            ...
            let running_requests_id_to_cortex_uuid = running_requests_id_to_cortex_uuid.clone();

            async move {
                crate::cortex_tool_runner::run_cortex_tool_session_reply(
                    cortex,
                    outgoing,
                    request_id,
                    prompt,
    ...
    async fn handle_cancelled_notification(
        ...
        let session_id = {
            let map_guard = self.running_requests_id_to_cortex_uuid.lock().await;
        ...
        // Obtain the Cortex conversation from the server.
        let cortex_arc = match self.conversation_manager.get_conversation(session_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for session_id: {session_id}");
                return;
        ...
        // Submit interrupt to Cortex.
        let err = cortex_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: cortex_core::protocol::Op::Interrupt,
            })
            .await;
        if let Err(e) = err {
            tracing::error!("Failed to submit interrupt to Cortex: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_cortex_uuid
            .lock()
            .await
            .remove(&request_id);
    }
