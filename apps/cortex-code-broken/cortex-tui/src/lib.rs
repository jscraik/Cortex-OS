...
// The standalone `cortex-tui` binary prints a short help message before the
// alternate‑screen mode starts; that file opts‑out locally via `allow`.
#![deny(clippy::print_stdout, clippy::print_stderr)]
#![deny(clippy::disallowed_methods)]
use app::App;
use cortex_core::AuthManager;
use cortex_core::BUILT_IN_OSS_MODEL_PROVIDER_ID;
...
use cortex_core::config::find_cortex_home;
use cortex_core::config::load_config_as_toml_with_cli_overrides;
use cortex_core::protocol::AskForApproval;
use cortex_core::protocol::SandboxPolicy;
use cortex_ollama::DEFAULT_OSS_MODEL;
...
pub async fn run_main(
...
) -> std::io::Result<cortex_core::protocol::TokenUsage> {
    ...
    let overrides_cli = cortex_common::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let config_toml = {
        let cortex_home = match find_cortex_home() {
            Ok(cortex_home) => cortex_home,
            Err(err) => {
                eprintln!("Error finding cortex home: {err}");
                std::process::exit(1);
            }
        };

        match load_config_as_toml_with_cli_overrides(&cortex_home, cli_kv_overrides) {
    ...
    let log_dir = cortex_core::config::log_dir(&config)?;
    ...
    // Cortex CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("cortex-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for cortex crates.
    let env_filter = || {
        EnvFilter::try_from_default_env()
            .unwrap_or_else(|_| EnvFilter::new("cortex_core=info,cortex_tui=info"))
    };
    ...
    if cli.oss {
        cortex_ollama::ensure_oss_ready(&config)
            .await
...
async fn run_ratatui_app(
...
) -> color_eyre::Result<cortex_core::protocol::TokenUsage> {
    ...
    if let Some(latest_version) = updates::get_upgrade_version(&config) {
        ...
        if managed_by_npm {
            let npm_cmd = "npm install -g @openai/cortex@latest";
            lines.push(Line::from(vec![
                "Run ".into(),
                npm_cmd.cyan(),
                " to update.".into(),
        ...
        {
            let brew_cmd = "brew upgrade cortex";
            lines.push(Line::from(vec![
                "Run ".into(),
                brew_cmd.cyan(),
                " to update.".into(),
            ...
            lines.push(Line::from(vec![
                "See ".into(),
                "https://github.com/openai/cortex/releases/latest".cyan(),
    ...
    let auth_manager = AuthManager::shared(config.cortex_home.clone(), config.preferred_auth_method);
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        let cortex_home = config.cortex_home.clone();
        match CortexAuth::from_cortex_home(&cortex_home, config.preferred_auth_method) {
