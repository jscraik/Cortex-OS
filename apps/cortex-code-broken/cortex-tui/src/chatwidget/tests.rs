...
use cortex_core::CortexAuth;
use cortex_core::config::Config;
use cortex_core::config::ConfigOverrides;
use cortex_core::config::ConfigToml;
use cortex_core::plan_tool::PlanItemArg;
use cortex_core::plan_tool::StepStatus;
use cortex_core::plan_tool::UpdatePlanArgs;
...
use cortex_core::protocol::PatchApplyEndEvent;
use cortex_core::protocol::StreamErrorEvent;
use cortex_core::protocol::TaskCompleteEvent;
use cortex_core::protocol::TaskStartedEvent;
...
fn test_config() -> Config {
    ...
    cortex_core::config::Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
...
fn final_answer_without_newline_is_flushed_immediately() {
    ...
    chat.handle_cortex_event(Event {
        id: "sub-a".into(),
        msg: EventMsg::AgentMessageDelta(AgentMessageDeltaEvent {
            delta: "Hi! How can I help with cortex-rs or anything else today?".into(),
        }),
    });
    ...
    chat.handle_cortex_event(Event {
        ...
        msg: EventMsg::AgentMessage(AgentMessageEvent {
            message: "Hi! How can I help with cortex-rs or anything else today?".into(),
        }),
    });
    ...
    assert!(
        ...
        "expected 'cortex' header to be emitted",
    );
    let found_final = cells.iter().any(|lines| {
        let s = lines
            ...
            .map(|sp| sp.content.clone())
            .collect::<String>();
        s.contains("Hi! How can I help with cortex-rs or anything else today?")
    });
    assert!(
        found_final,
...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let cfg = test_config();
    let conversation_manager = Arc::new(ConversationManager::with_auth(CortexAuth::from_api_key(
        "test",
    )));
...
fn make_chatwidget_manual() -> (
    ...
    let bottom = BottomPane::new(BottomPaneParams {
        ...
        placeholder_text: "Ask Cortex to do anything".to_string(),
...
fn begin_exec(chat: &mut ChatWidget, call_id: &str, raw_cmd: &str) {
    ...
    let parsed_cmd: Vec<cortex_protocol::parse_command::ParsedCommand> =
        cortex_core::parse_command::parse_command(&command)
            .into_iter()
            .map(Into::into)
            .collect();
    chat.handle_cortex_event(Event {
...
fn interrupt_exec_marks_failed_snapshot() {
    ...
    chat.handle_cortex_event(Event {
        id: "call-int".into(),
        msg: EventMsg::TurnAborted(cortex_core::protocol::TurnAbortedEvent {
...
async fn binary_size_transcript_snapshot() {
                ...
                if let Some(payload) = v.get("payload") {
                    ...
                    let ev = match ev {
                        Event {
                            ...
                            let parsed_cmd = cortex_core::parse_command::parse_command(&e.command);
                            Event {
                                id: ev.id,
                                msg: EventMsg::ExecCommandBegin(ExecCommandBeginEvent {
    ...
    const MARKER_PREFIX: &str = ">_ You are using OpenAI Cortex in ";
...
//
// Synthesizes a Cortex ExecApprovalRequest event to trigger the approval modal
...
fn approval_modal_exec_snapshot() {
    ...
    chat.config.approval_policy = cortex_core::protocol::AskForApproval::OnRequest;
...
fn interrupt_restores_queued_messages_into_composer() {
    ...
    chat.handle_cortex_event(Event {
        id: "turn-1".into(),
        msg: EventMsg::TurnAborted(cortex_core::protocol::TurnAbortedEvent {
            reason: cortex_core::protocol::TurnAbortReason::Interrupted,
        }),
    });
...
fn status_widget_and_approval_modal_snapshot() {
    ...
    let ev = ExecApprovalRequestEvent {
        ...
        reason: Some("Cortex wants to run a command".into()),
    };
    chat.handle_cortex_event(Event {
...
fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    // Expect a CortexOp with PatchApproval carrying the submission id, not call id
    let mut found = false;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::CortexOp(Op::PatchApproval { id, decision }) = app_ev {
            assert_eq!(id, "sub-123");
            assert!(matches!(
                decision,
                cortex_core::protocol::ReviewDecision::Approved
            ));
            found = true;
            break;
        }
...
fn apply_patch_full_flow_integration_like() {
    ...
    // 2) User approves via 'y' and App receives a CortexOp
    chat.handle_key_event(KeyEvent::new(KeyCode::Char('y'), KeyModifiers::NONE));
    let mut maybe_op: Option<Op> = None;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::CortexOp(op) = app_ev {
            maybe_op = Some(op);
            break;
        }
    }
    let op = maybe_op.expect("expected CortexOp after key press");

    // 3) App forwards to widget.submit_op, which pushes onto cortex_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to cortex channel");
    match forwarded {
...
fn chatwidget_exec_and_status_layout_vt100_snapshot() {
    ...
    chat.handle_cortex_event(Event {
        ...
        msg: EventMsg::ExecCommandBegin(ExecCommandBeginEvent {
            ...
            cwd: std::env::current_dir().unwrap_or_else(|_| PathBuf::from(".")),
            parsed_cmd: vec![
                cortex_core::parse_command::ParsedCommand::Search {
                    query: Some("Change Approved".into()),
                    path: None,
                    cmd: "rg \"Change Approved\"".into(),
                }
                .into(),
                cortex_core::parse_command::ParsedCommand::Read {
