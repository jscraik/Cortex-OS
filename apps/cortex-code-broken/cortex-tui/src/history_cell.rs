...
use base64::Engine;
use cortex_ansi_escape::ansi_escape_line;
use cortex_common::create_config_summary_entries;
use cortex_common::elapsed::format_duration;
use cortex_core::auth::get_auth_file;
use cortex_core::auth::try_read_auth_json;
...
use cortex_core::project_doc::discover_project_doc_paths;
...
impl HistoryCell for AgentMessageCell {
    ...
    fn transcript_lines(&self) -> Vec<Line<'static>> {
        ...
        if self.is_first_line {
            out.push("cortex".magenta().bold().into());
        }
...
pub(crate) fn new_session_info(
    ...
    if is_first_event {
        ...
        lines.push(Line::from(vec![
            ">_ ".dim(),
            "You are using OpenAI Cortex in".bold(),
            format!(" {cwd_str}").dim(),
        ]));
...
pub(crate) fn new_status_output(
    ...
    let auth_file = get_auth_file(&config.cortex_home);
        ...
        && let Some(tokens) = auth.tokens.clone()
    {
        ...
        match auth.openai_api_key.as_deref() {
            Some(key) if !key.is_empty() => {
                lines.push("  • Using API key. Run cortex login to use ChatGPT plan".into());
            }
            _ => {
                let plan_text = info
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            "    See the ".into(),
            "\u{1b}]8;;https://github.com/openai/cortex/blob/main/cortex-rs/config.md#mcp_servers\u{7}MCP docs\u{1b}]8;;\u{7}".underlined(),
