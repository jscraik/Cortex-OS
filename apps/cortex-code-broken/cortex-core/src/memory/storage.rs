...
impl MemoryStorage {
    pub async fn new(config: MemoryConfig) -> crate::Result<Self> {
    ...
    pub async fn end_conversation(&self, session_id: &str) -> crate::Result<()> {
        let context = {
            let mut contexts = self.active_contexts.write().await;
    ...
    pub async fn get_recent_context(&self, limit: usize) -> crate::Result<Vec<String>> {
    ...
    pub async fn search_memory(&self, query: &str) -> crate::Result<Vec<String>> {
        let agents_md = self.agents_md.read().await;
    ...
    pub async fn get_provider_history(&self, provider: &str) -> crate::Result<Vec<String>> {
        let agents_md = self.agents_md.read().await;
    ...
    pub async fn add_decision(&self, session_id: &str, decision: String) -> crate::Result<()> {
        let mut contexts = self.active_contexts.write().await;
        if let Some(context) = contexts.get_mut(session_id) {
    ...
    pub async fn add_tag(&self, session_id: &str, tag: String) -> crate::Result<()> {
        let mut contexts = self.active_contexts.write().await;
        if let Some(context) = contexts.get_mut(session_id) {
            context.add_tag(tag);
        }
    ...
    pub async fn get_memory_stats(&self) -> crate::Result<MemoryStorageStats> {
        let agents_md = self.agents_md.read().await;
    ...
    pub async fn cleanup_expired(&self) -> crate::Result<usize> {
        let mut agents_md = self.agents_md.write().await;
    ...
    pub async fn export_memory(&self, format: ExportFormat) -> crate::Result<String> {
        let agents_md = self.agents_md.read().await;
        ...
        match format {
            ExportFormat::Json => {
                ...
                serde_json::to_string_pretty(entries)
                    .map_err(|e| crate::error::CortexError::Memory(format!("JSON export failed: {}", e)))
            }
            ExportFormat::Markdown => {
                // Already in markdown format
                tokio::fs::read_to_string(&self.config.agents_md_path).await
                    .map_err(|e| crate::error::CortexError::Memory(format!("Failed to read AGENTS.md: {}", e)))
            }
            ExportFormat::Csv => {
                let entries = agents_md.get_entries();
    ...
    pub async fn active_sessions_summary(&self) -> crate::Result<(usize, Vec<String>)> {
        let contexts = self.active_contexts.read().await;
