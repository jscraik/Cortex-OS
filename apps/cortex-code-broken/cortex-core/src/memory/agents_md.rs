...
impl AgentsMd {
    ...
    pub async fn save(&mut self) -> crate::Result<()> {
        ...
        if let Some(parent) = self.path.parent() {
            fs::create_dir_all(parent).await
                .map_err(|e| crate::error::CortexError::Memory(format!("Failed to create directory: {}", e)))?;
        }

        fs::write(&self.path, markdown_content).await
            .map_err(|e| crate::error::CortexError::Memory(format!("Failed to write AGENTS.md: {}", e)))?;

        Ok(())
    }

    pub async fn add_entry(&mut self, entry: AgentEntry) -> crate::Result<()> {
    ...
    async fn cleanup_old_entries(&mut self) -> crate::Result<()> {
    ...
    async fn initialize(&mut self) -> crate::Result<()> {
        ...
        fs::write(&self.path, initial_content).await
            .map_err(|e| crate::error::CortexError::Memory(format!("Failed to create AGENTS.md: {}", e)))?;

        Ok(())
    }

    fn parse_markdown(&mut self, content: &str) -> crate::Result<()> {
