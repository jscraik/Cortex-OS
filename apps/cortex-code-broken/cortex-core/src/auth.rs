...
impl CortexAuth {
    ...
    pub fn from_cortex_home(
        cortex_home: &Path,
        preferred_auth_method: AuthMode,
    ) -> std::io::Result<Option<CortexAuth>> {
        load_auth(cortex_home, true, preferred_auth_method)
    }
...
pub fn get_auth_file(cortex_home: &Path) -> PathBuf {
    cortex_home.join("auth.json")
}

/// Delete the auth.json file inside `cortex_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(cortex_home: &Path) -> std::io::Result<bool> {
    let auth_file = get_auth_file(cortex_home);
...
pub fn login_with_api_key(cortex_home: &Path, api_key: &str) -> std::io::Result<()> {
    let auth_dot_json = AuthDotJson {
        openai_api_key: Some(api_key.to_string()),
        tokens: None,
        last_refresh: None,
    };
    write_auth_json(&get_auth_file(cortex_home), &auth_dot_json)
}
    ...
    let auth_dot_json = match try_read_auth_json(&auth_file) {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::NotFound && include_env_var => {
            return match read_openai_api_key_from_env() {
                Some(api_key) => Ok(Some(CortexAuth::from_api_key(&api_key))),
                None => Ok(None),
            };
        }
        ...
        match &tokens {
            Some(tokens) => {
                if tokens.should_use_api_key(preferred_auth_method, tokens.is_openai_email()) {
                    return Ok(Some(CortexAuth::from_api_key(api_key)));
                } else {
            ...
            None => {
                ...
                // Perhaps the user ran `cortex login --api-key <KEY>` or updated
...
mod tests {
    ...
    async fn roundtrip_auth_dot_json() {
        let cortex_home = tempdir().unwrap();
        ...
        let file = get_auth_file(cortex_home.path());
    ...
    async fn pro_account_with_no_api_key_uses_chatgpt_auth() {
        ...
        let CortexAuth {
            ...
            auth_dot_json,
            auth_file: _,
        } = super::load_auth(cortex_home.path(), false, AuthMode::ChatGPT)
            .unwrap()
            .unwrap();
    ...
    fn write_auth_file(params: AuthFileParams, cortex_home: &Path) -> std::io::Result<String> {
        let auth_file = get_auth_file(cortex_home);
...
/// hands out cloned `CortexAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
impl AuthManager {
    ...
    pub fn new(cortex_home: PathBuf, preferred_auth_mode: AuthMode) -> Self {
        let auth = CortexAuth::from_cortex_home(&cortex_home, preferred_auth_mode)
            .ok()
            .flatten();
        Self {
            cortex_home,
    ...
    /// Create an AuthManager with a specific CortexAuth, for testing only.
    pub fn from_auth_for_testing(auth: CortexAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            cortex_home: PathBuf::new(),
            inner: RwLock::new(cached),
        })
    }

    /// Current cached auth (clone). May be `None` if not logged in or load failed.
    pub fn auth(&self) -> Option<CortexAuth> {
    ...
    pub fn reload(&self) -> bool {
        ...
        let new_auth = CortexAuth::from_cortex_home(&self.cortex_home, preferred)
            .ok()
            .flatten();
    ...
    fn auths_equal(a: &Option<CortexAuth>, b: &Option<CortexAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    pub fn shared(cortex_home: PathBuf, preferred_auth_mode: AuthMode) -> Arc<Self> {
        Arc::new(Self::new(cortex_home, preferred_auth_mode))
    }
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = super::auth::logout(&self.cortex_home)?;
