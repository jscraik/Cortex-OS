...
///
/// 1. The process was spawned by Cortex as part of a shell tool call.
...
pub(crate) async fn spawn_child_async(
    ...
    // If this Cortex process dies (including being killed via SIGKILL), we want
    ...
    unsafe {
        cmd.pre_exec(|| {
            ...
            // run _after_ the parent (i.e., the Cortex process) has already
