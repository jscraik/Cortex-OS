...
//!
//! The history is stored at `~/.cortex/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.cortex`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.cortex_home.clone();
...
pub(crate) async fn append_entry(text: &str, session_id: &Uuid, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.cortex/history.jsonl` and ensure the parent directory exists.
