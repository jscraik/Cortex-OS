...
describe('createOllamaService', () => {
  test('returns null when disabled', () => {
    const service = createOllamaService({ baseUrl: 'http://localhost:11434', defaultModel: 'test', timeout: 1000, enabled: false });
  ...
  test('healthCheck uses global fetch', async () => {
    ...
    const service = createOllamaService({ baseUrl: 'http://localhost:11434', defaultModel: 'test', timeout: 1000, enabled: true });
