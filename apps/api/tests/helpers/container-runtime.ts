...
const decision: ContainerRuntimeDecision = (() => {
	...
	if (strategy === 'docker') {
		const dockerCandidates: readonly (string | undefined)[] = [
...
export function logMissingRuntimeWarning(scope: string): void {
	...
	switch (decision.strategy) {
		case 'docker':
			hint =
				'Ensure Docker Desktop or OrbStack is running, or set CORTEX_CONTAINER_RUNTIME to match your environment.';
			break;
		case 'colima':
		...
		default:
			hint =
				'Open Testcontainers Desktop and export TESTCONTAINERS_DAEMON_URL to point at its agent.';
			break;
	}
