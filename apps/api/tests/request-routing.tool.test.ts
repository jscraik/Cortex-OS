...
describe('ApiRequestRoutingTool', () => {
  it('resolves routes by operation identifier', async () => {
    ...
    const input: RoutingToolInput = { operationId: 'users.list', method: 'GET', path: '/users' };
  ...
  it('raises descriptive error when route cannot be found', async () => {
    ...
    await expect(env.routingTool.execute(input, env.context)).rejects.toBeInstanceOf(McpRouteNotFoundError);
