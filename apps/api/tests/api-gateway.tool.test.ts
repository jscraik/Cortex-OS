...
import { McpRateLimitError, McpRouteNotFoundError, McpSecurityError, McpValidationError } from '../src/mcp/errors.js';
...
describe('ApiGatewayTool', () => {
  ...
  it('rejects unauthenticated calls for protected routes', async () => {
    ...
    await expect(env.gatewayTool.execute(input, env.context)).rejects.toBeInstanceOf(McpSecurityError);
  });
  ...
  it('enforces rate limits per API key', async () => {
    ...
    await expect(env.gatewayTool.execute(input, env.context)).rejects.toBeInstanceOf(McpRateLimitError);
  });
  ...
  it('maps missing routes to descriptive errors', async () => {
    ...
    await expect(env.gatewayTool.execute(input, env.context)).rejects.toBeInstanceOf(McpRouteNotFoundError);
  });
});
