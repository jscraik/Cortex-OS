...
const resolveUserDelegate = (): PrismaUserDelegate | null => {
	const delegate = (prisma as unknown as Record<string, unknown>).user as
		| PrismaUserDelegate
		| undefined;
...
const resolveSessionDelegate = (): PrismaSessionDelegate | null => {
	const delegate = (prisma as unknown as Record<string, unknown>).session as
		| PrismaSessionDelegate
		| undefined;
	return delegate ?? null;
};
