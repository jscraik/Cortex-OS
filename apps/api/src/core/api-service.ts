...
import { AuditLogger, MetricsCollector, PerformanceMonitor, StructuredLogger } from './observability.js';
import { RequestRouter } from './request-router.js';
import { TransactionError, TransactionManager } from './transaction.js';
import type { GatewayRequest, GatewayResponse, InternalRequest, RouteResolution, ApiOperationMetadata, RouteDefinition } from './types.js';
...
export type ApiHandler = (request: InternalRequest, context: ApiHandlerContext) => Promise<ApiHandlerResult> | ApiHandlerResult;
...
export class ApiService {
  ...
  async execute(request: GatewayRequest): Promise<GatewayResponse> {
    ...
    if (!handler) {
      throw new ApiServiceError(`No handler registered for ${handlerKey}`, 'E_HANDLER_NOT_FOUND', { route: route.id });
    }
    ...
    try {
      ...
      if (error instanceof TransactionError || error instanceof SecurityError || error instanceof RateLimitError) {
        throw error;
      }
      this.metrics.increment('mcp.api.errors');
      this.logger.error('API handler threw unexpected error', { error: (error as Error).message, handler: handlerKey });
      throw new ApiServiceError('Internal API handler failure', 'E_HANDLER_FAILURE', { handler: handlerKey });
    }
    ...
    if (route.cacheTtlSeconds && gatewayResponse.statusCode >= 200 && gatewayResponse.statusCode < 300) {
  ...
  private createMetadata(request: GatewayRequest): ApiOperationMetadata {
    const correlationId = (request.metadata?.correlationId ?? request.headers?.['x-correlation-id']) as string | undefined;
    return {
  ...
  private createCacheKey(route: RouteDefinition, payload: unknown, headers: Record<string, string>): string {
...
export function createDefaultApiService(dependencies: Partial<ApiServiceDependencies> & Pick<ApiServiceDependencies, 'handlers'>): ApiService {
