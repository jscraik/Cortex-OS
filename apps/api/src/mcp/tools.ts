...
import { ToolExecutionError, ToolRegistry, ToolValidationError } from '@cortex-os/mcp-core';
import type { Express } from 'express';
import type { ApiBusIntegration } from '../core/a2a-integration.js';
...
import { API_TOOL_CONTRACTS } from './contracts.js';
import {
	McpApiError,
	McpHandlerError,
	McpRateLimitError,
...
export class ApiGatewayTool implements McpTool<GatewayToolInput, GatewayToolResult> {
	...
	async execute(
		...
		try {
			...
			return this.normalizeGatewayResponse(result);
		} catch (error) {
			...
			throw new McpApiError('Unexpected failure during API execution.', {
				cause: error,
			});
		}
	}
	...
	private normalizeGatewayResponse(raw: unknown): GatewayToolResult {
		const candidate = raw as Partial<GatewayToolResult> & { status?: number; headers?: unknown };
		...
		if (typeof statusCode !== 'number') {
			throw new McpApiError('Gateway response missing statusCode.', { cause: raw });
		}

		const requestId = typeof candidate.requestId === 'string' ? candidate.requestId : undefined;
		if (!requestId) {
			throw new McpApiError('Gateway response missing requestId.', { cause: raw });
		}
		...
		const headerSource =
			...
			!Array.isArray(candidate.headers)
				? Object.fromEntries(
						Object.entries(candidate.headers as Record<string, unknown>).map(([key, value]) => [
							key,
							String(value ?? ''),
						]),
					)
		...
		const normalized = {
			statusCode,
			body: candidate.body,
			headers: sanitizeHeaders(headerSource),
			durationMs,
			fromCache: candidate.fromCache ?? false,
			requestId,
			auditId: typeof candidate.auditId === 'string' ? candidate.auditId : '',
		};

		return gatewayResponseSchema.parse(normalized);
	}
}
...
export class ApiRequestRoutingTool implements McpTool<RoutingToolInput, RoutingToolResult> {
	...
	async execute(
		...
		try {
			...
			throw new McpRouteNotFoundError({
				input,
...
export class ApiResponseHandlingTool
...
export interface SetupMcpToolsResult {
	readonly registry: ToolRegistry;
	readonly contracts: readonly ToolContracts[];
}

export function setupMcpTools(app: Express, apiBus: ApiBusIntegration): SetupMcpToolsResult {
	...
	app.locals.mcp = {
		registry,
		contracts: API_TOOL_CONTRACTS,
	};

	apiBus.registerWebhookHandler('mcp.tool.audit.v1', async (event) => {
		app.locals.logger?.info?.('brAInwav MCP tool webhook received', {
			source: event.source,
			event: event.event,
	...
	return { registry, contracts: API_TOOL_CONTRACTS };
}
