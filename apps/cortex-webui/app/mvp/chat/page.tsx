...
export default function ChatPage() {
	...
	async function ensureSession() {
		...
		try {
			...
			const msg = e instanceof Error ? e.message : "Failed to load tool events";
			setError(msg);
		}
		return sid;
	}
	...
	function appendToken(token: string) {
		setMessages((xs) =>
			xs.map((m) =>
				m.id === "stream" ? { ...m, content: m.content + token } : m,
			),
		);
	}
	...
	async function sendMessage(e: React.FormEvent) {
		...
		setMessages((xs) => [
			...xs,
			userMsg,
			{ id: "stream", role: "assistant", content: "" },
		]);
		setInput("");
		setStreaming(true);
	...
	return (
		<main className="p-4 grid gap-3" aria-label="Chat interface">
			...
			<section
				className="grid md:grid-cols-[2fr_1fr] gap-3"
				aria-label="Conversation and tools"
			>
				<div
				...
				<section className="border rounded p-2" aria-labelledby={toolHeadingId}>
					<h2 id={toolHeadingId} className="text-sm font-semibold">
						Tool activity
					</h2>
					{toolEvents.length === 0 ? (
