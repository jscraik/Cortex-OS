...
const CallOverlay: React.FC<CallOverlayProps> = ({
  ...
  const transcribeHandler = async (audioBlob: Blob) => {
    ...
    const simulatedText = 'This is a simulated transcription of the spoken audio.';
  ...
  const analyseAudio = (stream: MediaStream) => {
    ...
    console.log('🔊 Sound detection started', lastSoundTime, localHasStartedSpeaking);
    ...
    return new Promise<void>((resolve) => {
      ...
      const getVoicesLoop = setInterval(async () => {
        ...
        if (voices.length > 0) {
          ...
          const voice = voices.find((v) => v.voiceURI === 'Google US English') || voices[0];
  ...
  return (
    <div className="max-w-lg w-full h-full max-h-[100dvh] flex flex-col justify-between p-3 md:p-6">
      ...
      <div className="flex justify-between items-center pb-2 w-full">
        <div>
          {camera ? (
            ...
            >
              <button className="p-3 rounded-full bg-gray-50 dark:bg-gray-900" type="button">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 20 20"
                  fill="currentColor"
        ...
        <div>
          ...
          >
            <div className="line-clamp-1 text-sm font-medium">
              {loading ? 'Thinking...' : assistantSpeaking ? 'Tap to interrupt' : 'Listening...'}
            </div>
          </button>
        </div>
      ...
      <audio id="audioElement" ref={audioElementRef} style={{ display: 'none' }} />
    </div>
  );
};
