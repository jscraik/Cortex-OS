    ...
    <div className={`bg-white rounded-lg shadow-md p-6 ${className}`}>
      ...
      <div className="space-y-4">
        {oauthProviders.map((provider) => {
          ...
          return (
            <div key={provider.id} className="flex items-center justify-between p-3 border border-gray-200 rounded-md">
              <div className="flex items-center space-x-3">
                <div
                  className="h-10 w-10 rounded-full flex items-center justify-center"
                  style={{ backgroundColor: `${provider.color}20`, border: `2px solid ${provider.color}` }}
                >
                  <span
                ...
                <div>
                  ...
                  {linked && account && (
                    <p className="text-sm text-gray-500">
                      {account.email || `Connected as ${account.accountUsername || account.accountId}`}
                    </p>
                  )}
                </div>
              </div>
