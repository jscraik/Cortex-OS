...
test.describe('brAInwav Test Environment Cleanup', () => {
  test('should cleanup test users', async ({ page }) => {
    ...
    if (adminLoginResponse.ok()) {
      ...
      if (usersResponse.ok()) {
        ...
        const testUsers = usersData.users.filter((user: any) =>
        ...
        for (const user of testUsers) {
          const deleteResponse = await page.request.delete(`http://localhost:3001/api/admin/users/${user.id}`, {
            headers: {
              'Authorization': `Bearer ${adminToken}`
            }
          });
  ...
  test('should cleanup test documents', async ({ page }) => {
    ...
    if (loginResponse.ok()) {
      ...
      if (docsResponse.ok()) {
        ...
        const testDocuments = docsData.documents.filter((doc: any) =>
        ...
        for (const doc of testDocuments) {
          const deleteResponse = await page.request.delete(`http://localhost:3001/api/documents/${doc.id}`, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
  ...
  test('should cleanup test workflows', async ({ page }) => {
    ...
    if (loginResponse.ok()) {
      ...
      if (workflowsResponse.ok()) {
        ...
        const testWorkflows = workflowsData.workflows.filter((workflow: any) =>
        ...
        for (const workflow of testWorkflows) {
          const deleteResponse = await page.request.delete(`http://localhost:3001/api/workflows/${workflow.id}`, {
            headers: {
              'Authorization': `Bearer ${token}`
            }
          });
  ...
  test('should cleanup temporary files', async ({ page }) => {
    ...
    const cleanupResponse = await page.request.post('http://localhost:3001/api/admin/cleanup-temp', {
      headers: {
  ...
  test('should verify cleanup completion', async ({ page }) => {
    ...
    if (loginResponse.ok()) {
      ...
      if (docsResponse.ok()) {
        const docsData = await docsResponse.json();
        const remainingTestDocs = docsData.documents.filter((doc: any) =>
          doc.title.includes('Test') || doc.title.includes('Setup')
        );
      ...
      if (workflowsResponse.ok()) {
        const workflowsData = await workflowsResponse.json();
        const remainingTestWorkflows = workflowsData.workflows.filter((workflow: any) =>
          workflow.name.includes('Test') || workflow.name.includes('Setup')
        );
  ...
  test('should generate cleanup report', async ({ page }) => {
    ...
    const memoryHealthResponse = await page.request.get('http://localhost:3028/api/v1/health');
    const memoryHealthData = await memoryHealthResponse.ok() ? await memoryHealthResponse.json() : {};
