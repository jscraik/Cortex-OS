...
function handleStreamingResponse(res, request, conversationId) {
...
function handleEmbeddings(req, res) {
  ...
  req.on('end', () => {
    try {
      ...
      const response = {
        object: 'list',
        data: embeddings.map((text, index) => ({
          object: 'embedding',
...
// Helper functions
function generateResponse(lastMessage, conversationHistory) {
  const responses = [
...
function generateMockEmbedding() {
  return Array.from({ length: 1536 }, () => faker.number.float({ min: -1, max: 1, precision: 0.0001 }));
}
...
function extractEntities(text) {
  return [
    ...
    { type: 'PRODUCT', text: 'Cortex-OS', confidence: 0.90 },
...
function generateMockText(minWords, maxWords) {
  ...
  const words = [
    'brAInwav', 'Cortex-OS', 'ASBR', 'runtime', 'architecture', 'testing',
    'development', 'production', 'quality', 'performance', 'accessibility',
    'workflows', 'agents', 'communication', 'events', 'API', 'integration'
  ];

  return Array.from({ length: wordCount }, () =>
    words[Math.floor(Math.random() * words.length)]
  ).join(' ').replace(/(\b\w+\b)(?=.*\b\1\b)/g, '') + '.';
}
