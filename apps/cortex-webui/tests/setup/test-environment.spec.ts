...
test.describe('brAInwav Test Environment Setup', () => {
	...
	test('should validate file upload capabilities', async ({ page }) => {
		...
		const getResponse = await page.request.get(
			`http://localhost:3001/api/documents/${docData.document.id}`,
			{
				headers: {
					Authorization: `Bearer ${token}`,
				},
	...
	test('should validate RAG system functionality', async ({ page }) => {
		...
		const ragDocument = {
			...
			content:
				'brAInwav Cortex-OS provides advanced RAG capabilities with semantic search and citation generation.',
