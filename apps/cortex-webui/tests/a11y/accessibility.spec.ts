      ...
      if (focusableElements.length > 1) {
        ...
        for (let i = 1; i < focusableElements.length; i++) {
          ...
          const focusedIndex = focusableElements.findIndex(
            async (el, index) => await el.evaluate(node => node === document.activeElement)
          );
  ...
  test.describe('Focus Management', () => {
    test('should manage focus in modal dialogs', async ({ page }) => {
      ...
      if (await modalTrigger.isVisible()) {
        ...
        if (await modal.isVisible()) {
          ...
          const firstFocusable = await modal.locator(
            'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'
          ).first();
    ...
    test('should restore focus after dynamic operations', async ({ page }) => {
      ...
      if (await trigger.isVisible()) {
        await trigger.focus();
        const triggerId = await trigger.evaluate(el => el.id || 'no-id');
    ...
    test('should provide clear form instructions', async ({ page }) => {
      ...
      for (const form of forms) {
        if (await form.isVisible()) {
          ...
          const instructions = await form.locator(
            '[aria-describedby], fieldset legend, .form-instructions'
          ).all();
