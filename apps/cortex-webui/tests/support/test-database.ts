...
export class TestDatabase {
	...
	async seed(): Promise<void> {
      ...
      `);
		...
		console.log(
			`✅ Database seeded with ${seedData.users.length} users, ${seedData.documents.length} documents`,
		);
	}
		...
		return {
			...
			messages: [
				...
				},
				{
					...
					role: 'assistant',
					content:
						'brAInwav Cortex-OS is an Autonomous Software Behavior Reasoning (ASBR) runtime...',
					metadata: {
						timestamp: new Date().toISOString(),
						model: 'claude-3-sonnet',
					},
