...
test.describe('brAInwav Cortex-OS Document Processing', () => {
	...
	test.describe('File Upload Workflows', () => {
		test('should upload PDF document successfully', async ({ page }) => {
			...
			// Select PDF file
			const _filePath = path.join(__dirname, '..', 'fixtures', 'sample-document.pdf');
			...
			await page.fill(
				...
				'Comprehensive guide to brAInwav Cortex-OS architecture',
			);
		...
		test('should validate file types and sizes', async ({ page }) => {
			...
			await expect(
				page.locator('text=Please upload PDF, image, audio, or text files'),
			).toBeVisible();
	...
	test.describe('RAG Query and Citation Validation', () => {
		...
		test('should handle multi-document queries', async ({ page }) => {
			...
			await page.fill(
				...
				'Compare brAInwav Cortex-OS with traditional AI systems and explain ASBR benefits',
			);
			await page.click('[data-testid="send-message-button"]');
	...
	test.describe('Search and Retrieval Workflows', () => {
		...
		test('should sort documents', async ({ page }) => {
			...
			const firstDate = await page
				.locator('[data-testid="document-item"]:first-child [data-testid="document-date"]')
				.textContent();
			const secondDate = await page
				.locator('[data-testid="document-item"]:nth-child(2) [data-testid="document-date"]')
				.textContent();
	...
	test.describe('Document Management', () => {
		...
		test('should export documents', async ({ page }) => {
			...
			await page.check(
				'[data-testid="document-item"]:nth-child(2) [data-testid="select-document"]',
			);
