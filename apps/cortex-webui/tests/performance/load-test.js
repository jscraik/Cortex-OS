...
function authenticateUser(user) {
	...
	check(response, {
		...
		'brAInwav auth: response time < 2s': (_r) => responseTime < 2000,
...
function testUserInterface(authToken) {
	...
	check(dashboardResponse, {
		...
		'brAInwav UI: response time < 3s': (_r) => responseTime < 3000,
...
function testAPIEndpoints(authToken) {
	...
	check(documentsResponse, {
		...
		'brAInwav API: response time < 2s': (_r) => responseTime < 2000,
	...
	check(workflowsResponse, {
		...
		'brAInwav API: workflows data structure': (r) => {
			...
			return Object.hasOwn(data, 'workflows');
		},
	});
}
...
function testDocumentWorkflows(authToken) {
	...
	if (createResponse.status === 201) {
		...
		check(processResponse, {
			...
			'brAInwav Document: processing time < 60s': (_r) => processTime < 60000,
...
function testAgentWorkflows(authToken) {
	...
	if (createResponse.status === 201) {
		...
		check(execResponse, {
			...
			'brAInwav Workflow: execution time < 30s': (_r) => execTime < 30000,
...
function testRAGQueries(authToken) {
	...
	check(ragResponse, {
		...
		'brAInwav RAG: response time < 5s': (_r) => responseTime < 5000,
...
export function teardown() {
	...
	console.log(
		`Workflow Execution Time (p95): ${brAInwavMetrics.workflowExecutionTime.p(95).toFixed(2)}ms`,
	);
	console.log(
		`Document Processing Time (p95): ${brAInwavMetrics.documentProcessingTime.p(95).toFixed(2)}ms`,
	);

	console.log('\nSuccess Rates:');
	console.log(`API Success Rate: ${(brAInwavMetrics.apiSuccessRate.rate * 100).toFixed(2)}%`);
	console.log(
		`Workflow Success Rate: ${(brAInwavMetrics.workflowSuccessRate.rate * 100).toFixed(2)}%`,
	);
	console.log(
		`Document Upload Success Rate: ${(brAInwavMetrics.documentUploadSuccessRate.rate * 100).toFixed(2)}%`,
	);
