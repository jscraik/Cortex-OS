...
import { createTestDatabase, clearDatabase } from '../../test/database';
...
describe('Authentication Flow Integration', () => {
  ...
  describe('Complete User Registration Flow', () => {
    it('should register, verify email, and login successfully', async () => {
      ...
      const dbUser = await db.select().from(schema.user).where(
  ...
  describe('Session Management Flow', () => {
    it('should create, use, and invalidate session', async () => {
      ...
      const dbSession = await db.select().from(schema.session).where(
        (session) => session.sessionToken.equals(sessionToken)
      ).limit(1);
      ...
      const deletedSession = await db.select().from(schema.session).where(
        (session) => session.sessionToken.equals(sessionToken)
      ).limit(1);
  ...
  describe('OAuth Integration Flow', () => {
    it('should handle complete OAuth flow with GitHub', async () => {
      ...
      vi.spyOn(betterAuthInstance.api, 'handleOAuthCallback').mockImplementationOnce(
        async ({ provider, code }) => {
          if (provider === 'github' && code === 'test-code') {
            ...
            let user = await db.select().from(schema.user).where(
              (u) => u.email.equals('github@example.com')
            ).limit(1);
      ...
      const createdUser = await db.select().from(schema.user).where(
        (u) => u.email.equals('github@example.com')
      ).limit(1);
  ...
  describe('Password Reset Flow', () => {
    it('should handle complete password reset flow', async () => {
      ...
      const verification = await db.select().from(schema.verification).where(
        (v) => v.identifier.equals('reset@example.com')
      ).limit(1);
  ...
  describe('Cross-Platform Session Management', () => {
    ...
    it('should handle concurrent sessions', async () => {
      ...
      const dbSessions = await db.select().from(schema.session).where(
