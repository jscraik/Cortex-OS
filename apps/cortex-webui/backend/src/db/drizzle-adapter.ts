    ...
    return {
      values: (values: any) => ({
        returning: async () => {
          ...
          await this.db.run(
            `INSERT INTO ${tableName} (id, ${Object.keys(values).filter(k => k !== 'id').join(', ')}) VALUES (?, ${Object.keys(values).filter(k => k !== 'id').map(() => '?').join(', ')})`,
            id, ...Object.values(values).filter((v: any, i: number) => i !== 0 || !values.id)
          );
          return [{ id, ...values }];
        }
      })
          ...
          returning: async () => {
            ...
            for (const record of results) {
              ...
              await this.db.run(
                ...
                ...Object.values(values), record.id
              );
            }

            // Return updated records
            const updatedResults = await this.db.all(`SELECT * FROM ${tableName}`);
            return updatedResults.filter(row => conditions.some(cond => row[cond.field] === cond.value));
          }
        })
      })
    };
