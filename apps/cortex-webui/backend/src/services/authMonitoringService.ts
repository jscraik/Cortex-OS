...
interface AuthEvent {
  id: string;
  userId?: string;
  eventType: 'login' | 'logout' | 'register' | 'password_reset' | 'email_verified' | '2fa_enabled' | '2fa_disabled' | 'oauth_signin' | 'failed_login';
  ipAddress?: string;
  userAgent?: string;
...
export class AuthMonitoringService {
  ...
  async logOAuthSignIn(userId: string, provider: string, ipAddress?: string, userAgent?: string): Promise<void> {
    await this.logEvent({
      userId,
