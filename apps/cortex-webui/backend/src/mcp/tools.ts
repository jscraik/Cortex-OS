...
export interface PanelService { open(input: OpenPanelInput): Promise<unknown>; }
export interface ComponentStateService { update(input: UpdateComponentStateInput): Promise<unknown>; }
export interface NavigationService { navigate(input: NavigateInput): Promise<unknown>; }
export interface InteractionService { click(input: SimulateClickInput): Promise<unknown>; }
export interface FormService { submit(input: SubmitFormInput): Promise<unknown>; }
export interface ChatService { send(input: SendChatMessageInput): Promise<unknown>; }
export interface ChartService { render(input: RenderChartInput): Promise<unknown>; }
export interface TimelineService { generate(input: GenerateTimelineInput): Promise<unknown>; }
export interface TreeService { render(input: RenderTreeInput): Promise<unknown>; }
...
const defaultServices: WebuiMcpServices = {
  ...
  componentState: { update: async (i) => ({ updated: true, componentId: i.componentId, path: i.path }) },
  ...
  chat: { send: async (i) => ({ queued: true, messageId: `msg_${Date.now()}`, role: i.role }) },
  chart: { render: async (i) => ({ chartId: i.chartId, status: 'scheduled' }) },
  timeline: { generate: async (i) => ({ timelineId: i.timelineId, events: i.events.length }) },
  tree: { render: async (i) => ({ treeId: i.treeId, nodes: countNodes(i.root) }) },
};
...
async function executeTool(tool: string, args: unknown, services: WebuiMcpServices = defaultServices) {
  switch (tool) {
    case 'open_panel': {
...
export function listWebuiMcpTools() {
  return webuiMcpTools.map((t: { name: string; description: string }) => ({ name: t.name, description: t.description }));
}
