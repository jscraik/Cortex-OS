...
impl CliTestHelper {
    ...
    pub fn new() -> Self {
        ...
        // Create a basic test configuration (JSON-based schema)
        ...
        fs::write(&config_path, serde_json::to_string_pretty(&test_config).unwrap()).expect("Failed to write test config");
...
mod error_tests {
    ...
    fs::write(&invalid_config, "{ invalid json" ).unwrap();
...
mod feature_tests {
    ...
    fn test_feature_disabled_behavior() {
        ...
        // Create config with features disabled (JSON)
        let mut disabled = serde_json::from_str::<serde_json::Value>(
            &fs::read_to_string(helper.temp_path().join("cortex.json")).unwrap()
        ).unwrap();
        // Disable some features for this test
        if let Some(obj) = disabled.get_mut("features").and_then(|v| v.as_object_mut()) {
            if let Some(streaming) = obj.get_mut("streaming").and_then(|v| v.as_object_mut()) {
                streaming.insert("enabled".to_string(), serde_json::json!(false));
            }
            if let Some(tui) = obj.get_mut("tui").and_then(|v| v.as_object_mut()) {
                tui.insert("enabled".to_string(), serde_json::json!(false));
            }
        }

        let config_path = helper.temp_path().join("disabled_features.json");
        fs::write(&config_path, serde_json::to_string_pretty(&disabled).unwrap()).unwrap();
