...
pub type Result<T> = std::result::Result<T, CortexErr>;
...
pub enum CortexErr {
    ...
    /// could not be captured). Analogous to the previous `CortexError::Spawn` variant.
    ...
    #[error(
        "To use Cortex with your ChatGPT plan, upgrade to Plus: https://openai.com/chatgpt/pricing."
    )]
    UsageNotIncluded,
    ...
    #[error("cortex-linux-sandbox was required but not provided")]
...
impl CortexErr {
    /// Minimal shim so that existing `e.downcast_ref::<CortexErr>()` checks continue to compile
...
pub fn get_error_message_ui(e: &CortexErr) -> String {
    match e {
        CortexErr::Sandbox(SandboxErr::Denied(_, _, stderr)) => stderr.to_string(),
        // Timeouts are not sandbox errors from a UX perspective; present them plainly
        CortexErr::Sandbox(SandboxErr::Timeout) => "error: command timed out".to_string(),
        _ => e.to_string(),
    }
}
