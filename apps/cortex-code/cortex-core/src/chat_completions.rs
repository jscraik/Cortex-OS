...
/// output is mapped onto Cortex's internal [`ResponseEvent`] so that the rest
    ...
    S: Stream<Item = Result<Bytes>> + Unpin,
{
    ...
    loop {
        let sse = match timeout(idle_timeout, stream.next()).await {
            ...
            Ok(Some(Err(e))) => {
                let _ = tx_event
                    .send(Err(CortexErr::Stream(e.to_string(), None)))
                    .await;
                return;
            }
            Ok(None) => {
            ...
            match Pin::new(&mut this.inner).poll_next(cx) {
                ...
                Poll::Ready(Some(Ok(ResponseEvent::OutputItemDone(item)))) => {
                    ...
                    let is_assistant_message = matches!(
                        &item,
                        cortex_protocol::models::ResponseItem::Message { role, .. } if role == "assistant"
                    );
                        ...
                        match this.mode {
                            AggregateMode::AggregatedOnly => {
                                ...
                                if this.cumulative.is_empty()
                                    && let cortex_protocol::models::ResponseItem::Message {
                                        content,
                                        ..
                                    } = &item
                                    && let Some(text) = content.iter().find_map(|c| match c {
                                        cortex_protocol::models::ContentItem::OutputText {
                                            text,
                                        } => Some(text),
                                        _ => None,
                                    })
                ...
                Poll::Ready(Some(Ok(ResponseEvent::Completed {
                        ...
                        && matches!(this.mode, AggregateMode::AggregatedOnly)
                    {
                        let aggregated_reasoning =
                            cortex_protocol::models::ResponseItem::Reasoning {
                                id: String::new(),
                                summary: Vec::new(),
                                content: Some(vec![
                                    cortex_protocol::models::ReasoningItemContent::ReasoningText {
                    ...
                    if !this.cumulative.is_empty() {
                        let aggregated_message = cortex_protocol::models::ResponseItem::Message {
                            id: None,
                            role: "assistant".to_string(),
                            content: vec![cortex_protocol::models::ContentItem::OutputText {
