...
use crate::user_agent::get_cortex_user_agent;
...
impl ModelClient {
    ...
    async fn stream_responses(&self, prompt: &Prompt) -> Result<ResponseStream> {
        ...
        loop {
            ...
            req_builder = req_builder.header("User-Agent", get_cortex_user_agent(Some(originator)));
            ...
            match res {
                Ok(resp) if resp.status().is_success() => {
                    ...
                    let stream = resp.bytes_stream().map_err(CortexErr::Reqwest);
                ...
                Ok(res) => {
                        ...
                        || status.is_server_error())
                    {
                        ...
                        return Err(CortexErr::UnexpectedStatus(status, body));
                    }
                    ...
                    if status == StatusCode::TOO_MANY_REQUESTS {
                        ...
                        if let Some(ErrorResponse { error }) = body {
                            if error.r#type.as_deref() == Some("usage_limit_reached") {
                                ...
                                return Err(CortexErr::UsageLimitReached(UsageLimitReachedError {
                                    plan_type,
                                    resets_in_seconds,
                                }));
                            } else if error.r#type.as_deref() == Some("usage_not_included") {
                                return Err(CortexErr::UsageNotIncluded);
                            }
                        }
                    }
                    ...
                    if attempt > max_retries {
                        if status == StatusCode::INTERNAL_SERVER_ERROR {
                            return Err(CortexErr::InternalServerError);
                        }

                        return Err(CortexErr::RetryLimit(status));
                    }
    ...
    S: Stream<Item = Result<Bytes>> + Unpin,
{
    ...
    let mut response_error: Option<CortexErr> = None;
    ...
    loop {
        let sse = match timeout(idle_timeout, stream.next()).await {
            ...
            Ok(Some(Err(e))) => {
                debug!("SSE Error: {e:#}");
                let event = CortexErr::Stream(e.to_string(), None);
            ...
            Ok(None) => {
                match response_completed {
                    ...
                    None => {
                        let _ = tx_event
                            .send(Err(response_error.unwrap_or(CortexErr::Stream(
        ...
        match event.kind.as_str() {
            ...
            "response.failed" => {
                if let Some(resp_val) = event.response {
                    response_error = Some(CortexErr::Stream(
                    ...
                    if let Some(error) = error {
                        match serde_json::from_value::<Error>(error.clone()) {
                            Ok(error) => {
                                ...
                                response_error = Some(CortexErr::Stream(message, delay));
                            }
                            Err(e) => {
...
async fn stream_from_fixture(
    ...
    let stream = ReaderStream::new(rdr).map_err(CortexErr::Io);
    tokio::spawn(process_sse(
        stream,
        tx_event,
...
mod tests {
    ...
    async fn collect_events(
        ...
        let stream = ReaderStream::new(reader).map_err(CortexErr::Io);
    ...
    async fn run_sse(
        ...
        let stream = ReaderStream::new(std::io::Cursor::new(body)).map_err(CortexErr::Io);
        tokio::spawn(process_sse(stream, tx, provider.stream_idle_timeout()));
    ...
    async fn error_when_error_event() {
        ...
        match &events[0] {
            Err(CortexErr::Stream(msg, delay)) => {
                assert_eq!(
                    msg,
