...
use cortex_apply_patch::ApplyPatchAction;
use cortex_apply_patch::MaybeApplyPatchVerified;
use cortex_apply_patch::maybe_parse_apply_patch_verified;
use cortex_protocol::protocol::ConversationHistoryResponseEvent;
use cortex_protocol::protocol::TaskStartedEvent;
use cortex_protocol::protocol::TurnAbortReason;
use cortex_protocol::protocol::TurnAbortedEvent;
use futures::prelude::*;
...
use cortex_protocol::config_types::ReasoningEffort as ReasoningEffortConfig;
use cortex_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use cortex_protocol::custom_prompts::CustomPrompt;
use cortex_protocol::models::ContentItem;
use cortex_protocol::models::FunctionCallOutputPayload;
use cortex_protocol::models::LocalShellAction;
use cortex_protocol::models::ReasoningItemContent;
use cortex_protocol::models::ReasoningItemReasoningSummary;
use cortex_protocol::models::ResponseInputItem;
use cortex_protocol::models::ResponseItem;
use cortex_protocol::models::ShellToolCallParams;
use cortex_protocol::models::WebSearchAction;
...
/// The high-level interface to the Cortex system.
...
/// Wrapper returned by [`Cortex::spawn`] containing the spawned [`Cortex`],
/// the submission id for the initial `ConfigureSession` request and the
/// unique session id.
pub struct CortexSpawnOk {
    pub cortex: Cortex,
    pub session_id: Uuid,
}
...
impl Cortex {
    /// Spawn a new [`Cortex`] and initialize the session.
    pub async fn spawn(
        config: Config,
        auth_manager: Arc<AuthManager>,
        conversation_history: InitialHistory,
    ) -> CortexResult<CortexSpawnOk> {
        ...
        // Generate a unique ID for the lifetime of this Cortex session.
        ...
        Ok(CortexSpawnOk { cortex, session_id })
    }
    ...
    /// Use sparingly: prefer `submit()` so Cortex is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, sub: Submission) -> CortexResult<()> {
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| CortexErr::InternalAgentDied)?;
        Ok(())
    }
...
impl Session {
    async fn new(
        ...
        let sess = Arc::new(Session {
            ...
            cortex_linux_sandbox_exe: config.cortex_linux_sandbox_exe.clone(),
    ...
    async fn run_exec_with_events<'a>(
        ...
        let result = process_exec_tool_call(
            ...
            exec_args.cortex_linux_sandbox_exe,
...
async fn submission_loop(
    ...
    while let Ok(sub) = rx_sub.recv().await {
        ...
        match sub.op {
                        ...
                        msg: EventMsg::GetHistoryEntryResponse(
                            crate::protocol::GetHistoryEntryResponseEvent {
                                ...
                                entry: entry_opt.map(|e| {
                                    cortex_protocol::message_history::HistoryEntry {
            ...
            Op::Shutdown => {
                info!("Shutting down Cortex instance");
...
async fn run_task(
    ...
    // Although from the perspective of cortex.rs, TurnDiffTracker has the lifecycle of a Task which contains
...
async fn run_turn(
...
) -> CortexResult<Vec<ProcessedResponseItem>> {
    ...
    loop {
        match try_run_turn(sess, turn_context, turn_diff_tracker, &sub_id, &prompt).await {
            ...
            Err(CortexErr::Interrupted) => return Err(CortexErr::Interrupted),
            Err(CortexErr::EnvVar(var)) => return Err(CortexErr::EnvVar(var)),
            Err(e @ (CortexErr::UsageLimitReached(_) | CortexErr::UsageNotIncluded)) => {
                return Err(e);
            }
            Err(e) => {
                ...
                if retries < max_retries {
                    retries += 1;
                    let delay = match e {
                        CortexErr::Stream(_, Some(delay)) => delay,
                        _ => backoff(retries),
                    };
                    warn!(
...
async fn run_compact_task(
    ...
    loop {
        ...
        match attempt_result {
            Ok(()) => break,
            Err(CortexErr::Interrupted) => return,
            Err(e) => {
                if retries < max_retries {
                    retries += 1;
...
async fn handle_response_item(
    ...
    sub_id: &str,
    item: ResponseItem,
) -> CortexResult<Option<ResponseInputItem>> {
...
pub struct ExecInvokeArgs<'a> {
    ...
    pub cortex_linux_sandbox_exe: &'a Option<PathBuf>,
...
async fn handle_container_exec_with_params(
    ...
    let (params, safety, command_for_display) = match &apply_patch_exec {
        Some(ApplyPatchExec {
            ...
            let path_to_cortex = std::env::current_exe()
                .ok()
                .map(|p| p.to_string_lossy().to_string());
            let Some(path_to_cortex) = path_to_cortex else {
                return ResponseInputItem::FunctionCallOutput {
                    call_id,
                    output: FunctionCallOutputPayload {
                        content: "failed to determine path to cortex executable".to_string(),
                        success: None,
                    },
                };
            };
    ...
    let output_result = sess
        .run_exec_with_events(
            ...
            ExecInvokeArgs {
                ...
                cortex_linux_sandbox_exe: &sess.cortex_linux_sandbox_exe,
