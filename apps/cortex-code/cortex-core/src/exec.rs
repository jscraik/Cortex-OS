...
pub async fn process_exec_tool_call(
    ...
    let raw_output_result: std::result::Result<RawExecToolCallOutput, CortexErr> = match sandbox_type
    {
        ...
        SandboxType::LinuxSeccomp => {
            ...
            let cortex_linux_sandbox_exe = cortex_linux_sandbox_exe
                .as_ref()
                .ok_or(CortexErr::LandlockSandboxExecutableNotProvided)?;
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            match raw_output.exit_status.signal() {
                Some(TIMEOUT_CODE) => return Err(CortexErr::Sandbox(SandboxErr::Timeout)),
                Some(signal) => {
                    return Err(CortexErr::Sandbox(SandboxErr::Signal(signal)));
                }
                None => {}
            }
            ...
            if exit_code != 0 && is_likely_sandbox_denied(sandbox_type, exit_code) {
                return Err(CortexErr::Sandbox(SandboxErr::Denied(
                    exit_code,
                    stdout.text,
                    stderr.text,
                )));
