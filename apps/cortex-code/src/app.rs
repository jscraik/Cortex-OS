...
use crate::model_picker::{ModelPicker, SelectionCriteria, TaskType, infer_task_type};
...
pub struct CortexApp {
    ...
    model_picker: Option<Arc<ModelPicker>>,
}
...
impl CortexApp {
    pub async fn new(config: Config) -> Result<Self> {
        ...
        let model_picker = match ModelPicker::new(config.clone()) {
            Ok(picker) => Some(Arc::new(picker)),
            Err(e) => {
                tracing::warn!("Failed to initialize model picker: {}. Auto-selection disabled.", e);
                None
            }
        };
    ...
    pub async fn get_ai_response(&mut self, prompt: &str) -> Result<String> {
        ...
        if std::env::var("CORTEX_AUTOPICK").unwrap_or("false".to_string()) == "true" {
            if let Some(response) = self.get_ai_response_with_autopick(prompt).await? {
                return Ok(response);
            }
        }
    ...
    pub async fn get_ai_response_with_autopick(&mut self, prompt: &str) -> Result<Option<String>> {
        let picker = match &self.model_picker {
            Some(picker) => picker,
            None => return Ok(None), // Auto-pick not available
        };

        // Use optimized criteria for coding AI CLI
        let criteria = if prompt.len() < 100 {
            ModelPicker::create_quick_criteria()
        } else {
            ModelPicker::create_coding_criteria_with_cloud()
        };

        match picker.select_best_model(criteria) {
            Ok(selected) => {
                tracing::info!("Auto-selected model: {} ({})", selected.name, selected.selection_reason);

                // Switch to the selected model/provider if different
                if selected.provider != self.get_current_provider_name().await {
                    if let Err(e) = self.switch_to_provider(&selected.provider, Some(&selected.name)).await {
                        tracing::warn!("Failed to switch to auto-selected provider {}: {}. Using current provider.", selected.provider, e);
                        return Ok(None);
                    }
                }
            ...
            Err(e) => {
                tracing::warn!("Model auto-selection failed: {}. Using current provider.", e);
                Ok(None)
            }
        }
    }

    pub async fn switch_to_provider(&mut self, provider_name: &str, model_name: Option<&str>) -> Result<()> {
        // Update the config with the new provider
        self.config.providers.default = provider_name.to_string();

        // If model name is provided, set it as environment variable for the provider
        ...
        *self.provider.lock().await = new_provider;
        ...
        tracing::info!("Switched to provider: {}", provider_name);
        Ok(())
    }
    ...
    pub fn list_available_models(&self) -> Result<Vec<(String, String, Vec<TaskType>)>> {
        match &self.model_picker {
            Some(picker) => Ok(picker.list_available_models()),
            None => Ok(Vec::new()),
        }
    }

    pub fn get_model_info(&self, model_name: &str) -> Option<(String, crate::model_picker::ModelCapabilities)> {
        self.model_picker.as_ref()
            .and_then(|picker| picker.get_model_info(model_name))
    }
