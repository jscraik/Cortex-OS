...
use cortex_code::{
    ...
    config::Config,
    error_panic_handler,
    model_picker::{ModelPicker, SelectionCriteria, TaskType, infer_task_type},
    view::{
        ...
        if input.starts_with('/') {
            ...
            match cmd {
                ...
                "/models" => {
                    ...
                    match ModelPicker::new(app.get_config().clone()) {
                        Ok(picker) => {
                            let models = picker.list_available_models();
                            println!("Available models:");
                            for (name, provider, tasks) in models {
                                let task_names: Vec<String> = tasks.iter().map(|t| format!("{:?}", t)).collect();
                                println!("  {} ({}) - Tasks: {}", name, provider, task_names.join(", "));
                            }
                        }
                        Err(e) => eprintln!("Failed to load model picker: {}", e),
                    }
                }
                "/model" => {
                    if let Some(model_name) = parts.next() {
                        match ModelPicker::new(app.get_config().clone()) {
                            Ok(picker) => {
                                if let Some((provider, caps)) = picker.get_model_info(model_name) {
                                    println!("Model: {} ({})", model_name, provider);
                                    println!("  Memory: {:.1} GB", caps.memory_gb);
                                    println!("  Context: {} tokens", caps.context_window);
                                    println!("  Quality: {:.2}", caps.quality_score);
                                    println!("  Speed: {:.2}", caps.speed_score);
                                    let task_names: Vec<String> = caps.task_types.iter().map(|t| format!("{:?}", t)).collect();
                                    println!("  Tasks: {}", task_names.join(", "));

                                    // Actually switch to this model and provider
                                    match app.switch_to_provider(&provider, Some(model_name)).await {
                                        Ok(()) => println!("✅ Switched to {} ({})", model_name, provider),
                                        Err(e) => eprintln!("❌ Failed to switch to {}: {}", model_name, e),
                                    }
                                } else {
                ...
                "/autopick" => {
                    if let Some(setting) = parts.next() {
                        match setting {
                            "on" => {
                                std::env::set_var("CORTEX_AUTOPICK", "true");
                                println!("Auto-pick enabled - models will be automatically selected based on task");
                            }
                            "off" => {
                                std::env::set_var("CORTEX_AUTOPICK", "false");
                                println!("Auto-pick disabled - using default provider");
                            }
                            _ => println!("Usage: /autopick <on|off>"),
                        }
                    } else {
                        let enabled = std::env::var("CORTEX_AUTOPICK").unwrap_or("false".to_string()) == "true";
                        println!("Auto-pick is {}", if enabled { "enabled" } else { "disabled" });
                    }
                }
                "/verify" => {
                    // Verify all models
                    match ModelPicker::new(app.get_config().clone()) {
                        Ok(picker) => {
                            ...
                            match picker.generate_health_report().await {
                                Ok(report) => println!("{}", report),
                                Err(e) => eprintln!("Failed to generate health report: {}", e),
                            }
                        }
                        Err(e) => eprintln!("Failed to load model picker: {}", e),
                    }
                ...
                "/check" => {
                    if let Some(model_name) = parts.next() {
                        match ModelPicker::new(app.get_config().clone()) {
                            Ok(picker) => {
                                let is_available = picker.is_model_available(model_name).await;
                                if is_available {
                                    println!("✅ Model '{}' is installed and working", model_name);
                                } else {
                                    println!("❌ Model '{}' is not available or not working", model_name);
                                }
                            }
                            Err(e) => eprintln!("Failed to load model picker: {}", e),
                        }
                    } else {
                        println!("Usage: /check <model_name>");
                    }
                }
                "/working" => {
                    // List only working models
                    match ModelPicker::new(app.get_config().clone()) {
                        Ok(picker) => {
                            match picker.verify_all_models().await {
                                Ok(health_status) => {
                                    println!("Working models:");
                                    for (name, is_working) in health_status {
                                        if is_working {
                                            println!("  ✅ {}", name);
                                        }
                                    }
                                }
                                Err(e) => eprintln!("Failed to verify models: {}", e),
                            }
                        }
                        Err(e) => eprintln!("Failed to load model picker: {}", e),
                    }
