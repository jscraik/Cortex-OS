...
pub async fn run_main(
    ...
    let overrides = ConfigOverrides {
        ...
        stream_mode_override: compute_stream_mode_override(&cli),
        model_reasoning_effort: cli.reasoning_effort,
        model_reasoning_summary: cli.reasoning_summary,
        model_verbosity: cli.verbosity,
    };
...
fn compute_stream_mode_override(cli: &Cli) -> Option<codex_core::config::StreamMode> {
    if let Some(mode) = cli.stream_mode {
        return Some(mode);
    }
    if cli.aggregate {
        return Some(codex_core::config::StreamMode::Aggregate);
    }
    if cli.raw {
        return Some(codex_core::config::StreamMode::Raw);
    }
    if cli.json_stream {
        return Some(codex_core::config::StreamMode::Json);
    }
    None
}
...
mod stream_mode_tests {
    ...
    #[test]
    fn compute_override_from_stream_mode_flag() {
        let cli = parse(&["--stream-mode", "raw"]);
        assert!(matches!(
            super::compute_stream_mode_override(&cli),
            Some(codex_core::config::StreamMode::Raw)
        ));
    }

    #[test]
    fn compute_override_from_aggregate() {
        let cli = parse(&["--aggregate"]);
        assert!(matches!(
            super::compute_stream_mode_override(&cli),
    ...
    #[test]
    fn compute_override_from_json_stream() {
        let cli = parse(&["--json-stream"]);
        assert!(matches!(
            super::compute_stream_mode_override(&cli),
