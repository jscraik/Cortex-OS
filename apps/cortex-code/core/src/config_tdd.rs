...
impl SimpleConfig {
        ...
        match fs::read_to_string(path) {
            Ok(content) => {
                let config: SimpleConfig =
                    toml::from_str(&content).map_err(|e| ConfigError::InvalidToml(e))?;
            ...
            if let Some(profile_config) = profiles.get(profile) {
                ...
                let config = toml::Value::Table(
                    profile_config
                        .as_table()
                        .ok_or_else(|| {
                            ConfigError::Validation(format!("Profile '{}' is not a table", profile))
                        })?
                        .clone(),
                )
    ...
    fn apply_override(config: &mut SimpleConfig, override_str: &str) -> Result<(), ConfigError> {
        ...
        if parts.len() != 2 {
            return Err(ConfigError::MalformedOverride(format!(
        ...
        match key {
            ...
            "model.temperature" => {
                config.model.temperature = value.parse().map_err(|_| {
                    ConfigError::MalformedOverride(format!("Invalid temperature: {}", value))
                })?;
            }
            "api.timeout_seconds" => {
                let parsed_value: i64 = value.parse().map_err(|_| {
                    ConfigError::MalformedOverride(format!("Invalid timeout: {}", value))
                })?;
                if parsed_value <= 0 {
                    return Err(ConfigError::Validation(
                        "API timeout must be positive".to_string(),
                    ));
                }
                config.api.timeout_seconds = parsed_value as u64;
            }
            "api.rate_limit.requests_per_minute" => {
                config.api.rate_limit.requests_per_minute = value.parse().map_err(|_| {
                    ConfigError::MalformedOverride(format!("Invalid rate limit: {}", value))
                })?;
            }
            "logging.level" => {
    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        ...
        if self.api.timeout_seconds <= 0 || self.api.timeout_seconds > 300 {
            return Err(ConfigError::Validation(
                "API timeout must be between 1 and 300 seconds".to_string(),
            ));
        }
        ...
        if self.model.temperature < 0.0 || self.model.temperature >= 2.0 {
            return Err(ConfigError::Validation(
                "Model temperature must be between 0.0 and 2.0 (exclusive)".to_string(),
            ));
        }
    ...
    pub fn to_toml_string(&self) -> Result<String, ConfigError> {
        toml::to_string(self).map_err(|e| ConfigError::Serialization(e))
    }
