...
impl CodexAuth {
    ...
    pub fn from_codex_home(
        ...
        // Allow tests or specific runtimes (e.g., MCP server in tests) to
        // disable reading OPENAI_API_KEY from the parent environment to avoid
        // flaky behavior across environments.
        let include_env = std::env::var("CODEX_MCP_IGNORE_ENV_API_KEY").is_err();
        load_auth(codex_home, include_env, preferred_auth_method)
    }
