...
pub struct ConfigOverrides {
    ...
    /// Optional overrides for model reasoning and verbosity parameters (Responses API)
    pub model_reasoning_effort: Option<ReasoningEffort>,
    pub model_reasoning_summary: Option<ReasoningSummary>,
    pub model_verbosity: Option<Verbosity>,
}
...
impl Config {
    ...
    pub fn load_from_base_config_with_overrides(
        ...
        let ConfigOverrides {
            ...
            model_reasoning_effort: override_reasoning_effort,
            model_reasoning_summary: override_reasoning_summary,
...
mod tests {
    ...
    use crate::client_common::{create_reasoning_param_for_request, create_text_param_for_request, OpenAiVerbosity};
...
"#;
    ...
    #[test]
    fn test_model_provider_override_selects_oss() -> std::io::Result<()> {
        // Minimal config TOML
        let toml_raw = r#"
model = "gpt-5"
approval_policy = "untrusted"
"#;
        let cfg: ConfigToml = toml::from_str(toml_raw).expect("toml");
        ...
        assert_eq!(config.model_provider.name, "gpt-oss");
        Ok(())
    }

    #[test]
    fn test_reasoning_and_verbosity_overrides_propagate() -> std::io::Result<()> {
        // Base config: gpt-5 supports reasoning summaries and text verbosity
        let toml_raw = r#"
model = "gpt-5"
approval_policy = "never"
"#;
        ...
        // Check values on Config
        assert_eq!(config.model_reasoning_effort, ReasoningEffort::High);
        assert_eq!(config.model_reasoning_summary, ReasoningSummary::Concise);
        assert_eq!(config.model_verbosity, Some(Verbosity::High));

        // Check request parameter builders
        let reasoning = create_reasoning_param_for_request(
            &config.model_family,
            config.model_reasoning_effort,
            config.model_reasoning_summary,
        );
        assert!(reasoning.is_some(), "gpt-5 should include reasoning param");

        let text = create_text_param_for_request(config.model_verbosity);
        let Some(text) = text else { panic!("expected text controls") };
        assert!(matches!(text.verbosity, Some(OpenAiVerbosity::High)));
        Ok(())
    }

    #[test]
    fn test_reasoning_omitted_when_not_supported() -> std::io::Result<()> {
        let toml_raw = r#"
model = "gpt-4.1"
approval_policy = "never"
"#;
        ...
        assert!(reasoning.is_none(), "gpt-4.1 should omit reasoning param");
        Ok(())
    }
