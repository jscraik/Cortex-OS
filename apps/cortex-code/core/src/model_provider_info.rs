...
env_http_headers = { "X-Example-Env-Header" = "EXAMPLE_ENV_VAR" }
    ...
    #[test]
    fn built_in_providers_contain_expected_entries() {
        let map = built_in_model_providers();
        assert!(map.contains_key("openai"), "missing openai provider");
        assert!(map.contains_key(BUILT_IN_OSS_MODEL_PROVIDER_ID), "missing oss provider");
        assert!(map.contains_key("anthropic"), "missing anthropic provider");
        assert!(map.contains_key("zai"), "missing zai provider");

        // spot-check wire_api assignments
        assert_eq!(map.get("openai").unwrap().wire_api, WireApi::Responses);
        assert_eq!(map.get(BUILT_IN_OSS_MODEL_PROVIDER_ID).unwrap().wire_api, WireApi::Chat);
    }
}
