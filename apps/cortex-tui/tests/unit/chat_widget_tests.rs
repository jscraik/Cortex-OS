...
use cortex_tui::view::chat::{ChatState, ChatWidget};
...
fn test_chat_widget_renders_empty_state() {
    ...
    assert!(buffer
        .content()
        .iter()
        .any(|cell| cell.symbol().contains("No messages")));
}
...
fn test_chat_widget_renders_messages() {
    ...
    assert!(buffer
        .content()
        .iter()
        .any(|cell| cell.symbol().contains("Hello, Cortex!")));
    // Verify assistant message is displayed
    assert!(buffer
        .content()
        .iter()
        .any(|cell| cell.symbol().contains("Hi there!")));
}
...
use cortex_tui::view::chat::{EventResponse, FocusElement, Theme};
