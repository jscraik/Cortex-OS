export const validateNeo4jInput = {
    ...
    nodeId(id) {
        ...
        if (id.length === 0 || id.length > 255) {
            return {
                success: false,
                error: 'Node ID must be between 1 and 255 characters',
            };
        }
    ...
    label(label) {
        ...
        if (!/^[a-zA-Z]\w*$/.test(label)) {
            return {
                success: false,
                error: 'Label must start with a letter and contain only alphanumeric characters and underscores',
            };
        }
    ...
    type(type) {
        ...
        if (type.length === 0 || type.length > 100) {
            return {
                success: false,
                error: 'Relationship type must be between 1 and 100 characters',
            };
        }
        ...
        if (!/^[A-Z][A-Z0-9_]*$/.test(type)) {
            return {
                success: false,
                error: 'Relationship type must start with an uppercase letter and contain only uppercase alphanumeric characters and underscores',
            };
        }
    ...
    propertyKey(key) {
        ...
        if (key.length === 0 || key.length > 100) {
            return {
                success: false,
                error: 'Property key must be between 1 and 100 characters',
            };
        }
        ...
        if (!/^[a-zA-Z][\w.]*$/.test(key)) {
            return {
                success: false,
                error: 'Property key must start with a letter and contain only alphanumeric characters, underscores, and dots',
            };
        }
    ...
    cypherFragment(fragment) {
        ...
        for (const pattern of dangerousPatterns) {
            if (pattern.test(fragment)) {
                return {
                    success: false,
                    error: 'Cypher fragment contains potentially dangerous patterns',
                };
            }
        }
