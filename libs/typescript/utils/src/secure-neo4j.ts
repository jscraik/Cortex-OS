...
export class SecureNeo4j {
  ...
  async upsertNode(node: { id: string; label: string; props: Record<string, any> }) {
    // Validate inputs
  ...
  async upsertRel(rel: { from: string; to: string; type: string; props?: Record<string, any> }) {
    // Validate inputs
  ...
  async neighborhood(nodeId: string, depth: number = 2) {
    // Validate inputs
  ...
  private validateProperties(props: Record<string, any>) {
