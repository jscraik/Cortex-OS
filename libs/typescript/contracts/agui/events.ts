...
export const UiComponentRenderedEventSchema = z.object({
    ...
    type: z.enum(['button', 'form', 'modal', 'chart', 'table', 'custom']).describe('Type of UI component'),
...
export const UserInteractionEventSchema = z.object({
    ...
    value: z.unknown().optional().describe('Value associated with the interaction (e.g., input text)'),
    coordinates: z
        .object({
...
export const AiRecommendationEventSchema = z.object({
    ...
    type: z.enum(['layout', 'accessibility', 'performance', 'ux', 'content']).describe('Category of recommendation'),
...
export const UiStateChangedEventSchema = z.object({
    ...
    componentId: z.string().optional().describe('ID of component whose state changed (if applicable)'),
    previousState: z.record(z.unknown()).describe('Previous state values'),
    newState: z.record(z.unknown()).describe('New state values'),
    trigger: z.enum(['user_action', 'data_update', 'navigation', 'system']).describe('What triggered the state change'),
