...
export const diagnosticCheckStatusSchema = z.object({
    status: z.enum(['ok', 'freed', 'error', 'degraded']).describe('Outcome status for this diagnostic component.'),
    details: z.string().optional().describe('Human readable context for this check.'),
    latencyMs: z.number().int().nonnegative().optional().describe('Latency in milliseconds (when relevant, e.g., health probe).'),
});
...
export function parseDiagnosticsResult(input: unknown): DiagnosticsResult {
