...
def run_suite(name: str):
    ...
    for case in suite:
        if name == 'chat' and 'input' in case:
            ...
            result = { 'name': case['name'], 'adapter': out.get('adapter'), 'ok': ok, 'snippet': out.get('text','')[:160] }
...
def main():
    ...
    if args.out:
        ...
        out_path.write_text(json.dumps({ 'suite': args.suite, 'failures': failures, 'results': results }, indent=2))
