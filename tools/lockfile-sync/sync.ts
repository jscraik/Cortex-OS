...
export async function syncLockfiles(checkOnly = false): Promise<void> {
	...
	try {
		const pnpmJson = execSync("pnpm list --json --depth 0", {
			encoding: "utf8",
		});
		const pnpmDeps = JSON.parse(pnpmJson);
		const uvContent = readFileSync("uv.lock", "utf8");
		const uvLock = parseToml(uvContent) as unknown;
		const npmMap = new Map<string, string>();
		for (const node of pnpmDeps) {
			if (node.dependencies) {
				for (const [name, info] of Object.entries<unknown>(node.dependencies)) {
					npmMap.set(name, info.version);
				}
			}
		}
		const pyMap = new Map<string, string>();
		const pkgs = (uvLock?.package ?? []) as Array<{
			name: string;
			version: string;
		}>;
		for (const p of pkgs)
			pyMap.set(p.name.toLowerCase().replace(/_/g, "-"), p.version);
