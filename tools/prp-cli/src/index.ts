...
function registerManifestCommands(program: Command) {
	const manifest = program
		.command('manifest')
		.description('Inspect and validate PRP run manifests');
    ...
    `,
		...
		.action(async (path: string) => {
			try {
				...
				if (!result.ok) {
					console.error(chalk.red(`${BRAND} Manifest validation failed:`));
				...
				console.log(chalk.green(`${BRAND} Manifest validation passed`));
				result.issues.forEach((issue) => {
					const prefix = issue.level === 'error' ? chalk.red('ERROR') : chalk.yellow('WARN');
		...
		.action(async (options: { manifest: string; policy: string; json?: boolean }) => {
			try {
				...
				if (options.json) {
					...
					console.error(chalk.red(`${BRAND} Policy violations detected:`));
					...
					console.log(chalk.green(`${BRAND} Policy check passed`));
					if (evaluation.findings.length > 0) {
						for (const finding of evaluation.findings) {
							console.log(`${BRAND} NOTE: ${finding.message}`);
						}
					}
				}
				if (!evaluation.ok) {
		...
		.action(async (options: { manifest: string; proof?: string[]; all?: string }) => {
			try {
				...
				if (proofPaths.length === 0) {
					console.warn(`${BRAND} prp: no proof files provided`);
					return;
				}
				const verification = await verifyStageProofs(options.manifest, proofPaths);
				if (!verification.ok) {
					console.error(chalk.red(`${BRAND} Signature verification failed:`));
...
async function resolveProofList(options: { proof?: string[]; all?: string }): Promise<string[]> {
	...
	if (options.all) {
		...
		const matches = await glob.default('**/*.proof.json', {
			cwd: resolvePath(options.all),
			absolute: true,
		});
