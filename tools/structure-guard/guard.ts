...
await (async () => {
  ...
  // Enforce root-only location for allowedRootEntries files
  const rootOnlyEnforcedFiles = [
    'pnpm-workspace.yaml',
    'pnpm-lock.yaml',
    'tsconfig.json',
    'tsconfig.base.json',
  ...
  if (misplacedEnforcedRootFiles.length) {
    console.error(`Misplaced root-only files detected (should be at repo root):\n  ${misplacedEnforcedRootFiles.join('\n  ')}`);
    console.error('Auto-fix: move these files to the monorepo root.');
    process.exitCode = 5;
  }
