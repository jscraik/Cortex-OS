/**
 * @fileoverview Structure Guard Policy Schema and Validation
 *
	...
	.refine(
		(rules) => {
			...
			const conflicts = rules.bannedPatterns.filter((banned) =>
				rules.allowedCrossPkgImports.some(
					(allowed) => allowed === banned || new RegExp(banned).test(allowed),
				),
			);
			if (conflicts.length > 0) {
				throw new Error(
					`Import rule conflict: ${conflicts.join(', ')} appears in both banned and allowed`,
				);
			}
			return true;
		},
...
const secureGlobArraySchema = z.array(z.string()).refine(
	(globs) => {
		...
		if (dangerous.length > 0) {
			throw new Error(
				`Security violation: dangerous glob patterns detected: ${dangerous.join(', ')}`,
			);
		}
		return true;
	},
...
export function loadPolicy(
	...
	try {
		...
		return {
			valid: false,
			errors: [
				`Failed to load policy from ${filePath}: ${error instanceof Error ? error.message : String(error)}`,
			],
		};
	}
}
