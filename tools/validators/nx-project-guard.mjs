...
function normalizeOptions(options, issues) {
	...
	if (typeof opts.command === 'string') {
		...
		if (res.cmd !== opts.command) {
			opts.command = res.cmd;
			mutated = true;
		}
		if (res.options !== opts) {
			Object.assign(opts, res.options);
			mutated = true;
		}
	}
	if (Array.isArray(opts.commands)) {
		const updated = opts.commands.map((c) => normalizeCommand(String(c), opts, issues).cmd);
		if (JSON.stringify(updated) !== JSON.stringify(opts.commands)) {
			opts.commands = updated;
			mutated = true;
		}
	}
	return { opts, mutated };
...
function processProjectJson(file) {
	...
	try {
		...
		return { file, changed: false, issues: [`invalid JSON: ${e.message}`] };
	}
	const issues = [];
...
function main() {
	...
	if (files.length === 0) {
		console.log(COLOR.yellow('No project.json files found.'));
		process.exit(0);
	}
	let changedCount = 0;
	let issueCount = 0;
	...
	for (const f of files) {
		...
		if (r.issues.length) {
			...
			lines.push(`${r.changed ? 'fix' : 'chk'} ${f}\n - ${r.issues.join('\n - ')}`);
		}
	...
	if (FIX) {
		console.log(
			COLOR.green(`nx-project-guard: fixed ${changedCount} files; ${issueCount} issues addressed`),
		);
		process.exit(0);
	}
	if (issueCount > 0) {
		console.error(
			COLOR.red(
				`nx-project-guard: ${issueCount} issues found across ${files.length} files. Run with --fix to apply.`,
			),
		);
		process.exit(1);
	}
