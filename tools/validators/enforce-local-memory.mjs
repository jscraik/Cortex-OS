...
const skip =
	process.env.CI_SKIP_LOCAL_MEMORY_ENFORCE === '1' || process.env.CI_SKIP_MEMORY_ENFORCE === '1';
const strict =
	process.env.LOCAL_MEMORY_ENFORCE_STRICT === '1' || process.env.CI_LOCAL_MEMORY_STRICT === '1';
...
async function main() {
	if (skip) {
		console.log(
			'[enforce-local-memory] Skipped by CI_SKIP_LOCAL_MEMORY_ENFORCE/CI_SKIP_MEMORY_ENFORCE',
		);
		return;
	}
	...
	if (!process.env.LOCAL_MEMORY_NAMESPACE) {
		warn(
			'`LOCAL_MEMORY_NAMESPACE` not set. Defaulting to unspecified namespace may reduce isolation.',
		);
	}
	...
	if (strict) {
		...
		if (!bin) {
			return fail(
				'Could not locate `local-memory` binary (set LOCAL_MEMORY_BIN or install to ~/.local/bin/local-memory).',
			);
		}
