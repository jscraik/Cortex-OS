...
function parseArgs() {
    ...
    while (i < args.length) {
        ...
        if (a === '--force') { opts.force = true; i += 1; continue; }
        ...
        if (a === '--help' || a === '-h') {
            console.log('Usage: node scripts/memory-baseline-auto.mjs [--max-age-days N] [--force] [--max-allowed-pct P]');
            process.exit(0);
        }
...
function fileAgeDays(path) {
    try { const m = statSync(path).mtimeMs; return (Date.now() - m) / (1000 * 60 * 60 * 24); } catch { return Infinity; }
}
...
function main() {
    ...
    const pctIncrease = baseline.peakMB > 0 ? ((lastPeak - baseline.peakMB) / baseline.peakMB) * 100 : 0;
    ...
    if (!opts.force) {
        ...
        if (pctIncrease > opts.maxAllowedPct) {
            log('WARN', `Refusing refresh: lastPeak increased ${pctIncrease.toFixed(1)}% (> ${opts.maxAllowedPct}%) vs existing baseline.`);
            process.exit(2);
        }
    } else {
    ...
    function promote(peak, reason) {
        ...
        const newBaseline = { peakMB: peak, updatedAt: new Date().toISOString(), reason };
