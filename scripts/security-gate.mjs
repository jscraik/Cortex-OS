...
function runPnpmAudit() {
    ...
    if (!result.success) {
        if (result.error && result.error.includes('vulnerabilities found')) {
...
function main() {
    ...
    if (auditResults.critical > VULNERABILITY_THRESHOLDS.critical) {
        issues.push(`❌ CRITICAL: ${auditResults.critical} critical vulnerabilities (threshold: ${VULNERABILITY_THRESHOLDS.critical})`);
        hasBlockingIssues = true;
    }

    if (auditResults.high > VULNERABILITY_THRESHOLDS.high) {
        issues.push(`❌ HIGH: ${auditResults.high} high-severity vulnerabilities (threshold: ${VULNERABILITY_THRESHOLDS.high})`);
        hasBlockingIssues = true;
    }

    if (auditResults.moderate > VULNERABILITY_THRESHOLDS.moderate) {
        issues.push(`⚠️  MODERATE: ${auditResults.moderate} moderate vulnerabilities (threshold: ${VULNERABILITY_THRESHOLDS.moderate})`);
    }
