...
async function ensureFileList() {
    ...
    const raw = execSync("git ls-files '*.ts' '*.tsx' '*.js' '*.jsx'", { encoding: 'utf8' })
        .split(/\n+/)
...
(async () => {
    ...
    console.log(`[eslint-batch] Wrote ${aggregated.length} file results with ${totalMessages} total messages to ${outFile}`);
