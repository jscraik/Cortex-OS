...
if (!fs.existsSync(file)) {
    console.error('[mutation-threshold] mutation.json not found – run Stryker first.');
    process.exit(1);
}
try {
    ...
    if (typeof score !== 'number') {
        console.error('[mutation-threshold] mutationScore missing in mutation.json');
        process.exit(1);
    }
    if (score + 0.0001 < min) {
        console.error(`[mutation-threshold] FAIL ${score.toFixed(2)}% < required ${min}%`);
        process.exit(1);
    }
