...
if (!fs.existsSync(sparkFile)) {
    console.warn('[sparkline-inline] Skipping inline embed: reports/badges/branch-trend.svg not found (generate badges first)');
    process.exit(0); // graceful no-op for early pipelines
}
const svg = fs.readFileSync(sparkFile, 'utf8')
