...
async function runForDir(dir) {
  ...
  try {
    ...
    if (overrideConfig && overrideConfig.files) {
    ...
    if (overrideConfig && overrideConfig.languageOptions) {
    ...
    if (overrideConfig && overrideConfig.plugins && typeof overrideConfig.plugins === 'object') {
    ...
    const msg = `ERROR running sonar scan for ${name}: ${err && err.stack ? err.stack : String(err)}`;
