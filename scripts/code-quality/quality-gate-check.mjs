...
try {
    ...
    if (!pass) {
        console.error(`[quality-gate] FAIL branch=${b}% (min ${branchMin}%) mutation=${m}% (min ${mutationMin}%)`);
        process.exit(2);
    }
    console.log(`[quality-gate] PASS branch=${b}% >= ${branchMin}% & mutation=${m}% >= ${mutationMin}%`);
} catch (e) {
