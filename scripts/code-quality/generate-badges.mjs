...
// 1. Standard badges
fs.writeFileSync(
	path.join(OUT_DIR, 'branch-coverage.svg'),
	svgBadge('branch', branchPct, '%', colorScale(branchPct)),
);
fs.writeFileSync(
	path.join(OUT_DIR, 'mutation-score.svg'),
	svgBadge('mutation', mutationPct, '%', colorScale(mutationPct)),
);
...
if (fs.existsSync(mutationHistoryFile)) {
	try {
		mutationHistory = JSON.parse(fs.readFileSync(mutationHistoryFile, 'utf8'));
		if (!Array.isArray(mutationHistory)) mutationHistory = [];
	} catch {
		mutationHistory = [];
	}
}
...
try {
	...
	if (fs.existsSync(mutationFile)) {
		...
		if (raw.files) {
			...
			fs.writeFileSync(
				path.join(OUT_DIR, 'mutation-operators-summary.json'),
				JSON.stringify(operatorArray, null, 2),
			);
			// Also emit a lightweight markdown table for human inspection
			const mdLines = [
				'| Mutator | Total | Killed | Survived | NoCoverage | Timeout | Detection % |',
				'|---------|-------|--------|----------|------------|---------|-------------|',
			];
...
console.log(
	`[generate-badges] branch=${branchPct.toFixed(1)}% mutation=${mutationPct.toFixed(1)}% gate=${qualityGatePass ? 'PASS' : 'FAIL'} -> ${OUT_DIR}`,
);
