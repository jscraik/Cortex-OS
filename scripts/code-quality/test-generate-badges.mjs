...
fs.writeFileSync('coverage/coverage-summary.json', JSON.stringify({ total: { branches: { pct: 70.2 } } }));
fs.mkdirSync('reports/mutation', { recursive: true });
fs.writeFileSync('reports/mutation/mutation.json', JSON.stringify({
  mutationScore: 82.4,
  files: {
    'src/file.ts': {
      mutants: [
...
const needed = ['branch-coverage.svg', 'mutation-score.svg', 'quality-gate.svg', 'metrics.json', 'mutation-operators-summary.json'];
...
const ops = JSON.parse(fs.readFileSync(path.join(outDir, 'mutation-operators-summary.json'), 'utf8'));
if (!ops.find(o => o.mutator === 'ArithmeticOperator' && o.total === 2)) fail('ArithmeticOperator summary missing');
if (!ops.find(o => o.mutator === 'StringLiteral' && o.total === 3)) fail('StringLiteral summary missing');

// Validate SVG structure (basic check)
const branchSvg = fs.readFileSync(path.join(outDir, 'branch-coverage.svg'), 'utf8');
if (!branchSvg.includes('70.2%') || !branchSvg.includes('role="img"')) fail('branch coverage SVG malformed');
