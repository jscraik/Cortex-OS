...
if (args.length === 0) {
  console.error('Usage: nx-smart <target> [--json] [--verbose] [--dry-run] [--interactive] [--force]');
...
const forceRun = flags.includes('--force') || process.env.CORTEX_SMART_FORCE === '1' || process.env.NX_SMART_FORCE === '1';
...
if (['build', 'test', 'lint', 'typecheck'].includes(target) && nonDocChanged.length === 0 && !forceRun) {
  meta.skipped = true;
  if (json) {
...
if (['build', 'test', 'lint', 'typecheck'].includes(target) && nonDocChanged.length === 0 && forceRun && !json) {
