...
if (telemetryEnabled) {
	try {
		...
		runCounter = telemetry.createCounter(
			'nx_smart_runs_total',
			'Total nx-smart wrapper runs',
		);
...
if (args.length === 0) {
	console.error(
		'Usage: nx-smart <target> [--json] [--verbose] [--dry-run] [--interactive]',
	);
...
function run(command) {
	...
	if (verbose && !json)
		console.log(
			'[exec]',
			command,
			`(NX_INTERACTIVE=${process.env.NX_INTERACTIVE})`,
		);
...
if (strategy === 'affected') {
	...
	if (
		...
		if (filtered.length > 0) {
			if (!json)
				log(
					`[nx-smart] focus (pre-summary) ${original.length}->${filtered.length}`,
				);
	...
	if (isDryRun) {
		if (strategy === 'affected') {
			...
			console.log(
				`[nx-smart][warn] affected strategy failed - would fall back to full run-many`,
			);
			console.log(`Target: ${target}`);
	...
	if (strategy === 'affected' && affectedList.length === 0) {
		...
		if (json) {
			...
			console.log(
				'[nx-smart] No affected projects for target',
				target,
				'- skipping.',
			);
		}
	...
	if (strategy === 'affected') {
				...
				if (fs.existsSync(graphTmp)) {
					...
					if (jsonStart !== -1 && jsonEnd !== -1) {
						const graphJson = JSON.parse(
							graphRaw.slice(jsonStart, jsonEnd + 1),
						);
		...
		run(
			`nx affected -t ${target} --base=${baseRef} --head=${headRef} ${narrowedProjectsArg} ${interactiveFlag} ${forwardedFlags.join(' ')}`
				.replace(/ {2,}/g, ' ')
				.trim(),
		);
	} else {
		const interactiveFlag = forceInteractive ? '--interactive' : '';
		run(
			`nx run-many -t ${target} --parallel ${interactiveFlag} ${forwardedFlags.join(' ')}`.trim(),
		);
	}
} else {
	const interactiveFlag = forceInteractive ? '--interactive' : '';
