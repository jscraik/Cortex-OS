...
function fail(msg) { console.error(`[perf-check] ${msg}`); process.exit(1); }
...
try { baseline = JSON.parse(fs.readFileSync(baselinePath, 'utf8')); } catch (e) { fail(`Invalid baseline JSON: ${e.message}`); }
try { metrics = JSON.parse(fs.readFileSync(metricsPath, 'utf8')); } catch (e) { fail(`Invalid metrics JSON: ${e.message}`); }
...
if (duration > maxMs) {
    if (process.env.ALLOW_PERF_DRIFT === '1') {
        console.warn(`[perf-check] WARNING: target ${target} duration ${duration}ms exceeds baseline ${maxMs}ms but ALLOW_PERF_DRIFT=1 set.`);
        process.exit(0);
    }
