...
function parseArgs() {
    ...
    while (i < raw.length) {
        ...
        switch (a) {
            case '--target': {
                const v = raw[i + 1];
                if (v) { opts.target = v; i += 2; break; }
                i += 1; break;
            }
            case '--json':
                opts.json = true; i += 1; break;
            case '--focus': {
                const v = raw[i + 1];
                if (v) { opts.focus = v; i += 2; break; }
                i += 1; break;
            }
            case '--dry-run':
                opts.dryRun = true; i += 1; break;
            case '--help':
            case '-h':
                console.log('Usage: node scripts/prebuild-graph.mjs [--target build|test|lint|typecheck] [--json] [--focus pkga,pkgB] [--dry-run]');
                process.exit(0);
            default:
                i += 1; break;
        }
...
function gitRev(ref) {
    try { return execSync(`git rev-parse ${ref}`, { encoding: 'utf8' }).trim(); } catch { return null; }
}
...
function memoryHygieneCheck() {
    try {
        const procList = execSync("ps -Ao pid,rss,comm | grep -E '(vitest|tsc|node)' | grep -v grep | head -25", { encoding: 'utf8' });
...
function computeAffected(target, focus, dryRun) {
    ...
    try {
        ...
        try {
            const fallback = execSync('npx nx show projects --with-target=' + target + ' --noInteractive', { encoding: 'utf8' });
    ...
    if (focus) {
        const focusSet = new Set(focus.split(',').map(s => s.trim()).filter(Boolean));
    ...
    if (!dryRun && target === 'build' && focused.length > 0) {
        try {
            ...
            spawnSync('npx', ['nx', 'run-many', '-t', 'typecheck', '--projects', focused.join(','), '--parallel=1'], { stdio: 'inherit' });
            prewarmRan = true;
        } catch (err) {
...
function writeSummary(summary, jsonMode) {
    ...
    try { if (!existsSync(outDir)) mkdirSync(outDir, { recursive: true }); } catch { }
    const file = join(outDir, 'prebuild-summary.json');
    try { writeFileSync(file, JSON.stringify(summary, null, 2)); } catch (e) { log('WARN', 'Failed to write summary: ' + e.message); }
    if (jsonMode) {
        console.log(JSON.stringify(summary));
    } else {
        log('INFO', `Affected projects (${summary.projects.length}): ${summary.projects.join(', ') || '(none)'}`);
        if (summary.focusNotice) log('INFO', summary.focusNotice);
        log('INFO', `Focused set (${summary.focused.length}): ${summary.focused.join(', ') || '(none)'}`);
