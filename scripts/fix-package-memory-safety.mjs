    ...
    try {
        ...
        if (packageJson.scripts) {
            ...
            const unsafePatterns = [
                { pattern: /^vitest run$/, replacement: 'node ../../scripts/vitest-safe.mjs run' },
                { pattern: /^vitest run --coverage$/, replacement: 'node ../../scripts/vitest-safe.mjs run --coverage' },
                { pattern: /^vitest run tests\/unit$/, replacement: 'node ../../scripts/vitest-safe.mjs run tests/unit' },
                { pattern: /^vitest run tests\/integration$/, replacement: 'node ../../scripts/vitest-safe.mjs run tests/integration' },
                { pattern: /^vitest run tests\/e2e$/, replacement: 'node ../../scripts/vitest-safe.mjs run tests/e2e' },
                { pattern: /^vitest run --coverage --reporter=verbose$/, replacement: 'node ../../scripts/vitest-safe.mjs run --coverage --reporter=verbose' },
                { pattern: /^vitest$/, replacement: 'echo "DISABLED: vitest watch mode causes memory leaks. Use test:safe instead."' },
                { pattern: /^vitest --watch$/, replacement: 'echo "DISABLED: vitest watch mode causes memory leaks. Use test:safe instead."' },
                { pattern: /^vitest watch$/, replacement: 'echo "DISABLED: vitest watch mode causes memory leaks. Use test:safe instead."' },
            ];
                ...
                if (typeof scriptCommand === 'string') {
                    for (const { pattern, replacement } of unsafePatterns) {
                        if (pattern.test(scriptCommand)) {
                            log('INFO', `Updating ${packagePath} script "${scriptName}": ${scriptCommand} -> ${replacement}`);
