...
function parseArgs() {
    ...
    while (i < args.length) {
        const a = args[i];
        if (a === '--') { opts.cmd = args.slice(i + 1); break; }
        switch (a) {
            case '--label': opts.label = args[i + 1] || opts.label; i += 2; break;
            case '--interval': opts.interval = parseInt(args[i + 1] || '3000', 10); i += 2; break;
            case '--no-manual-snapshot': opts.manual = false; i += 1; break;
            case '--limit-mb': opts.limitMb = parseInt(args[i + 1], 10); i += 2; break;
            case '--fail-over-mb': opts.failOverMb = parseInt(args[i + 1], 10); i += 2; break;
            case '--json': opts.json = true; i += 1; break;
            case '--prom': opts.prom = true; i += 1; break;
            case '--help':
            case '-h':
                console.log('Usage: node scripts/memory-snapshot-runner.mjs [options] -- <command> [args...]');
                process.exit(0);
            default: i += 1; break;
        }
    }
...
function listRecentSnapshots(limit = 5) {
    try {
        const files = readdirSync(snapshotDir)
            ...
            .map(f => ({ file: f, path: join(snapshotDir, f), mtime: statSync(join(snapshotDir, f)).mtimeMs, size: statSync(join(snapshotDir, f)).size }))
...
function manualHeapSnapshot(label, ts) {
...
function formatMB(kb) { return (kb / 1024).toFixed(2); }
...
function main() {
    ...
    const childEnv = {
        ...process.env,
        NODE_OPTIONS: `${process.env.NODE_OPTIONS || ''} --max-old-space-size=1536 --heapsnapshot-near-heap-limit=1 --expose-gc`.trim(),
    };
    ...
    const sampleInterval = setInterval(() => {
        ...
        if (opts.failOverMb && rssKB / 1024 > opts.failOverMb) {
            log('ERROR', `RSS exceeded hard limit (${formatMB(rssKB)}MB > ${opts.failOverMb}MB). Terminating.`);
    ...
    child.on('close', (code, signal) => {
        ...
        try { mkdirSync(join(rootDir, 'reports'), { recursive: true }); } catch { }
        try { writeFileSync(join(rootDir, 'reports', 'memory-snapshot-summary.json'), JSON.stringify(summary, null, 2)); } catch { }
        ...
        if (promEnabled) {
            try {
                ...
                const promLines = [];
                promLines.push('# HELP cortex_memory_peak_rss_bytes Peak resident set size during monitored run');
                ...
                promLines.push('# HELP cortex_memory_observation_window_ms Milliseconds covered by sampling window');
        ...
        if (opts.json) {
            ...
            const recentStr = recent.map(r => `${r.file}(${(r.size / 1024 / 1024).toFixed(1)}MB)`).join(', ');
