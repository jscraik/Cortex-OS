...
import { join, dirname, sep, posix, relative as rel } from 'node:path';
...
const ignoreDirs = new Set([
  'node_modules', 'dist', 'build', 'coverage', '.git', '.nx', '.pnpm-store',
  '.vitest-tmp-docs', '.vitest-tmp-target', '.venv', '.uv-cache', 'reports',
]);
...
for (const f of allFiles) {
  ...
  try { if (!statSync(srcRoot).isDirectory()) continue; } catch { continue; }
