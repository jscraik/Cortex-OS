...
function riskLevel(diff) {
    switch (diff) {
        ...
        case 'new': return 'informational';
        case 'downgrade': return 'medium';
...
function main() {
    ...
    Object.keys(curDeps).forEach(name => {
        ...
        if (oldV !== newV) {
            ...
            changes.push({ name, old: oldV || null, new: newV, diff, risk: riskLevel(diff) });
        }
    });
    const summary = {
        totalChanges: changes.length,
        byRisk: changes.reduce((acc, c) => { acc[c.risk] = (acc[c.risk] || 0) + 1; return acc; }, {}),
