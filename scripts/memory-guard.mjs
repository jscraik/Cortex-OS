...
export function startGuard({ pids = [], pattern = 'node', maxRssMB, intervalMs }) {
  ...
  const timer = setInterval(() => {
    const targets = pids.length > 0 ? pids.map(pid => ({ pid, rssMB: getRssMB(pid) })) : getProcessList(pattern);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  startGuard({ pids: opts.pid || [], pattern: opts.pid ? '' : opts.pattern, maxRssMB: opts.max, intervalMs: opts.interval });
}
