...
const featureDirs = fs
	...
	.filter((d) => fs.statSync(path.join(featureRoot, d)).isDirectory());
...
for (const feature of featureDirs) {
	...
	const tsSources = sourceFiles
		.filter((f) => f.endsWith('.ts') || f.endsWith('.tsx'))
		.filter((f) => !f.includes('__tests__'));
	if (tsSources.length > 0 && tsSources.length < 3) {
		warnings.push(
			`Feature package '${feature}' has only ${tsSources.length} source files (<3). Consider merging until boundary is clear.`,
		);
	}
		...
		if (spec.startsWith('..')) {
			...
			if (resolved.startsWith(featureRoot)) {
				...
				if (targetFeature && targetFeature !== feature) {
					violations.push(
						`${rel}:${idx + 1} reach-through relative import to sibling feature '${targetFeature}' -> '${spec}'`,
					);
				}
			}
		}
		...
		if (spec.startsWith('.') && spec.includes('/infra/')) {
			...
			if (!resolved.startsWith(featurePath) && resolved.startsWith(featureRoot)) {
				...
				if (targetFeature !== feature) {
					violations.push(
						`${rel}:${idx + 1} direct infra import from feature '${targetFeature}' -> '${spec}'`,
					);
				}
			}
		}
...
function collectAllTsFiles(dir, acc = []) {
	for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
		...
		const full = path.join(dir, e.name);
		if (e.isDirectory()) collectAllTsFiles(full, acc);
		else if (/\.[cm]?tsx?$/.test(e.name)) acc.push(full);
	}
	return acc;
}

const scanRoots = [
	path.join(root, 'apps', 'cortex-os', 'packages'),
		...
		for (const [feature, infraAbs] of featureInfraPaths.entries()) {
			if (resolved.startsWith(infraAbs)) {
				...
				if (!f.startsWith(featureRootPath)) {
					violations.push(
						`${rel}: import into ${feature}/src/infra from outside feature -> '${spec}'`,
					);
				}
			}
		}
