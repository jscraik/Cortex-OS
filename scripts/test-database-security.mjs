...
try {
  ...
  execSync('semgrep --config=.semgrep/owasp-precise.yaml --severity=ERROR apps/cortex-os/packages/agents/src/legacy-instructions/DatabaseManager.ts', { 
    stdio: 'inherit',
    cwd: process.cwd()
  });
  ...
  try {
    execSync('grep -r -i "TODO.*security\\|TODO.*secure\\|FIXME.*security\\|FIXME.*secure" apps/cortex-os/packages/agents/src/legacy-instructions/DatabaseManager.ts', { 
      stdio: 'pipe',
      cwd: process.cwd()
    });
  ...
  try {
    execSync('pnpm test:unit apps/cortex-os/packages/agents/src/legacy-instructions/DatabaseManager.test.ts', { 
      stdio: 'inherit',
      cwd: process.cwd()
    });
