...
console.log(
	`🔍 Monitoring pnpm/node processes every ${interval}s (leak threshold: ${threshold}MB)`,
);
...
const monitor = () => {
	try {
		const ps = execSync(
			"ps -Ao pid,etime,rss,command | grep -E '(pnpm|node)' | grep -v grep | grep -v 'monitor-pnpm-memory'",
			{ encoding: 'utf8' },
		);
