...
function parseArgs() {
	...
	const opts = {
		...
		allowedPct: process.env.MEMORY_GUARD_ALLOWED_PCT
			...
			: 25,
		allowedDeltaMb: process.env.MEMORY_GUARD_ALLOWED_DELTA_MB
			? parseInt(process.env.MEMORY_GUARD_ALLOWED_DELTA_MB, 10)
			: null,
		json: false,
	...
	while (i < args.length) {
		...
		switch (a) {
			...
			case '--baseline':
				...
				break;
			case '--max-mb':
				opts.maxMb = parseInt(args[i + 1], 10);
				i += 2;
				break;
			case '--allowed-pct':
				opts.allowedPct = parseFloat(args[i + 1]);
				i += 2;
				break;
			case '--allowed-delta-mb':
				opts.allowedDeltaMb = parseInt(args[i + 1], 10);
				i += 2;
				break;
			case '--json':
				opts.json = true;
			...
			case '-h':
				console.log(
					'Usage: node scripts/memory-regression-guard.mjs [--file path] [--baseline path] [--max-mb N] [--allowed-pct P] [--allowed-delta-mb D] [--tiered] [--tiers spec] [--prom] [--json]',
				);
				process.exit(0);
			default:
				i += 1;
...
function _evaluateFailure({
	...
	if (hardMaxDecision.hardMaxHit) {
		...
		reasonCode = 'HARD_MAX';
		reasons.push(
			`Current peak ${currentPeak}MB exceeded hardMax ${hardMaxDecision.hardMaxValue}MB (tier threshold ${hardMaxDecision.matchedTier?.thresholdMB ?? 'n/a'})`,
		);
	...
	if (!failed && typeof baselinePeak === 'number' && baselinePeak >= 0) {
		...
		if (currentPeak > absoluteAllowed) {
			...
			reasonCode = 'DELTA';
			reasons.push(
				`Current peak ${currentPeak}MB exceeds allowed delta (${baselinePeak}MB + ${deltaAllowedMB}MB = ${absoluteAllowed}MB)`,
			);
		}
	}
	if (!failed && typeof baselinePeak === 'number' && baselinePeak > 0 && pctAllowed != null) {
		...
		if (currentPeak > pctLimit) {
			...
			reasonCode = 'PCT_TIER';
			reasons.push(
				`Current peak ${currentPeak}MB exceeds pct tier limit ${pctLimit}MB (${pctAllowed}% over baseline ${baselinePeak}MB)`,
			);
		}
	}
	return { failed, reasons, reasonCode };
...
function _buildPromMetrics(summary) {
	...
	const labels = (extra = {}) => {
		const all = {
			...
			decision: summary.tierDecision || 'none',
			outcome: summary.failed ? 'fail' : 'pass',
			...extra,
		};
	...
	pushGauge(
		...
		'Hard max MB from tier or explicit',
	);
...
function decideTierPolicy({ baselinePeak, opts, peakMB }) {
	if (!opts.tiered)
		return {
			effectiveAllowedPct: opts.allowedPct,
			tierDecision: null,
			tierHardMaxTriggered: false,
		};
	...
	for (const t of tiers) {
		if (baselinePeak <= t.thresholdMB) {
			const tierDecision = {
				limit: t.thresholdMB,
				pct: t.allowedPct,
				hardMax: t.hardMax ?? null,
				tiers,
			};
			const tierHardMaxTriggered = t.hardMax != null && peakMB > t.hardMax;
			log(
				'INFO',
				`Tiered policy active: allowedPct adjusted to ${t.allowedPct}% (baseline=${baselinePeak.toFixed(2)}MB)`,
			);
			return {
				effectiveAllowedPct: t.allowedPct,
				tierDecision,
				tierHardMaxTriggered,
			};
		}
	}
	const last = tiers[tiers.length - 1];
	return {
		effectiveAllowedPct: last.allowedPct,
		tierDecision: {
			limit: last.thresholdMB,
			pct: last.allowedPct,
			hardMax: last.hardMax ?? null,
			tiers,
		},
		tierHardMaxTriggered: last.hardMax != null && peakMB > last.hardMax,
	};
}

function writeGuardPromMetrics(summary, _opts, peakMB) {
	try {
		...
		if (summary.policy.tierDecision) {
			...
			lines.push(
				'# HELP cortex_memory_guard_tier_info Tier decision metadata (limit & allowedPct)',
			);
...
function buildEvaluation({ peakMB, baseline, opts }) {
	...
	if (!failed && tierInfo.tierHardMaxTriggered) {
		failed = true;
		reasonCode = 'HARD_MAX_TIER';
		reasons.push(
			`Peak ${peakMB.toFixed(2)}MB exceeds tier hardMax ${tierInfo.tierDecision.hardMax}MB (tier limit ${tierInfo.tierDecision.limit})`,
		);
	}
...
function main() {
	...
	if (!file) {
		log(
			'ERROR',
			'No memory log file found (.memory/logs/*.jsonl). Run memory-snapshot-runner first.',
		);
		process.exit(1);
	}
	...
	log(
		'INFO',
		`Baseline peak: ${evalResult.baselinePeak.toFixed(2)}MB | Delta: ${evalResult.delta.toFixed(2)}MB (${evalResult.pct.toFixed(1)}%)`,
	);
	...
	if (opts.json) console.log(JSON.stringify(summary));
		...
		log(
			'ERROR',
			`Memory regression detected (reasonCode=${summary.reasonCode}): ${summary.reasons.join('; ')}`,
		);
