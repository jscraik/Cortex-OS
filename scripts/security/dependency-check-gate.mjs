...
class DependencySecurityGate {
	...
	processNpmAuditResults(audit) {
		...
		console.log(
			`   Critical: ${this.results.vulnerabilities.critical}, High: ${this.results.vulnerabilities.high}`,
		);
	}
	...
	shouldRunFullScan() {
		...
		const criticalAndHigh =
			this.results.vulnerabilities.critical + this.results.vulnerabilities.high;
		return (
			criticalAndHigh > this.config.allowedVulns ||
			this.config.threshold === 'moderate' ||
			this.config.threshold === 'low'
		);
	}
	...
	logFailureDetails() {
		...
		const highSevVulns = this.results.details.filter(
...
function parseArgs() {
	...
	args.forEach((arg) => {
		if (arg.startsWith('--threshold=')) {
			...
			config.allowedVulns = parseInt(arg.split('=')[1], 10);
