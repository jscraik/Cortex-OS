...
const insecurePatterns = [
	...
	{
		pattern:
			/this\.db\s*\.\s*prepare\(\s*"UPDATE agents SET status = \? WHERE id = \?"\s*\)\s*\.\s*run\(\s*([^,]+)\s*,\s*([^)]+)\s*\);/g,
	...
	{
		pattern:
			/this\.db\s*\.\s*prepare\(\s*`[\s\S]*?UPDATE memory[\s\S]*?access_count = access_count \+ 1[\s\S]*?WHERE key = \? AND namespace = \?"[\s\S]*?`\s*\)\s*\.\s*run\(\s*([^,]+)\s*,\s*([^)]+)\s*\);/g,
	...
	{
		pattern:
			/this\.db\s*\.\s*prepare\(\s*"DELETE FROM memory WHERE key = \? AND namespace = \?"\s*\)\s*\.\s*run\(\s*([^,]+)\s*,\s*([^)]+)\s*\);/g,
	...
	{
		pattern:
			/this\.db\s*\.\s*prepare\(\s*`[\s\S]*?INSERT INTO consensus[\s\S]*?VALUES[\s\S]*?`\s*\)\s*\.\s*run\(\{[\s\S]*?\}\);/g,
	...
	{
		pattern:
			/this\.db\s*\.\s*prepare\(\s*`[\s\S]*?UPDATE consensus[\s\S]*?SET votes = \?[\s\S]*?WHERE id = \?"[\s\S]*?`\s*\)\s*\.\s*run\([\s\S]*?\);/g,
