...
function listPackageJson() {
    const out = execSync("find packages apps -name package.json -not -path '*/node_modules/*'", { encoding: 'utf-8' });
    return out.split(/\n/).filter(Boolean);
}

const VIOLATION_REGEX = /"(test|test:[^"]+)"\s*:\s*"(?:(?:npx |pnpm |NODE_OPTIONS=[^"]* )*)vitest(?!-safe).*?(run|watch|--watch|--ui)/;
