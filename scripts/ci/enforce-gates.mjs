...
class BrAInwavQualityGateEnforcer {
	...
	async checkSecurity() {
		...
		console.log(
			`[brAInwav] Security: ${security.critical || 0} critical, ${security.high || 0} high vulnerabilities`,
		);
	}
	...
	async checkPerformance() {
		...
		console.log(
			`[brAInwav] Performance: ${perf.p95_latency || 'N/A'}ms P95, ${perf.error_rate || 'N/A'}% errors, ${perf.throughput || 'N/A'} RPS`,
		);
	}
	...
	async checkOpsReadiness() {
		...
		console.log(
			`[brAInwav] Operational readiness: ${(actualScore * 100).toFixed(1)}% (${ops.score || 0}/${ops.max_score || 20} criteria)`,
		);
	}
	...
	validateBrandCompliance(branding) {
		if (branding.brand_compliance_required) {
			...
			if (brandingCheck && brandingCheck.violations > 0) {
				this.violations.push(
					`Brand compliance violations detected: ${brandingCheck.violations} instances`,
				);
			}
		}
