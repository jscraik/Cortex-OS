...
describe('brAInwav Quality Gate Enforcer', () => {
  ...
  describe('Coverage Gate Enforcement', () => {
    it('should fail when line coverage is below 95% threshold', async () => {
      ...
      expect(result.violations).toContain(
        expect.stringMatching(/brAInwav standard/)
      );
    });
    ...
    it('should fail when mutation score is below threshold', async () => {
      ...
      await fs.writeFile(
        path.join(metricsDir, 'coverage.json'),
        JSON.stringify(goodCoverage)
      );
      await fs.writeFile(
        path.join(metricsDir, 'mutation.json'),
        JSON.stringify(poorMutation)
      );
      ...
      expect(result.violations).toContain(
        expect.stringMatching(/prevents vacuous tests/)
      );
    });
  });
  ...
  describe('Security Gate Enforcement', () => {
    it('should fail when critical vulnerabilities are detected', async () => {
      ...
      await fs.writeFile(
        path.join(metricsDir, 'coverage.json'),
        JSON.stringify({ lines: { percentage: 96 }, branches: { percentage: 92 }, total: 1000, covered: 960 })
      );
      await fs.writeFile(
        path.join(metricsDir, 'security.json'),
      ...
      expect(result.violations).toContain(
        expect.stringMatching(/brAInwav zero-tolerance policy/)
      );
    });
    ...
    it('should fail when secrets scan is not clean', async () => {
      ...
      expect(result.passed).toBe(false);
      expect(result.violations).toContain(
        expect.stringMatching(/Secrets scan failed/)
      );
      expect(result.violations).toContain(
    ...
    it('should fail when SBOM is not generated', async () => {
      ...
      expect(result.passed).toBe(false);
      expect(result.violations).toContain(
        expect.stringMatching(/SBOM generation required/)
      );
      expect(result.violations).toContain(
        expect.stringMatching(/brAInwav compliance requirement/)
      );
    });
  });
  ...
  describe('Comprehensive Quality Gate Success', () => {
    it('should pass when all quality gates are met', async () => {
      ...
      await fs.writeFile(
        path.join(metricsDir, 'coverage.json'),
        JSON.stringify(excellentCoverage)
      );
      await fs.writeFile(
        path.join(metricsDir, 'mutation.json'),
        JSON.stringify(goodMutation)
      );
      await fs.writeFile(
        path.join(metricsDir, 'security.json'),
        JSON.stringify(cleanSecurity)
      );
      await fs.writeFile(
  ...
  describe('Contract File Validation', () => {
    ...
    it('should fail when required metrics files are missing', async () => {
      ...
      expect(result.passed).toBe(false);
      expect(result.violations).toContain(
        expect.stringMatching(/Coverage metrics required/)
      );
    });
  });
  ...
  describe('Report Generation', () => {
    it('should generate comprehensive quality gate report', async () => {
      ...
      await fs.writeFile(
        path.join(metricsDir, 'coverage.json'),
        JSON.stringify(passingCoverage)
      );
