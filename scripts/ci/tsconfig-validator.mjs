...
function collectFailures(files) {
  ...
  for (const file of files) {
    ...
    if (moduleResolution === 'NodeNext' && moduleVal !== 'NodeNext') {
      failures.push({ file, reason: `moduleResolution=NodeNext but module=${moduleVal || '<missing>'}` });
    }
    if (ignoreVal === '6.0') {
      failures.push({ file, reason: `ignoreDeprecations=6.0 is unsupported; set to 5.0 or remove` });
    }
  }
  return failures;
}
...
function applyFixesToContent(content) {
  ...
  if (/"moduleResolution"\s*:\s*"NodeNext"/.test(out)) {
    if (/"module"\s*:\s*"[^"]+"/.test(out)) {
      ...
      out = out.replace(/("moduleResolution"\s*:\s*"NodeNext"\s*,?)/, '$1\n    "module": "NodeNext",');
    } else {
