#!/bin/bash
# Enhanced memory management script for MCP services - TDD compliant with proper error handling
# Usage: ./memory-manager-mcp.sh [--gentle|--aggressive] [--dry-run]
...
readonly MCP_PROCESS_THRESHOLD_KB=512000  # 500MB - Higher threshold for MCP processes
...
readonly LOG_FILE="${HOME}/.cortex-memory-manager-mcp.log"
...
# Check if a process is an MCP process
is_mcp_process() {
    local command="$1"
    echo "$command" | grep -qE "(mcp|mcp-registry|mcp-bridge|mcp-core)"
}
...
cleanup_excessive_node_processes() {
    ...
    while IFS=$'\t' read -r pid memory_kb command; do
        # Use higher threshold for MCP processes
        local effective_threshold="$memory_threshold"
        if is_mcp_process "$command"; then
            effective_threshold="$MCP_PROCESS_THRESHOLD_KB"
            log_info "Using higher threshold for MCP process: $command (PID: $pid)"
        fi

        if [[ "$memory_kb" -gt "$effective_threshold" ]]; then
...
force_cleanup_remaining() {
    ...
    for pattern in "${force_patterns[@]}"; do
        while IFS= read -r pid; do
            # Skip MCP processes even in aggressive mode
            local command
            command=$(ps -p "$pid" -o command= 2>/dev/null || echo "")
            if is_mcp_process "$command"; then
                log_info "Skipping MCP process: $command (PID: $pid)"
                continue
            fi
...
main() {
    ...
    log_info "Starting MCP-aware memory management in $MODE mode${DRY_RUN:+ (DRY RUN)}"
    ...
    show_memory_report
    log_info "MCP-aware memory management complete"
}
