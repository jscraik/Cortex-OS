...
if (existsSync(externalSSDPath)) {
  try {
    ...
    for (const dir of keyDirs) {
      ...
      if (exists) {
        try {
          const count = spawnSync('find', [dir, '-maxdepth', '2', '-type', 'd'], { encoding: 'utf8' });
...
console.log('\n📦 mlx-knife Version:');
const versionResult = spawnSync('bash', ['-lc', 'mlx-knife --version 2>/dev/null || echo "(not installed)"'], { encoding: 'utf8' });
console.log(`  ${versionResult.stdout.trim()}`);

console.log('\n🎯 Available MLX Models:');
const listResult = spawnSync('bash', ['-lc', 'mlx-knife list 2>/dev/null || echo "(mlx-knife not installed or no models found)"'], { encoding: 'utf8' });
...
if (process.env.MLX_MODEL_PATH) {
  ...
  if (existsSync(process.env.MLX_MODEL_PATH)) {
    const listFiles = spawnSync('bash', ['-lc', `ls -la "${process.env.MLX_MODEL_PATH}" 2>/dev/null | head -20 || echo "(path not readable)"`], { encoding: 'utf8' });
    console.log('  ✅ Path accessible:');
    console.log(listFiles.stdout.split('\n').map(line => `    ${line}`).join('\n'));
  } else {
...
if (process.env.MLX_MODEL_PATH && existsSync(process.env.MLX_MODEL_PATH)) summary.push('✅ MLX_MODEL_PATH configured');
