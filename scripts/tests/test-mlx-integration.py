...
def test_model_config():
    ...
    try:
        ...
        print(
            f"Available models: {list(adapter.config.get('embedding_models', {}).keys())}"
        )
        return adapter
    ...
    for model_name in ["qwen3-0.6b", "qwen3-4b", "qwen3-8b"]:
        try:
            ...
            if embeddings and len(embeddings) == len(test_texts):
                ...
                print(
                    f"✅ Generated {len(embeddings)} embeddings with {dims} dimensions"
                )
            else:
                print("❌ Failed to generate proper embeddings")
