...
const updateMemoryEntryPattern = /async updateMemoryEntry\([^}]*?SecureDatabaseWrapper for this operation[^}]*?this\.db\s*\.\s*prepare\([^}]*?UPDATE memory[^}]*?SET value = \?, access_count = \?, last_accessed_at = \?[^}]*?run\([^)]*\);\s*}/s;
