...
function usage() {
    console.error('Usage: perf-autotune.mjs <baseline.json> <history.json> [--window N] [--headroom PCT]');
    process.exit(1);
}
...
try { baseline = JSON.parse(fs.readFileSync(baselinePath, 'utf8')); } catch (e) { console.error('Invalid baseline JSON:', e.message); process.exit(1); }
try { history = JSON.parse(fs.readFileSync(historyPath, 'utf8')); } catch (e) { console.error('Invalid history JSON:', e.message); process.exit(1); }

if (!Array.isArray(history)) { console.error('History file must be an array of entries'); process.exit(1); }
...
report.forEach(r => console.log(`  - ${r.target}: median=${r.median} oldMax=${r.oldMax} newMax=${r.newMax}`));
