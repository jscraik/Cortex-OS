	...
	try {
		...
		if (tsconfig.compilerOptions.rootDir) {
			...
			const hasConflict = includes.some((pattern: string) => {
				...
				return (
					!normalizedPattern.startsWith(normalizedRootDir) &&
					normalizedPattern !== normalizedRootDir
				);
			});
		...
		const hasTestExcludes = tsconfig.exclude.some(
			(ex: string) => ex.includes('test') || ex.includes('spec'),
		);
		...
		if (hasTests && !hasSpecConfig) {
			...
			if (fs.existsSync(templatePath)) {
				...
				if (!options.dryRun) {
					fs.writeFileSync(path.join(packagePath, 'tsconfig.spec.json'), specTemplate);
				}
