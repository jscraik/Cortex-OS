 ...
 */
function isFeatureBoundaryViolation(currentFile: string, importPath: string): boolean {
...
export function createStructureGuardRule(): ESLintRule {
  return {
    meta: {
      ...
      fixable: false,
      messages: {
        forbiddenImport: 'Cross-feature imports are not allowed: "{{source}}". Use A2A events or MCP tools instead.',
