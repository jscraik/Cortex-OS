...
const citationTypeSchema = z.enum(['documentation', 'academic', 'changelog', 'code', 'news', 'blog']);
...
export function validateEvidenceEvent(event: unknown): ValidationResult {
  ...
  const candidate = (typeof event === 'object' && event !== null) ? event as Partial<EvidenceEvent> : {};
...
export function validateCitation(citation: Citation): ValidationResult {
  ...
  try {
    ...
    if (citation.type === 'documentation' && (!citation.snippet || citation.snippet.trim() === '')) {
...
export function deserializeEvidenceEvent(cloudEvent: CloudEventSerialized | Record<string, unknown>): EvidenceEvent {
  ...
  if (typeof rawData === 'string') {
    try {
      ...
      parsedData = {
        ...
        level: 'low',
        citations: [],
        metadata: { confidence: 0, relevance: 0, createdAt: new Date().toISOString() }
      };
    }
