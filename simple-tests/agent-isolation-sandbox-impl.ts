...
export interface AgentSandbox {
  run<T = unknown>(code: (api: SandboxApi) => T | Promise<T>): Promise<SandboxResult & { returnValue?: T }>;
...
type WorkerMessage = { type: 'violation'; violation: AuditEvent } | { type: 'return'; value: unknown } | { type: 'error'; error: string };

async function executeInWorker<T>(code: (api: SandboxApi) => T | Promise<T>, injected: unknown, deadlineMs: number, state: InternalState, options: SandboxOptions): Promise<{ value?: T; error?: Error; durationMs: number; }> {
  ...
  worker.on('error', (err) => { if (!finished) { runError = err; finished = true; } });
  worker.on('exit', (code) => { if (!finished && code !== 0) { runError = new Error('worker exited abnormally'); finished = true; } });
...
export function createAgentSandbox(options: SandboxOptions): AgentSandbox {
  ...
  return {
    async run<T = unknown>(code: (api: SandboxApi) => T | Promise<T>) {
      ...
      const { value, error, durationMs } = await executeInWorker(code, injected, options.maxExecutionMs, state, options);
      if (error) {
        return { success: false, error, violations: [...state.violations], durationMs } as SandboxResult & { returnValue?: T };
      }
      ...
      if (high) {
        return { success: false, error: new Error(`policy violation: ${high.type}`), violations: [...state.violations], durationMs } as SandboxResult & { returnValue?: T };
      }
      return { success: true, returnValue: value as T, violations: [...state.violations], durationMs } as SandboxResult & { returnValue?: T };
    },
    async dispose() {
      state.disposed = true;
    }
