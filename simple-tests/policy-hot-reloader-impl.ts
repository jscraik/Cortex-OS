		...
		try {
			...
			try {
				...
				this.emit(
					'parseError',
					new Error(
						`JSON parse error in ${this.policyFilePath}: ${parseError instanceof Error ? parseError.message : String(parseError)}`,
					),
				);
				return;
			}
			...
			if (!this.validatePolicyStructure(parsedPolicy)) {
				this.emit(
					'validationError',
					new Error('Policy validation failed: missing required fields'),
				);
				return;
			}
			...
			if (isChange) {
				...
				if (process.env.POLICY_RELOADER_DEBUG) {
					// eslint-disable-next-line no-console
					console.log(
						'[PolicyHotReloader-TS] emitting policyReloaded version',
			...
			this.emit(
				'policyError',
				new Error(
					`Failed to read policy file ${this.policyFilePath}: ${fileError instanceof Error ? fileError.message : String(fileError)}`,
				),
			);
		}
	}
		...
		try {
			...
			try {
				...
				this.emit(
					'parseError',
					new Error(
						`JSON parse error in ${this.policyFilePath}: ${e instanceof Error ? e.message : String(e)}`,
					),
				);
