...
import { createMCPToolValidator, MCPToolPolicy } from './mcp-tool-validator-impl';
...
describe('MCP Tool Allowlist Validation', () => {
  ...
  describe('Tool access control', () => {
    ...
    it('should enforce scope requirements', () => {
      ...
      expect(validator.isToolAllowed('secure_tool', { scopes: ['admin:read', 'admin:write'] })).toBe(true);
    });
  ...
  describe('Error handling and reporting', () => {
    it('should provide structured violation errors', () => {
      ...
      expect(validator.isToolAllowed('restricted_tool', { callCount: 15, scopes: ['admin:read'] })).toBe(false);
