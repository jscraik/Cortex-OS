...
export interface MCPToolValidator {
  ...
  getViolationReason(toolName: string, context?: { scopes?: string[]; callCount?: number }): string | null;
}
...
class MCPToolValidatorImpl implements MCPToolValidator {
  ...
  isToolAllowed(toolName: string, context?: { scopes?: string[]; callCount?: number }): boolean {
    ...
    if (allowedTool.restrictions) {
      ...
      if (allowedTool.restrictions.requiredScopes && allowedTool.restrictions.requiredScopes.length > 0) {
  ...
  getViolationReason(toolName: string, context?: { scopes?: string[]; callCount?: number }): string | null {
    ...
    if (allowedTool.restrictions) {
      ...
      // Check call limits
      if (allowedTool.restrictions.maxCalls !== undefined && contextToUse?.callCount !== undefined) {
