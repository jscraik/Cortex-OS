...
export class AgentResourceManagerImpl implements AgentResourceManager {
    ...
    return new Promise<T>((resolve, reject) => {
      ...
      operation()
        .then((result) => {
          ...
          this.currentUsage.executionTimeMs = Math.max(this.currentUsage.executionTimeMs, executionTime);
          resolve(result);
        })
        .catch((error) => {
          clearTimeout(timer);
  ...
  trackMemoryUsage<T>(operation: () => T): T {
    try {
      ...
      this.currentUsage.memoryMB = Math.max(this.currentUsage.memoryMB, finalMemory / (1024 * 1024));
  ...
  validateResourceRequest(requestedResources: Partial<ResourceLimits>): boolean {
    if (requestedResources.maxMemoryMB && requestedResources.maxMemoryMB > this.limits.maxMemoryMB) {
      return false;
    }
    if (requestedResources.maxCpuPercent && requestedResources.maxCpuPercent > this.limits.maxCpuPercent) {
      return false;
    }
    if (requestedResources.maxExecutionTimeMs && requestedResources.maxExecutionTimeMs > this.limits.maxExecutionTimeMs) {
      return false;
    }
    if (requestedResources.maxFileSystemMB && requestedResources.maxFileSystemMB > this.limits.maxFileSystemMB) {
      return false;
    }
    if (requestedResources.maxNetworkRequests && requestedResources.maxNetworkRequests > this.limits.maxNetworkRequests) {
      return false;
    }
    return true;
  }
