...
const SecurityConfigSchema = z.object({
  ...
  allowedOrigins: z.array(z.string().refine(
    (url) => {
      try {
  ...
  sessionTimeout: z.number().min(60).max(86400).refine(
...
export interface EnvironmentValidator {
  ...
  mergeConfigurations(defaultConfig: EnvironmentConfig, overrideConfig: Partial<EnvironmentConfig>): EnvironmentConfig;
...
class EnvironmentValidatorImpl implements EnvironmentValidator {
  ...
  private addMemoryConfig(config: EnvironmentConfig, env: Record<string, string | undefined>): void {
  ...
  private checkProductionSecurity(config: EnvironmentConfig, warnings: string[]): void {
    ...
    if (config.security.sessionTimeout > 7200) { // 2 hours
  ...
  mergeConfigurations(defaultConfig: EnvironmentConfig, overrideConfig: Partial<EnvironmentConfig>): EnvironmentConfig {
    ...
    if (defaultConfig.security && overrideConfig.security) {
      merged.security = { ...defaultConfig.security, ...overrideConfig.security };
    ...
    if (defaultConfig.database && overrideConfig.database) {
      merged.database = { ...defaultConfig.database, ...overrideConfig.database };
